...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    coutv << "(Re-entrant lock) Locked_device returned value after first "
             "unlock = "
          << WEXITSTATUS(ret) << endl;
    assert(WEXITSTATUS(ret) == 1);
    ...
    coutv << "(Re-entrant lock) Locked_device returned value after second "
             "unlock = "
          << WEXITSTATUS(ret) << endl;
    assert(WEXITSTATUS(ret) == 0);
    ...
    coutv << "(Destroying device) Locked_device returned value after the "
             "delete = "
          << WEXITSTATUS(ret) << endl;
    assert(WEXITSTATUS(ret) == 0);
    ...
    cout << "  Impossible to change polling on a device locked by another "
            "client --> OK"
         << endl;
    ...
    cout << "  Impossible to change logging on a device locked by another "
            "client --> OK"
         << endl;
