...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    string query = "select host from event where name like 'notifd/%' order by "
                   "`started` desc limit 1";
    din << query;
    try
    {
    ...
    int cache_size = kBase            // unchanged number of information related to the admin device, DS
                                      // events, DServer class properties, default properties, admin
                                      // device properties, class properties, class attribute properties
                                      // and control system object properties
                     + notifd_size    // if there is at least one host on which there is notifd
                                      // deamon running : 6 properties, if not : 2 properties
                     + 2              // CtrlSystem object + number describing the amount of its properties
                     + (2 * nb_prop)  // (property name + number describing the amount of its
                                      // values) x number of properties
                     + prop_elt       // number of all properties values
                     + acc_ctrl_size; // if AccessControl defined : 8 properties, if not : 0
                                      // properties
    ...
    coutv << "Class prop data between " << class_elt->class_prop.first_idx << " and " << class_elt->class_prop.last_idx
          << endl;
    first = last + 3;
    last  = first + 1;
    ...
    coutv << "Class attribute prop data between " << class_elt->class_att_prop.first_idx << " and "
          << class_elt->class_att_prop.last_idx << endl;
    first = last + 1;
    last  = first + 1;
