// Testprogram to demonstrate a server internal data update problem
// in 7.0.2-bugfix (snapshot: 24.7.2009)
//
...
//
// This problem occures for setting: abs_change and rel_change to control event
// generation.
//
// RUNNING:
...
void ConfigChangedEventCallback::push_event(Tango::AttrConfEventData *ed)
{
  ...
  //	cout << "[ConfigChangedEventCallback]: push_event( AttrConfEventData )
  // entering for:" << endl
  //             << "[ConfigChangedEventCallback]: " << ed->attr_name << endl
  //             << "[ConfigChangedEventCallback]: event type: " << ed->event <<
  //             endl
  //             << "[ConfigChangedEventCallback]: err: " << ed->err << endl
  //            << "[ConfigChangedEventCallback]: abs_change: " <<
  //            abs_change_according_last_event << endl
  //             << "[ConfigChangedEventCallback]: rel_change: " <<
  //             rel_change_according_last_event << endl
  //             << "[ConfigChangedEventCallback]: push_event( AttrConfEventData
  //             ) leaving." << endl;
}
...
int main(int argc, char **argv)
{
  ...
  //        cout << "[main]: This program assumes that  \"TangoTest test\"  is
  //        running" << endl; cout << "[main]: connect to " << dn
  //             << "  TANGO_HOST="
  //             << (getenv( "TANGO_HOST" ) != NULL ? getenv( "TANGO_HOST" ) :
  //             "not set")
  //             << endl;
  ...
  //        cout << "[main]: for " << an << ": set abs_change to: " <<
  //        ai.events.ch_event.abs_change << endl; cout << "[main]: for " << an
  //        << ": set rel_change to: " << ai.events.ch_event.rel_change << endl;
  ail.push_back(ai);
  dev->set_attribute_config(ail);

  //        cout << "[main]: make a subscribe_event() for: attribute
  //        configuration changed, " << an << endl;
  ...
  Tango_sleep(1);
  //        cout << "[main]: current setting according configuration changed
  //        event: abs_chang=" << abs_change_according_last_event << endl; cout
  //        << "[main]: current setting according configuration changed event:
  //        rel_chang=" <<  rel_change_according_last_event << endl;
  ...
  //        cout << "[main]: clearing setting for rel/abs_change.  Wait for a
  //        config changed event ..." << endl;
  Tango_sleep(1);

  // bug demonstration:
  //	cout << "[main]: doing a query for configuration from server:
  // dev->attribute_query() ..." << endl;
  ai = dev->attribute_query(an);
  //	cout << "[main]: setting as seen by server: abs_change=" <<
  // ai.events.ch_event.abs_change << endl; 	cout << "[main]: setting as seen
  // by client due to the config change event: abs_change=" <<
  // abs_change_according_last_event << endl; 	cout << "[main]: setting as seen
  // by server: rel_change=" << ai.events.ch_event.rel_change << endl; 	cout <<
  // "[main]: setting as seen by client due to the config change event:
  // rel_change=" << rel_change_according_last_event << endl;
  ...
  /*	if( abs_change_according_last_event != resetValue
       || rel_change_according_last_event != resetValue )
          {
                  rc = -1;
                  cout << endl << "[main]: BUG: could not reset setting." <<
     endl;
          }*/
  ...
  /*	if( ai.events.ch_event.abs_change != abs_change_according_last_event
       || ai.events.ch_event.rel_change != rel_change_according_last_event )
          {
                  rc = -1;
                  cout << endl << "[main]: BUG: server sent a config changed
     event with correct data, but did not update the data internal." << endl;
          }*/
...
[main]: make a subscribe_event() for: attribute configuration changed,
double_scalar [ConfigChangedEventCallback]: push_event( AttrConfEventData )
entering for: [ConfigChangedEventCallback]:
tango://taco10.taco.frm2:10000/sys/tg_test/1/double_scalar
...
[main]: setting as seen by server: abs_change=33333 [main]: setting as seen by
client due to the config change event: abs_change=Not specified [main]: setting
as seen by server:                                rel_change=99.99 [main]:
setting as seen by client due to the config change event: rel_change=Not
specified

[main]: BUG: server sent a config changed event with correct data, but did not
update the data internal.
