...
// The setting is also lost for events.per_event.period even if a DevRestart is
// not done.
//
...
// min_alarm, alarms.min_alarm, max_alarm and alarms.max_alarm in
// Tango::AttributeInfoEx if pedantic_test is false:
// Tango::AttributeInfoEx::mXX_alarm is not checked but
// Tango::AttributeInfoEx::alarms.mXX_alarm
...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    {
      rc = 1;
      //			cout << "test 1: FAILED, set and control numerical changes." <<
      // endl;
    ...
    {
      rc = rc * 10 + 1;
      //			cout << "test 2: FAILED, set all to '" << notset << "' control
      // changes"  << endl;
    ...
    //		cout << endl << "test 3:  change to numerical/string values with
    // device restart" << endl;
    ai_expected = ai_orig;
    setValues(ai_expected, false);
    ail.clear();
    ...
    {
      rc = rc * 10 + 1;
      //			cout << "test 3: FAILED, set all to numerical/string values and
      // control changes after a
      // DevRestart"  << endl;
      ...
      cout << "   Set all to numerical/string values and control changes after "
              "a DevRestart --> OK"
           << endl;

    // test 4: device restart, change to 'Not specified'
    //		cout << endl << "test 4:  change to 'Not specified' with device
    // restart" << endl;
    ai_expected = ai_orig;
    setValues(ai_expected, true);
    ail.clear();
    ...
    {
      rc = rc * 10 + 1;
      //			cout << "test 4: FAILED, set all to '" << notset << "' control
      // changes after a
      // DevRestart"  << endl;
    }
    else
      cout << "   Set all to 'Not specified' + control changes after a "
              "DevRestart --> OK"
           << endl;
...
#define CMP(x)                                                                           \
  ...
  {                                                                                      \
    rc = 1;                                                                              \
    cout << #x << ":\tBUG got: " << ai_act.x << "\texpected: " << ai_expected.x << endl; \
  }
