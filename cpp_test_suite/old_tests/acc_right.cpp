...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    if(allo_cmds.size() == 0)
    {
      cout << "Your control system is not configured property to run the "
              "controlled access!"
           << endl;
      exit(-1);
    }
  ...
  cout << "  Access allowed/blocked for first config (user not defined, * "
          "read) --> OK"
       << endl;
  ...
  cout << "  Access allowed/blocked for second config (user not defined, * "
          "WRITE) --> OK"
       << endl;
  ...
  cout << "  Access allowed/blocked for third config (* READ, user defined "
          "with device WRITE) --> OK"
       << endl;

  //
  // Set cmd line user right to READ on the command line device and WRITE on all
  // other device
  //
  ...
  cout << "  Access allowed/blocked for forth config (* READ, user defined "
          "with default device WRITE, cmd line device "
          "READ) --> OK"
       << endl;
  ...
  cout << "  Access allowed/blocked for fifth config (cmd line device WRITE + "
          "allowed from wrong address) --> OK"
       << endl;
  ...
  cout << "  Access allowed/blocked for sixth config (cmd line device WRITE + "
          "allowed from user address) --> OK"
       << endl;

  //--------------- CONTROLLED ACCESS AND CONTROL SYSTEM CONFIG
  // TEST--------------------
  ...
  cout << "  Read access if access service defined in DB but is not running "
          "(even for allowed commands) --> OK"
       << endl;
  ...
  string cmd_line("export MYSQL_USER=root;export MYSQL_PASSWORD=root;export "
                  "SUPER_TANGO=true;/home/taurel/tango/cppserver/"
                  "TangoAccessControl/bin/ubuntu810/TangoAccessControl 1 "
  ...
  cout << "  Device rights after re-connection (without calls between DS "
          "kill/start) --> OK"
       << endl;
  ...
  cout << "  Device rights after re-connection (with calls between DS "
          "kill/start) --> OK"
       << endl;
  ...
  cout << "  Device rights after re-connection (kill -9 and without calls "
          "between DS kill/start) --> OK"
       << endl;
  ...
  cout << "  Device rights after re-connection (kill -9 and with calls between "
          "DS kill/start) --> OK"
       << endl;

  delete another_dev_dp;

  //--------------- RE_CONNECTION TO CONTROLLED ACCESS SERVICE
  // TEST--------------------
  ...
  try
  {
    ...
    ca_pid = start_ds("/home/taurel/tango/cppserver/TangoAccessControl/bin/"
                      "ubuntu810/TangoAccessControl",
  ...
  try
  {
    ...
    //		check_device_access(dev_dp,false,false);
    coutv << "Read access for DeviceProxy created while the CA is killed -9 "
             "--> OK"
          << endl;
  ...
  cout << "  Device rights after re-start of Controlled Access Service killed "
          "by -9 --> OK"
       << endl;
