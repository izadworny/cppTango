...
int main(int argc, char **argv)
{
  ...
  for(i = 0; i < loop; i++)
  {
    ...
    // Strangi behaviour with CC 5.5 oin kidiboo. This loop works well if you
    // run it 3 times  but fails if you rin it forth. Replace the vector
    // equality test by a vector element equality test (20/07/05)
    //		assert( vout == vin );
    assert(vin[0] == vout[0]);
    assert(vin[1] == vout[1]);
    assert(vin[2] == vout[2]);
