...
void EventCallBack::push_event(Tango::DataReadyEventData *event_data)
{
  ...
  try
  {
    ...
    if(!event_data->err)
    {
      coutv << "Event data: Attribute data type = " << event_data->attr_data_type << ", User ctr = " << event_data->ctr
            << std::endl;
...
int main(int argc, char **argv)
{
  ...
  {
    //
    // Subscribe to an attribute which is not data ready event enabled should
    // fail
    //
    ...
    cout << "   data_ready_event --> OK" << std::endl;
