...
int main(int argc, char **argv)
{
  ...
  try
  {
    ...
    cout << "   CallBack executed when attribute throw exception only once --> "
            "OK"
         << std::endl;
    ...
    // Wait one second for the callback sent because exception not thrown any
    // more
    //
    ...
    // Note that like archive_period is set to 5 seconds, we should have
    // received an archive periodic callback
    //
    ...
    assert(cb.cb_executed >= (nb_cb + 1));
    cout << "   CallBack executed for the periodic part of the event "
            "(archive_period set) --> OK"
         << std::endl;
    ...
    // Let's hope the polling thread will not start just between the two
    // command_inout
    ...
    cout << "   CallBack executed when spectrum size increases (with filter) "
            "--> OK"
         << std::endl;
    ...
    cout << "   CallBack executed when spectrum size decreases (with filter) "
            "--> OK"
         << std::endl;
