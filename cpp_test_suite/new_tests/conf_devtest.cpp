...
void print_changes(std::string &desc, std::string &server, DbData &db_data)
{
  ...
  while(it != db_data.end())
  {
    ...
    //			cout << "   " << ((i == 0) ? ((str << it->value_string[i], (str >>
    // nb_prop && str.eof()) ? " : " : (" = "
...
/*
 * This is a utility that configures properties of test device servers provided
 * as command line parameters. Attribute values, their ranges and other
 * properties are set to defaults used in the test suite.
 */
int main(int argc, char **argv)
{
  ...
  {
    cout << "usage: " << argv[0]
         << " dserver device1 device2 device3 device1_alias attribute_alias "
            "fwd_device device20"
         << std::endl;
    exit(-1);
  }
  ...
  try
  {
    ...
    {
      cout << "Added test server : " << str << " -> " << info.name << ", class : " << info._class << std::endl;
    }
    cout << std::endl;
  }
  catch(...)
  ...
  try
  {
    ...
    for(size_t i = 0; i < db_dev_infos.size(); i++)
      cout << "Added pseudo server : " << str << " -> " << db_dev_infos[i].name
  ...
  {
    cout << "Exception: cannot set specific attribute properties for the "
            "device: "
         << device1_name << std::endl;
  }
