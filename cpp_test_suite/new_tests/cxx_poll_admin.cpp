...
public:
  ...
  virtual ~SUITE_NAME()
  {
    ...
    // clean up in case test suite terminates before polling state is restored
    // to defaults
  ...
  void test_check_if_command_polling_is_started(void)
  {
    ...
    TS_ASSERT_THROWS_NOTHING(dout = dserver->command_inout("DevPollStatus", din));
    dout >> status_arr;
    // extract the first 3 lines of the status and compare with the reference
    // string
    // TODO: although device/time dependent, the other 3 lines could also be
    // compared
  ...
  void test_poll_object_which_returns_an_exception(void)
  {
    ...
    status_ref[1] = "Last attribute read FAILED :\n\tReason = "
                    ...
                    "attr_wrong_size exceeds given limit";
    din << device1_name;
    TS_ASSERT_THROWS_NOTHING(dout = dserver->command_inout("DevPollStatus", din));
    dout >> status_arr;
    status_arr_str = (*status_arr)[0].in();
    // extract the first and the last 3 lines of the status and compare with the
    // reference string
    // TODO: although device/time dependent, the other, middle 3 lines could
    // also be compared
  ...
  void test_start_a_command_externally_triggered(void)
  {
    ...
    // extract the first 3 lines of the status and compare with the reference
    // string
    // TODO: although device/time dependent, the other line could also be
    // compared
    status = string((*status_arr)[0].in()).substr(0, status_ref.length());
    TS_ASSERT_EQUALS(status, status_ref);
