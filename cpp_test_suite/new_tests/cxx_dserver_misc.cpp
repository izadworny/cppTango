...
public:
  ...
  void test_ping_the_device(void) { TS_ASSERT_THROWS_NOTHING(dserver->ping()); }
  ...
  void DISABLED_test_archive_periodic_events_after_polling_restart()
  {
    ...
    int subscription = 0;
    TS_ASSERT_THROWS_NOTHING(subscription = device1->subscribe_event(attribute_name, Tango::ARCHIVE_EVENT, &callback));
  ...
  void test_unsubscription_during_deletion_of_multiple_proxies()
  {
    ...
    int subscription1 = 0;
    TS_ASSERT_THROWS_NOTHING(subscription1 = proxy1->subscribe_event(attribute_name, Tango::USER_EVENT, &callback1));

    auto proxy2       = std::make_unique<DeviceProxy>(device1_name);
    int subscription2 = 0;
    TS_ASSERT_THROWS_NOTHING(subscription2 = proxy2->subscribe_event(attribute_name, Tango::USER_EVENT, &callback2));
