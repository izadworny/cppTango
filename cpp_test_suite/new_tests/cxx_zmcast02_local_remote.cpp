...
// The other event comes from a device running in a different host (therefore
// using PGM to communicate) These two events use the same callback object
//
...
public:
  ...
  {
  public:
    EventCallBack(McastLocalRemoteTestSuite *ptr)
        : parent(ptr)
    {
    }
