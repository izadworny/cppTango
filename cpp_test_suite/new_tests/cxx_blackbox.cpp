...
public:
  ...
  void test_blackbox_device_feature(void)
  {
    ...
    // sets the "command_inout" suffix (refer to blackbox.cpp
    // build_info_as_str())
    ...
    string query = "select attribute, name  from property_attribute_device "
                   "where name like '\\_\\_%' and  device='" +
                   device3_name + "'";
    ...
    for(unsigned int i = 0; i < result.svalue.length(); i++)
    {
      if(i % 2 == 0)
      {
        //				cout << "===> attr_name: " << string(result.svalue[i].in()) <<
        // endl;
        attr_name = device3_name + "/" + string(result.svalue[i].in());
      }
      else
      {
        //				cout << "===> attr_prop: " << string(result.svalue[i].in()) <<
        // endl;
    ...
    /*		TS_ASSERT_THROWS_ASSERT(device3->black_box(2), Tango::DevFailed &e,
       ...
       API_BlackBoxEmpty); TS_ASSERT_EQUALS(e.errors[0].severity,
       Tango::ERR));*/
    delete device3;
