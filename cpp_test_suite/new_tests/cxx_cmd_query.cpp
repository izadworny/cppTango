...
public:
  ...
  //		cout << "name: " << cmd_inf.cmd_name << " tag: " << cmd_inf.cmd_tag << "
  // displ_level: " <<
  // cmd_inf.disp_level << " in_type: " << cmd_inf.in_type << " type_desc: " <<
  // cmd_inf.in_type_desc << " out_type: " << cmd_inf.out_type << "
  ...
  void test_command_list_query_AddObjPolling(void)
  {
    ...
    TS_ASSERT_EQUALS(cmd_inf.out_type, Tango::DEV_VOID);
    TS_ASSERT_EQUALS(cmd_inf.in_type_desc, "Lg[0]=Upd period. Str[0]=Device name. Str[1]=Object "
                                           "type. Str[2]=Object name");
    TS_ASSERT_EQUALS(cmd_inf.out_type_desc, "Uninitialised");
  }
  ...
  {
    //		CommandInfo cmd_inf =
    // dserver->command_query("EventConfirmSubscription");
    CommandInfo cmd_inf = cmd_inf_list[5];
    TS_ASSERT_EQUALS(cmd_inf.cmd_name, "EventConfirmSubscription");
    TS_ASSERT_EQUALS(cmd_inf.in_type, Tango::DEVVAR_STRINGARRAY);
    TS_ASSERT_EQUALS(cmd_inf.out_type, Tango::DEV_VOID);
    TS_ASSERT_EQUALS(cmd_inf.in_type_desc, "Str[0] = dev1 name, Str[1] = att1 name, Str[2] = event "
                                           "name, Str[3] = dev2 "
  ...
  {
    //		CommandInfo cmd_inf =
    // dserver->command_query("EventSubscriptionChange");
  ...
  {
    //		CommandInfo cmd_inf =
    // dserver->command_query("QueryWizardClassProperty");
  ...
  {
    //		CommandInfo cmd_inf =
    // dserver->command_query("QueryWizardDevProperty");
  ...
  {
    //		CommandInfo cmd_inf =
    // dserver->command_query("ZmqEventSubscriptionChange");
    ...
    TS_ASSERT_EQUALS(cmd_inf.out_type_desc, "Str[0] = Heartbeat pub endpoint - Str[1] = Event pub endpoint\n"
                                            "...\n"
                                            "Str[n] = Alternate Heartbeat pub endpoint - Str[n+1] = Alternate "
                                            "Event pub endpoint\n"
                                            "Str[n+1] = event name used by this server as zmq topic to send "
                                            "events\n"
