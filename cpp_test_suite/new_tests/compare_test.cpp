...
bool is_line_equal(const std::string &ref_line, const std::string &out_line) { return ref_line.compare(out_line) == 0; }
...
void CmpTst::CompareTest::out_set_event_properties(string file, map<string, string> prop_val_map)
{
  if(prop_val_map.size() > 0)
  {
    ...
    while(infile)
    {
      ...
      if(line.find("<log4j:event ") != string::npos)
      {
        ...
        for(map<string, string>::iterator it = prop_val_map.begin(); it != prop_val_map.end(); ++it)
        {
          ...
          // replaces property with value each time the keyword is found in the
          // line
          size_t begin = line.find(property_str);
          while(begin != string::npos)
          {
            begin += property_str.length();
            size_t end = line.find("\"", begin); // looking for the property value
                                                 // closing double quotation mark
            line.replace(begin, end - begin,
                         (*it).second); // replacing with new value
    ...
    outfile.close();
    if(infile.bad())
      throw CmpTst::CompareTestException("[CmpTst::CompareTest::out_set_event_"
                                         "properties] Cannot close file: " +
                                         file);
    if(outfile.bad())
      throw CmpTst::CompareTestException("[CmpTst::CompareTest::out_set_event_"
                                         "properties] Cannot close file: " +
  ...
  if(prefix_num_map.size() > 0)
  {
    ...
    while(infile)
    {
      ...
      for(map<string, string>::iterator it = prefix_num_map.begin(); it != prefix_num_map.end(); ++it)
      {
        ...
        // replaces property with value each time the prefix is found in the
        // line
...
//			Original tag in the output file:
//			<log4j:event logger="DSERVER" timestamp="10" level="DEBUG" thread="1">
//			The tag after ref_replace_keywords("outfile", "DSERVER",
//"dserver/devTest/test") call: 			<log4j:event logger="dserver/devTest/test"
// timestamp="10" level="DEBUG" thread="1">
// argument : in :	- file : 	output file to be modified
...
//			<log4j:event logger="dserver/devTest/test" timestamp="10" level="INFO"
// thread="1">
// argument : in :	- file : 	output file to be modified
...
void CmpTst::CompareTest::ref_replace_keywords(string file, map<string, string> key_val_map)
{
  if(key_val_map.size() > 0)
  {
    ...
    outfile.open(string(outfile_name).c_str());
    if(!outfile)
      throw CmpTst::CompareTestException("[CmpTst::CompareTest::ref_replace_keywords] Cannot open file for "
                                         "writing: " +
                                         outfile_name);
    ...
    while(infile)
    {
      ...
      {
        // replaces keyword with value each time the keyword is found in the
        // line
...
void CmpTst::CompareTest::out_remove_entries(string file, vector<string> &v_entries)
{
  if(v_entries.empty() == false)
  {
    ...
    outfile.open(string(outfile_name).c_str());
    if(!outfile)
      throw CmpTst::CompareTestException("[CmpTst::CompareTest::out_remove_entries] Cannot open file for "
                                         "writing: " +
                                         outfile_name);
...
//
//--------------------------------------------------------------------------
void CmpTst::CompareTest::print_output(string out) { print_output(out, false); }
