...
//
// In this test, the local server is used to send mcast event locally (to this
// client) but also to another client running on a remote host. This another
// client is in fact a device server as well with the commands IOSubscribeEvent,
// IOGetCbExecuted and IOUnsubscribeEvent. This allows us to test the case where
// we have a server firing multicast event to local and remote clients
//
...
public:
  ...
  {
  public:
    EventCallBack(SvrMcastLocalRemoteTestSuite *ptr)
        : parent(ptr)
    {
    }
...
public:
  ...
  // unsubscribe to the event. No need to stop polling on local device. The
  // IOUnsubscribeEvent on remote device will take care of this
...
void SvrMcastLocalRemoteTestSuite::EventCallBack::push_event(Tango::EventData *event_data)
{
  ...
  try
  {
    coutv_cb << "EventCallBack::push_event(): called attribute " << event_data->attr_name << " event "
             << event_data->event << "\n";
    if(!event_data->err)
    {
