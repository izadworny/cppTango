...
RealTestDescription::RealTestDescription(List &argList, SuiteDescription &argSuite, unsigned argLine,
                                         const char *argTestName)
{
  initialize(argList, argSuite, argLine, argTestName);
}

void RealTestDescription::initialize(List &argList, SuiteDescription &argSuite, unsigned argLine,
                                     const char *argTestName)
{
  _suite    = &argSuite;
  _line     = argLine;
...
bool RealTestDescription::setUp()
{
  ...
  _TS_TRY { _TSM_ASSERT_THROWS_NOTHING(file(), line(), "Exception thrown from setUp()", suite()->setUp()); }
...
bool RealTestDescription::tearDown()
{
  ...
  _TS_TRY { _TSM_ASSERT_THROWS_NOTHING(file(), line(), "Exception thrown from tearDown()", suite()->tearDown()); }
  _TS_CATCH_ABORT({ return false; });
...
RealSuiteDescription::RealSuiteDescription(const char *argFile, unsigned argLine, const char *argSuiteName,
                                           List &argTests)
{
  initialize(argFile, argLine, argSuiteName, argTests);
}

void RealSuiteDescription::initialize(const char *argFile, unsigned argLine, const char *argSuiteName, List &argTests)
{
  _file      = argFile;
  _line      = argLine;
...
StaticSuiteDescription::StaticSuiteDescription(const char *argFile, unsigned argLine, const char *argSuiteName,
...
void StaticSuiteDescription::initialize(const char *argFile, unsigned argLine, const char *argSuiteName,
                                        TestSuite &argSuite, List &argTests)
{
...
void CommonDynamicSuiteDescription::initialize(const char *argFile, unsigned argLine, const char *argSuiteName,
...
SuiteDescription *RealWorldDescription::firstSuite() { return (RealSuiteDescription *) suites().head(); }
