...
//<< "\n\nException : \n" << endl;
// Tango::Except::print_exception(e); throw;} ) _TS_CATCH_TYPE( (const
// CxxTest::AbortTest &), b )
#undef _TS_LAST_CATCH
#define _TS_LAST_CATCH(b)                       \
  _TS_CATCH_TYPE((const Tango::DevFailed &e), { \
    cout << "\n\nException : \n" << endl;       \
...
// This is a trick to deal with compiler warnings while catching DevFailed
// multiple times
// TODO: Remove this code when issue #53
// (https://github.com/CxxTest/cxxtest/issues/53) is fixed
...
private:
   ...
   * the 'params' map
   * (example: ./run_tests user_param_1 user_param_2 --loop=10 ==> uargv[0] ==
   * "user_param_1"; uargv[1] == "user_param_2")
   */
  ...
  /*
   * map of predefined parameter names and their declarations (the way of usage
   * in the command line) (example: params["loop"] = "--loop=" ==> ./run_tests
   * user_param --loop=10)
   */
  ...
  /*
   * list of expected user arguments; printed out if provided arguments do not
   * match the expected in quantity
   */
  static vector<string> uargs_validate;
  /*
   * descriptions of expected user arguments; printed out if provided arguments
   * do not match the expected in quantity
   */
  static vector<string> uargs_desc;
  /*
   * list of expected mandatory parameters; printed out if provided arguments do
   * not match the expected in quantity
   */
  static set<string> params_validate;
  /*
   * list of expected optional parameters; printed out if provided arguments do
   * not match the expected in quantity
   */
  static set<string> params_opt_validate;
  /*
   * list of expected local parameters; printed out if provided arguments do not
   * match the expected in quantity
   */
  ...
  /*
   * set containing registered names of the methods which are to be called to
   * restore original device settings in case a test case fails before restoring
...
#define cout cout_stored
  ...
  int run(int argc_tmp, char **argv_tmp)
  {
    ...
    for(int i = 1; i < argc; i++)
    {
      ...
      for(map<string, vector<string>>::iterator it = params.begin(); it != params.end(); ++it)
      {
        ...
        {
          // checks if the parameter has already been used to accept value of
          // only the first occurrence of the parameter
    ...
    // prints out the list of all parameters if required by user ("--?" or
    // "--help")
    if(TangoPrinter::is_param_set("?") || TangoPrinter::is_param_set("help") || TangoPrinter::is_param_set("-?") ||
       TangoPrinter::is_param_set("-help"))
    {
      cout << "\nAll parameters:";
      for(map<string, vector<string>>::iterator it = TangoPrinter::params.begin(); it != TangoPrinter::params.end();
          ++it)
      {
   ...
   */
  static void restore_set(const char *name) { restore_set(string(name)); }

  /*
   * registers a restore point to take up an action in TestSuite tearDown method
   * in case a test case does not restore the default device properties
   */
  static void restore_set(const string &name) { restore_points.insert(name); }

  /*
   * unregisters the restore point (after the test case had successfully
   * restored the default device properties)
   */
  static void restore_unset(const char *name) { restore_unset(string(name)); }

  /*
   * unregisters the restore point (after the test case had successfully
   * restored the default device properties)
   */
  static void restore_unset(const string &name) { restore_points.erase(name); }

  /*
   * checks if the restore point has been registered
   */
  static bool is_restore_set(const char *name) { return is_restore_set(string(name)); }
  ...
  static unsigned int get_argc(void) { return argc; }

  static char **get_argv(void) { return argv; }

  static vector<string> &get_uargv(void) { return uargv; }

  static unsigned int get_uargc(void) { return uargc; }

  /*
   * returns number of predefined parameters used in command line by user
   */
  static unsigned int get_pargc(void) { return pargc; }
  ...
  static map<string, vector<string>> &get_params(void) { return params; }

  static string &get_executable_name(void) { return executable_name; }

  /*
   * registers user argument on the list, returns its value (if set) or empty
   * string
   */
  ...
  /*
   * registers user argument and its description on the list, returns its value
   * (if set) or empty string
   */
  ...
  /*
   * registers description of mandatory parameter on the list, returns its value
   * (if set) or empty string
   */
  ...
  /*
   * registers description of optional parameter on the list, returns its value
   * (if set) or empty string
   */
    ...
    else
    {
      ...
      cout << "Parameter '" << param
           << "' is one of the predefined parameters.\nUse "
              "CxxTest::TangoPrinter::get_param(\""
           << param << "\") method instead.\n";
      exit(-1);
    }
    return param_val;
  }
  ...
  /*
   * registers description of optional parameter on the list, returns true if
   * parameter has been set (useful when no value for parameter is expected e.g.
   * "--v")
   */
  ...
  /*
   * checks if all required arguments have been properly set, if not prints out
   * usage hint and terminates
   */
  ...
  static map<string, vector<string>> create_params()
  {
    ...
    params_tmp["loop"]          = param_desc("--loop=", "execute test cases marked with '__loop' suffix "
                                               "the indicated number of times");
    params_tmp["suiteloop"]     = param_desc("--suiteloop=",
                                         "execute test suites marked with '__loop' suffix the "
                                         "indicated number of times"); // executes suite in a loop
    ...
    params_tmp["clienthost"]    = param_desc("--clienthost=", "client host's fully qualified domain name, e.g. "
                                                           "mypc.myinstitute.com (small caps)");
    params_tmp["serverhost"]    = param_desc("--serverhost=", "fully qualified domain name of the host "
                                                           "on which the server is running, e.g. "
                                                           "myserver.myinstitute.com (small caps)");
    ...
    params_tmp["loglevel"]      = param_desc("--loglevel=",
                                        "default device logging level, e.g. 0"); // default device logging
                                                                                      // level, e.g. 0
    params_tmp["dsloglevel"] = param_desc("--dsloglevel=",
                                          "default device server logging level, e.g. 3"); // default device server
                                                                                          // logging level, e.g. 3
    ...
    params_tmp["outpath"] = param_desc("--outpath=",
                                       "device server logging target directory, e.g. /tmp/"); // device server
                                                                                              // logging target
                                                                                              // directory,
                                                                                              // e.g. /tmp/
    params_tmp["refpath"] = param_desc("--refpath=",
                                       "directory where the 'compare test' reference files (*.out) "
                                       "are stored");                             // directory where the compare test
                                                                                  // reference files (*.out) are stored
  ...
  static void kill_server() { system(Tango::kKillServerCmd.c_str()); }
