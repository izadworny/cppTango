...
void DevTest::always_executed_hook() { cout2 << "In always_executed_hook method" << std::endl; }

void DevTest::server_init_hook() { cout2 << "In server_init_hook method" << std::endl; }
...
void DevTest::IOTempl() { cout << "[DevTest::IOTempl]" << std::endl; }
...
void DevTest::IOTemplIn(Tango::DevLong received) { cout << "[DevTest::IOTemplIn] received " << received << std::endl; }
...
Tango::DevLong DevTest::IOSubscribeEvent(const Tango::DevVarStringArray *in_data)
{
  ...
  // TODO: modify conf_devtest.cpp to automatically create a remote device based
  ...
  // tg->get_device_list_by_class("DevTest"); 		if(dev_list.size() > 1) // the
  // second device on the list is chosen
  //		{
  ...
  //		}
  //		else
  //			cout << "EXCEPTION: at least 2 devices have to be defined" <<
  // std::endl;
  //	}
  ...
  //	eve_id =
  // remote_dev->subscribe_event(att_name,Tango::PERIODIC_EVENT,&cb,filters);
...
{
  //	cout << "In write_Short_attr_w for attribute " << att.get_name() <<
  // std::endl;
...
{
  //	cout << "In write_String_attr_w for attribute " << att.get_name() <<
  // std::endl;
...
void DevTest::write_attr_hardware(std::vector<long> &att_idx)
{
  ...
  case 3:
    if(att_idx.size() >= 2)
      Tango::Except::throw_named_exception(this, att_idx[2], "DevTest_WriteAttrHardware", "bbb",
...
void DevTest::read_Encoded_attr_rw(Tango::Attribute &att)
{
  ...
  /*		if ((count % 2) == 0)
                  {
                          enc_attr.encoded_format = Tango::string_dup("First
     string"); enc_attr.encoded_data[0] = 11;
                  }
                  else
                  {
                          enc_attr.encoded_format = Tango::string_dup("Second
     string"); enc_attr.encoded_data[0] = 22;
                  }*/
...
/*		enc_attr_ptr = new Tango::DevEncoded;
                enc_attr_ptr->encoded_format = Tango::string_dup("Which
   format?"); enc_attr_ptr->encoded_data.length(4);
...
bool DevTest::is_RWPipe_allowed(Tango::PipeReqType) { return true; }
