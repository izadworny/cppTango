...
public:
   ...
   * @exception DevFailed If the command is not found, if the command is not
   * allowed in the actual device state and re-throws of all the exception
   * thrown by the <i>always_executed_hook</i>, <i>is_alloed</i> and
   * <i>execute</i> methods. Click <a
   ...
   *
   */
  CORBA::Any *command_handler(DeviceImpl *device, const std::string &command, const CORBA::Any &in_any);
  /**
...
//@{
   ...
   *
   * @param signo The signal number
   * @param own_handler A boolean set to true if you want the device signal
   * handler to be executed in its own handler instead of being executed by the
   * signal thread. If this parameter is set to true, care should be taken on
   * how the handler is written. A default false value is provided * @exception
   * DevFailed Thrown if the signal number is out of range or if the operating
   * system failed to register a signal for the process. Click <a
   * href="https://tango-controls.readthedocs.io/en/latest/development/advanced/IDL.html#exceptions">here</a>
   * to read <b>DevFailed</b> exception specification
   */
