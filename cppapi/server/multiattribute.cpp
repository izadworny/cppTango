...
auto find_property(std::vector<Tango::AttrProperty> &cont, std::string name)
{
...
static OptAttrProp Tango_OptAttrProp[] = {{"label", LabelNotSpec},
...
//
// description :
//		Constructor for the MultiAttribute class from the device device name and a
// pointer to the DeviceClass
// object
//
// argument :
//		in :
    ...
    : ext(new MultiAttribute::MultiAttributeExt)
{
  ...
  // No need to implement a retry here (in case of db server restart) because
  // the db reconnection is forced by the get_property call executed during
  ...
  if(nb_attr != 0)
  {
    ...
    if(tg->_UseDb == true)
    {
      ...
      // same time some other processes also access the device attribute
      // properties table. This has been experimented at ESRF. Increase timeout
      // to cover this case
      //
    ...
    for(i = 0; i < nb_attr; i++)
    {
      ...
      //
      // If the attribute has some properties defined at device level, build a
      // vector of these properties
      //
      ...
      if(fwd_ok == true)
      {
        ...
        //
        // If one of the alarm properties is defined, add it to the alarmed
        // attribute list
        //
    ...
    //
    // Remove attdesc entry in class object for forwarded attribute with root
    // device not started
    //
...
//
// description :
//		Destructor for the MultiAttribute class. It simply delete all the
// Attribute object stored in its 		attr_list data member
//
//-----------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Concatenate porperties defined at the class level and at the device level.
// Prperties defined at the
// device 		level have the highest level
//
// arguments:
// 		in :
...
void MultiAttribute::concat(const std::vector<AttrProperty> &dev_prop, const std::vector<AttrProperty> &class_prop,
                            std::vector<AttrProperty> &result)
{
  ...
  {
    auto pos = find_property(tmp_result, class_prop[i].get_name());
...
{
  //
  // Overwrite the label attribute property in order to be set to the attribute
  // name
  //
  ...
  //
  // Also overwrite the format attribute property according to attribute data
  // type
  //
  ...
  // vector of attributes if they are defined. If yes, continue. Otherwise, add
  // a new property with the default value
  //

  for(long i = 0; i < nb_opt_prop; i++)
  {
    auto pos = find_property(prop_list, Tango_OptAttrProp[i].name);
...
void MultiAttribute::add_user_default(std::vector<AttrProperty> &prop_list,
                                      const std::vector<AttrProperty> &user_default)
{
  //
  ...
  {
    auto pos = find_property(prop_list, user_default[i].get_name());
...
//
// description :
//		Check if the writable_attr_name property is set and in this case, check if
// the associated attribute
// exists 		and is writable. This is necessary only for attribute of the
// READ_WITH_WRITE or READ_WRITE types
//
// argument :
//		in :
...
//
// description :
//		Check some features according to which IDL release the device implement.
// This can not be done in 		DeviceImpl class ctor because when the DeviceImpl
// ctor
// is executed, it is always IDL release 1. 		The real supported IDL release
// number is set during each Device_XImpl ctor which is executed after
// DeviceImpl ctor. 		Today, we check :
  ...
  for(size_t i = 0; i < nb_attr; i++)
  {
    ...
    if(attr_list[i]->get_data_type() == DEV_ENUM && idl_version < 5)
    {
      try
      {
        ...
        ss << "This is supported oonly for device inheriting from IDL 5 or "
              "more";
    ...
    // If IDL < 5, on top of setting wrongly configured attribute in device,
    // remove the root att from root attribute registry
    //
...
void MultiAttribute::add_attribute(const std::string &dev_name, DeviceClass *dev_class_ptr, long index)
{
  ...
  //
  // Create an Attribute instance and insert it in the attribute list. If the
  // device implement IDL 3 (with state and status as attributes), add it at the
  // end of the list but before state and status.
  //
  ...
  //
  // Check if the writable_attr_name property is set and in this case, check if
  // the associated attribute exists and is writable
  //
...
//
// description :
//		Construct a new forwarded attribute object and add it to the device
// attribute list
//
// argument :
//		in :
...
void MultiAttribute::add_fwd_attribute(const std::string &dev_name, DeviceClass *dev_class_ptr, long index,
                                       Attr *new_attr)
{
  ...
  if(new_attr->is_fwd() == true)
  {
    ...
    // If forwarded attribute is dynamically created and was constructed without
    // specifying the root_attribute parameter then we have to get its
    // __root_att property from tango DB prior to calling validate_fwd_att()
    std::vector<AttrProperty> dev_prop;
    Tango::Util *tg = Tango::Util::instance();
    ...
    {
      // If any error add this attribute to the device list of wrong configured
      // attributes
      DeviceImpl::FwdWrongConf fwc;
...
//			- update_idx : Flag set to true if the attributes object index used to
// retrieve the
// corresponding Attr 						   object has to be updated (because one Attr
...
void MultiAttribute::remove_attribute(const std::string &attr_name, bool update_idx)
{
  ...
  // Update all the att_index_in_vector indexes for the attributes following the
  // one which has been deleted
  ...
  if(update_idx == true)
  {
    ...
    // MultiClassAttribute vector for the attributes which are following (in
    // MultiClassAttribute vector) the one which has been deleted This is a 2
    // steps process: 1 - Update indexes in local device 2 - Update indexes in
    // remaining device(s) belonging to the same class Update indexes in local
    // device
  ...
  //
  // Clear the writable attribute index vector and rebuild it. This is necessary
  // because this vector containd index in the main attribute list which has
  // been modified
  //
...
//
// return :
// 		A reference to the wanted attribute or throws an exception id the
...
//
// return :
//		The index of the wanted attribute or throws an exception id the attribute
// is not found
//
//--------------------------------------------------------------------------------------------------------------------
...
//
// return :
//		A boolen set to true if one of the attribute with an alarm defined is in
// alarm state
//
//--------------------------------------------------------------------------------------------------------------------
...
bool MultiAttribute::check_alarm()
{
  ...
  for(i = 0; i < alarm_attr_list.size(); i++)
  {
    ...
    {
      //
      // If the attribute is polled, the check_alarm method has already been
      // called when the polling thread has read the attribute.
      //
...
//
// description :
//		Add a message in the device status string if one of the device attribute
...
// argument :
// 		in :
//			- eve : One structure in this vector for each attribute with events
// subscribed
//
//------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		For scalar attribute with an associated write attribute, the
// read_attributes CORBA operation also
// returns 		the write value. This method gets the associated write attribute
// value and adds it to the read attribute
//
// argument :
// 		in :
...
//
// description :
//		Check for all attribute if one of them has its quality factor set to
// ALARM. 		Returns true in this case. Otherwise, returns false
//
//------------------------------------------------------------------------------------------------------------------
...
bool MultiAttribute::is_att_quality_alarmed()
{
  ...
  {
    if((attr_list[i]->get_quality() == Tango::ATTR_ALARM) || (attr_list[i]->get_quality() == Tango::ATTR_WARNING))
    {
      ret = true;
      break;
    }
...
//
// description :
//		Add to the status string name of attributes with a quality factor set to
// alarm
//
//-------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Update attribute lists when a forwarded attribute conf. is now completely
// known (after a device with fwd 		attributes started before the device with
// root attribute(s))
//
// argument:
//		in :
...
void MultiAttribute::update(const Attribute &att, const std::string &dev_name)
{
  ...
  //
  // If it is writable, add it to the writable attribute list if not there
  // already
  //
...
//
// description :
//		This method returns true if the property name passed as argument is the
// name of one of the attribute
// optional
//      properties
//
// argument:
...
//
// return:
//      True if prop_name is the name of one of the optional properties. False
//      otherwise
//
//-------------------------------------------------------------------------------------------------------------------
