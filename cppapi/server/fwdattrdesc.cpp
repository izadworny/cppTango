...
//
// description :
//		Constructor for the FwdAttr class. Initialize data with default value for
// a wrongly configured 		forwarded attribute
//
// argument :
//		in :
...
bool FwdAttr::validate_fwd_att(std::vector<AttrProperty> &prop_list, const std::string &dev_name)
{
  ...
  try
  {
    ...
    {
      //
      // Write something in DB to help user to create the right entry except if
      // the root att name is hard coded
      //
  ...
  //
  // Check root att syntax and add TANGO_HOST info in root device name of not
  // given Also add dns suffix if not defined in provided TANGO_HOST host name
  //
...
//
// description :
//		This method initialize FwdAttr class data member in case of forwarded
// attribute from the root attribute 		configuration received during the
// attribute change event subscription
//
// argument :
//		in :
...
//
// description :
//		Get the label property from the user default properties list. Throw
...
//
// description :
//		Remove classical attribute property(ies) if found in prop_list input
// argument. For forwarded attribute,
//      the configuration is taken from the root att and should not be taken
//      from the DB. If we found some useless properties, remove them from the
//      prop list and inform the user to clean up the database.
//
// argument :
//		in :
...
void FwdAttr::remove_useless_prop(std::vector<AttrProperty> &prop_list, const std::string &dev_name,
                                  MultiAttribute *m_attr)
{
  ...
  for(ite = prop_list.begin(); ite != prop_list.end();)
  {
    ...
    if(m_attr->is_opt_prop(ite->get_name()) == true)
    {
      ...
      std::cerr << "This property will not be taken into account. Please clean "
                   "up your DB."
                << std::endl;
