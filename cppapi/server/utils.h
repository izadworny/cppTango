...
public:
   ...
   * It is the user responsability to code this function in a way that it
   * implements some kind of blocking in order not to load the computer CPU
   *
   * @param f_ptr The event loop function pointer. This function will not
   * receive any argument. It returns a boolean value. If this boolean is set to
   * true, the device server process exit.
   */
...
public:
  ...
  //
  // Some methods are duplicated here (with different names). It is for
