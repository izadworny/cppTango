//+==================================================================================================================
//
//  zmqeventsupplier.cpp : C++ classes for implementing the event server and
//  client singleton classes -ZmqEventSupplier
//						   This class is used to send events from the server to the
// client(s) when zmq is used to 						   transport the events
//
      ...
      double_send_heartbeat(false)
{
  ...
  // If the user has specified one IP address on the command line, re-use it in
  // the endpoint But if the address was specified as a name (supported by
  // omniORB), convert this name to IP address but uses the name in the endpoint
  // given to client
  //
  ...
  if(specified_addr.empty() == false &&
     WSAStringToAddress(src_copy, AF_INET, NULL, (struct sockaddr *) &ss, &size) != 0)
#endif
    specified_name = true;
  ...
  // Leave the OID and method name un-initialized. Marshall the structure into
  // CORBA CDR
  //
  ...
  //   tango://host:port/dserver/exec_name/inst_name#dbase=no.heartbeat when
...
//      This dtor delete the zmq socket. The ZMQ context will be deleted (then
//      calling zmq_term) when the object is destroyed
//
//-------------------------------------------------------------------------------------------------------------------
...
void ZmqEventSupplier::create_event_socket()
{
  if(event_pub_sock == NULL)
  {
    ...
    // If the user has specified one IP address on the command line, re-use it
    // in the endpoint
    //
...
//
// description :
//		Create and bind the publisher socket used to publish the real events when
...
void ZmqEventSupplier::create_mcast_event_socket(const std::string &mcast_data, const std::string &ev_name, int rate,
                                                 bool local_call)
{
  std::map<std::string, McastSocketPub>::iterator ite;

  //
  // If the event is already in the mcast event map, check if it is already used
  // by local clients
  //
...
//
// description :
//		This method checks if the event is already defined in the map of multicast
// event.
//
// argument :
//		in :
...
//
// description :
//		This method returns the multicast socket endpoint for the event passed as
// parameter
//
// argument :
//		in :
...
void ZmqEventSupplier::push_heartbeat_event()
{
  ...
  //
  // Heartbeat - check wether a heartbeat event has been sent recently. If not
  // then send it. A heartbeat contains no data, it is used by the consumer to
  // know that the supplier is still alive.
  //
  ...
  cout3 << "ZmqEventSupplier::push_heartbeat_event(): delta time since last "
           "heartbeat "
        << delta_time << std::endl;

  //
  // We here compare delta_time to 8 and not to 10.
  // This is necessary because, sometimes the polling thread is some milli
  // second in advance. The computation here is done in seconds. So, if the
  // polling thread is in advance, delta_time computed in seconds will be 9 even
  // if in reality it is 9,9
  //
  ...
  if(delta_time >= 8)
  {
    ...
    cout3 << "ZmqEventSupplier::push_heartbeat_event(): detected heartbeat "
             "event for "
    ...
    while(nb_event != 0)
    {
      ...
      try
      {
        ...
        //
        // Push a dummy message in the event socket. This is required by the ZMQ
        // layer. In case of reconnection, if nothing is sent on the socket
        // between two reconnections, some memory will be "leaked". ZMQ is
        // deleting that memory at process end (zmq context delete) OR when
        // something is sent to the socket. Therefore, send a dummy message to
        // the socket to force ZMQ "garbage collection" because we cannot wait
        ...
        //
        // For reference counting on zmq messages which do not have a local
        // scope
        //
...
//
// Small callback used by ZMQ when using the no-copy API to signal that the
// message has been sent. Take care, this method is also called when ZMQ
// connection is closed. In such a case, the calling thread is the thread doing
// the zmq::send() call and it's not the ZMQ thread. When this happens, we do
// not need to send a signal to the calling thread.
//
...
void ZmqEventSupplier::push_event(DeviceImpl *device_impl, std::string event_type,
                                  ...
                                  const struct SuppliedEventData &ev_value, const std::string &obj_name,
                                  DevFailed *except, bool inc_cptr)
{
  if(device_impl == NULL)
    return;
  ...
  // Don't forget case where we have notifd client (thus with a fqdn_prefix
  // modified)
  //
  ...
  // it does not give any performance improvement in this case (too small amount
  // of data)
  //
  ...
  else
  {
    ...
    if(except == NULL)
    {
      ...
      {
        //
        // Get number of data exchanged by this event. If this value is greater
        // than a threshold, set a flag In such a case, we will use ZMQ no-copy
        // message call
        //
    ...
    //
    // For event with small amount of data, use memcpy to initialize the zmq
    // message. For large amount of data, use zmq message with no-copy option
    //
  ...
  try
  {
    ...
    //
    // If we have a multicast socket with also a local client we are obliged to
    // send two times the messages. ZMQ does not support local client with PGM
    // socket
    //
    ...
    while(send_nb > 0)
    {
      ...
      if(send_nb == 1)
      {
        ...
        {
          //
          // Case of multicast socket with a local client. Send the event also
          // on the local socket
          //
...
std::string ZmqEventSupplier::create_full_event_name(DeviceImpl *device_impl, const std::string &event_type,
                                                     const std::string &obj_name_lower, bool intr_change)
{
...
bool ZmqEventSupplier::update_connected_client(client_addr *cl)
{
  ...
  //
  // Immediately return if client identification not possible. (Very old
  // client....)
  //
...
//
// description :
//		Method to send the event in a loop due to possible different client
// release (event compatibility)
//
// argument :
//		in :
//			- device_impl : The device
                                       ...
                                       const struct SuppliedEventData &attr_value, Attribute &att, DevFailed *except)
{
  ...
  for(ite = client_libs.begin(); ite != client_libs.end(); ++ite)
  {
    ...
    push_event(device_impl, ev_name, filterable_names, filterable_data, filterable_names_lg, filterable_data_lg,
               sent_value, att.get_name_lower(), except, inc_ctr);
...
//
// description :
//		Get how many data are transferred in the blob given as parameter. For
// instance, for a blob transporting 		one array of 1000 double, the returned
// value will be 1000
//
// argument :
//		in :
...
//
// description :
//		Get how many data are transferred in the data element given as parameter.
// For instance, for a data
// element 		transporting one array of 250 floats, the returned value will be
// 250
//
// argument :
//		in :
