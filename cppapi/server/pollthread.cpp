...
// file :               PollThread.cpp
//
// description :        C++ source code for the PollThread class. This class is
// used for the polling thread. The rule
//						of this thread is to regulary exceute command on device or read
// attribute and store result 						in a ring buffer.
//
// project :            TANGO
//
// author(s) :          E.Taurel
...
//			- heartbeat : Flag set to true if this polling thread is the thread used
// to send the heartbeat
// event
//
//------------------------------------------------------------------------------------------------------------------

PollThread::PollThread(PollThCmd &cmd, TangoMonitor &m, bool heartbeat)
    : shared_cmd(cmd),
      p_mon(m),
...
void *PollThread::run_undetached(TANGO_UNUSED(void *ptr))
{
  ...
  //
  // If the thread is the event heartbeat thread, use it also for the storage of
  // sub device properties. Declare a work item to check the for new sub devices
  // regularly.
  //
  ...
  while(1)
  {
    ...
    {
      std::cerr << "OUPS !! A omni thread fatal exception received by a "
                   "polling thread !!!!!!!!"
                << std::endl;
#ifndef _TG_WINDOWS_
      time_t t = time(NULL);
    ...
    {
      std::cerr << "OUPS !! A Tango::DevFailed exception received by a polling "
                   "thread !!!!!!!!"
                << std::endl;
    ...
    {
      std::cerr << "OUPS !! A CORBA::Exception exception received by a polling "
                   "thread !!!!!!!!"
                << std::endl;
      Tango::Except::print_exception(e);
#ifndef _TG_WINDOWS_
    ...
    {
      std::cerr << "OUPS !! An unforeseen standard exception has been received "
                   "by a polling thread !!!!!!"
                << std::endl;
...
//
// description :
//		This method wait on the shared monitor for a new command to be sent to the
// polling thread. The thread
// waits 		with a timeout. If the thread is awaken due to the timeout, false is
// returned. 		If the work list is empty, the thread waits for ever.
//
// returns :
// 		Enumeration indicating the type of detected event when waiting for next
// command.
//
//------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		This method is called when a command has been received. It execute the
// command!
//
// args :
//		in :
...
bool pred_dev(const WorkItem &w) { return w.dev == PollThread::dev_to_del; }
...
void PollThread::execute_cmd()
{
  ...
  case Tango::POLL_ADD_OBJ:
  {
    ...
    if(found == false)
    {
      ...
      if(wo.update != PollClock::duration::zero())
      {
        ...
        {
          cout5 << "Received a delta from now of " << std::fixed << duration_ms(local_cmd.new_upd) << " ms"
                << std::endl;
  ...
  case Tango::POLL_REM_DEV:
    ...
    //     polling period means removing the already inserted object from the
    //     work list, compute its new polling time and insert it in the work
    //     list with its new polling time and polling period
    // 2 - This is executed by the polling thread itself
    // 2-1 - The command updates polling period for another object: idem than
    // previous 2-2 - The commands updates polling period for the object it is
    // actually polled.
    //       In this case, the object is not in the work list. It has been
    //       removed from the
    //	 work list at the beginning of the "one_more_poll" method and is
    // memorized there 	 Therefore, simply stores new polling period in a data
    // member. The "one_more_poll" 	 method will get its new polling period
    // before re-inserting the object in the work
    // 	 list with the new update period.
    //	 We detect this case because the object is not in any work list (either
    // the work 	 list or the trigger list)
    //
  ...
  case Tango::POLL_UPD_PERIOD:
  {
    ...
    //
    // If not found in work list, it should be in the externally triggered
    // object. Therefore, remove it from externally triggered list and insert it
    // in work list. If not found in work list and in trig list, we are in case
    // 2-2 as described above (polling thread itself updating polling period of
    // the object it actually polls)
    //
...
void PollThread::one_more_poll()
{
  ...
  //
  // For case where the polling thread itself modify the polling period of the
  // object it already polls
  //
  ...
  if(auto_upd.empty() == false)
  {
    ...
    for(const auto &entry : auto_upd)
    {
      ...
      {
        if(ite->dev == tmp.dev && ite->type == tmp.type && ite->update == entry.first)
        {
...
void PollThread::one_more_trigg()
{
  ...
  //
  // Check that the object to poll has been installed. If not, simply returns.
  // This case should never happens because it is tested in the
  // Util::trigger_polling() method before the trigger is effectively sent to
  // this thread.
  //
    ...
    if(ite->type != EVENT_HEARTBEAT)
    {
      if(ite->type != STORE_SUBDEV)
      {
        ...
        cout5 << "Dev name = " << ite->dev->get_name() << ", obj name = " << obj_list << ", next wake_up at "
              << std::fixed << duration_s(ite->wake_up_date.time_since_epoch()) << " s " << std::fixed << "(in "
              << duration_ms(ite->wake_up_date - PollClock::now()) << " ms)" << std::endl;
      }
      else
      {
        cout5 << ite->name[0] << ", next wake_up at " << std::fixed << duration_s(ite->wake_up_date.time_since_epoch())
              << " s " << std::fixed << "(in " << duration_ms(ite->wake_up_date - PollClock::now()) << " ms)"
              << std::endl;
      }
    }
    else
    {
      cout5 << "Event heartbeat, next wake_up at " << std::fixed << duration_s(ite->wake_up_date.time_since_epoch())
            << " s " << std::fixed << "(in " << duration_ms(ite->wake_up_date - PollClock::now()) << " ms)"
            << std::endl;
    }
...
//
// description :
//		To insert (at the correct place) a new Work Item in the work list but
// before this for attribute, if we
//      already have a work item with the same device at the same period, simply
//      add the obj name in it. (in order to use read_attributes() in a optimal
//      way)
//
// args :
//		in :
...
// args :
//		in :
// 			- from_needed : Set to true if the delta between work should be at least
// equal to the
...
void PollThread::tune_list(bool from_needed)
{
  ...
  //
  // If we try to tune the list with respect to works needed time, compute works
  // needed time sum and find minimun update period
  //
  ...
  if(from_needed == true)
  {
    ...
    for(++ite; ite != works.end(); ++ite, ++ite_prev)
    {
      ...
      if(next_work < next_tuning)
      {
        ...
        {
          // Explicit calculation of n (as integer) is needed and cannot be
          // skipped.
...
void PollThread::compute_sleep_time()
{
  ...
  if(works.empty() == false)
  {
    ...
    if(polling_bef_9 == false)
    {
      ...
      //  - Seriously late (number of late items equal number of items) --> We
      //  will discard items
      //  - Late for the first time: Poll immediately but memorize the number of
      //  items for which we are late
...
//
// description :
//		To force one event if the polling thread has discarded one work item
// because it is late
//
// args :
//		in :
...
void PollThread::err_out_of_sync(WorkItem &to_do)
{
  ...
  for(size_t ctr = 0; ctr < nb_obj; ctr++)
  {
    ...
    if((event_supplier_nd != NULL) || (event_supplier_zmq != NULL))
    {
      ...
      errs[0].desc     = Tango::string_dup("The polling thread is late and discard this object "
      ...
      if(event_supplier_nd != NULL)
        send_event =
            event_supplier_nd->detect_and_push_events(to_do.dev, ad, &except, to_do.name[ctr], PollClock::now());
      if(event_supplier_zmq != NULL)
      {
        if(event_supplier_nd != NULL)
        {
          ...
          if(send_event.change == true)
            event_supplier_zmq->push_event_loop(to_do.dev, CHANGE_EVENT, f_names, f_data, f_names_lg, f_data_lg, ad,
                                                att, &except);
          if(send_event.archive == true)
            event_supplier_zmq->push_event_loop(to_do.dev, ARCHIVE_EVENT, f_names, f_data, f_names_lg, f_data_lg, ad,
                                                att, &except);
          if(send_event.periodic == true)
            event_supplier_zmq->push_event_loop(to_do.dev, PERIODIC_EVENT, f_names, f_data, f_names_lg, f_data_lg, ad,
                                                att, &except);
        }
        else
          event_supplier_zmq->detect_and_push_events(to_do.dev, ad, &except, to_do.name[ctr], PollClock::now());
      }
    }
  }
}
...
void PollThread::poll_cmd(WorkItem &to_do)
{
  ...
  //
  // Insert result in polling buffer and simply forget this command if it is not
  // possible to insert the result in polling buffer
  //
...
void PollThread::poll_attr(WorkItem &to_do)
{
  ...
  //
  // Starting with IDl release 3, an attribute in error is not an exception any
  // more. Re-create one. Don't forget that it is still possible to receive
  // classical exception (in case of Monitor timeout for instance)
  //
  ...
  //
  // Events - for each event call the detect_and_push() method this method will
  // fire events if there are clients registered and if there is an event
  // (on_change, on_alarm or periodic) We also have to retrieve which kind of
  // clients made the subscription (zmq or notifd) and send the event
  // accordingly
  //
  ...
  for(size_t ctr = 0; ctr < nb_obj; ctr++)
  {
    ...
    if((event_supplier_nd != NULL) || (event_supplier_zmq != NULL))
    {
      if(attr_failed == true)
      {
        ...
        //
        // Eventually push the event (if detected). When we have both notifd and
        // zmq event supplier, do not detect the event two times. The
        // detect_and_push_events() method returns true if the event is
        // detected.
        //
        ...
        if(event_supplier_nd != NULL)
          send_event =
              event_supplier_nd->detect_and_push_events(to_do.dev, ad, save_except, to_do.name[ctr], before_cmd);
        if(event_supplier_zmq != NULL)
        {
          if(event_supplier_nd != NULL)
          {
      ...
      else
      {
        ...
        if(event_supplier_nd != NULL)
          send_event =
              event_supplier_nd->detect_and_push_events(to_do.dev, ad, tmp_except, to_do.name[ctr], before_cmd);
        if(event_supplier_zmq != NULL)
        {
          if(event_supplier_nd != NULL)
          {
  ...
  // not possible to insert the result in polling buffer Different case
  // according to the number of attributes which has been read by this poll. The
  // PollObj object are supposed to store data coming from a read_attributes()
  // executed for 1 attribute. Sonce Tango 9.0.8, it is not always the case.
  // This is why we differantiate cases when we read several att in once call.
  // Sin the last case, re-create memory layout as the one coming from
  // read_attributes called for single attribute. This new feature is available
  // only for devices with IDL 4 or more
  //
