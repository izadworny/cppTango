...
// classes. These classes are the root class
//						for all derived Device classes. They are abstract classes. The
// DeviceImpl class is
// the 						CORBA servant which is "exported" onto the network and
...
//
// description :
//		Constructors for the device_impl class from the class object pointer, the
// device name, the description
// field, 		the state and the status. Device_3Impl inherits from DeviceImpl.
// These constructors simply call the correct 		DeviceImpl class constructor
//
//--------------------------------------------------------------------------------------------------------------------
...
Device_3Impl::Device_3Impl(DeviceClass *device_class, const char *dev_name, const char *desc, Tango::DevState dev_state,
...
//
// description :
//		Method called for each read_attributes operation executed from any client
// on a Tango device version 3.
//
//--------------------------------------------------------------------------------------------------------------------
                                                             ...
                                                             Tango::DevSource source)
{
  ...
  {
    //
    // It must be now CACHE_DEVICE (no other choice), first try to get values
    // from cache
    //
    ...
    //
    // Now, build the list of attributes which it was not possible to get their
    // value from cache
    //
...
//
// description :
//		Read attributes from device but do not throw exception if it fails. This
// method is mainly a copy of the 		original DeviceImpl::read_attributes
// method.
//
// arguments:
//		in :
//			- names: The names of the attribute to read
//			- aid : Structure with pointers for data to be returned to caller
//(several pointers acoording to 					caller IDL level)
//			- second_try : Flag set to true when this method is called due to a
// client request with source set
// to 						   CACHE_DEVICE and the reading the value from the cache
// failed for one reason or another
//			- idx : Vector with index in back array for which we have to read data
//(to store error at the
// right 					place in the array for instance)
//
//-------------------------------------------------------------------------------------------------------------------

void Device_3Impl::read_attributes_no_except(const Tango::DevVarStringArray &names, Tango::AttributeIdlData &aid,
                                             bool second_try, std::vector<long> &idx)
{
  //
  ...
  try
  {
      ...
      else
      {
        try
        {
          ...
          else
          {
            ...
            {
              //
              // If the attribute is a forwarded one, force reading it from  the
              // root device. Another client could have written its value
              //
    ...
    for(i = 0; i < nb_wanted_attr; i++)
    {
      if(wanted_attr[i].idx_in_multi_attr != -1)
      {
        ...
        try
        {
          ...
          //
          // Call the user read method except if the attribute is writable and
          // memorized and if the write failed during the device startup
          // sequence
          //
          ...
          else
          {
            ...
            o << " It failed during the write call of the device startup "
                 "sequence";
        ...
        catch(...)
        {
          ...
          if(aid.data_5 != nullptr)
          {
            if((att.get_attr_serial_model() == ATTR_BY_KERNEL) && (is_allowed_failed == false))
            {
              cout4 << "Releasing attribute mutex for attribute " << att.get_name()
    ...
    // If the device has some alarmed attributes and some of them have already
    // been read and failed, it is not necessary to read state, simply copy
    // faulty alarmed attribute error message to the state attribute error
    // messages
    //
    ...
    if(state_wanted == true)
    {
      ...
      {
        if(aid.data_5 != nullptr)
          error_from_errorlist((*aid.data_5)[state_idx], (*aid.data_5)[wanted_attr[id].idx_in_names].err_list,
                               names[state_idx]);
        else if(aid.data_4 != nullptr)
          error_from_errorlist((*aid.data_4)[state_idx], (*aid.data_4)[wanted_attr[id].idx_in_names].err_list,
                               names[state_idx]);
        else
          error_from_errorlist((*aid.data_3)[state_idx], (*aid.data_3)[wanted_attr[id].idx_in_names].err_list,
                               names[state_idx]);
      }
    }
          ...
          if(att.get_value_flag() == false)
          {
            ...
            try
            {
              ...
              if(upd == PollClock::duration::zero())
              {
                ...
                o << "Attribute values are set by external polling buffer "
                     "filling"
                  << std::ends;
              }
              else
          ...
          else
          {
            try
            {
              ...
              if(aid.data_5 != nullptr)
              {
                ...
                {
                  cout4 << "Giving attribute mutex to CORBA structure for "
                           "attribute "
            ...
            catch(Tango::DevFailed &e)
            {
              if(aid.data_5 != nullptr)
              {
                cout4 << "Asking CORBA structure to release attribute mutex "
                         "for attribute "
                      << att.get_name() << std::endl;
...
//
// description :
//		Read attributes from cache but do not throw exception if it fails. This
// method is mainly a copy of the
// original 		DeviceImpl::read_attributes method.
//
// argument:
//		in :
//			- names: The names of the attribute to read
//			- aid : Structure with pointers to data which should be returned to the
// caller. This method is
// called for 					3 differents IDL releases (3, 4 and 5). In this struct,
// there is one ptr for each
// possible IDL. 					Only the ptr for the caller IDL is not null.
//
//--------------------------------------------------------------------------------------------------------------------
...
void Device_3Impl::read_attributes_from_cache(const Tango::DevVarStringArray &names, Tango::AttributeIdlData &aid)
{
  ...
  //
  // Check that device supports the wanted attribute and that the attribute is
  // polled. If some are non polled, store their index in the real_names
  ...
  //
  // If some attributes are not polled but their polling update period is
  // defined, and the attribute is not in the device list of attr which should
  // not be polled, start to poll them
  //
  ...
  if(non_polled.empty() == false)
  {
    ...
    //
    // Leave method if number of attributes which should not be polled is equal
    // to the requested attribute number
    //
  ...
  //
  // For each attribute, check that some data are available in cache and that
  ...
  for(i = 0; i < nb_names; i++)
  {
    ...
    {
#ifdef _TG_WINDOWS_
      if((poll_list[j]->get_type() == Tango::POLL_ATTR) && (_stricmp(poll_list[j]->get_name().c_str(), names[i]) == 0))
#else
    ...
    //
    // In some cases where data from polling are required by a DS for devices
    // marked as polled but for which the polling is not sarted yet, polled_attr
    // could be NULL at the end of this loop. Return "No data yet" in this case
    //
    ...
    // sent back to user In order to avoid unnecessary copy, don't use the
    // assignement operator of the AttributeValue structure which copy each
    // element and therefore also copy the Any object. The Any assignement
    // operator is a deep copy! Create a new sequence using the attribute buffer
    // and insert it into the Any. The sequence inside the source Any has been
    // created using the attribute data buffer.
    //
    ...
    try
    {
      long vers = get_dev_idl_version();
      {
        ...
        //
        // Get device IDL release. Since release 4, devices are polled using
        // read_attribute_4
        //
        ...
        //
        // Init remaining structure members according to IDL client (aid.xxxx)
        // and IDL device (vers)
        //
...
void Device_3Impl::write_attributes_3(const Tango::AttributeValueList &values)
{
  ...
  //
  // Record operation request in black box. If this method is executed with the
  // request to store info in blackbox (store_in_bb == true), this means that
  // the request arrives through a Device_2 CORBA interface. Check locking
  // feature in this case. Otherwise the request has arrived through Device_4
  // and the check is already done
  //
...
//
// description :
//		Method to write the attribute. This method is common to the IDL interface
// 3 and 4.
//
// argument:
//		in :
  ...
  try
  {
    ...
    for(i = 0; i < nb_updated_attr; i++)
    {
      ...
      try
      {
        ...
        // For attributes which are not scalar, also check that their dimensions
        // are correct
        //
        ...
        //
        // Check if there are some startup exceptions for the attribute (due to
        // invalid attribute properties configuration). If so, do not allow to
        // write the attribute.
        //
      ...
      try
      {
        if(values_3 == NULL)
          dev_attr->get_w_attr_by_ind(ctr->idx_in_multi_attr)
              .check_written_value((*values_4)[ctr->idx_in_names].value, (unsigned long) single_att_dimx,
                                   (unsigned long) single_att_dimy);
        else
          dev_attr->get_w_attr_by_ind(ctr->idx_in_multi_attr)
              .check_written_value((*values_3)[ctr->idx_in_names].value, (unsigned long) single_att_dimx,
                                   (unsigned long) single_att_dimy);
      }
      catch(Tango::DevFailed &e)
      {
    ...
    if(nb_failed != nb_updated_attr)
    {
      ...
      for(ite = updated_attr.begin(); ite != updated_attr.end();)
      {
        ...
        try
        {
          ...
          //
          // If the write call succeed and if the attribute was memorized but
          // with an exception thrown during the device startup sequence, clear
          // the memorized exception
          //
      ...
      // If it throws NamedDevFailedList exception, mark only referenced
      // attributes as faulty
      //
    ...
    //
    // Copy data into Attribute object, store the memorized one in db and if the
    // attribute has a RDS alarm, set the write date
    //
    // Warning: Do not copy caller value if the user has manually set the
    // attribute written value in its write method
    //
    // WARNING: --> The DevEncoded data type is supported only as SCALAR and is
    // not memorizable. Therefore, no need to call copy_data
    //
...
// DevAttrHistory structure for each
//		attribute record
//
//--------------------------------------------------------------------------------------------------------------------
...
// 		WARNING !!!!!!!!!!!!!!!!!!
//
// 		This is the release 3 of this CORBA operation which returns much more
// parameter than in release 2 		The code has been duplicated in order to keep
// it clean (avoid many "if" on version number in a common method)
//
// argument:
//		in :
...
Tango::AttributeConfigList_3 *Device_3Impl::get_attribute_config_3(const Tango::DevVarStringArray &names)
{
  ...
  // If the device implements IDL 3 (State and status as attributes) and the
  // client is an old one (not able to read state/status as attribute),
  // decrement attribute number
  //
...
//
// description :
//		CORBA operation to set attribute configuration locally and in the Tango
// database
//
// argument:
//		in :
//			- new_conf: The new attribute(s) configuration. One AttributeConfig
// structure is needed for each 				    	attribute to update
//
//--------------------------------------------------------------------------------------------------------------------
...
void Device_3Impl::set_attribute_config_3(const Tango::AttributeConfigList_3 &new_conf)
{
  ...
  //
  // Record operation request in black box
  // If this method is executed with the request to store info in blackbox
  // (store_in_bb == true), this means that the request arrives through a
  // Device_2 CORBA interface. Check locking feature in this case. Otherwise the
...
// return:
// 		This method returns a pointer to a DevAttrHistoryList with one
// DevAttrHistory structure for each attribute
//		record
//
//--------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Get attribute properties. This method is used to retrieve properties for
// state and status.
//
// argument:
//		in :
...
void Device_3Impl::get_attr_props(const char *attr_name, std::vector<AttrProperty> &prop_list)
{
  ...
  if(tg->_UseDb == true)
  {
    ...
    //
    // Insert AttrProperty element in suplied vector for att. properties found
    // in DB
    //
...
//
// description :
//		Method to add alarmed attributes (if not already there) in the attribute
...
//
// description :
//		Method to check if it is necessary to read state. If the device has some
// alarmed attribute and one of
// these 		attributes has already been read and failed, it is not necessary to
// read state. It will also fail.
//
// argument:
//		in :
...
//
// return:
// 		This  method returns -1 if reading state is possible. Otherwise, it
// returns the index in the wanted_attr list 		of the alarmed attribute which
// failed
//
//--------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		This method find all the attributes which will be read by the state
//(because alarmed) and which have
// been 		already read. It builds a vector with the list of attribute not read
//
// argument:
//		in :
//			- wanted_attr : The list of attribute to be read by this call
