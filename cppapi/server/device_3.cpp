...
static bool is_error_stored(const Tango::AttributeIdlData& data, int index)
{
    const Tango::DevErrorList& errors =
        data.data_5 ? (*data.data_5)[index].err_list :
        data.data_4 ? (*data.data_4)[index].err_list :
...
static void store_error(AttributeValueT& attr_val, const DevErrorList& errors, const char* name)
{
...
static void store_error(Tango::AttributeIdlData& data, int index, const Tango::DevErrorList& errors, const char* name)
{
    if (data.data_5)
...
static void store_error(Tango::AttributeIdlData& data, int index, const Tango::DevFailed& dev_failed, const char* name)
{
    ...
    const AttributeIndicesInData& name_to_data_mapping)
{
    ...
    for (std::size_t i = 0; i < attributes.size(); ++i)
    {
        ...
        const Tango::AttrWriteType access_mode = att.get_writable();
    ...
    long& status_index_in_data)
{
    ...
    for (long i = 0; i < num_of_names; ++i)
    {
        ...
        else
        {
            try
            {
                const long attr_index = dev_attr->get_attr_ind_by_name(name.c_str());
    ...
    if (qual != Tango::ATTR_INVALID)
    {
        if (att.get_value_flag() == false)
        {
            ...
            if (aid.data_5 != NULL)
            {
                ...
                one_error((*aid.data_5)[index_in_data],reas,ori,s,att);
            }
            else if (aid.data_4 != NULL)
            {
                ...
                one_error((*aid.data_4)[index_in_data],reas,ori,s,att);
            }
            else
                one_error((*aid.data_3)[index_in_data],reas,ori,s,att);
        }
        else
        {
            try
