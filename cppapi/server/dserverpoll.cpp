...
// class is derived from Device. It
//						represents the CORBA servant object which will be accessed from
// the
// network. 						All commands which can be executed on a DServer object
...
Tango::DevVarStringArray *DServer::dev_poll_status(const std::string &dev_name)
{
  ...
  //
  // Compute how many cmds and/or attributes are polled. Since IDL V3, state and
  // status are polled as attributes For compatibility, if one of these
  // "attributes" is polled, also returns the info as the command is polled
  //
  ...
  for(i = 0; i < nb_poll_obj; i++)
  {
    ...
    {
      //
      // Take polled object ownership in order to have coherent info returned to
      // caller. We don't want the polling thread to insert a new elt in polling
      // ring while we are getting these data. Therefore, use the xxx_i methods
      //
    ...
    else
    {
      ...
      //
      // If the attribute is state or status, also add the string in command
      // list after replacing all occurences of "attributes" by "command" in the
      // returned string
      //
...
void DServer::add_obj_polling(const Tango::DevVarLongStringArray *argin, bool with_db_upd, int delta_ms)
{
  ...
  if(obj_type == PollCommand)
  {
    ...
    //
    // Since IDl release 3, state and status command must be polled as
    // attributes to be able to generate event on state or status.
    //
  ...
  //
  // Check that the requested polling period is not below the one authorized (if
  // defined) 0 as polling period is always authorized for polling buffer
  // externally filled
  //
  ...
  //
  // Create a new PollObj instance for this object. Protect this code by a
  // monitor in case of the polling thread using one of the vector element.
  //
  ...
  //
  // Find out which thread is in charge of the device. If none exists already,
  // create one
  //
  ...
  if(th_id != poll_th_id)
  {
    ...
    //
    // Wait for thread to execute command except if the command is requested by
  ...
  //
  // Update polling parameters in database (if wanted and possible). If the
  // property is already there (it should not but...), only update its polling
  // period
  //
  ...
  //
  // Also update the polling threads pool conf if one thread has been created by
  // this call
  //
...
void DServer::upd_obj_polling_period(const Tango::DevVarLongStringArray *argin, bool with_db_upd)
{
  ...
  // already defined in the polling property. Add object name and update period
  // if the object is not known in the property
  //
...
void DServer::rem_obj_polling(const Tango::DevVarStringArray *argin, bool with_db_upd)
{
  ...
  if(tg->is_svr_shutting_down() == false)
  {
    ...
    if(th_info->poll_th != NULL)
    {
      ...
      if(th_id != poll_th_id)
      {
        ...
        //
        // Wait for thread to execute command except if the command is requested
  ...
  // properties if they exist or add it to the list of device not polled for
  // automatic polling defined at command/attribute level. Do this if possible
  // and wanted.
  //
  ...
  // Also update the map<device name,thread id> If this device was the only one
  // for a polling thread, kill the thread then in Db if possible
  //
  ...
  if(poll_list.empty() == true)
  {
    ...
    //
    // Kill the thread if needed and join but don do this now if the executing
    // thread is the polling thread itself (case of a polled command which
    // itself decide to stop its own polling)
    //
  ...
  //
  // In case there are some subscribers for event on this attribute relying on
  // polling, fire event with error
  //
  ...
  //
  // In case of local_request and executing thread is the polling thread, ask
  // our self to exit now that eveything else is done
  //
...
//
// description :
//		command to ask the heartbeat thread to send the event heartbeat every 9
// seconds
//
//--------------------------------------------------------------------------------------------------------------------
...
void DServer::add_event_heartbeat()
{
  ...
  //
  // Send command to the heartbeat thread but wait in case of previous cmd still
  // not executed
  //
...
//
// description :
//		In case a minimun update polling period is defined (via the
// min_poll_period, cmd_min_poll_period, 		attr_min_poll_period) check if the
// requested period is not smaller
//
// args :
// 		in:
//			- dev : The device
