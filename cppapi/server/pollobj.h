...
public:
  ...
  PollClock::duration get_needed_time()
  {
    omni_mutex_lock sync(*this);
  ...
  {
    if(type == POLL_CMD)
      return ring.is_last_cmd_an_error();
    else
      return ring.is_last_attr_an_error();
  }
...
inline bool operator<(const PollObj &, const PollObj &) { return true; }
