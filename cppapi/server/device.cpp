...
// file :		Device.cpp
//
// description :	C++ source code for the DeviceImpl class. This class is the
// root class for all derived Device
//					classes. It is an abstract class. The DeviceImpl class is the CORBA
// servant which is "exported" 					onto the network and accessed by the
// client.
//
// project :		TANGO
//
...
//
// The per thread data storage key (The client identification is stored in
// thread specific storage) defined in utils.cpp
//
...
DeviceImpl::DeviceImpl(DeviceClass *cl_ptr, const std::string &d_name, const std::string &de, Tango::DevState st,
...
void DeviceImpl::real_ctor()
{
  ...
  //
  // Create device pipe and finish the pipe config init since we now have device
  // name
  //
...
void DeviceImpl::get_dev_system_resource()
{
  ...
  if(tg->_UseDb == true)
  {
    ...
    {
      TangoSys_OMemStream o;
      o << "Database error while trying to retrieve device prperties for "
           "device "
        << device_name.c_str() << std::ends;
    ...
    if(db_data[8].is_empty() == false)
    {
      ...
      {
        std::transform(cmd_poll_ring_depth[i].begin(), cmd_poll_ring_depth[i].end(), cmd_poll_ring_depth[i].begin(),
                       ::tolower);
      }
    }
    if(db_data[9].is_empty() == false)
    {
      ...
      for(unsigned int i = 0; i < nb_prop; i = i + 2)
      {
        std::transform(attr_poll_ring_depth[i].begin(), attr_poll_ring_depth[i].end(), attr_poll_ring_depth[i].begin(),
                       ::tolower);
      }
    }
    ...
    if(db_data[11].is_empty() == false)
    {
      ...
      for(unsigned int i = 0; i < nb_prop; i = i + 2)
      {
        std::transform(cmd_min_poll_period[i].begin(), cmd_min_poll_period[i].end(), cmd_min_poll_period[i].begin(),
                       ::tolower);
      }
    }
    ...
    if(db_data[12].is_empty() == false)
    {
      ...
      for(unsigned int i = 0; i < nb_prop; i = i + 2)
      {
        std::transform(attr_min_poll_period[i].begin(), attr_min_poll_period[i].end(), attr_min_poll_period[i].begin(),
                       ::tolower);
      }
    }
...
PortableServer::POA_ptr DeviceImpl::_default_POA() { return Util::instance()->get_poa(); }
...
Command *DeviceImpl::get_command(const std::string &cmd_name)
{
  ...
  //
  // This piece of code is added for VC++ compiler. As they advice, I have try
  // to use the __decspec(noreturn) for the throw_exception method, but it seems
...
//
// description :
//		This method returns the polling buffer depth. Most of the times, this is
// defined at device level 		via the device "poll_ring_depth" property.
// Nevertheless, in some cases, this value can be overwritten via the 		device
// "cmd_poll_ring_depth" property.
//
// args :
// 		in :
...
//
// description :
//		The default method called by the DevState command. If the device is ON,
// this method checks attribute 		with a defined alarm and set the state to
// ALARM if one of these attribute is in alarm. Otherwise, simply 		returns
// device state
//
//--------------------------------------------------------------------------------------------------------------------
  ...
  else
  {
    if((device_state == Tango::ON) || (device_state == Tango::ALARM))
    {
      ...
      if(nb_wanted_attr != 0)
      {
        ...
        {
          //
          // Starting with IDL 3, it is possible that some of the alarmed
          // attribute have already been read.
          //
          ...
          try
          {
            ...
            else
            {
              ...
              if((qua != Tango::ATTR_INVALID) && (att.get_value_flag() == false))
              {
                ...
                o << "Hint: Did the server follow Tango V5 attribute reading "
                     "framework ?"
                  << std::ends;
          ...
          catch(Tango::DevFailed &)
          {
            ...
            {
              WARN_STREAM << "Attribute has no value, forcing INVALID quality for: " << att.get_name() << std::endl;
      ...
      //
      // Check if one of the remaining attributes has its quality factor set to
      // ALARM or WARNING. It is not necessary to do this if we have already
      // detected that the state must switch to ALARM
      //
...
//
// description :
//		The default method called by the DevStatus command. If the device is ON,
// this method add Attribute
// status 		for all device attribute in alarm state.
//
//---------------------------------------------------------------------------------------------------------------------
...
Tango::DevState DeviceImpl::state()
{
  ...
  try
  {
    ...
    //
    // Return data to caller. If the state_cmd throw an exception, catch it and
    // do not re-throw it because we are in a CORBA attribute implementation
...
// AttributeConfig structure for each atribute
//
//--------------------------------------------------------------------------
...
void DeviceImpl::set_attribute_config(const Tango::AttributeConfigList &new_conf)
{
  ...
  //
  // The attribute conf. is protected by two monitors. One protects access
  // between get and set attribute conf. The second one protects access between
  // set and usage. This is the classical device monitor
  //
  ...
  try
  {
    for(i = 0; i < nb_attr; i++)
    {
      ...
      //
      // In case the attribute quality factor was set to ALARM, reset it to
      // VALID
      //
...
//
// This method returns a pointer to a AttributeConfigList with one
// AttributeValue structure for each atribute. This structure contains the
// attribute value, the value quality and the date.
//
//--------------------------------------------------------------------------
...
Tango::AttributeValueList *DeviceImpl::read_attributes(const Tango::DevVarStringArray &names)
{
  ...
  try
  {
    ...
    //
    // Retrieve index of wanted attributes in the device attribute list and
    // clear their value set flag
    //
    ...
    for(i = 0; i < nb_wanted_attr; i++)
    {
      ...
      else
      {
        ...
        if(idx == -1)
        {
          ...
          o << "It is not possible to read state/status as attributes with "
               "your\n";
...
void DeviceImpl::add_attribute(Tango::Attr *new_attr)
{
  ...
  //
  // Check that this attribute is not already defined for this device. If it is
  // already there, immediately returns. Trick : If you add an attribute to a
  // device, this attribute will be inserted in the device class attribute list.
  // Therefore, all devices created after this attribute addition will also have
  // this attribute.
  //
  ...
  //
  // If device is IDL 5 or more and if enabled, and if there is some client(s)
  // listening on the device interface change event, get device interface.
  //
  ...
  //
  // Add this attribute in the MultiClassAttribute attr_list vector if it does
  // not already exist
  //
  ...
  else
  {
    ...
    if((attr_list[i]->get_type() != new_attr->get_type()) || (attr_list[i]->get_format() != new_attr->get_format()) ||
...
void DeviceImpl::remove_attribute(Tango::Attr *rem_attr, bool free_it, bool clean_db)
{
  ...
  if(ite_attr != poll_attr.end())
  {
    ...
    {
      //
      // There is no need to stop the polling because we are in the server
      // shutdown sequence and the polling is already stopped.
      //

      if(clean_db == true && Tango::Util::_UseDb == true)
      {
        //
        // Memorize the fact that the dynamic polling properties has to be
        // removed from db. The classical attribute properties as well
        //
  ...
  //
  // Now remove all configured attribute properties from the database. Do it in
  // one go if the Db server support this
  //
  ...
  //
  // Remove attribute in MultiClassAttribute in case there is only one device in
  // the class or it is the last device in this class with this attribute
  //
...
//			- device_level : flag set to true if the command must be added at the
// device level (instead of class level)
//
//--------------------------------------------------------------------------------------------------------------------
...
void DeviceImpl::add_command(Tango::Command *new_cmd, bool device_level)
{
  ...
  //
  // Check that this command is not already defined for this device. If it is
  // already there, immediately returns.
  //
  ...
  if(already_there == false)
  {
    ...
    try
    {
      ...
      if((al_cmd_dev.get_in_type() != new_cmd->get_in_type()) || (al_cmd_dev.get_out_type() != new_cmd->get_out_type()))
      {
        throw_ex = true;
      }
    }
  ...
  //
  // Throw exception if the device already have a command with the same name but
...
{
  //
  // Search for command first at class level and then at device level (for
  // dynamic cmd)
  //
...
//
// description :
//		Started from Tango V5, state and status are polled as attributes.
// Previously, they were polled as
// commands. 		If state or status are polled as commands, move them to the list
// of polled attributes
//
//------------------------------------------------------------------------------------------------------------------
...
void DeviceImpl::poll_lists_2_v5()
{
  ...
  //
  // Try to find state in list of polled command(s). If found, remove it from
  // poll cmd and move it to poll attr
  //
...
//
// description :
//		Write the command name to the list of polled commands in the database. The
// polling period is set to 0 to 		indicate that the polling buffer is filled
// externally from the device server code.
//
// args :
//		in :
//			- cmd_name : The command name
...
void DeviceImpl::init_cmd_poll_ext_trig(const std::string &cmd_name)
{
  ...
  //
  // never do the for the state or status commands, they are handled as
  // attributes!
  //
...
//
// description :
//		Checks the specified polling period for all commands of the device. If a
// polling period is specified for
// a 		command the command name and the period are written to the list of
// polled commands in the database. This happens only if the command is not yet
// in the list of polled commands.
//
//--------------------------------------------------------------------------------------------------------------------
...
void DeviceImpl::init_cmd_poll_period()
{
  ...
  if(tg->_UseDb == true)
  {
    ...
    for(i = 0; i < cmd_list.size(); i++)
    {
      ...
      //
      // check the validity of the polling period. must be longer than min
      // polling period
      //
...
// The polling period is set to
// 0 		to indicate that the polling buffer is filled externally from the device
// server code.
//
// args :
//		in :
...
//
// description :
//		Checks the specified polling period for all attributes of the device. If a
// polling period is specified for
// an 		attribute the attribute name and the period are written to the list of
// polled attributes in the database. 		This happens only if the attribute is
// not yet in the list of polled attributes.
//
//-------------------------------------------------------------------------------------------------------------------
...
void DeviceImpl::init_attr_poll_period()
{
  ...
  if(tg->_UseDb == true)
  {
    ...
    for(i = 0; i < attr_list.size(); i++)
    {
      ...
      //
      // Special case for state and status attributes. They are polled as
      // attribute but they are managed by Pogo as commands (historical
      // reasons). If the polling is set in the state or status defined as
      // command, report this info when they are defined as attributes
      //
    ...
    //
    // Another loop to correctly initialize polling period data in Attribute
    // instance
    //
...
//
// description :
//		Get client identification. This method returns a pointer to the client
// identification
//
//-----------------------------------------------------------------------------------------------------------------
...
{
  //
  // Check if the device is already locked and if it is a valid lock. If the
  // lock is not valid any more, clear it
  //
...
{
  //
  // Check if the device is already locked and if it is a valid lock. A ReLock
  // is valid only if the device is already locked by the same client and if
  // this lock is valid
  //
...
void DeviceImpl::basic_unlock(bool forced)
{
  ...
  //
  // Also unlock root device(s) in case it is needed (due to forwarded
  // attributes)
  //
...
//
// description :
//		Check lock validity (according to lock validity time). This method returns
// true if the lock is still
...
//		This method returns a sequence with longs and strings. The strings
// contain: 			1 - The locker process hostname 			2 - The java main class
// (in case of Java locker) 			3 - A string which summarizes the locking
// status 		The longs contain:
...
//
// description :
//		Method called for each command_inout operation executed from any client on
// a Tango device.
//
// argument:
//		in :
...
void DeviceImpl::check_lock(const char *meth, const char *cmd)
{
  if(device_locked == true)
  {
    if(valid_lock() == true)
    {
      ...
      {
        //
        // Old client, before throwing the exception, in case the CORBA
        // operation is a command_inout, checks if the command is an "allowed"
        // one
        //
      ...
      {
        //
        // Wrong client, before throwing the exception, in case the CORBA
        // operation is a command_inout, checks if the command is an "allowed"
        // one
        //
...
//
// description :
//		Put the attribute data within the object used on the wire to transfer the
// attribute. For IDL release <=
// 3, 		it's an Any object. Then, it is an IDL union
//
// argument:
//		in :
//			- att :
...
void DeviceImpl::data_into_net_object(Attribute &att, AttributeIdlData &aid, long index, AttrWriteType w_type,
                                      bool del_seq)
{
  //
  ...
  case Tango::DEV_ENCODED:
  {
    if(aid.data_3 != nullptr)
    {
      ...
      (*aid.data_3)[index].err_list[0].desc     = Tango::string_dup("The DevEncoded data type is available only for "
                                                                        "device implementing IDL 4 and above");
    ...
    else
    {
      ...
      if(aid.data_5 != nullptr)
      {
        ...
        {
          the_seq[0].encoded_data.replace((*ptr)[0].encoded_data.length(), (*ptr)[0].encoded_data.length(),
        ...
        if((w_type == Tango::READ_WRITE) || (w_type == Tango::READ_WITH_WRITE))
        {
          ...
          the_seq[1].encoded_data.replace((*ptr)[1].encoded_data.length(), (*ptr)[1].encoded_data.length(),
...
void DeviceImpl::polled_data_into_net_object(AttributeIdlData &aid, long index, long type, long vers,
                                             PollObj *polled_att, const DevVarStringArray &names)
{
  ...
  case Tango::DEV_STATE:
    ...
    else
    {
      if(vers >= 5)
      {
        ...
        {
          DevVarStateArray &union_seq = att_val.value.state_att_value();
          new_tmp_state               = new DevVarStateArray(union_seq.length(), union_seq.length(),
                                                             const_cast<DevState *>(union_seq.get_buffer()), false);
  ...
  case Tango::DEV_ENCODED:
    ...
    else
    {
      ...
      o << " It's not possible to retrieve this data type through the "
           "interface you are using (IDL V3)"
        << std::ends;
...
//
// description :
//			Set flags in DeviceImpl if any of the device attributes has some wrong
// configuration in DB
// generating 			startup exception when the server started. 			In
// DeviceImpl class, this method set the force_alarm_state flag and fills in the
...
void DeviceImpl::att_conf_loop()
{
  ...
  //
  // Run the loop for wrong attribute conf. or memorized att which failed at
  // startup
  //
  ...
  if(att_type == Tango::DeviceImpl::FWD)
  {
    for(size_t i = 0; i < nb_att; ++i)
    {
      ...
      case FWD_ROOT_DEV_NOT_STARTED:
        alarm_status = alarm_status + "Root device not started yet. Polling (if any) for "
      ...
      {
        alarm_status = alarm_status + "Root attribute already used in this "
                                      "device server process for attribute ";
...
//
// description :
//		Remove one forwarded attribute from the list of errored forwarded
// attribute
//
// argument:
//		in :
...
//
// description :
//		Update error code for one forwarded attribute in the list of errored
// forwarded attribute
//
// argument:
//		in :
...
//
// description :
//		Lock/Unlock all root devices for all the forwarded attributes defined for
// this device
//
// argument:
//		in :
//			- validity : The lock validity interval (used only in case of locking)
//			- lock_action : Flag set to true if root device(s) must be locked. If
// false, root devices will be unlocked
//
//---------------------------------------------------------------------------------------------------------------------
...
// argument :
// 		in :
//			- ev_client : Flag set to true if some clients are listening on the
// event
//
//------------------------------------------------------------------------------------------------------------------
...
{
  //
  // If device is IDL 5 or more and if enabled, push a device interface change
  // event but only if there is some client(s) listening on the event. This is
  // done by starting a dedicated thread (if not already started). The rule of
  // this thread is to delayed the event in case of attributes/commands
  // added/removed in a loop in order to minimize the event number.
  //
...
//
// description :
//		Get all pipe properties defined at device level and aggregate all the pipe
// properties defined at
// different 		level (device, user default, class default
//
//------------------------------------------------------------------------------------------------------------------
...
void DeviceImpl::end_pipe_config()
{
  ...
  if(nb_pipe != 0)
  {
    ...
    if(tg->_UseDb == true)
    {
      ...
      //
      // On some small and old computers, this request could take time if at the
      // same time some other processes also access the device pipe properties
      // table. This has been experimented at ESRF. Increase timeout to cover
      // this case
      //
      ...
      {
        //
        // If pipe has some properties defined at device level, build a vector
        // of PipeProperty with them
        //
        ...
        //
        // Call method which will aggregate prop definition retrieved at
        // different levels
        //
...
//
// description :
//		Set a pipe property. A pipe property can be defined at device level in DB,
// by a user default or at 		class level in DB. Properties defined in DB at
// device
// level are given to the method as the first 		parameter. If the user has
// defined some user default, the pipe object already has them. The properties
// defined at class level are available in the MultiClassPipe object available
// in the DeviceClass instance
//
// argument :
//		in:
...
void DeviceImpl::set_pipe_prop(std::vector<PipeProperty> &dev_prop, Pipe *pi_ptr, PipePropType ppt)
{
  ...
  // - User default
  // - Class pipe
  // The pipe instance we have here already has config set to user default (if
  // any)
  //
  ...
  {
    //
    // Prop not defined at device level. If the prop is still the lib default
    // one, search if it is defined at class level
    //
