...
public:
  ...
  SendEventType detect_and_push_events(DeviceImpl *, struct SuppliedEventData &, DevFailed *, std::string &,
                                       PollClock::time_point);

  //------------------ Change event ---------------------------

  bool detect_change(Attribute &, struct SuppliedEventData &, bool, double &, double &, DevFailed *, bool &,
                     DeviceImpl *);

  //------------------ Detect, push change event --------------

  bool detect_and_push_change_event(DeviceImpl *, struct SuppliedEventData &, Attribute &, std::string &, DevFailed *,
                                    bool user_push = false);

  //------------------ Detect, push archive event --------------

  bool detect_and_push_archive_event(DeviceImpl *, struct SuppliedEventData &, Attribute &, std::string &, DevFailed *,
                                     PollClock::time_point, bool user_push = false);

  //------------------ Detect, push periodic event -------------

  bool detect_and_push_periodic_event(DeviceImpl *, struct SuppliedEventData &, Attribute &, std::string &, DevFailed *,
                                      PollClock::time_point);

  //------------------ Push event -------------------------------

  virtual void push_event(DeviceImpl *, std::string, const std::vector<std::string> &, const std::vector<double> &,
                          const std::vector<std::string> &, const std::vector<long> &, const struct SuppliedEventData &,
                          const std::string &, DevFailed *, bool)                          = 0;
  virtual void push_event_loop(DeviceImpl *, EventType, const std::vector<std::string> &, const std::vector<double> &,
                               const std::vector<std::string> &, const std::vector<long> &,
                               const struct SuppliedEventData &, Attribute &, DevFailed *) = 0;
  virtual void push_heartbeat_event()                                                      = 0;

  //------------------- Attribute conf change event ---------------------

  void push_att_conf_events(DeviceImpl *device_impl, const SuppliedEventData &, DevFailed *, const std::string &);
  ...
  void convert_att_event_to_5(const struct SuppliedEventData &, struct SuppliedEventData &, bool &, Attribute &);
  void convert_att_event_to_4(const struct SuppliedEventData &, struct SuppliedEventData &, bool &, Attribute &);
  void convert_att_event_to_3(const struct SuppliedEventData &, struct SuppliedEventData &, bool &, Attribute &);
...
public:
  ...
  virtual void push_event(DeviceImpl *, std::string, const std::vector<std::string> &, const std::vector<double> &,
                          const std::vector<std::string> &, const std::vector<long> &, const struct SuppliedEventData &,
                          const std::string &, DevFailed *, bool);

  virtual void push_event_loop(DeviceImpl *, EventType, const std::vector<std::string> &, const std::vector<double> &,
                               const std::vector<std::string> &, const std::vector<long> &,
                               const struct SuppliedEventData &, Attribute &, DevFailed *)
  {
  }

protected:
  NotifdEventSupplier(CORBA::ORB_var, CosNotifyChannelAdmin::SupplierAdmin_var, CosNotifyChannelAdmin::ProxyID,
                      CosNotifyChannelAdmin::ProxyConsumer_var, CosNotifyChannelAdmin::StructuredProxyPushConsumer_var,
                      CosNotifyChannelAdmin::EventChannelFactory_var, CosNotifyChannelAdmin::EventChannel_var,
                      std::string &, Util *);
...
public:
  ...
  std::string create_full_event_name(DeviceImpl *device_impl, const std::string &event_type,
...
private:
  ...
  std::vector<std::string> alternate_h_endpoint; // Alternate heartbeat endpoint
                                                 // (host with several NIC)
