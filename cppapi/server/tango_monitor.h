...
//
// description :
//		This class is used to synchronise device access between polling thread and
// CORBA request. It is used only for 		the command_inout and read_attribute
// calls
//
//--------------------------------------------------------------------------------------------------------------------
...
// already locked. If the thread is
// already 		the monitor owner thread, simply increment the locking counter
//
//--------------------------------------------------------------------------------------------------------------------
  ...
  else if(th != locking_thread)
  {
    while(locked_ctr > 0)
    {
      ...
      if(interupted == false)
      {
        ...
        TANGO_THROW_EXCEPTION(API_CommandTimedOut, "Not able to acquire serialization (dev, class "
                                                   "or process) monitor");
      }
    }
    locking_thread = th;
...
//
// description :
//		Release a monitor if the caller thread is the locking thread. Signal other
// threads if the locking
