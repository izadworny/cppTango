...
namespace Tango
{
TangoAppender::TangoAppender(const std::string &src_name, const std::string &name, const std::string &dev_name,
...
TangoAppender::~TangoAppender() { close(); }

bool TangoAppender::requires_layout(void) const { return false; }
...
int TangoAppender::_append(const log4tango::LoggingEvent &event)
{
  ...
  try
  {
    ...
    if(dvsa)
    {
      ...
      ts_ms_str << std::fixed << std::noshowpoint << std::setprecision(0) << ts_ms << std::ends;
