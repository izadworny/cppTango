...
// singleton classes -
// EventSupplier 						This class is used to send events from the server
//
...
{
  //
  // Specify the precision interval for the event period testing 2% are used for
  // periods < 5000 ms and 100ms are used for periods > 5000 ms.
  //
...
SendEventType EventSupplier::detect_and_push_events(DeviceImpl *device_impl, struct SuppliedEventData &attr_value,
                                                    DevFailed *except, std::string &attr_name,
                                                    PollClock::time_point time_bef_attr)
{
  ...
  cout3 << "EventSupplier::detect_and_push_events(): last subscription for "
           "change5 "
        << change5_subscription << " periodic5 " << periodic5_subscription << " archive5 " << archive5_subscription
        << std::endl;

  //
  // For change event
  // First check if it is necessary to send the event in case clients are not
  // there any more
  //
...
bool EventSupplier::detect_and_push_change_event(DeviceImpl *device_impl, struct SuppliedEventData &attr_value,
                                                 Attribute &attr, std::string &attr_name, DevFailed *except,
                                                 TANGO_UNUSED(bool user_push))
{
  ...
  //
  // if no attribute of this name is registered with change then insert the
  // current value
  //

  if(attr.prev_change_event.inited == false)
  {
    attr.prev_change_event.store(attr_value.attr_val_5, attr_value.attr_val_4, attr_value.attr_val_3,
                                 attr_value.attr_val, except);
    is_change = true;
  }
  else
  {
    ...
    is_change =
        detect_change(attr, attr_value, false, delta_change_rel, delta_change_abs, except, force_change, device_impl);
  ...
  if(is_change)
  {
    ...
    for(ite = client_libs.begin(); ite != client_libs.end(); ++ite)
    {
      ...
      push_event(device_impl, ev_name, filterable_names, filterable_data, filterable_names_lg, filterable_data_lg,
                 sent_value, attr_name, except, inc_ctr);
...
bool EventSupplier::detect_and_push_archive_event(DeviceImpl *device_impl, SuppliedEventData &attr_value,
                                                  Attribute &attr, std::string &attr_name, DevFailed *except,
                                                  PollClock::time_point time_bef_attr, TANGO_UNUSED(bool user_push))
{
  std::string event, domain_name;
  double delta_change_rel = 0.0;
  double delta_change_abs = 0.0;
  ...
  //
  // Do not get time now. This method is executed after the attribute has been
  // read. For some device, reading one attribute could be long and even worse
  // could have an unstable reading time. If we takes time now, it will also be
  // unstable. Use the time taken in the polling thread before the attribute was
  // read. This one is much more stable
  //
  ...
  //
  // If the attribute archive period is INT_MAX, this means that the user does
  // not want the periodic part of the archive event
  //
  ...
  {
    attr.prev_archive_event.store(attr_value.attr_val_5, attr_value.attr_val_4, attr_value.attr_val_3,
                                  attr_value.attr_val, except);
  ...
  else
  {
    ...
    {
      is_change =
          detect_change(attr, attr_value, true, delta_change_rel, delta_change_abs, except, force_change, device_impl);
    }
  }
  ...
  if(except == NULL && attr.prev_archive_event.quality != the_quality)
  {
    is_change      = true;
    quality_change = true;
  }
                                                   ...
                                                   PollClock::time_point time_bef_attr)
{
  ...
  //
  // Do not get time now. This metthod is executed after the attribute has been
  // read. For some device, reading one attribute could be long and even worse
  // could have an unstabe reading time. If we takes time now, it will also be
  // unstable. Use the time taken inthe polling thread befor the attribute was
  // read. This one is much more stable
  //
  ...
  if(ms_since_last_periodic > eve_period)
  {
    ...
    cout3 << "EventSupplier::detect_and_push_is_periodic_event(): detected "
             "periodic event for "
...
//
// description :
//		Method to detect if there is a change according to the criterions and
// return a boolean set to true if a
// change 		is detected
//
// argument :
//		in :
...
//			- except : The exception thrown during the last attribute reading. NULL
// if no exception
//			- force_change : A flag set to true if the change is due to a non
// mathematical reason 				       (array size change, from exception to
// classic...)
                                  ...
                                  bool &force_change, DeviceImpl *dev)
{
  ...
  //
  // Send event, if the read_attribute failed or if it is the first time that
  // the read_attribute succeed after a failure. Same thing if the attribute
  // quality factor changes to INVALID
  //

  if(archive == true)
  {
    //
    // force an event only when the last reading was not returning an exception
    // or not returning the same exception
    //
    ...
    //
    // force an archive event when the last reading was still returning an
    // exception
    //
    ...
    //
    // check wether the quality is invalid. Force an event only if the last
    // reading was valid
    //
    ...
    //
    // force an archive event when the last reding was still marked as invalid
    // data
    //
  ...
  else
  {
    ...
    //
    // force an change event when the last reding was still returning an
    // exception
    //
    ...
    //
    // force an change event when the last reding was still marked as invalid
    // data
    //
        ...
        if(double_type == true)
        {
          ...
          for(i = 0; i < curr_seq_db->length(); i++)
          {
            ...
            if(abs_change[0] != INT_MAX)
            {
              ...
              // if (delta_change_abs <= abs_change[0] || delta_change_abs >=
              // abs_change[1])
...
void EventSupplier::push_att_data_ready_event(DeviceImpl *device_impl, const std::string &attr_name, long data_type,
                                              DevLong ctr)
{
  ...
  push_event(device_impl, ev_type, filterable_names, filterable_data, filterable_names_lg, filterable_data_lg, ad,
             const_cast<std::string &>(attr_name), NULL, true);
}
...
void EventSupplier::push_att_conf_events(DeviceImpl *device_impl, const SuppliedEventData &attr_conf, DevFailed *except,
                                         const std::string &attr_name)
{
  ...
  //
  // Return if there is no client or if the last client subscription is more
  // than 10 mins ago
  //
  ...
  cout3 << "EventSupplier::push_att_conf_events(): delta since last subscription " << att_conf_subscription
        << std::endl;
  ...
  push_event(device_impl, ev_type, filterable_names, filterable_data, filterable_names_lg, filterable_data_lg,
             attr_conf, attr_name, except, true);
}
...
void EventSupplier::push_dev_intr_change_event(DeviceImpl *device_impl, bool dev_start, DevCmdInfoList_2 *cmds_list,
...
void EventSupplier::convert_att_event_to_5(const struct EventSupplier::SuppliedEventData &attr_value,
                                           struct EventSupplier::SuppliedEventData &sent_value, bool &need_free,
                                           Attribute &attr)
{
