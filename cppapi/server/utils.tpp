...
void Util::fill_attr_polling_buffer(DeviceImpl *dev, std::string &att_name, AttrHistoryStack<T> &data)
{
  ...
  if((att.get_data_type() == DEV_ENCODED) && (att.get_data_format() != Tango::SCALAR) && (w_type == Tango::READ_WRITE))
  {
    for(size_t i = 0; i < nb_elt; i++)
    {
      if((data.get_data())[i].wr_ptr != NULL)
  ...
  for(i = 0; i < nb_elt; i++)
  {
    ...
    else
    {
         ...
         (qu == Tango::ATTR_CHANGING))
      {
        ...
        att.set_value((T *) (data.get_data())[i].ptr, (data.get_data())[i].x, (data.get_data())[i].y,
        ...
        else if(w_type == Tango::READ_WRITE)
        {
          if((data.get_data())[i].wr_ptr != NULL)
          {
            w_att_ptr->set_write_value((T *) (data.get_data())[i].wr_ptr, (data.get_data())[i].wr_x,
