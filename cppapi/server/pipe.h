...
public:
   ...
   *
   * @param [in] ser_model The new serialisation model. The serialization model
   * must be one of PIPE_BY_KERNEL, PIPE_BY_USER or PIPE_NO_SYNC
   */
...
//@{
   ...
   *
   * Inserting data into a Pipe instance is simlar to inserting data into a
   * DevicePipeBlob class instance. See doc of DevicePipeBlob class insertion
   * methods (DevicePipeBlob::operator<<) to get a complete documentation on how
   * to insert data into a Pipe object
   *
   ...
   *when trying to insert/extract data from a Pipe object. The following flags
   *are supported :
   ...
   *
   * Allow the user to check if insertion/extraction into/from Pipe instance was
   ...
   * into/from Pipe failure. This method has to be used when exceptions are
   * disabled. Here is an example of how methods has_failed() and state() could
   * be used
   * @code
   * Pipe dpb = ....
   *
  ...
  /// @privatesection
  virtual bool is_allowed(DeviceImpl *dev, PipeReqType)
  {
    (void) dev;
    return true;
  }
  ...
  void set_one_str_prop(const char *, const CORBA::String_member &, std::string &, std::vector<Attribute::AttPropDb> &,
                        std::vector<PipeProperty> &, std::vector<PipeProperty> &, const char *);
...
inline bool Pipe::prop_in_list(const char *prop_name, std::string &prop_str, size_t list_size,
                               std::vector<PipeProperty> &list)
{
  bool ret = false;
