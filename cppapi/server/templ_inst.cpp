...
// file :               templ_inst.cpp
//
// description :        C++ source code to instantiate template methods/class in
// order to have explicit instanciation
//						This makes user life easier in term of getting bug fixes (no
...
// - A file with template methods/class definition (attribute.tpp for Attribute
// class)
// - A file with template methods/class specialization definition
// (attribute_spec.tpp for Attribute class)
//
// We did this in order to have explicit instanciation of templates method
// except for specialized templates for which we have instanciation following
// the inclusion model See C++ template book chapter 6
//
...
//
// The DevEncoded and DevString data types are not there. DevString is handle by
// 2 (for const) classic methods forwarding the call to the same method using
// string data type as parameter. DevEncoded and string are managed as
...
//+----------------------------------------------------------------------------
//
// Instanciate Util::fill_XXX_polling buffers methods and associate template
// classes
//
//-----------------------------------------------------------------------------
...
template void WAttribute::check_min_max(const unsigned int, const DevVarLong64Array &, const DevLong64 &,
                                        const DevLong64 &);
template void WAttribute::check_min_max(const unsigned int, const DevVarUShortArray &, const unsigned short &,
                                        const unsigned short &);
template void WAttribute::check_min_max(const unsigned int, const DevVarCharArray &, const unsigned char &,
                                        const unsigned char &);
template void WAttribute::check_min_max(const unsigned int, const DevVarULongArray &, const DevULong &,
                                        const DevULong &);
template void WAttribute::check_min_max(const unsigned int, const DevVarULong64Array &, const DevULong64 &,
                                        const DevULong64 &);
template void WAttribute::check_min_max(const unsigned int, const DevVarStateArray &, const DevState &,
                                        const DevState &);
...
//
// Note that there is no instanciation for type DevString. For this type, a  non
// template method has been written Otherwise, due the partial specialization
// for pointer types, it was not possible to use extracttion into DevString type
//
...
template DevicePipe &operator>>(DevicePipe &, DataElement<std::vector<std::string>> &);
template DevicePipe &operator>>(DevicePipe &, DataElement<std::vector<DevState>> &);
// template DevicePipe &operator>>(DevicePipe
// &,DataElement<std::vector<DevEncoded> > &);
...
// template DevicePipeBlob &operator<<(DevicePipeBlob
// &,DataElement<std::vector<DevEncoded> > &);
...
// template DevicePipeBlob &operator<<(DevicePipeBlob
// &,DataElement<DevVarEncodedArray &> &);
