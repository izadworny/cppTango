...
PollRing::PollRing(long max_size)
    : ring(max_size)
{
  insert_elt = 0;
  nb_elt     = 0;
...
void PollRing::get_cmd_history(long n, Tango::DevCmdHistory_4 *ptr, Tango::CmdArgType &cmd_type)
{
  ...
  if((cmd_type <= DEV_STRING) || (cmd_type == DEV_STATE) || (cmd_type == CONST_DEV_STRING) || (cmd_type == DEV_UCHAR) ||
     (cmd_type == DEV_LONG64) || (cmd_type == DEV_ULONG64) || (cmd_type == DEV_ENCODED))
  {
    for(i = 0; i < n; i++)
    {
      if(ring[index].except != NULL)
  ...
  for(i = 0; i < n; i++)
  {
    ...
    if(ring[index].except != NULL)
    {
      ...
      if((last_dim.dim_x == 0) && (last_dim.dim_y == 0))
      {
    ...
    if(ring[index].except == NULL)
    {
      ...
      if((*ptr)[seq_index].value.quality != Tango::ATTR_INVALID)
      {
        ...
        case Tango::DEV_ENUM:
          ...
          new_tmp_sh = new DevVarShortArray(tmp_sh->length(), tmp_sh->length(),
                                            const_cast<short *>(tmp_sh->get_buffer()), false);
          (*ptr)[seq_index].value.value <<= new_tmp_sh;
          break;

        case Tango::DEV_DOUBLE:
          (*ring[index].attr_value)[0].value >>= tmp_db;
          new_tmp_db = new DevVarDoubleArray(tmp_db->length(), tmp_db->length(),
                                             const_cast<double *>(tmp_db->get_buffer()), false);
        ...
        case Tango::DEV_STRING:
          ...
          new_tmp_str = new DevVarStringArray(tmp_str->length(), tmp_str->length(),
                                              const_cast<char **>(tmp_str->get_buffer()), false);
          (*ptr)[seq_index].value.value <<= new_tmp_str;
          break;

        case Tango::DEV_FLOAT:
          (*ring[index].attr_value)[0].value >>= tmp_fl;
          new_tmp_fl = new DevVarFloatArray(tmp_fl->length(), tmp_fl->length(),
                                            const_cast<float *>(tmp_fl->get_buffer()), false);
        ...
        case Tango::DEV_UCHAR:
          ...
          new_tmp_uch = new DevVarCharArray(tmp_uch->length(), tmp_uch->length(),
                                            const_cast<DevUChar *>(tmp_uch->get_buffer()), false);
