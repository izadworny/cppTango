...
//
// description :        C++ source code for the DevIntrThread class. This class
// is used for the device interface
//						change thread. The rule of this thread is to delay the event
// sending in order to
// minimize 						the number of events thrown when dynamic command(s)
// and/or attribute(s) are added/removed 						in a loop.
//
// project :            TANGO
//
// author(s) :          E.Taurel
...
void DevIntrThread::run(TANGO_UNUSED(void *ptr))
{
  ...
  try
  {
    while(exit == false)
    {
      ...
      case DEV_INTR_TIME_OUT:
        ...
        {
          std::cerr << "Received a DevFailed exception while trying to push "
                       "the device interface change event!!!"
                    << std::endl;
  ...
  {
    std::cerr << "OUPS !! A omni thread fatal exception in a device interface "
                 "change event thread!!!!!!!!"
              << std::endl;
  }
}
...
// thread. 		The thread waits with a timeout.
//
// argument :
//		in :
...
//
// return :
//		One enum with two possible value: Thread awaken due to timeout or thread
// awaken due to a received
// command
//
//------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		This method pushes the device interface change event if there is a
// interface change
//
//------------------------------------------------------------------------------------------------------------------
