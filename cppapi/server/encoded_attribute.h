...
public:
   ...
   *
   * This constructor allows the user to define the size of the buffer pool used
   * to store the encoded images. This buffer pool is managed as a circular
   * pool. A different buffer is used each time an image is encoded. The last
   * used buffer is then passed to the attribute with the attribute::set_value()
   * method
   *
   * @param buf_pool_size    Buffer pool size
   * @param serialization	   Set to true if the instance manage the data buffer
   * serialization
   *
   */
  ...
  /**
   * Decode a 8 bits grayscale image (JPEG_GRAY8 or GRAY8) and returns a 8 bits
   * gray scale image. Throws DevFailed in case of failure.
   *
   * @param attr     DeviceAttribute that contains the image
   * @param width    Width of the image
   * @param height   Height of the image
  ...
  /**
   * Decode a 16 bits grayscale image (GRAY16) and returns a 16 bits gray scale
   * image. Throws DevFailed in case of failure.
   *
   * @param attr     DeviceAttribute that contains the image
   * @param width    Width of the image
   * @param height   Height of the image
...
private:
  ...
  // ----------------------------------------------------------------------------
  void jpeg_encode_rgb32(int width, int height, unsigned char *rgb32, double quality, std::size_t *jpegSize,
                         unsigned char **jpegData);

  void jpeg_encode_rgb24(int width, int height, unsigned char *rgb24, double quality, std::size_t *jpegSize,
                         unsigned char **jpegData);

  void jpeg_encode_gray8(int width, int height, unsigned char *gray8, double quality, std::size_t *jpegSize,
                         unsigned char **jpegData);
  ...
  // the caller.
  // ----------------------------------------------------------------------------
  void jpeg_decode(std::size_t jpegSize, unsigned char *jpegData, int *width, int *height, int *format,
                   unsigned char *&frame);
};
