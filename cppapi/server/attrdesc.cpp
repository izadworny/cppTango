...
// file :               attrdesc.cpp
//
// description :        C++ source code for the Attr, SpectrumAttr and ImageAttr
// classes. Those classes are used
//						by DS devcelopper to define their attributes (Scalar, Spectrum and
// Image)
//
// project :            TANGO
//
...
//
// description :
//		Constructor for the Attr class. This constructor simply set the internal
// values
//
//------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		This method checks data type and throws an exception in case of
...
void Attr::check_type()
{
  ...
  if(type == Tango::DEV_SHORT || type == Tango::DEV_LONG || type == Tango::DEV_LONG64 || type == Tango::DEV_DOUBLE ||
     type == Tango::DEV_STRING || type == Tango::DEV_FLOAT || type == Tango::DEV_BOOLEAN || type == Tango::DEV_USHORT ||
     type == Tango::DEV_UCHAR || type == Tango::DEV_ULONG || type == Tango::DEV_ULONG64 || type == Tango::DEV_STATE ||
     type == Tango::DEV_ENCODED || type == Tango::DEV_ENUM)
    unsuported = false;
...
//
// description :
//		This method set the default user properties in the Attr object. At this
// level, each attribute property 		is represented by one instance of the
// Attrproperty class.
//
// arguments :
//		in :
...
void Attr::set_default_properties(UserDefaultAttrProp &prop_list)
{
  ...
  if(prop_list.min_warning.empty() == false && (TG_strcasecmp(prop_list.min_warning.c_str(), AlrmValueNotSpec) != 0) &&
  ...
  if(prop_list.max_warning.empty() == false && (TG_strcasecmp(prop_list.max_warning.c_str(), AlrmValueNotSpec) != 0) &&
  ...
  {
    std::string err_msg = "Just one of the user default properties : delta_val "
                          "or delta_t is set. Both or none of the "
...
//
// description :
//		This method set the attribute as memorized in database. This is allowed
// only for scalar attribute and
// for 		writable one
//
//------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Constructor for the SpectrumAttr class. This constructor simply set the
// internal values
//
//-------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Constructor for the ImageAttr class. This constructor simply set the
// internal values
//
//-------------------------------------------------------------------------------------------------------------------
...
ImageAttr::ImageAttr(const char *att_name, long att_type, Tango::AttrWriteType w_type, long x, long y, DispLevel level)
