...
NotifdEventSupplier::NotifdEventSupplier(
    CORBA::ORB_var _orb, CosNotifyChannelAdmin::SupplierAdmin_var _supplierAdmin,
    CosNotifyChannelAdmin::ProxyID _proxId, CosNotifyChannelAdmin::ProxyConsumer_var _proxyConsumer,
    ...
    CosNotifyChannelAdmin::EventChannel_var _eventChannel, std::string &event_ior, Util *tg)
    : EventSupplier(tg)
{
  orb_                        = _orb;
...
NotifdEventSupplier *NotifdEventSupplier::create(CORBA::ORB_var _orb, std::string server_name, Util *tg)
{
  ...
  //
  // does the NotifdEventSupplier singleton exist already ? if so simply return
  // it
  //
  ...
  NotifdEventSupplier *_event_supplier = new NotifdEventSupplier(_orb, ns.SupAdm, ns.pID, ns.ProCon, ns.StrProPush,
                                                                 ns.EveChaFac, ns.EveCha, ns.ec_ior, tg);
...
void NotifdEventSupplier::connect()
{
  ...
  {
    std::cerr << "Tango::NotifdEventSupplier::connect() caught "
...
{
  cout4 << "calling "
           "Tango::NotifdEventSupplier::disconnect_structured_push_supplier() \n";
}
...
void NotifdEventSupplier::connect_to_notifd(NotifService &ns, const CORBA::ORB_var &_orb,
                                            const std::string &server_name, Util *tg)
{
  CosNotifyChannelAdmin::EventChannelFactory_var _eventChannelFactory;
  ...
  if(Util::_FileDb == false)
  {
    ...
    {
      //
      // Impossible to connect to notifd. In case there is already an entry in
      // the db for this server event channel, clear it.
      //
      ...
      if(tg->is_svr_starting() == true)
      {
        ...
        //
        // There is a cout and a cerr here to have the message displayed on the
        // console AND sent to the logging system (cout is redirected to the
        // logging when compiled with -D_TANGO_LIB)
        //

        cout4 << "Failed to import EventChannelFactory " << factory_name << " from the Tango database" << std::endl;
      }
  ...
  else
  {
    ...
    catch(Tango::DevFailed &)
    {
      if(tg->is_svr_starting() == true)
      {
        cout4 << "Failed to import EventChannelFactory from the Device Server "
                 "property file"
              << std::endl;
        cout4 << "Notifd event will not be generated" << std::endl;
      }

      TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                                "Failed to import the EventChannelFactory from "
                                "the Device Server property file");
    }
...
//		omniORB::setClientConnectTimeout(NARROW_CLNT_TIMEOUT);
  ...
  catch(...)
  {
    ...
    //
    // Impossible to connect to notifd. In case there is already an entry in the
    // db for this server event channel, clear it.
    //
    ...
    //
    // There is a cout and a cerr here to have the message displayed on the
    // console AND sent to the logging system (cout is redirected to the logging
    // when compiled with -D_TANGO_LIB) Print these messages only during DS
    // startup sequence
    //

    if(tg->is_svr_starting() == true)
    {
      cout4 << "Failed to narrow the EventChannelFactory - Notifd events will "
               "not be generated (hint: start the notifd "
               "daemon on this host)"
            << std::endl;
    }

    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                              "Failed to narrow the EventChannelFactory, make sure the notifd "
  ...
  if(!channel_exported)
  {
    ...
    {
      std::cerr << "Failed to create event channel - events will not be "
                   "generated (hint: start the notifd daemon on "
                   "this host)"
                << std::endl;
      TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                                "Failed to create a new EventChannel, make sure the notifd process "
  ...
  {
    cout4 << "Tango::NotifdEventSupplier::create(): _narrow worked, use this "
             "event channel\n";
  ...
  if(CORBA::is_nil(_supplierAdmin))
  {
    ...
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                              "Failed to get the default supplier admin from "
                              "the notification daemon (hint: make sure "
  ...
  try
  {
    ...
    if(CORBA::is_nil(_proxyConsumer))
    {
      ...
      TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                                "Failed to obtain a Notification push consumer, make sure the notifd "
                                "process is running on this host");
    }
  }
  catch(const CosNotifyChannelAdmin::AdminLimitExceeded &)
  {
    std::cerr << "Failed to get push consumer from notification daemon - "
                 "events will not be generated (hint: start the "
                 "notifd daemon on this host)"
              << std::endl;
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                              "Failed to get push consumer from notification "
                              "daemon (hint: make sure the notifd "
                              "process is running on this host)");
  }
  ...
  {
    std::cerr << "Tango::NotifdEventSupplier::create() could not get "
...
void NotifdEventSupplier::push_heartbeat_event()
{
  ...
  cout3 << "NotifdEventSupplier::push_heartbeat_event(): delta time since last "
           "heartbeat "
        << delta_time << std::endl;
  ...
  if(delta_time >= 8)
  {
    ...
    cout3 << "NotifdEventSupplier::push_heartbeat_event(): detected heartbeat "
             "event for "
          << domain_name << std::endl;
    cout3 << "NotifdEventSupplier::push_heartbeat_event(): delta _time " << delta_time << std::endl;
    ...
    {
      cout3 << "NotifdEventSupplier::push_heartbeat_event() event channel "
               "disconnected !\n";
      fail = true;
    }
    catch(const CORBA::TRANSIENT &)
    {
      cout3 << "NotifdEventSupplier::push_heartbeat_event() caught a "
               "CORBA::TRANSIENT ! "
            << std::endl;
      fail = true;
    }
...
void NotifdEventSupplier::reconnect_notifd()
{
  ...
  {
    cout3 << "Can't reconnect.............." << std::endl;
  }
...
void NotifdEventSupplier::push_event(DeviceImpl *device_impl, std::string event_type,
                                     ...
                                     const struct SuppliedEventData &attr_value, const std::string &attr_name,
                                     DevFailed *except, TANGO_UNUSED(bool inc_cptr))
{
  CosNotification::StructuredEvent struct_event;
  std::string domain_name;

  cout3 << "NotifdEventSupplier::push_event(): called for attribute " << attr_name << std::endl;

  //
  // If we are called for IDL 5 (AttributeValue_5 or AttributeConfig_5), simply
  // return. IDL 5 is only for ZMQ
  //
  ...
  if(except == NULL)
  {
    ...
    else if(attr_value.attr_val_4 != NULL)
    {
      ...
      // Clean the mutex ptr in the structure inserted in the Any to prevent
      // this bad behavior
      //
  ...
  cout3 << "EventSupplier::push_event(): push event " << event_type << " for "
  ...
  {
    cout3 << "EventSupplier::push_event() caught a CORBA::TRANSIENT ! " << std::endl;
    fail = true;
  }
  catch(const CORBA::COMM_FAILURE &)
  {
    cout3 << "EventSupplier::push_event() caught a CORBA::COMM_FAILURE ! " << std::endl;
    fail = true;
  }
  catch(const CORBA::SystemException &)
  {
    cout3 << "EventSupplier::push_event() caught a CORBA::SystemException ! " << std::endl;
    fail = true;
  }
  catch(...)
  {
...
void NotifdEventSupplier::file_db_svr() { fqdn_prefix = fqdn_prefix + '#'; }
