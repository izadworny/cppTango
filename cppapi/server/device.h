...
public:
   ...
   *
   */
  DeviceImpl(DeviceClass *device_class, const std::string &dev_name, const std::string &desc, Tango::DevState dev_state,
  ...
  DeviceImpl(DeviceClass *device_class, const char *dev_name, const char *desc = "A TANGO device",
   ...
   * Get device status.
   *
   * Return the device dev_status field. This method does the same thing than
   * the default status_cmd method.
   *
   * @return Device status
   */
  std::string &get_status() { return device_status; }
   ...
   */
  void set_status(const std::string &new_status) { device_status = new_status; }
   ...
   *
   * Return the device dev_prev_state field. This method is used for the
...
public:
   ...
   * Hook method.
   *
   * Default method to implement an action necessary on a device after server
   ...
   * this vector
   * is the index in the device object attribute vector of an attribute to be
   * read.
   * @exception DevFailed This method does not throw exception but a
   * redefined method can.
   * Click <a
   ...
   * constructed in the DeviceClass::attribute_factory() method. Nevertheless,
   * it is still possible to add a new attribute to a device with this method.
   ...
   *
   * @param new_attr Pointer to the new attribute to be added to the list. This
   * pointer must point to "heap" allocated memory (or to static memory) and not
   * to "stack" allocated memory
   * @exception DevFailed
   * Click <a
   * href="https://tango-controls.readthedocs.io/en/latest/development/advanced/IDL.html#exceptions">here</a>
   * to read <b>DevFailed</b> exception specification
   ...
   * Attributes are normally
   * constructed in the DeviceClass::attribute_factory() method. Nevertheless,
   *it is still possible to add a new attribute to a device with the
   *DeviceImpl::add_attribute method. This remove_attribute method delete the
   *attribute from the device attribute list.
   *
   ...
   *         info if the attribute is polled) from database. Default value is
   *true
   * @exception DevFailed
   * Click <a
   *href="https://tango-controls.readthedocs.io/en/latest/development/advanced/IDL.html#exceptions">here</a>
   ...
   *
   * @param new_cmd Pointer to the new command to be added to the list. This
   * pointer must point to "heap" allocated memory (or to static memory) and not
   * to "stack" allocated memory
   * @param device Set this flag to true if the command must be added for only
   * this device Default is false (command added for the device class)
   * @exception DevFailed
   * Click <a
   * href="https://tango-controls.readthedocs.io/en/latest/development/advanced/IDL.html#exceptions">here</a>
   * to read <b>DevFailed</b> exception specification
   ...
   * Commands are normally
   * constructed in the DeviceClass::command_factory() method. Nevertheless, it
   * is still possible to add a new command to a device with the
   *DeviceImpl::add_command method. This remove_command method delete the
   *command from the device command list.
   *
   ...
   *
   * This method returns a boolean set to true if there are some subscriber(s)
   * listening on the event specified by the two method arguments. Be aware that
   * there is some delay (up to 600 sec) between this method returning false and
   * the last subscriber unsubscription or crash... The device interface change
   * event is not supported by this method.
   *
   * @param att_name The attribute name
   * @param event_type The event type
   * @return A boolean set to true if there are some subscriber listening on
   * this event
   ...
   *
   * Create a DevVarShortArray type data and return a pointer to it. The array
   ...
   *
   * Create a DevVarLong64Array type data and return a pointer to it. The array
   * is build using the input pointer with the given length
   *
   * @param ptr	Pointer to the basic type data buffer
   * @param length Number of element in the previous buffer
   ...
   *
   * Create a DevVarFloatArray type data and return a pointer to it. The array
   * is build using the input pointer with the given length
   *
   * @param ptr	Pointer to the basic type data buffer
   * @param length Number of element in  the previous buffer
   ...
   *
   * Create a DevVarDoubleArray type data and return a pointer to it. The array
   * is build using the input pointer with the given length
   *
   * @param ptr	Pointer to the basic type data buffer
   * @param length Number of element in  the previous buffer
   ...
   *
   * Create a DevVarUShortArray type data and return a pointer to it. The array
   * is build using the input pointer with the given length
   *
   * @param ptr	Pointer to the basic type data buffer
   * @param length Number of element in  the previous buffer
   ...
   *
   * Create a DevVarULongArray type data and return a pointer to it. The array
   * is build using the input pointer with the given length
   *
   * @param ptr	Pointer to the basic type data buffer
   * @param length Number of element in  the previous buffer
   ...
   *
   * Create a DevVarULong64Array type data and return a pointer to it. The array
   * is build using the input pointer with the given length
   *
   * @param ptr	Pointer to the basic type data buffer
   * @param length Number of element in the previous buffer
   ...
   *
   * Create a DevVarStringArray type data and return a pointer to it. The array
   * is build using the input pointer with the given length
   *
   * @param ptr	Pointer to the basic type data buffer
   * @param length Number of element in  the previous buffer
   ...
   * change events manually, without the polling to be started. If the detect
   ...
   * set to true. Default value is true.
   */
  void set_change_event(const std::string &attr_name, bool implemented, bool detect = true);

  /**
   * Push a change event for a state or status attribute or return an exception
   * as change event for any attribute. The event is pushed to the event system.
   *
   * The method needs the attribue name as input.
   * For the state and status attributes the actual state and status values are
   * pushed. In case of an exception, the exception is pushed as a change event
   * for the attribute.
   *
   * @param attr_name The name of the attribute
   * @param except Pointer to a Tango::DevFailed exception. Default value is
   * NULL.
   */
   ...
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   * be given. The time stamp of the event is set to the actual time and the
   * attribute quality is set to valid. The event is triggered with or without
   * checking of the change event criteria depending on the configuration
   * choosen with set_change_event().
   *
   * @param attr_name The name of the attribute
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevShort *p_data, long x = 1, long y = 0,
                         bool release = false);
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevLong *p_data, long x = 1, long y = 0,
                         bool release = false);

  /**
   * Push a change event for an attribute with Tango::DevLong64 attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevLong64 *p_data, long x = 1, long y = 0,
                         bool release = false);
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevFloat *p_data, long x = 1, long y = 0,
                         bool release = false);

  /**
   * Push a change event for an attribute with Tango::DevDouble attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevDouble *p_data, long x = 1, long y = 0,
                         bool release = false);

  /**
   * Push a change event for an attribute with Tango::DevString attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevString *p_data, long x = 1, long y = 0,
                         bool release = false);

  /**
   * Push a change event for an attribute with Tango::DevBoolean attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevBoolean *p_data, long x = 1, long y = 0,
                         bool release = false);

  /**void push_change_event (string attr_name, Tango::DevBoolea
   * Push a change event for an attribute with Tango::DevUShort attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevUShort *p_data, long x = 1, long y = 0,
                         bool release = false);
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevUChar *p_data, long x = 1, long y = 0,
                         bool release = false);
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevULong *p_data, long x = 1, long y = 0,
                         bool release = false);

  /**
   * Push a change event for an attribute with Tango::DevULong64 attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevULong64 *p_data, long x = 1, long y = 0,
                         bool release = false);
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevState *p_data, long x = 1, long y = 0,
                         bool release = false);

  /**
   * Push a change event for an attribute with Tango::DevEncoded attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevEncoded *p_data, long x = 1, long y = 0,
                         bool release = false);

  /**
   * Push a change event for an attribute with Tango::DevEncoded attribute data
   * type when the DevEncoded data are specified by two pointers. The event is
   * pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   * be given. The time stamp of the event is set to the actual time and the
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_change_event(const std::string &attr_name, Tango::DevString *p_str_data, Tango::DevUChar *p_data, long size,
                         bool release = false);
 ...
 * type. The event is pushed to the event system.
 *
 * The method needs the attribue name, a pointer to the data to be pushed, the
 * time stamp for the data and the attribute quality factor as input. Depending
 * on the attribute type the dimensions x and why need to be given. The event is
 * triggered with or without checking of the change event criteria depending on
 * the configuration choosen with set_change_event().
 *
 * @param attr_name The name of the attribute
 * @param p_data Pointer to the data to be pushed
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevShort *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevShort *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevLong *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevLong *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevLong64 *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevLong64 *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevFloat *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevFloat *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push a change event for an attribute with Tango::DevDouble attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevDouble *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevDouble *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push a change event for an attribute with Tango::DevString attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevString *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevString *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push a change event for an attribute with Tango::DevBoolean attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevBoolean *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevBoolean *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push a change event for an attribute with Tango::DevUShort attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevUShort *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevUShort *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push a change event for an attribute with Tango::DevUChar attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevUChar *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevUChar *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevULong *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevULong *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevULong64 *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevULong64 *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevState *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevState *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevEncoded *p_data, struct _timeb &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevEncoded *p_data, struct timeval &t,
                         Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif

/**
 * Push a change event for an attribute with Tango::DevEncoded attribute data
 * type when the data rea specified with two pointers. The event is pushed to
 * the event system.
 *
 * The method needs the attribue name, a pointer to the data to be pushed, the
 * time stamp for the data and the attribute quality factor as input. Depending
 ...
 */
#ifdef _TG_WINDOWS_
  void push_change_event(const std::string &attr_name, Tango::DevString *p_str_data, Tango::DevUChar *p_data, long size,
                         struct _timeb &t, Tango::AttrQuality qual, bool release = false);
#else
  void push_change_event(const std::string &attr_name, Tango::DevString *p_str_data, Tango::DevUChar *p_data, long size,
  ...
  /**
   * Set an implemented flag for the attribute to indicate that the server fires
   * archive events manually, without the polling to be started. If the detect
   * parameter is set to true, the criteria specified for the archive event are
   * verified and the event is only pushed if they are fulfilled. If detect is
   * set to false the event is fired without any value checking!
   *
   * @param attr_name The name of the attribute
  ...
  /**
   * Push an archive event for state or status attribute or push an exception as
   * archive event for any attribute. The event is pushed to the event system.
   *
   * The method needs the attribue name as input.
   * For the state and status attributes the actual state and status values are
   * pushed. In case of an exception, the exception is pushed as an archive
  ...
  /**
   * Push an archive event for an attribute with Tango::DevShort attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   * be given. The time stamp of the event is set to the actual time and the
   * attribute quality is set to valid. The event is triggered with or without
   * checking of the archive event criteria depending on the configuration
   * choosen with set_archive_event().
   *
   * @param attr_name The name of the attribute
   * @param p_data Pointer to the data to be pushed
   * @param x The attribute x length. Default value is 1
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevShort *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevLong attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevLong *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevLong64 attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevLong64 *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevFloat attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevFloat *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevDouble attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevDouble *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevString attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevString *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevBoolean attribute
   * data type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   * be given. The time stamp of the event is set to the actual time and the
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevBoolean *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevUShort attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevUShort *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevUChar attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevUChar *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevULong attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevULong *p_data, long x = 1, long y = 0,
                          bool release = false);
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevULong64 *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevState attribute data
   * type. The event is pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevState *p_data, long x = 1, long y = 0,
                          bool release = false);
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevEncoded *p_data, long x = 1, long y = 0,
                          bool release = false);

  /**
   * Push an archive event for an attribute with Tango::DevEncoded attribute
   * data type when the data are specified using two pointers. The event is
   * pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
   ...
   * to read <b>DevFailed</b> exception specification
   */
  void push_archive_event(const std::string &attr_name, Tango::DevString *p_str_data, Tango::DevUChar *p_data,
 ...
 * time stamp for the data and the attribute quality factor as input. Depending
 * on the attribute type the dimensions x and why need to be given. The event is
 * triggered with or without checking of the archive event criteria depending on
 * the configuration choosen with set_archive_event().
 *
 * @param attr_name The name of the attribute
 * @param p_data Pointer to the data to be pushed
 * @param t The time stamp
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevShort *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevShort *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevLong attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevLong *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevLong *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevLong64 attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevLong64 *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevLong64 *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevFloat attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevFloat *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevFloat *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevDouble attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevDouble *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevDouble *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevString attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevString *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevString *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevBoolean attribute data
 * type. The event is pushed to the event system.
 *
 * The method needs the attribue name, a pointer to the data to be pushed, the
 * time stamp for the data and the attribute quality factor as input. Depending
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevBoolean *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevBoolean *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevUShort attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevUShort *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevUShort *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevUChar attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevUChar *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevUChar *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevULong attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevULong *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevULong *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevULong64 attribute data
 * type. The event is pushed to the event system.
 *
 * The method needs the attribue name, a pointer to the data to be pushed, the
 * time stamp for the data and the attribute quality factor as input. Depending
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevULong64 *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevULong64 *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif
/**
 * Push an archive event for an attribute with Tango::DevState attribute data
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevState *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevState *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif

/**
 * Push an archive event for an attribute with Tango::DevEncoded attribute data
 * type. The event is pushed to the event system.
 *
 * The method needs the attribue name, a pointer to the data to be pushed, the
 * time stamp for the data and the attribute quality factor as input. Depending
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevEncoded *p_data, struct _timeb &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevEncoded *p_data, struct timeval &t,
                          Tango::AttrQuality qual, long x = 1, long y = 0, bool release = false);
#endif

/**
 * Push an archive event for an attribute with Tango::DevEncoded attribute data
 * type when it is specified using two pointers. The event is pushed to the
 * event system.
 *
 * The method needs the attribue name, a pointer to the data to be pushed, the
 * time stamp for the data and the attribute quality factor as input. Depending
 ...
 */
#ifdef _TG_WINDOWS_
  void push_archive_event(const std::string &attr_name, Tango::DevString *p_str_data, Tango::DevUChar *p_data,
                          long size, struct _timeb &t, Tango::AttrQuality qual, bool release = false);
#else
  void push_archive_event(const std::string &attr_name, Tango::DevString *p_str_data, Tango::DevUChar *p_data,
                          long size, struct timeval &t, Tango::AttrQuality qual, bool release = false);
#endif
  //@}
  ...
  //@{
  /**
   * Push a user event for a state or status attribute or return an exception as
   * user event for any attribute. The event is pushed to the event system.
   *
   * The method needs the attribue name as input.
   * For the state and status attributes the actual state and status values are
   * pushed. In case of an exception, the exception is pushed as a user event
   * for the attribute.
   *
   * @param attr_name The name of the attribute
   ...
   * NULL.
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, DevFailed *except = NULL);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevShort *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevLong *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevLong64 *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevFloat *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevDouble *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevString *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevBoolean *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevUShort *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevUChar *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevULong *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevULong64 *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevState *p_data, long x = 1, long y = 0,
                  bool release = false);
   ...
   */
  void push_event(const std::string &attr_name, const std::vector<std::string> &filt_names,
                  const std::vector<double> &filt_vals, Tango::DevEncoded *p_data, long x = 1, long y = 0,
                  bool release = false);

  /**
   * Push a user event for an attribute with Tango::DevEncoded attribute data
   * type when the attribute data are specified with 2 pointers. The event is
   * pushed to the event system.
   *
   * The method needs the attribue name and a pointer to the data to be pushed
   * as input. Depending on the attribute type the dimensions x and why need to
 ...
 * The method needs the attribue name, a pointer to the data to be pushed, the
 * time stamp for the data and the attribute quality factor as input. Depending
 * on the attribute type the dimensions x and why need to be given.
 *
 * @param attr_name The name of the attribute
 * @param filt_names The filterable fields name
 * @param filt_vals The filterable fields value (as double)
 ...
 */
  ...
  /**
   * Set an implemented flag for the attribute to indicate that the server fires
   * data ready event for this attribute.
   *
   * @param attr_name The name of the attribute
   ...
   * Push a pipe event.
   *
   * The method needs the pipe name and a pointer to the pipe blob to be pushed
   * as input. The time stamp of the event is set to the actual time.
   *
   * @param pipe_name The name of the pipe
   * @param p_data Pointer to the Pipe blob to be sent with the event
   * @param reuse_it Flag set to true if you don want the push_pipe_event to
   * consume Pipe internal memory. Default value is false which covers 95% of
   * use cases
 ...
 *
 * The method needs the pipe name, a pointer to the pipe blob to be pushed and
 * the time stamp for the data as input.
 *
 * @param pipe_name The name of the pipe
 * @param p_data Pointer to the data to be pushed
 * @param t The time stamp
 * @param reuse_it Flag set to true if you don want the push_pipe_event to
 * consume Pipe internal memory. Default value is false which covers 95% of use
 * cases
 * @exception DevFailed If the pipe data type is not coherent.
 * Click <a
 * href="https://tango-controls.readthedocs.io/en/latest/development/advanced/IDL.html#exceptions">here</a>
 ...
 */
#ifdef _TG_WINDOWS_
  void push_pipe_event(const std::string &pipe_name, Tango::DevicePipeBlob *p_data, struct _timeb &t,
                       bool reuse_it = false);
#else
  void push_pipe_event(const std::string &pipe_name, Tango::DevicePipeBlob *p_data, struct timeval &t,
                       bool reuse_it = false);
#endif
//@}
...
//@{
   ...
   * how the handler is written. A default false value is provided
   ...
   * Unregister a signal.
   *
   * Unregister this device as device to be informed when signal signo is sent
...
public:
  ...
  std::vector<std::string> &get_polled_cmd() { return polled_cmd; }

  std::vector<std::string> &get_polled_attr() { return polled_attr; }

  std::vector<std::string> &get_non_auto_polled_cmd() { return non_auto_polled_cmd; }

  std::vector<std::string> &get_non_auto_polled_attr() { return non_auto_polled_attr; }

  std::vector<PollObj *> &get_poll_obj_list() { return poll_obj_list; }
  ...
  std::string &get_name_lower() { return device_name_lower; }
  ...
  std::vector<std::string> &get_cmd_min_poll_period() { return cmd_min_poll_period; }

  std::vector<std::string> &get_attr_min_poll_period() { return attr_min_poll_period; }
  ...
  std::vector<std::string> &get_att_wrong_db_conf() { return att_wrong_db_conf; }

  void check_att_conf();

  bool is_alarm_state_forced() { return force_alarm_state; }

  std::vector<std::string> &get_att_mem_failed() { return att_mem_failed; }

  std::vector<FwdWrongConf> &get_fwd_att_wrong_conf() { return fwd_att_wrong_conf; }
  ...
  inline log4tango::Logger *get_logger(void) { return logger ? logger : get_logger_i(); }
  ...
  {                                                                                                        \
    ...
    {                                                                                                      \
      ...
      {                                                                                                    \
        ...
        D = new B(union_seq.length(), union_seq.length(), const_cast<C *>(union_seq.get_buffer()), false); \
      }                                                                                                    \
      else if(vers == 4)                                                                                   \
      {                                                                                                    \
        AttributeValue_4 &att_val = polled_att->get_last_attr_value_4(false);                              \
      ...
      {                                                                                                    \
        ...
        att_val.value >>= E;                                                                               \
        D = new B(E->length(), E->length(), const_cast<C *>(E->get_buffer()), false);                      \
      }                                                                                                    \
