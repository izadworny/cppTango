...
//
// description :	C++ source code for the DeviceClass class. This class is one
// the root class for all derived
...
static void lower_cmd_name(std::string &cmd) { std::transform(cmd.begin(), cmd.end(), cmd.begin(), ::tolower); }
...
//
// description :
//		DeviceClass constructor. Protected method which will be called
...
// argument :
// 		in :
//			- all : Flag set to true if memorized values must be applied to all
// class devices
//			- idx : Index of the device in the device_list vector of the device for
// which memorized values
// must 			        be applied. Used if "all" is set to false
//			- from_init : Flag set to true if the method is called due to a device
...
void DeviceClass::set_memorized_values(bool all, long idx, bool from_init)
{
  ...
  for(unsigned long i = start; i < stop; i++)
  {
    ...
    for(unsigned long j = 0; j < att_list.size(); j++)
    {
      ...
      if(att.is_memorized() == true)
      {
        ...
        if(mem_value != MemNotUsed)
        {
          ...
          //
          // In order to not send a new time the already memorized value into
          // db, mark it as not memorized before writing the value
          //

          att.set_memorized(false);

          //
          // The memorized value gotten from db is a string, we need to convert
          // this string to its real type before inserting it into an Any
          //
          ...
          try
          {
            ...
            //
            // Check the initialisation flag for memorized attributes. If the
            // flag is false, do not add the element to the att_val vector. This
            // avoids a call to write the memorized value to the attribute.
            //
    ...
    if(nb_wr != 0)
    {
      ...
      try
      {
        cout4 << "Writing data for " << att_val.length() << " attribute(s) for device " << device_list[i]->get_name()
              << std::endl;
        (static_cast<Device_3Impl *>(device_list[i]))->write_attributes_3(att_val);
      }
      catch(Tango::DevFailed &e)
      {
        cout3 << "Cannot write setpoint(s) value for any memorized "
                 "attribute(s) on device "
              << device_list[i]->get_name() << std::endl;
        Tango::Except::print_exception(e);
      }
      catch(Tango::MultiDevFailed &e)
      {
        cout3 << "Cannot write setpoint(s) value for memorized attribute(s) of "
                 "device "
              << device_list[i]->get_name() << std::endl;
        for(unsigned long k = 0; k < e.errors.length(); k++)
        {
          WAttribute &att =
              device_list[i]->get_device_attr()->get_w_attr_by_name(att_val[e.errors[k].index_in_call].name.in());
          att.set_mem_exception(e.errors[k].err_list);

          DEV_WARN_STREAM(device_list[i])
              << "Writing set_point for attribute " << att.get_name() << " failed" << std::endl;
          DEV_WARN_STREAM(device_list[i]) << "\tException desc = " << e.errors[k].err_list[0].desc.in() << std::endl;
          DEV_WARN_STREAM(device_list[i])
              << "\tException reason = " << e.errors[k].err_list[0].reason.in() << std::endl;
        }
        device_list[i]->set_run_att_conf_loop(true);
        Tango::NamedDevFailedList e_list(e, device_list[i]->get_name(),
      ...
      catch(...)
      {
        cout3 << "Cannot write setpoint(s) value for memorized attribute(s) of "
                 "device "
              << device_list[i]->get_name() << std::endl;
        std::cerr << "Received unknown exception while trying to write "
                     "memorized attribute(s)"
                  << std::endl;
        ...
        {
          std::stringstream ss;
          ss << "Received unknown exception (nor a devfailed, nor a "
                "MultiDevFailed !!";
...
DeviceClass::~DeviceClass()
{
  ...
  if(device_list.size() != 0)
  {
    ...
    {
      //
      // Clear vectors used to memorize info used to clean db in case of devices
      // with dyn attr removed during device destruction
      //
...
void DeviceClass::delete_dev(long idx, Tango::Util *tg, PortableServer::POA_ptr r_poa)
{
  ...
  //
  // If the polling thread is alive and if device is polled, ask polling thread
  // to stop polling
  //
...
//
// description :
//		Method to register a class on a signal. When the signal is sent to the
// process, the signal_handler 		method of this class will be executed
//
// argument :
// 		in :
...
//
// description :
//		This is the signal handler for the class. This method is defined as
// virtual and therefore, can be
// redefined 		by DS programmers in their own classes derived from DeviceClass
// class
//
// argument :
// 		in :
...
//
// description :
//		This method exports a device to the outside world. This is done by sending
// its CORBA network parameter 		(mainly the IOR) to the Tango database
//
// argument :
// 		in :
//			- dev : The device to be exported
//			- corba_obj_name : The CORBA object name associated with the device. A
// default value is
// provided. 							   This field is mainly use for specific device server
...
void DeviceClass::export_device(DeviceImpl *dev, const char *corba_obj_name)
{
  ...
  {
    //
    // Take the Tango monitor in order to protect the device against external
    // world access until memorized attribute (if any) are set but forget the
    // admin device (which does not have any memorized attribute) But don't do
    // this for dynamic devices which are not created in the
    // DServer::init_device method class loop
    //
  ...
  {
    //
    // For server started without db usage (Mostly the database server). In this
    // case, it is necessary to create our own CORBA object id and to bind it
    // into the Boot Manager for access through a stringified object reference
    // constructed using the corbaloc style. The API will try to connect to
    // device using lower case letters. Register device in POA with lower case
    // letters
    //
  ...
  if((Tango::Util::_UseDb == true) && (Tango::Util::_FileDb == false))
  {
    ...
    //
    // Call db server
    // We are still in the server starting phase. Therefore, the db timeout is
    // still high (13 sec the 07/01/2011) with 3 retries in case of timeout
    //
  ...
  //
  // Set the DeviceImpl exported flag to true. Also enable device interface
  // change event
  //
...
//
// description :
//		Command handler which is called by Device when a command is received. It
// will check to see if the command is 		implemented. If so it will test to see
// if it is allowed in this state. Finally it will execute
// the 		command by calling its execute method. If an error occurs it will
// throw a DevFailed exception.
//
//-------------------------------------------------------------------------------------------------------------------
...
CORBA::Any *DeviceClass::command_handler(DeviceImpl *device, const std::string &command, const CORBA::Any &in_any)
{
  ...
  //
  // Search for command object first at class level then at device level (case
  // of dynamic command installed at device level)
  //
  ...
  if(found == true)
  {
    ...
    {
      TangoSys_OMemStream o;
      o << "Command " << command << " not allowed when the device is in " << Tango::DevStateName[device->get_state()]
        << " state" << std::ends;
...
//
// description :
//		Method to remove and delete a device from the running devices belonging to
// the Tango class
//
// argument :
// 		in :
...
void DeviceClass::device_destroyer(const std::string &dev_name)
{
  ...
  //
  // Check if the device is polled. If yes, ask polling thread to stop polling
  // it
  //
...
//
// description :
//		Method to check if a command is allowed even if the device is locked by
// another client. It follows the 		definition of the Tango control access
// system to define what is an allowed command
//
// argument :
// 		in :
...
//
// description :
//		Get for all class devices and for all attributes multicast event
// parameters (if any)
//
// argument :
// 		in :
...
//
// description :
//		Check attribute configuration (for wrong conf. in db exception) for all
...
//
// description :
//		Release all device(s) class monitor. This is needed in DS startup
// sequence. Each device monitor is taken 		before the device is known to
// CORBA and released after the memorized attributes (if any) are set
//
//------------------------------------------------------------------------------------------------------------------
...
// argument :
// 		in :
//			- cl : The device class (we need here the pointer to the real device
// class not to the base
// class)
//          - dev : Device ptr
//
//------------------------------------------------------------------------------------------------------------------
