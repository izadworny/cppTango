...
// method : Logging::add_logging_target
//--------------------------------------------------------------------------
void Logging::add_logging_target(log4tango::Logger *logger, const std::string &tg_type, const std::string &tg_name,
                                 int throw_exception)
{
...
void Logging::add_logging_target(log4tango::Logger *logger, const std::string &in_type_name, int throw_exception)
{
  try
  {
    ...
    {
      cout4 << "Target " << appender_name << " is already attached to " << logger->get_name() << std::endl;
    }
    ...
    for(i = 0; i < argin->length();)
    {
      ...
      for(unsigned int j = 0; j < dl.size(); j++)
      {
        ...
        if(remove_all_targets == 0)
        {
          ...
          cout4 << "removing target " << full_tg_name << " from " << dl[j]->get_name() << std::endl;
        ...
        {
          cout4 << "removing ALL <" << tg_type_str << "> targets from " << dl[j]->get_name() << std::endl;
  ...
  for(i = 0; i < dl.size(); i++)
  {
    ...
    if(logger)
    {
      ...
      for(j = 0; j < al.size(); j++)
      {
        ...
        {
          cout4 << "Removing zombie appender " << dl[i]->get_name() << "::" << al[j]->get_name() << std::endl;
...
std::string Logging::dev_to_file_name(const std::string &_dev_name)
{
  ...
  cout4 << "Leaving Logging::dev_to_file_name (output " << file_name << ")" << std::endl;
  return file_name;
}
...
int Logging::get_target_type_and_name(const std::string &input, std::string &type, std::string &name)
{
  ...
  cout4 << "Logging::get_target_type_and_name (output " << type << " " << name << ")" << std::endl;
  return 0;
}
