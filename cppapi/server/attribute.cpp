...
// file :               Attribute.cpp
//
// description :        C++ source code for the Attribute class. This class is
// used to manage attribute.
//						A Tango Device object instance has one MultiAttribute object which
// is an aggregate of 						Attribute or WAttribute objects
//
// project :            TANGO
//
// author(s) :          E.Taurel
...
void LastAttrValue::store(const AttributeValue_5 *attr_5, const AttributeValue_4 *attr_4,
                          const AttributeValue_3 *attr_3, const AttributeValue *attr, DevFailed *error)
{
  if(error)
  {
    except = *error;
...
//
// description :
//		Constructor for the Attribute class from the attribute property vector,
// its type and the device name
//
// argument :
//		in :
...
void Attribute::init_event_prop(std::vector<AttrProperty> &prop_list, const std::string &dev_name, Attr &att)
{
  ...
  try
  {
    ...
    if(rel_change_defined)
    {
      if((data_type != Tango::DEV_STRING) && (data_type != Tango::DEV_BOOLEAN) && (data_type != Tango::DEV_STATE))
      {
        ...
        validate_change_properties(dev_name, "rel_change", rel_change_str, rel_change_tmp, rel_change_set_usr_def,
                                   unused);
  ...
  try
  {
    ...
    if(abs_change_defined)
    {
      if((data_type != Tango::DEV_STRING) && (data_type != Tango::DEV_BOOLEAN) && (data_type != Tango::DEV_STATE))
      {
        ...
        std::vector<bool> unused;
        validate_change_properties(dev_name, "abs_change", abs_change_str, abs_change_tmp, abs_change_set_usr_def,
                                   unused);
  ...
  try
  {
    ...
    if(archive_rel_change_defined)
    {
      if((data_type != Tango::DEV_STRING) && (data_type != Tango::DEV_BOOLEAN) && (data_type != Tango::DEV_STATE))
      {
        ...
        std::vector<bool> unused;
        validate_change_properties(dev_name, "archive_rel_change", archive_rel_change_str, archive_rel_change_tmp,
                                   archive_rel_change_set_usr_def, unused);
        ...
        if(archive_rel_change_set_usr_def[0] || archive_rel_change_set_usr_def[1])
        {
          if(nb_user != 0)
          {
            ...
            if(i != nb_user)
            {
              ...
              validate_change_properties(dev_name, "archive_rel_change", def_user_prop[i].get_value(),
        ...
        cout1 << "Attribute::Attribute(): archive_rel_change = " << archive_rel_change[0] << " "
  ...
  try
  {
    ...
    if(archive_abs_change_defined)
    {
      if((data_type != Tango::DEV_STRING) && (data_type != Tango::DEV_BOOLEAN) && (data_type != Tango::DEV_STATE))
      {
        ...
        std::vector<bool> unused;
        validate_change_properties(dev_name, "archive_abs_change", archive_abs_change_str, archive_abs_change_tmp,
                                   archive_abs_change_set_usr_def, unused);
        ...
        if(archive_abs_change_set_usr_def[0] || archive_abs_change_set_usr_def[1])
        {
          if(nb_user != 0)
          {
            ...
            if(i != nb_user)
            {
              ...
              validate_change_properties(dev_name, "archive_abs_change", def_user_prop[i].get_value(),
        ...
        cout1 << "Attribute::Attribute(): archive_abs_change = " << archive_abs_change[0] << " "
  ...
  try
  {
    ...
    if(event_period_defined)
    {
      ...
      if(str >> event_period_tmp && str.eof())
      {
        ...
        cout1 << "Attribute::Attribute(): event_period_str " << event_period_str << " event_period = " << event_period
              << std::endl;
      }
      else
  ...
  try
  {
    ...
    if(archive_period_defined)
    {
      ...
      if(str >> archive_period_tmp && str.eof())
      {
        ...
        cout1 << "Attribute::Attribute(): archive_period_str " << archive_period_str
  ...
  //
  // do not start sending events automatically, wait for the first client to
  // subscribe. Sending events automatically will put an unnecessary load on the
  // server because all attributes will be polled
  //
...
bool Attribute::init_check_val_prop(std::vector<AttrProperty> &prop_list, const std::string &dev_name,
                                    const char *prop_name, std::string &prop_str, Tango::Attr_CheckVal &prop,
...
void Attribute::init_opt_prop(std::vector<AttrProperty> &prop_list, const std::string &dev_name)
{
  ...
  try
  {
       ...
       ((delta_t_defined == false) && (delta_val_defined == true)))
    {
      ...
      o << "RDS alarm properties (delta_t and delta_val) are not correctly "
  ...
  catch(DevFailed &e)
  {
    ...
    if(desc.find("Property enum_labels is missing") != std::string::npos)
    {
      ...
      ss << "The attribute " << name
         << " has the DEV_ENUM data type but there is no enumeration label(s) "
            "defined";
...
//
// description :
//		Create vector of enum labels and  check that the same label is not used
// several times
//
// argument :
// 		in :
...
//
// description :
//		Deletes the exception related to the property name from startup_exceptions
// map
//
// argument :
// 		in :
//			- prop_name : The property name as a key for which the exception is to
// be deleted from
...
//
// description :
//		Throw a Tango DevFailed exception when an error format is detected in the
...
void Attribute::throw_incoherent_val_err(const char *min_prop, const char *max_prop, const std::string &dev_name,
                                         const char *origin)
{
  TangoSys_OMemStream o;
...
void Attribute::throw_min_max_value(const std::string &dev_name, const std::string &memorized_value,
...
bool Attribute::is_polled()
{
  ...
  for(unsigned int i = 0; i < attr_list.size(); i = i + 2)
  {
    ...
    if(att_name == name_lowercase)
    {
      ...
      // mark the attribute as not polled! No events can be send by the polling
      // thread!
      //
...
//
// returns:
//		This method returns a boolean set to true if the atribute has an
// associatied writable attribute
//
//-------------------------------------------------------------------------------------------------------------------
...
bool Attribute::check_alarm()
{
  ...
  //
  // If the attribute quality is different than VALID don`t do any checking to
  // avoid to override a user positioned quality value. If no alarms levels are
  // specified, just return without alarm.
  //
  ...
  if((bs.test(Attribute::min_level) == true) || (bs.test(Attribute::max_level) == true))
  {
...
//
// description :
//		Build an AttributeValue_3 object from the AttributeValue_4 object. This
// method is used in case an event
// is 		requested by a client knowing only IDL release 3
//
// args :
// 		in :
...
//
// description :
//		Build an AttributeConfig_3 object from the AttributeConfig_5 object. This
// method is used in case an event is 		requested by a client knowing only IDL
// release 4
//
// argument:
// 		in :
...
// arguments:
// 		in :
//			- ptr : Pointer to a DevFailed exception to fire in case of an error to
// indicate.
//
//---------------------------------------------------------------------------------------------------------------------
...
void Attribute::fire_change_event(DevFailed *except)
{
  ...
  try
  {
    ...
    if(is_check_change_criteria() == true)
    {
      ...
      // When we have both notifd and zmq event supplier, do not detect the
      // event two times. The detect_and_push_events() method returns true if
      ...
      if(event_supplier_zmq != NULL)
      {
        if(event_supplier_nd != NULL)
        {
          if(send_event == true && pub_socket_created == true)
          {
            ...
            event_supplier_zmq->push_event_loop(dev, CHANGE_EVENT, f_names, f_data, f_names_lg, f_data_lg, ad, *this,
                                                except);
          }
        }
        else
    ...
    else
    {
      ...
      // comming with the standard mechanism or coming from a manual fire event
      // call.
      //
...

      {
        ...
        if(except || quality == Tango::ATTR_INVALID || prev_change_event.err || old_quality == Tango::ATTR_INVALID)
        {
          force_change = true;
        }

        prev_change_event.store(send_attr_5, send_attr_4, send_attr, Tango_nullptr, except);
      ...
      if(event_supplier_nd != NULL)
        event_supplier_nd->push_event(dev, "change", filterable_names, filterable_data, filterable_names_lg,
                                      filterable_data_lg, ad, name, except, false);

      if(event_supplier_zmq != NULL && pub_socket_created == true)
      {
        event_supplier_zmq->push_event_loop(dev, CHANGE_EVENT, filterable_names, filterable_data, filterable_names_lg,
                                            filterable_data_lg, ad, *this, except);
      }
    }
      ...
      if(event_supplier_zmq != NULL)
      {
        if(event_supplier_nd != NULL)
        {
          if(send_event == true && pub_socket_created == true)
          {
            ...
            event_supplier_zmq->push_event_loop(dev, ARCHIVE_EVENT, f_names, f_data, f_names_lg, f_data_lg, ad, *this,
                                                except);
          }
        }
        else
    ...
    else
    {
...

      {
        ...
        if(event_supplier_nd || event_supplier_zmq)
        {
          ...
          event_supplier->detect_change(*this, ad, true, delta_change_rel, delta_change_abs, except, force_change, dev);
        }

        prev_archive_event.store(send_attr_5, send_attr_4, send_attr, Tango_nullptr, except);
      ...
      if(event_supplier_nd != NULL)
        event_supplier_nd->push_event(dev, "archive", filterable_names, filterable_data, filterable_names_lg,
                                      filterable_data_lg, ad, name, except, false);

      if(event_supplier_zmq != NULL && pub_socket_created == true)
      {
        event_supplier_zmq->push_event_loop(dev, ARCHIVE_EVENT, filterable_names, filterable_data, filterable_names_lg,
                                            filterable_data_lg, ad, *this, except);
      }
    }
...
//			- except : Pointer to a DevFailed exception to fire in case of an error
// to indicate.
//
//--------------------------------------------------------------------------------------------------------------------

void Attribute::fire_event(const std::vector<std::string> &filt_names, const std::vector<double> &filt_vals,
                           DevFailed *except)
{
  ...
  try
  {
    ...
    if(event_supplier_nd != NULL)
      event_supplier_nd->push_event(dev, "user_event", filt_names, filt_vals, filterable_names_lg, filterable_data_lg,
                                    ad, name, except, false);
    if(event_supplier_zmq != NULL && pub_socket_created == true)
    {
      event_supplier_zmq->push_event_loop(dev, USER_EVENT, filt_names, filt_vals, filterable_names_lg,
                                          filterable_data_lg, ad, *this, except);
    }
...
//
// description :
//		Fire a periodic event with error set. This is used when attribute polling
// is stopped
//
// arguments:
// 		in :
...
void Attribute::fire_error_periodic_event(DevFailed *except)
{
  ...
  if(event_supplier_nd != NULL)
    event_supplier_nd->push_event(dev, "periodic_event", filt_names, filt_vals, filterable_names_lg, filterable_data_lg,
                                  ad, name, except, false);
  if(event_supplier_zmq != NULL)
  {
    event_supplier_zmq->push_event_loop(dev, PERIODIC_EVENT, filt_names, filt_vals, filterable_names_lg,
                                        filterable_data_lg, ad, *this, except);
  }
}
...
void Attribute::upd_att_prop_db(const Tango::Attr_CheckVal &new_value, const char *prop_name)
{
...
//                  remove all configuration of a dynamic attribute before
//                  deleting it.
//
// 					The method removes all configured attribute properties
//                  and removes the attribute from the list of polled
//                  attributes.
//--------------------------------------------------------------------------
...
void Attribute::set_attr_serial_model(AttrSerialModel ser_model)
{
  if(ser_model == Tango::ATTR_BY_USER)
  {
    ...
    {
      TANGO_THROW_EXCEPTION(API_AttrNotAllowed, "Attribute serial model by user is not allowed when the process is "
                                                "not in BY_DEVICE serialization model");
    }
  }
...
{
  //
  // Get device class. When the server is started, it's an easy task. When the
  // server is in its starting phase, it's more tricky. Get from the DeviceClass
  // list the first one for which the device_factory method has not yet been
  // fully executed. This is the DeviceClass with the device in its
  // init_device() method has called Attribute::set_properties()
  //
  ...
  else
  {
    ...
    for(loop = 0; loop < tmp_cl_list.size(); ++loop)
    {
      ...
      {
        // If a server wants to set attribute properties during a "write
        // memorized attribute value at init" phase, we might be in this case...
        ...
        for(size_t i = 0; i < dev_list.size(); ++i)
        {
          ...
          {
            // Our device is listed in this class, returns the corresponding
            // DeviceClass pointer
...
//
// description :
//		Store in att the famous AVNS (AlrmValueNotSpec) for a specific attribute
// property
//
// Arguments:
//		in :
...
//
// description :
//		Set the attribute format property to the default value which depends on
// attribute data type
//
//--------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Insert the default format string in a DbDatum instance. This default value
// depends on the attribute 		data type
//
// argument :
//		in :
...
//
// description :
//		Returns true if there are some subscriber listening for event. This is a
// method family. There is one
// method 		for each event type 		Cannot replce this methods family by one
// method with event type as parameter for compatibility reason
//
//--------------------------------------------------------------------------------------------------------------------
...
{
  cout4 << "Attribute::set_client_lib(" << client_lib_version << "," << EventName[event_type] << ")" << std::endl;
