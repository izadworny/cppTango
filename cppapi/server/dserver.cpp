...
// file :               DServer.cpp
//
// description :        C++ source for the DServer class and its commands. The
// class is derived from Device.
//						It represents the CORBA servant object which will be accessed from
// the network.
// All commands 						which can be executed on a DServer object are
...
void DServer::init_device()
{
  ...
  try
  {
    ...
    {
      //
      // Set the class list pointer in the Util class and add the DServer object
      // class
      //
      ...
      for(i = 0; i < class_list.size(); i++)
      {
        ...
        // a retry here (in case of db server restart) because the db
        // reconnection is forced by the get_property call executed during
        ...
        if(tg->_UseDb == true)
        {
          ...
          catch(Tango::DevFailed &)
          {
            ...
            o << "Database error while trying to retrieve device list for "
                 "class "
          ...
          //
          // Set value for each device with memorized writable attr. This is
          // necessary only if db is used
          //
        ...
        {
          //
          // Retrieve devices name list from either the command line or from the
          // device_name_factory method
          //
  ...
  catch(std::bad_alloc &)
  {
    ...
    // all classes already built. If the error occurs during the command or
    // device factories, erase only the following classes
    //
    ...
    {
      o << "Can't allocate memory in server while building command(s) or "
           "device(s) for class number "
        << i + 1 << std::ends;
  ...
  for(DeviceClass *dclass : this->get_class_list())
  {
    for(DeviceImpl *device : dclass->get_device_list())
    {
      ...
      catch(const DevFailed &devFailed)
      {
        ...
        ss << "Device[" << device->get_name_lower() << "] server_init_hook has failed due to DevFailed:" << std::endl;
        ss << devFailed;
...
void DServer::delete_devices()
{
  if(class_list.empty() == false)
  {
    for(long i = class_list.size() - 1; i >= 0; i--)
    {
      ...
      {
        //
        // Clear vectors used to memorize info used to clean db in case of
        // devices with dyn attr removed during device destruction
        //
...
void DServer::add_class(DeviceClass *class_ptr) { class_list.push_back(class_ptr); }
...
void DServer::restart(const std::string &d_name)
{
  ...
  //
  // Change device name to lower case letters and remove extra field in name in
  // case of (#dbase=xxx or protocol specif) Check if the wanted device exists
  // in each class
  //
  ...
  //
  // Memorize device interface if some client(s) are listening on device
  ...
  //
  // If the device is locked and if the client is not the lock owner, refuse to
  // do the job
  //
  ...
  try
  {
    ...
    //
    // Re-create device. Take the monitor in case of class or process
    // serialisation model
    //
...
void ServRestartThread::run(void *ptr)
{
  ...
  //
  // Change the POA manager to discarding state. This is necessary to discard
  // all request arriving while the server restart.
  //
...
//
// description :
//		command to return the list of property device at class level for the
// specified class
//
//-------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		command to return the list of property device at device level for the
// specified class
//
//------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		command to kill the device server process. This is done by starting a
// thread which will kill the
// process. 		Starting a thread allows the client to receive something from
// the server before it is killed
//
//-----------------------------------------------------------------------------------------------------------------
...
void DServer::create_cpp_class(const char *cl_name, const char *par_name)
{
  ...
  if((mod = LoadLibrary(lib_name.c_str())) == NULL)
  {
    ...
    ::FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, l_err,
                    MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (char *) &str, 0, NULL);
...
void DServer::get_dev_prop(Tango::Util *tg)
{
  ...
  if(tg->_UseDb == true)
  {
    ...
    {
      TangoSys_OMemStream o;
      o << "Database error while trying to retrieve device properties for "
           "device "
    ...
    //
    // If the prop is not defined in db and if the user has defined it in the
    // Util class, takes the user definition
    //
    ...
    if(db_data[1].is_empty() == false)
    {
      ...
      //
      // If the polling threads pool conf. has been splitted due to the max
      // device property length of 255 chars, rebuild a real pool conf
      //
...
//
// description :
//		Get the properties defining which event has to be transported using
// multicast protocol. Also retrieve 		property for Nan allowed in writing
// attribute
//
// args :
// 		in :
    ...
    try
    {
      ...
      for(unsigned int i = 0; i < nb_elt; ++i)
      {
        if(is_ip_address(mcast_event_prop[i]) == true)
        {
          ...
          //
          // If the property is uncoherent, clear it but inform user. No event
          // will use multicasting in this case
          //

          if(uncoherent == true)
          {
            std::cerr << "Database CtrlSystem/MulticastEvent property is uncoherent" << std::endl;
            std::cerr << "Prop syntax = mcast ip adr (must be between "
                         "224.x.y.z and 239.x.y.z) - port - [rate] - [ivl] "
                         "- event name"
                      << std::endl;
...
//
// description :
//		Return in m_event vector, list of mcast event for the specified
// device/attribute. 		For each event, the returned string in the vector
// follows the syntax 						event_name:ip_adr:port:rate:ivl 		The last
// two are optionals. Please note that the same dev/att may have several event
// using multicasting
//
// args :
//		in :
...
void DServer::mcast_event_for_att(const std::string &dev_name, const std::string &att_name,
...
//
// description :
//		Store event parameters for all class/devices in a DS. This is necessary in
// case of command RestartServer
//
// args :
//		out :
...
//
// description :
//		Apply event parameters for all class/devices in a DS. This is necessary in
// case of command RestartServer
//
// args :
//		in :
...
//
// description :
//		Memorize device interface for devices where some client(s) are listening
// on device interface change 		event
//
// arguments :
//		out :
//			- _map : reference to the map (name,device interface) where the
// interface must be stored
//
//------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Check for each device with client(s) listening on device interface change
// event if the device interface 		has changed and if true, send event
//
// arguments :
//		in :
//			- _map : reference to the map (name,device interface) where devices
