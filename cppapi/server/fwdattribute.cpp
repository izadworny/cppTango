...
//
// description :        C++ source code for the FwdAttribute class. This class
// is used to manage attribute.
//						A Tango Device object instance has one MultiAttribute object which
// is an aggregate of 						Attribute, FwdAttribute, WAttribute and
// FwdWAttribute objects
//
// project :            TANGO
//
// author(s) :          E.Taurel
...
//
// description :
//		Constructor for the FwdAttribute class from the attribute property vector,
...
//
// description :
//		Set attribute configuration. In this case, it is much easier than for
// classical attribute. The config.
// comes 		from the root attribute and therefore, all tests have already been
// done in the root attribute and we don't 		need to store anything in DB Note
// that the main attribute characteristics (format, type, memorized...) are set
// via FwdAttr instance and are 		not changeable here
//
//		This method is called when a new attribute configuration is received via
// the Callback installed for 		forwarded attribute
//
// argument :
//		in :
...
//
// Same method than previous but the argument is not the same and this one is
// called when a forwarded attribute configuration is written via a call in the
// Forwarded attribute callback in the case of root attribute not available when
// the DS was started
//
...
void FwdAttribute::upd_att_config_base(const char *new_label)
{
  ...
  //
  // Throw exception in case of fwd att wrongly configured or if the root device
...
//
// description :
//		Update a forwarded attribute config. Only label is maintained locally.
// Send att conf. to root device
// only 		if it is modified compared to what we have
//
// argument :
//		in :
//			- conf : The attribute new configuration
...
{
  //
  // Send new config to root attribute if received configuration if different
...
//
// description :
//		Update the forwarded attribute label property. It is the only property
// which stays local for forwarded 		attribute
//
// argument :
//		in :
...
void FwdAttribute::upd_att_label(const char *new_label)
{
  ...
  //	LabelNotSpec or AlrmvalueNotSpec means return to lib default (att name in
  // this case)
  //	"" (empty string) means returns to user def if any otherwise return to lib
  // default
  //  NaN means returns to class default (if any), user default (if any), lib
  //  default
  //
  ...
  {
    // set class default value if defined, otherwise use the user default or
    // library defaults
  ...
  //
  // Remove value from DB because label is now the default one (class, user or
  // lib)
  //
...
//		Note that we do not use the classical API DeviceProxy class instance. This
// is because for this call, the 		network transfer uses RLE algorithm and the
// API is doing the decoding. In our case, we want data from 		root attribute
// in RLE and directly transfer them in RLE to our client.
//
// argument :
//		in :
...
DevAttrHistory_5 *FwdAttribute::read_root_att_history(long n)
{
  ...
  //
  // Get data from root device (Reminder: we don't use the classical API. See
  // above)
  //
