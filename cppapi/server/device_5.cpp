...
// file :               Device_5.cpp
//
// description :        C++ source code for the Device_5Impl class. This class
// is the root class for all derived Device
...
//
// description :
//		Constructors for the Device_5impl class from the class object pointer, the
// device name, the description
// field,
// 		the state and the status. Device_5Impl inherits from DeviceImpl. These
// constructors simply call the correct
//		DeviceImpl class constructor
//
//--------------------------------------------------------------------------------------------------------------------

Device_5Impl::Device_5Impl(DeviceClass *device_class, const std::string &dev_name)
    : Device_4Impl(device_class, dev_name),
      ext_5(nullptr)
{
  idl_version = 5;
}

Device_5Impl::Device_5Impl(DeviceClass *device_class, const std::string &dev_name, const std::string &desc)
    : Device_4Impl(device_class, dev_name, desc),
      ext_5(nullptr)
{
  idl_version = 5;
}
...
Device_5Impl::Device_5Impl(DeviceClass *device_class, const std::string &dev_name, const std::string &desc,
    ...
    : Device_4Impl(device_class, dev_name, desc, dev_state, dev_status),
      ext_5(nullptr)
{
  idl_version = 5;
}

Device_5Impl::Device_5Impl(DeviceClass *device_class, const char *dev_name, const char *desc, Tango::DevState dev_state,
                                                             ...
                                                             Tango::DevSource source, const Tango::ClntIdent &cl_id)
{
  ...
  {
    //
    // If the device has some forwarded attribute, do not try to get data from
    // the local cache but get it from the root device Create two arrays with
    // local attribute names and fwd attribute names
    //
    ...
    {
      //
      // Some fwd attributes in call: First get data for local attributes then
      // get data for forwarded attributes from root devices
      //
...
//
// description :
//		CORBA operation to write then read attribute(s) value. Nowadays, it is
// possible to write then read
// attribute 		for only 1 attribute at a time even if the IDL is written for
// several attribute
//
//------------------------------------------------------------------------------------------------------------------
...
//
// return :
//		Pointer to a AttributeConfigList_5 with one AttributeConfig_5 structure
// for each attribute
//
//-------------------------------------------------------------------------------------------------------------------
...
Tango::AttributeConfigList_5 *Device_5Impl::get_attribute_config_5(const Tango::DevVarStringArray &names)
{
  ...
  // If the device implements IDL 3 or above  (State and status as attributes)
  // and the client is an old one (not able to read state/status as attribute),
...
Tango::DevAttrHistory_5 *Device_5Impl::read_attribute_history_5(const char *name, CORBA::Long n)
{
  ...
  cout4 << "Device_5Impl::read_attribute_history_5 arrived, requested history "
           "depth = "
        << n << std::endl;
  ...
  //
  // Check that the wanted attribute is polled (Except in case of forwarded
  // attribute)
  //
  ...
  else
  {
    ...
    // Trick: To identify the state used as an attribute from a classical
    // attribute with type DEV_STATE, use DEV_VOID for state as data type.
    //
...
//
// description :
//		CORBA operation to set pipe configuration locally and in the Tango
// database
//
// argument:
//		in :
//			- new_conf: The new pipe(s) configuration. One PipeConfig structure is
// needed for each 				    	pipe to update
