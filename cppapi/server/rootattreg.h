...
// file :		rootattreg.h
//
// description :	Include file for the RootAttRegistry and RootAttConfCallBack
// classes hierarchy.
...
public:
  ...
  void add_root_att(const std::string &, const std::string &, const std::string &, const std::string &, FwdAttr *,
                    DeviceImpl *);
  ...
  void update_label(const std::string &_d, const std::string &_a, const std::string &_l)
  {
    std::string s(_d + '/' + _a);
    cbp.update_label(s, _l);
  }
...
private:
  ...
  {
  public:
    RootAttConfCallBack(RootAttRegistry *_r)
        : Tango::CallBack(),
          rar(_r)
    {
      ci.cpp_clnt(0);
  ...
  {
  public:
    RootAttUserCallBack(RootAttRegistry *_r)
        : Tango::CallBack(),
          rar(_r)
    {
    }
