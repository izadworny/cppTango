...
//
// description :
//		Instance method for DServerSignal object. This class is a singleton and
// this method creates the object
// the 		first time it is called or simply returns a pointer to the already
// created object for all the other calls.
//
//-------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		constructor for DServerSignal object. As this class is a singleton, this
// method is protected
//
//-----------------------------------------------------------------------------------------------------------------
    ...
    : TangoMonitor("signal")
{
...
#ifndef _TG_WINDOWS_
  //
  // With Solaris/Linux, the SIGINT and SIGQUIT default actions are set to
  // SIG_IGN for all processes started in the background (POSIX requirement).
  // Signal SIGINT is used by Tango in its signal management, reset the default
  // action to default
  //
  ...
  if(sigaction(SIGINT, &sa, NULL) == -1)
    std::cerr << "DServerSignal::DServerSignal --> Can't reset default action "
                 "for SIGINT to SIG_DFL"
              << std::endl;

  if(sigaction(SIGQUIT, &sa, NULL) == -1)
    std::cerr << "DServerSignal::DServerSignal --> Can't reset default action "
                 "for SIGQUIT to SIG_DFL"
              << std::endl;
...
#endif
{
  ...
  //
  // Check if class is already registered for this signal. If it is already
  // done, leave method. Otherwise, record class pointer
  //
...
//
// description :
//		Method to check if a class is already registered for a signal. If it is
// true, this method returns in
// which 		element of the vector the class is registered
//
// argument :
// 		in :
//			- signo : Signal number
...
#endif
{
  ...
  //
  // Check if devices is already registered for this signal. If it is already
  // done, leave method. Otherwise, record class pointer
  //
...
//
// description :
//		Method to check if a device is already registered for a signal. If it is
// true, this method returns  in
// which 		element of the vector the device is registered
//
// argument :
// 		in :
//			 - signo : Signal number
...
void DServerSignal::unregister_dev_signal(DeviceImpl *dev_ptr)
{
  ...
  for(i = 0; i < _NSIG; i++)
  {
    ...
    //
    // If nothing is registered for this signal, unregister the OS signal
    // handler
    //
...
//
// description :
//		Method to unregister from the OS the main signal handler for a given
// signal
//
// argument :
// 		in :
...
//
// description :
//		This is a dummy signal handler used only with solaris which needs one for
// signal with a default ignore 		action to work correctly with the sigwait()
// call.
//
// argument :
// 		in :
