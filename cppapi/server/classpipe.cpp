...
MultiClassPipe::MultiClassPipe() { cout4 << "Entering MultiClassPipe constructor" << std::endl; }
...
// ClassPipe object for each pipe 		with defined properties
//
// argument :
//		in :
...
void MultiClassPipe::init_class_pipe(DeviceClass *cl_ptr)
{
  ...
  if((nb_pipe != 0) && (Tango::Util::_UseDb == true))
  {
    ...
    //
    // Ask for class default prop in DB only for pipe with label or desc not
    // already set to some user default value
    //
...
//
// description :
//		Get list of pipe property (defined at class level) for the pipe specified
// as input arg
//
// argument :
// 		in :
...
//
// return :
//		Reference to the vector of pipe properties or throw an exception if the
// pipe is not found
//
//--------------------------------------------------------------------------------------------------------------------
