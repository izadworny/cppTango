...
// file :        eventcmds.cpp
//
// description : C++ source for the event commands which are part of the DServer
// class.
//
...
DevLong DServer::event_subscription_change(const Tango::DevVarStringArray *argin)
{
  if(argin->length() < 4)
  {
    ...
    o << "Not enough input arguments, needs 4 i.e. device name, attribute "
  ...
  //
  // If we receive this command while the DS is in its shuting down sequence, do
  // nothing
  //
  ...
  //
  // Check if the request comes from a Tango 6 client (without client
  // identification) If true, the event has to be sent using AttributeValue_3
  // data structure If cl is NULL, this means that the call is local (Two tango
  // classes within the same process and with events between device from class 1
  // and device from classs 2)
  //
...
//
// description :
//		method to do all the necessary checks on attribute config to generate
// events
//
// args :
// 		in :
...
void DServer::event_subscription(const std::string &dev_name, const std::string &obj_name, const std::string &action,
                                 std::string &event, const std::string &obj_name_lower, ChannelType ct,
                                 std::string &mcast_data, int &rate, int &ivl, DeviceImpl *dev, int client_lib)
{
  Tango::Util *tg = Tango::Util::instance();
  ...
  if(event != EventName[INTERFACE_CHANGE_EVENT] && event != EventName[PIPE_EVENT])
  {
    ...
    if(ct == NOTIFD)
    {
      if(attribute.is_fwd_att() == true)
      {
        ...
        ss << "\nIt is not supported to subscribe events from forwarded "
              "attribute using Tango < 9. Please update!!";
    ...
    if(action == "subscribe")
    {
      if(event == "user_event")
      {
        cout4 << "DServer::event_subscription(): update user_event "
                 "subscription\n";
      ...
      else if(event == "data_ready")
      {
        ...
        cout4 << "DServer::event_subscription(): update data_ready "
                 "subscription\n";
      ...
      {
        //
        // If the polling is necessary to send events, check whether the polling
        // is started for the requested attribute.
        //
        ...
        if(event == "change")
        {
          ...
          {
            if((attribute.get_data_type() != Tango::DEV_STRING) && (attribute.get_data_type() != Tango::DEV_BOOLEAN) &&
               (attribute.get_data_type() != Tango::DEV_ENCODED) && (attribute.get_data_type() != Tango::DEV_STATE) &&
               (attribute.get_data_type() != Tango::DEV_ENUM))
            {
              if(attribute.is_check_change_criteria() == true)
              {
                ...
                {
                  TangoSys_OMemStream o;
                  o << "Event properties (abs_change or rel_change) for "
                       "attribute ";
                  o << obj_name;
        ...
        {
          cout4 << "DServer::event_subscription(): update quality_change "
                   "subscription\n";
          attribute.event_quality_subscription = time(NULL);
        }
        else if(event == "periodic")
        {
          cout4 << "DServer::event_subscription(): update periodic "
                   "subscription\n";
        ...
        else if(event == "archive")
        {
          ...
          if(obj_name_lower != "state")
          {
               ...
               (attribute.get_data_type() != Tango::DEV_ENUM))
            {
              ...
              {
                if((attribute.archive_abs_change[0] == INT_MAX) && (attribute.archive_abs_change[1] == INT_MAX) &&
                   (attribute.archive_rel_change[0] == INT_MAX) && (attribute.archive_rel_change[1] == INT_MAX) &&
                   (attribute.archive_period == INT_MAX))
                {
                  TangoSys_OMemStream o;
                  o << "Archive event properties (archive_abs_change or "
                       "archive_rel_change or archive_period) for "
                       "attribute ";
                  o << obj_name;
                  o << " are not set" << std::ends;
      ...
      //
      // Check if multicast has to be used for event transport (only for ZMQ
      // event) Don't forget syntax in attribute mcast_event string:
    ...
    //
    // Memorize client lib release. Protect this setting in case of user thread
    // pushing event when the subscription command is received
    //
  ...
  else
  {
    if(action == "subscribe")
    {
      cout4 << "DServer::event_subscription(): update device interface_change "
               "subscription\n";
  ...
  //
  // Ask polling thread in charge of heartbeat to send them (if not already
  // done)
  //
...
DevVarLongStringArray *DServer::zmq_event_subscription_change(const Tango::DevVarStringArray *argin)
{
  ...
  {
    TangoSys_OMemStream o;
    o << "Not enough input arguments, needs at least 4 i.e. device name, "
         "attribute/pipe name, action, event name, "
         "<Tango lib release>"
      << std::ends;
  ...
  if(argin->length() == 1)
  {
    ...
    {
      TangoSys_OMemStream o;
      o << "Not enough input arguments, needs 4 i.e. device name, "
           "attribute/pipe name, action, event name"
        << std::ends;
  ...
  else
  {
      ...
      if(client_release == 0)
      {
        ...
        else
        {
          ...
          else
          {
            //
            // Check if the request comes from a Tango 6 client (without client
            // identification) If true, the event has to be sent using
            // AttributeValue_3 data structure If cl is NULL, this means that
            // the call is local (Two tango classes within the same process and
            // with events between device from class 1 and device from classs 2)
            //
    ...
    cout4 << "ZmqEventSubscriptionChangeCmd: subscription for device " << dev_name << " attribute/pipe " << obj_name
          << " action " << action << " event " << event << " client lib = " << client_release << std::endl;

    //
    // If we receive this command while the DS is in its shuting down sequence,
    // do nothing
    //
    ...
    {
      TangoSys_OMemStream o;
      o << "The device server is shutting down! You can no longer subscribe "
           "for events"
        << std::ends;
    ...
    //
    // Get device pointer and check which IDL release it implements. If it is
    // less than IDL 4, refuse to use ZMQ event. To do so, simulate a Tango 7 DS
    // (throw command not exist exception) Also change event name if both device
    // and client supports IDL5 and lib 9 (For attribute conf. change event)
    //
    ...
    if(idl_vers < 4)
    {
      ...
      o << "\nSimulate a CommandNotFound exception to move to notifd event "
           "system"
        << std::ends;
    ...
    //
    // Create the event publisher socket (if not already done). Take care for
    // case where the device is running with db in a file
    //
    ...
    //
    // For forwarded attribute, eventually subscribe to events coming from root
    // attribute
    //
    ...
    if(intr_change == false && pipe_event == false)
    {
      ...
      if(attribute.is_fwd_att() == true && et != ATTR_CONF_EVENT)
      {
        ...
        //
        // We unsubscribe and subscribe. This is mandatory for following case:
        // The appli is killed and re-started but in the meantime, polling for
        // the root attribute has been stopped. The error that the polling is
        // not started for the root attribute is sent at subscription time
        //
    ...
    if(client_release >= 5 && add_compat_info) // client_release here is the minimum of the client
                                               // release and dev IDL version
    {
