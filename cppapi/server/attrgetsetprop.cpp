...
//
// description :        C++ source code for the Attribute class methods erlated
// to set/get attribute properties
//
// project :            TANGO
//
// author(s) :          E.Taurel
...
{
  //
  // Throw exception in case of Fwd attribute and the att configuration is not
  // yet received
  //
...
//
// description :
//		Add into the received structure configuration release 5 specific data
//(root att, mem info,...)
//
// argument :
// 		out :
...
void Attribute::set_properties(const Tango::AttributeConfig &conf, const std::string &dev_name,
                               TANGO_UNUSED(bool from_ds), std::vector<AttPropDb> &v_db)
{
  if(name_lower == "state" || name_lower == "status")
    return;

  //
  // Check if the caller try to change "hard coded" properties. Throw exception
  // in case of
  //
  ...
  // For each "string" property, an empty string means returns to its default
  // value which could be the library default value or the user defined default
  // value
  //
  ...
  set_one_str_prop("standard_unit", conf.standard_unit, standard_unit, v_db, def_user_prop, def_class_prop,
                   StdUnitNotSpec);
  delete_startup_exception("standard_unit", dev_name);

  set_one_str_prop("display_unit", conf.display_unit, display_unit, v_db, def_user_prop, def_class_prop,
                   DispUnitNotSpec);
  ...
  set_one_alarm_prop("min_value", conf.min_value, min_value_str, min_value, v_db, def_user_prop, def_class_prop,
                     check_min_value);
  delete_startup_exception("min_value", dev_name);

  set_one_alarm_prop("max_value", conf.max_value, max_value_str, max_value, v_db, def_user_prop, def_class_prop,
                     check_max_value);
  delete_startup_exception("max_value", dev_name);

  bool alrm_set;
  set_one_alarm_prop("min_alarm", conf.min_alarm, min_alarm_str, min_alarm, v_db, def_user_prop, def_class_prop,
                     alrm_set);
  if(alrm_set == false)
  ...
  set_one_alarm_prop("max_alarm", conf.max_alarm, max_alarm_str, max_alarm, v_db, def_user_prop, def_class_prop,
                     alrm_set);
  if(alrm_set == false)
...
void Attribute::set_properties(const Tango::AttributeConfig_3 &conf, const std::string &dev_name, bool from_ds,
                               std::vector<AttPropDb> &v_db)
{
  ...
  //
  // Add a check of the display level property because it is not checked by the
  // check_hard_coded_properties() template method called by the
  // set_properties() method. Display level is available only in
  // AttributeConfig_3
  //
     ...
     ((data_type != Tango::DEV_STRING) && (data_type != Tango::DEV_BOOLEAN) && (data_type != Tango::DEV_STATE)))
  {
    ...
    set_one_alarm_prop("min_warning", conf.att_alarm.min_warning, min_warning_str, min_warning, v_db, def_user_prop,
                       def_class_prop, alrm_set);
    if(alrm_set == false)
    ...
    set_one_alarm_prop("max_warning", conf.att_alarm.max_warning, max_warning_str, max_warning, v_db, def_user_prop,
                       def_class_prop, alrm_set);
    if(alrm_set == false)
    ...
    set_one_event_prop("rel_change", conf.event_prop.ch_event.rel_change, rel_change, v_db, def_user_prop,
                       def_class_prop);
    delete_startup_exception("rel_change", dev_name);

    set_one_event_prop("abs_change", conf.event_prop.ch_event.abs_change, abs_change, v_db, def_user_prop,
                       def_class_prop);
    delete_startup_exception("rel_change", dev_name);

    set_one_event_prop("archive_rel_change", conf.event_prop.arch_event.rel_change, archive_rel_change, v_db,
                       def_user_prop, def_class_prop);
    delete_startup_exception("archive_rel_change", dev_name);

    set_one_event_prop("archive_abs_change", conf.event_prop.arch_event.abs_change, archive_abs_change, v_db,
                       def_user_prop, def_class_prop);
  ...
  set_one_event_period("event_period", conf.event_prop.per_event.period, event_period, DEFAULT_EVENT_PERIOD, v_db,
                       def_user_prop, def_class_prop);
  delete_startup_exception("event_period", dev_name);

  set_one_event_period("archive_period", conf.event_prop.arch_event.period, archive_period, INT_MAX, v_db,
                       def_user_prop, def_class_prop);
  delete_startup_exception("archive_period", dev_name);
}

void Attribute::set_properties(const Tango::AttributeConfig_5 &conf, const std::string &dev_name, bool from_ds,
                               std::vector<AttPropDb> &v_db)
{
  //
  // Check that the request is not to change some unmutable properties added in
  // IDL 5
  //
...
//
// description :
//		Analyse one of the string properties. String properties are description,
// label, unit, standard_unit, 		display_unit and format
//
// argument :
// 		in :
...
void Attribute::set_one_str_prop(const char *prop_name, const CORBA::String_member &conf_val, std::string &att_conf,
                                 ...
                                 const std::vector<AttrProperty> &def_class_prop, const char *lib_def)
{
  ...
  {
    //
    // Return to lib default. If something defined as user default or class
    // default, put entry in DB to overwrite these defaults
    //
  ...
  {
    //
    // Return to user default or lib default. If something defined as class
    // default, put entry in DB in order to overwrite this default value.
    //
...
//
// description :
// 		Comment 1: If the user input value is equal to user default value do not
// store it in the database.
//
// 		Comment 2: User default value (if defined) has to be converted to double
// before it is compared with the user
//		input to determine if to store the new value in the database. String
// comparison is not appropriate in case of 		floating point numbers, e.g.
// "5.0"
// is numerically equal to "5.00" but the strings differ.
//
// 		Comment 3: If user defaults are defined - at first place the input string
// is converted to double to determine 		if it is a number. If so, the double
// value is cast to the type corresponding with the type of the attribute and
//		further compared with the double representation of the user default value.
// 		The purpose of casting is as follows. Lets take an example of an attribute
// of DevShort data type with user
//		default value for min_alarm set to 5. Now, if the user inputs "5.678" as a
// new value for min_alarm
// 		it would normally be cast to DevShort and stored in the database as "5"
// (this is the standard behaviour if the
//		user inputs a floating point value for a property of non-floating point
// type). But now the outcome "5" is
// equal 		to the user default value 5 and should not be stored in the
// database. This is why there is the cast of the 		user input value to the
// attribute data type before comparison with the user default value.
//
// Arguments:
//		in :
...
void Attribute::set_one_alarm_prop(const char *prop_name, const CORBA::String_member &conf_val,
                                   std::string &att_conf_str, Tango::Attr_CheckVal &att_conf,
                                   std::vector<AttPropDb> &v_db, const std::vector<AttrProperty> &def_user_prop,
                                   const std::vector<AttrProperty> &def_class_prop, bool &check_it)
{
  ...
  {
    // set class default if defined, user default value if defined, otherwise
  ...
  //
  // For string representation: If there is a default (class or user) and if the
  // user entered a value equal to the default but with a different precision
  // (3.2 and 3.20), take the default one instead of the user one
  //
...
void Attribute::set_rds_prop(const AttributeAlarm &att_alarm, const std::string &dev_name, std::vector<AttPropDb> &v_db,
...
//
// description :
//		Write into the attribute object the properties related to rds (delta_val
// and delta_t)
//
// Arguments:
//		in :
                                 ...
                                 const std::vector<AttrProperty> &def_class_prop)
{
  ...
  if(TG_strcasecmp(att_alarm.delta_t, AlrmValueNotSpec) == 0 || TG_strcasecmp(att_alarm.delta_t, "0") == 0 ||
  ...
  else if(delta_t_defined || delta_val_defined)
  {
    ...
    try
    {
      ...
      DEV_WARN_STREAM(dev) << "RDS (Read Different Set) incoherent in attribute " << name << " (only "
                           << (delta_t_defined ? "delta_t" : "delta_val") << " is set) " << std::endl;
    }
    catch(...)
    {
      std::cerr.clear();
...
//
// description :
//		Check which of the two properties related to RDS (delta_val, delta_t) has
// to be updated/deleted in Db
//
// Arguments:
//		in :
//			- att_alarm : New attribute properties
...
void Attribute::set_rds_prop_db(const AttributeAlarm &att_alarm, std::vector<AttPropDb> &v_db,
                                const std::vector<AttrProperty> &def_user_prop,
                                const std::vector<AttrProperty> &def_class_prop)
{
  std::stringstream str;
  ...
  //
  // RDS alarm values (delta_val and delta_t) are stored in or deleted from the
  // database only if both are set or both not specified.
  //
  ...
  {
    if((TG_strcasecmp(att_alarm.delta_val, NotANumber) == 0) ||
       (strcmp(att_alarm.delta_val, delta_val_usr_def_val.c_str()) == 0) || (strlen(att_alarm.delta_val) == 0))
    {
      delta_val_tmp_str   = delta_val_usr_def_val;
      delta_val_is_number = false;
    }
    else if(TG_strcasecmp(att_alarm.delta_val, AlrmValueNotSpec) == 0)
    {
      ...
      delta_val_is_number = false;
          ...
          (TG_strcasecmp(att_alarm.delta_val, NotANumber) == 0) || (strlen(att_alarm.delta_val) == 0))
  {
    delta_val_tmp_str   = AlrmValueNotSpec;
    delta_val_is_number = false;
  }
  ...
  {
    if((TG_strcasecmp(att_alarm.delta_t, NotANumber) == 0) ||
       (strcmp(att_alarm.delta_t, delta_t_usr_def_val.c_str()) == 0) || (strlen(att_alarm.delta_t) == 0))
    {
      delta_t_tmp_str   = delta_t_usr_def_val;
      delta_t_is_number = false;
    }
    else if((TG_strcasecmp(att_alarm.delta_t, AlrmValueNotSpec) == 0) || (TG_strcasecmp(att_alarm.delta_t, "0") == 0) ||
            (TG_strcasecmp(att_alarm.delta_t, "0.0") == 0))
    {
      delta_t_tmp_str   = "0";
      delta_t_is_number = false;
          ...
          (TG_strcasecmp(att_alarm.delta_t, "0.0") == 0) || (TG_strcasecmp(att_alarm.delta_t, NotANumber) == 0) ||
  ...
  //
  // Check if to store both delta_val and delta_t in database, delete or do
  // nothing
  //
  ...
  {
    if((TG_strcasecmp(delta_val_tmp_str.c_str(), delta_val_class_def_val.c_str()) == 0) &&
       (TG_strcasecmp(delta_t_tmp_str.c_str(), delta_t_class_def_val.c_str()) == 0))
      rds_delete_both = true;
    else if(((TG_strcasecmp(delta_val_tmp_str.c_str(), AlrmValueNotSpec) == 0) &&
             (TG_strcasecmp(delta_t_tmp_str.c_str(), "0") == 0)) ||
            ((TG_strcasecmp(delta_val_tmp_str.c_str(), AlrmValueNotSpec) != 0) &&
             (TG_strcasecmp(delta_t_tmp_str.c_str(), "0") != 0)))
      rds_store_both = true;
  }
  else if(delta_val_user_defaults || delta_t_user_defaults)
  {
    if((TG_strcasecmp(delta_val_tmp_str.c_str(), delta_val_usr_def_val.c_str()) == 0) &&
       (TG_strcasecmp(delta_t_tmp_str.c_str(), delta_t_usr_def_val.c_str()) == 0))
      rds_delete_both = true;
    else if(((TG_strcasecmp(delta_val_tmp_str.c_str(), AlrmValueNotSpec) == 0) &&
             (TG_strcasecmp(delta_t_tmp_str.c_str(), "0") == 0)) ||
            ((TG_strcasecmp(delta_val_tmp_str.c_str(), AlrmValueNotSpec) != 0) &&
...
//
// description :
//		Analyse new property when they are related to event. We have four of these
// properties: rel_change,
// abs_change, 		archive_rel_change, archive_abs_change
//
// Arguments:
//		in :
//			- prop_name : The property name
                                   ...
                                   const std::vector<AttrProperty> &def_class_prop)
{
  ...
  std::string rel_change_str(conf_val); // provided, comma separated min and/or
                                        // max values for the property
  std::string rel_change_usr_str;       // user default, comma separated min and/or
                                        // max values for the property, if defined
  std::string rel_change_class_str;     // class default, comma separated min and/or
                                        // max values for the property, if defined
  std::vector<double> rel_change_tmp;   // vector containing min and max values of the property
  std::vector<double> rel_change_usr;   // vector containing user default min and
                                        // max values of the property
  std::vector<double> rel_change_class; // vector containing class default min
                                        // and max values of the property
  bool rel_change_usr_def;              // true if there are user defaults defined for the
                                        // property
  bool rel_change_class_def;            // true if there are class defaults defined for the
                                        // property

  std::vector<bool> rel_change_set_usr_def;   // vector indicating if to use provided values for
                                              // the property or the user defaults if defined
  std::vector<bool> rel_change_set_class_def; // vector indicating if to use provided values
                                              // for the property or the class defaults if
                                              // defined
  ...
  validate_change_properties(d_name, prop_name, rel_change_str, rel_change_tmp, rel_change_set_usr_def,
                             rel_change_set_class_def);
  ...
  {
    prop_val[0] = (rel_change_set_class_def[0]) ? rel_change_usr[0]
                                                : ((rel_change_set_usr_def[0]) ? rel_change_usr[0] : rel_change_tmp[0]);
    prop_val[1] = (rel_change_set_class_def[1]) ? rel_change_usr[1]
...
//
// description :
//		Set IDl 5 specific attribute configuration. It's only the enum labels for
// attribute of the DEV_ENUM data
// type. 		The other IDL 5 specific attribute properties are non-mutable.
//
// argument :
//		in :
...
void Attribute::set_prop_5_specific(const AttributeConfig_5 &conf, const std::string &dev_name, bool from_ds,
                                    std::vector<AttPropDb> &v_db)
{
  if(data_type == Tango::DEV_ENUM)
  {
    ...
    if(from_ds == false)
    {
            ...
            TG_strcasecmp(conf.enum_labels[0], NotANumber) == 0 || strlen(conf.enum_labels[0]) == 0)))
      {
        if(conf.enum_labels.length() != enum_labels.size())
        {
          std::stringstream ss;
          ss << "Device " << dev_name << "-> Attribute : " << name;
          ss << "\nIt's not supported to change enumeration labels number from "
                "outside the Tango device class code";
    ...
    if(TG_strcasecmp(conf.enum_labels[0], AlrmValueNotSpec) == 0)
    {
      ...
      std::stringstream ss;
      ss << "Device " << dev_name << "-> Attribute : " << name;
      ss << "\nNo enumeration label(s) default library value for attribute of "
            "the Tango::DEV_ENUM data type";
    ...
    else if(strlen(conf.enum_labels[0]) == 0)
    {
      ...
      if(usr_defaults == false)
      {
        std::stringstream ss;
        ss << "Device " << dev_name << "-> Attribute : " << name;
        ss << "\nNo enumeration labels default library value for attribute of "
              "the Tango::DEV_ENUM data type";
    ...
    else if(TG_strcasecmp(conf.enum_labels[0], NotANumber) == 0)
    {
      ...
      if(class_defaults == false)
      {
        if(usr_defaults == false)
        {
          std::stringstream ss;
          ss << "Device " << dev_name << "-> Attribute : " << name;
          ss << "\nNo enumeration labels default library value for attribute "
                "of the Tango::DEV_ENUM data type";
...
void Attribute::set_min_alarm(char *new_min_alarm_str) { set_min_alarm(std::string(new_min_alarm_str)); }

void Attribute::set_min_alarm(const char *new_min_alarm_str) { set_min_alarm(std::string(new_min_alarm_str)); }

void Attribute::set_max_alarm(char *new_max_alarm_str) { set_max_alarm(std::string(new_max_alarm_str)); }

void Attribute::set_max_alarm(const char *new_max_alarm_str) { set_max_alarm(std::string(new_max_alarm_str)); }

void Attribute::set_min_warning(char *new_min_warning_str) { set_min_warning(std::string(new_min_warning_str)); }

void Attribute::set_min_warning(const char *new_min_warning_str) { set_min_warning(std::string(new_min_warning_str)); }

void Attribute::set_max_warning(char *new_max_warning_str) { set_max_warning(std::string(new_max_warning_str)); }

void Attribute::set_max_warning(const char *new_max_warning_str) { set_max_warning(std::string(new_max_warning_str)); }
...
//
// description :
//		Check coherency between min and max value for properties where a min and a
// max is used
//
// argument :
//		in :
...
//		    - change_prop_str : A string representing the change property (in the
// form of either "value" or 								"value1,value2")
//
// 		out :
//			- validated_prop : A vector of parsed change properties values in the
// form of numbers of type
// double
//
//-------------------------------------------------------------------------------------------------------------------

void Attribute::validate_change_properties(const std::string &dev_name, const char *prop_name,
                                           std::string &change_prop_str, std::vector<double> &validated_prop)
{
...
// form of numbers of type
// double
//			- bring_usr_def : A vector of boolean values indicating if for a
// corresponding value in
// validated_prop 							  vector there was a request to restore the user
// defined value
//			- bring_class_def : A vector of boolean values indicating if for a
// corresponding value in
// validated_prop 								vector there was a request to restore the
// class defined value
//
//------------------------------------------------------------------------------------------------------------------
    ...
    if(str >> prop_tmp && str.eof())
    {
      if(fabs(prop_tmp) > 0 && prop_tmp != INT_MAX)
      {
        if((data_type != Tango::DEV_STRING) && (data_type != Tango::DEV_BOOLEAN) && (data_type != Tango::DEV_STATE))
        {
          ...
          throw_err_data_type(prop_name, const_cast<std::string &>(dev_name),
                              "Attribute::validate_change_properties()");
      }
    }
    else
...
void Attribute::event_prop_db_xxx(const std::vector<double> &rel_change_tmp, const std::vector<double> &rel_change_usr,
                                  std::vector<AttPropDb> &v_db, AttPropDb &apd)
{
...
void Attribute::set_one_event_period(const char *prop_name, const CORBA::String_member &conf_val, int &prop_val,
                                     const int &prop_def, std::vector<AttPropDb> &v_db,
                                     const std::vector<AttrProperty> &def_user_prop,
                                     const std::vector<AttrProperty> &def_class_prop)
{
  AttPropDb apd;
  ...
  if(TG_strcasecmp(conf_val, AlrmValueNotSpec) == 0 || TG_strcasecmp(conf_val, def_event_period.str().c_str()) == 0)
  {
    // force library defaults (even if user defaults defined)
    prop_val = prop_def;
  }
  ...
  if(prop_val != old_prop_val)
  {
    if(class_defaults)
    {
      if((TG_strcasecmp(class_def_val.c_str(), AlrmValueNotSpec) == 0) ||
         (TG_strcasecmp(class_def_val.c_str(), NotANumber) == 0) || (strlen(class_def_val.c_str()) == 0))
      {
        std::stringstream str;
        str << (int) (prop_def);
    ...
    else
    {
      ...
      if((TG_strcasecmp(conf_val, AlrmValueNotSpec) == 0) ||
         (TG_strcasecmp(conf_val, def_event_period.str().c_str()) == 0) || (TG_strcasecmp(conf_val, NotANumber) == 0) ||
         (strlen(conf_val) == 0) || (input_equal_def == true))
        store_in_db = false;
    }
...
//
// description :
//		Check if the user tries to change attribute properties considered as hard
// coded added by IDL 5
...
void Attribute::convert_prop_value(const char *prop_name, std::string &value_str, Attr_CheckVal &val,
...
//
// description :
//		Update database according to the info received in the v_db parameter. We
// update or delete info in db for 		attribute configuration.
//
// Arguments:
//		in :
