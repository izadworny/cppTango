...
CORBA::Any *DevInitCmd::execute(DeviceImpl *device, TANGO_UNUSED(const CORBA::Any &in_any))
{
  ...
  //
  // Get device interface only if necessary (some client(s) listening on device
  ...
  try
  {
    ...
    //
    // Re-configure polling in device on which the Init cmd been done is the
    // admin device but only if the Init is not called during the DS startup
    // sequence
    //
    ...
    //
    // Apply memorized values for memorized attributes (if any). For Py DS, if
    // some attributes are memorized, the write_attributes call will take the
    // Python lock
    //
  ...
  catch(Tango::DevFailed &e)
  {
    ...
    o << "\nHINT: RESTART device with the Restart command of the device server "
         "adm. device";
  ...
  //
  // Check if device interface has changed and eventually fire device interface
  // change event
  //
