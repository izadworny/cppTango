...
// polling configuration stored in
// db. 		This is done in separate thread in order to equally spread all the
// polled objects polling time on the smallest polling period.
//
//------------------------------------------------------------------------------------------------------------------
...
void Util::polling_configure()
{
  ...
  //
  // Create the structure used to send data to the polling thread and store them
  // in a vector
  //
  ...
  for(i = 0; i < tmp_cl_list.size(); i++)
  {
    ...
    for(j = 0; j < dev_list.size(); j++)
    {
      ...
      for(k = 0; k < poll_cmd_list.size(); k += 2)
      {
        ...
        //
        // Convert polling period to a number and store polling thread command
        // params
        //
      ...
      if(nb_cmd != 0)
      {
        ...
        PollingThreadInfo *th_info =
            get_polling_thread_info_by_id(get_polling_thread_id_by_name(v_poll_cmd[0]->svalue[0]));
  ...
  //
  // Send command to polling thread one by one for each threads. In the
  // following computation , I try to take into account the non real time aspect
  // of our OS. I remove 15 mS from each sleeping time due to thread wake-up
  // time
  //

  unsigned long nb_thread = poll_ths.size();
  cout4 << "POLLING: " << nb_thread << " thread(s) needed for polling from a pool of "
        << get_polling_threads_pool_size() << std::endl;
  ...
  //
  // If some polling related prop. (polling conf or dev conf) has to be updated
  // in db, do it now Add a check to verify that it is possible to store polling
  // pool conf in database. Property length cannot be longer than 256 characters
  //
...
//
// description :
//		Trigger the polling thread for polled attributes registered with a polling
// update period set as 		"externally triggered" (0 mS)
//
// args :
// 		in :
...
void Util::trigger_attr_polling(Tango::DeviceImpl *dev, const std::string &name)
{
  ...
  //
  // Send command to the polling thread but wait in case of previous cmd still
  // not executed
  //
...

  {
    ...
    while(shared_cmd.trigger == true)
    {
      ...
      // attribute methods) which are rapidly sent by the client. Client request
      // cmd1 which send trigger to the polling thread The polling thread wake
      // up clear shared_cmd.trigger and try to execute the command. But cmd 1
      // thread still owns the device monitor and polling thread wait. cmd 1
      // finished and client immediately send the command a new time. On
      // Windows, it may happens that the polling thread is not activated just
      // after the cmd thread has released the device monitor. As the client
      // sent a new command, the device monitor is immediately re-taken by the
      // thread executing the new command sent by the client. An order is sent
      // to the polling thread and the cmd thread reach this code. It will wait
      // for polling thread to clear shared_cmd.trigger. But, the polling thread
      // is already waiting for the device monitor and ..... deadlock....
      //
...
//
// description :
//		Trigger the polling thread for polled command registered with a polling
// update period set as 		"externally triggered" (0 mS)
//
// args :
// 		in :
...
//
// description :
//		Create a polling thread for the device specified as parameter. If the
// thread already exist, do nothing 		If the pool is full, associate this
// device to an already existing thread
//
// args :
// 		in :
// 			- dev_name : The device name
//		   	- startup : True if this method is called during DS startup. In such
// a case, some exception
// should not 						be thrown
//          - polling_9 : The polling strict period flag
//		   	- smallest_upd : The smallest upd !
//
// return :
// 		This method returns -2 if the pool conf. does not need any update. It
// returns -1 if a new thread has been
//		created. It returns the index in the pool conf of the entry which has to
// be modified when the device is 		associated to an already existing thread
//
//-------------------------------------------------------------------------------------------------------------------
...
int Util::create_poll_thread(const char *dev_name, bool startup, bool polling_9, int smallest_upd)
{
  ...
  if(poll_ths.size() != poll_pool_size)
  {
    ...
    if(asso_devs.empty() != true)
    {
      ...
      //
      // If we find a thread for one of the associated device, no need to create
      // a new one. Simply add the device entry in the map <dev_name,thread id>
      //
  ...
  else
  {
    ...
    if(asso_devs.empty() != true)
    {
      ...
      //
      // If we find a thread for one of the associated device, simply add the
      // device entry in the map <dev_name,thread id>
      //
    ...
    //
    // Find a device already assigned to this thread and then get the device
    // entry in the thread pool
    //
...
//
// description :
//		Return the PollingThreadInfo for the thread with the ID specified as the
// input argument
//
// args :
//		in :
...
void Util::check_pool_conf(DServer *admin_dev, unsigned long pool_size)
{
  ...
  //
  // First, get the list of devices instantiated in this server and check if the
  // polled devices are defined within the server
  //
  ...
  for(iter = mod_conf.begin(); iter != mod_conf.end(); ++iter)
  {
    ...
    for(iter_entry = splitted.begin(); iter_entry != splitted.end(); ++iter_entry)
    {
      ...
      //
      // The device is not defined in the DS, remove it from the conf. (Do not
      // copy it in the new conf built by this method)
      //
      ...
      {
        //
        // Now, we know that the device exists in the server, but is it really
        // polled ?
        //
  ...
  {
    cout << "WARNING: More threads defined in the polling threads pool "
            "configuration";
    cout << " than in its size (" << mod_conf.size() << " > " << pool_size << ")" << std::endl;
    cout << "The pool configuration will be automatically updated" << std::endl;

    //
    // If we have more threads in the conf than in the pool, distribute the
    // extra thread devices to the still existing threads
    //
...
//
// description :
//		Check if all the cmd/attr to be polled for a device are really supported
// by the device
//
// args :
//		in :
...
//
// return :
// 		This method returns 0 if no polling conf has been modified. Otherwise, it
// returns -1 if only the command list
//		has been modified, -2 if the only the attribute list has been modified and
//-3 if both of them have been
// 		modified
//
//------------------------------------------------------------------------------------------------------------------

int Util::check_dev_poll(std::vector<std::string> &poll_cmd_list, std::vector<std::string> &poll_attr_list,
                         DeviceImpl *dev)
{
  int ret = 0;
...
//			- splited_str : The splitted string returned in a vector of individual
// elt
//
//-------------------------------------------------------------------------------------------------------------------
...
// args :
//		in :
// 			- upd_devs : ref to a vector with one elt for each dev with polling
// prop. to be updated
//			- admin_dev : The DS admin device
//
//------------------------------------------------------------------------------------------------------------------
...
void Util::upd_polling_prop(const std::vector<DevDbUpd> &upd_devs, DServer *admin_dev)
{
  ...
  for(i = 0; i < upd_devs.size(); i++)
  {
    ...
    //
    // If now the device does not have any objects polled (no cmd, no attr), it
    // must be removed from the pool conf.
    //
    ...
    while(retry == true)
    {
      try
      {
        ...
        else
        {
          ...
          // If we have only one thread, it is easy to catch this threshold. In
          // case this threshold is reached, split entry in several sub-entries
...
//
// description :
//		Get from the polling pool configuration which devices should be polled by
// the thread which is in charge
// of 		device dev
//
// args :
//		in :
// 			- dev : The device name
//			- th_polled_devs : List of devices also polled by the thread in charge
// of dev
//
//------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Remove all the polling thread info from the vector of polling thread info
