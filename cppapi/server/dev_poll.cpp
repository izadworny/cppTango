...
// file :		dev_poll.cpp
//
// description :	C++ source code for part of the DeviceImpl class. This class
// is the root class for all derived
...
//
// description :
//		Init polling info for device running without DB. In such a case, polling
// is available only for
//      object with polling defined in code. Fill in string vectors which are in
//      case of DS using database initialised from the db.
//
//------------------------------------------------------------------------------------------------------------------
...
bool DeviceImpl::is_attribute_polled(const std::string &att_name)
{
  ...
  /*    Tango::Attribute &the_att =
  dev_attr->get_attr_by_name(att_name.c_str()); if (
  the_att.get_polling_period() > 0 )
          {

  //
  // check the list of non_auto_polled attributes to verify wether the polling
  was disabled
  //
  ...
  #else
                          if (strcasecmp(napa[j].c_str(), att_name.c_str()) ==
  0) #endif
                          {
                                  return false;
                          }
...
bool DeviceImpl::is_command_polled(const std::string &cmd_name)
{
  ...
  #else
                          if (strcasecmp(napa[j].c_str(), cmd_name.c_str()) ==
  0) #endif
                          {
                                  return false;
                          }
...
//
// description :
//		Poll one attribute. If the attribute is already polled, update its polling
// period to the new value
//
// argument:
//		in 	:
//			- att_name : The attribute name
...
void DeviceImpl::poll_attribute(const std::string &att_name, int period) { poll_object(att_name, period, POLL_ATTR); }
...
//
// description :
//		Poll one command. If the command is already polled, update its polling
// period to the new value
//
// argument:
//		in :
...
void DeviceImpl::poll_command(const std::string &cmd_name, int period) { poll_object(cmd_name, period, POLL_CMD); }
...
//
// description :
//		Poll one object. If the object is already polled, update its polling
// period to the new value
//
// argument:
//		in :
...
void DeviceImpl::poll_object(const std::string &obj_name, int period, PollObjType type)
{
  ...
  {
    TANGO_THROW_EXCEPTION(API_NotSupported, "It's not supported to start polling on any device "
                                            "cmd/attr while the device is shutting down");
  }

  if(tg->is_svr_starting() == true)
  {
    //
    // If server is starting, we rely on the Util::polling_configure method to
    // effectively start the polling Nevertheless, some tests are coded before
    // doing the job
    //
    ...
    //
    // Just to be sure that the command/attribute exist. Also init ptr to the
    // command/attribute polled list
    //
    ...
    // If yes, only update polling period in vector. Otherwise, add cmd name and
    // polling period in vector
    //
    // Util::polling_configure will ask dserver polling command to store info in
    // db only if polling period is negative.
    //
  ...
  {
    //
    // Ask the admin device to do the work (simulating the classical way to tune
    // polling) If the attribute is already polled, it's an update polling
    // period. Otherwise, it's a add object polling command
    //
...
void DeviceImpl::stop_poll_attribute(const std::string &att_name) { stop_poll_object(att_name, POLL_ATTR); }
...
void DeviceImpl::stop_poll_command(const std::string &cmd_name) { stop_poll_object(cmd_name, POLL_CMD); }
...
void DeviceImpl::stop_poll_object(const std::string &obj_name, PollObjType type)
{
  ...
  else
  {
    ...
    {
      //
      // Ask the admin device to do the work (simulating the classical way to
      // tune polling)
      //
