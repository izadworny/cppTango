...
Device_2Impl::Device_2Impl(DeviceClass *device_class, const char *dev_name, const char *desc, Tango::DevState dev_state,
...
CORBA::Any *Device_2Impl::command_inout_2(const char *in_cmd, const CORBA::Any &in_data, Tango::DevSource source)
{
  ...
  else
  {
    ...
    try
    {
      ...
      // Warning : Since IDL 3 (Tango V5), state and status are polled as
      // attributes
      //
      ...
      // and the command is not in the device list of command which should not
      be
      ...
      //
                                      ...
                                      {
                                              found = false;
                                              std::vector<std::string> &napc =
      get_non_auto_polled_cmd(); for (i = 0;i < napc.size();i++)
                                              {
                                                      if (napc[i] == cmd_str)
                                                              found = true;
                                              }
                                              ...
                                              if (found == true)
                                              {
                                                      TangoSys_OMemStream o;
                                                      o << "Command " << in_cmd
      << " not polled" << std::ends; TANGO_THROW_EXCEPTION(API_CmdNotPolled,
      o.str());
                                              }
                                              else
                                              {
                                                      Tango::Util *tg =
      Tango::Util::instance(); DServer *adm_dev = tg->get_dserver_device();

                                                      DevVarLongStringArray
      *send = new DevVarLongStringArray(); send->lvalue.length(1);
                                                      send->svalue.length(3);

                                                      send->lvalue[0] =
      poll_period; send->svalue[0] = device_name.c_str(); send->svalue[1] =
      Tango::string_dup("command"); send->svalue[2] = in_cmd;
      ...
      #else
                                                      struct timespec
      to_wait,inter; to_wait.tv_sec = 0; to_wait.tv_nsec = 600000000;
  ...
  try
  {
    ...
    else
    {
      ...
      try
      {
        ...
        //
        // Check that all wanted attributes are polled. If some are non polled,
        ...
        // defined, and the attribute is not in the device list of attr which
        // should not be polled, start to poll them
        //
        ...
        //
        // For each attribute, check that some data are available in cache and
        // that they are not too old
        //
        ...
        for(i = 0; i < nb_attr; i++)
        {
          ...
          //
          // Check that data are still refreshed by the polling thread
          // Skip this test for object with external polling triggering (upd =
          // 0)
          //
          ...
          //
          // Finally, after all these checks, get value and store it in the
          // sequence sent back to user In order to avoid unnecessary copy,
          // don't use the assignement operator of the AttributeValue structure
          // which copy each element and therefore also copy the Any object. The
          // Any assignement operator is a deep copy! Create a new sequence
          // using the attribute buffer and insert it into the Any. The sequence
          // inside the source Any has been created using the attribute data
          // buffer.
          //
              ...
              if(vers >= 4)
              {
                ...
                if(att_val_4.quality != Tango::ATTR_INVALID)
                {
                  if(type == Tango::DEV_ENCODED)
                  {
                    ...
                    o << " It's not possible to retrieve this data type "
                         "through the interface you are using (IDL V2)"
                      << std::ends;
                    TANGO_THROW_EXCEPTION(API_NotSupportedFeature, o.str());
                  }
      ...
      {
        //
        // Re-throw exception if the source parameter is CACHE or if the source
        // param. is CACHE_DEV but one of the attribute is really in fault (not
        // the polling)
        //
    ...
    //
    // If the source parameter is CACHE, returned data, else called method
    // reading attribute from device
    //
...
Tango::DevCmdInfo_2 *Device_2Impl::command_query_2(const char *command)
{
  ...
  //
  // Try to retrieve the command in the command list first at class level then
  // at device level (in case of dyn command instaleld at device level)
  //
...
void Device_2Impl::Polled_2_Live(long data_type, CORBA::Any &hist_any, CORBA::Any &live_any)
{
  ...
  case Tango::DEV_ENUM:
    hist_any >>= tmp_sh;
    new_tmp_sh =
        new DevVarShortArray(tmp_sh->length(), tmp_sh->length(), const_cast<short *>(tmp_sh->get_buffer()), false);
    live_any <<= new_tmp_sh;
    break;
  ...
  case Tango::DEV_DOUBLE:
    hist_any >>= tmp_db;
    new_tmp_db =
        new DevVarDoubleArray(tmp_db->length(), tmp_db->length(), const_cast<double *>(tmp_db->get_buffer()), false);
    live_any <<= new_tmp_db;
    break;
  ...
  case Tango::DEV_LONG:
    hist_any >>= tmp_lg;
    new_tmp_lg =
        new DevVarLongArray(tmp_lg->length(), tmp_lg->length(), const_cast<DevLong *>(tmp_lg->get_buffer()), false);
    live_any <<= new_tmp_lg;
    break;
  ...
  case Tango::DEV_STRING:
    hist_any >>= tmp_str;
    new_tmp_str =
        new DevVarStringArray(tmp_str->length(), tmp_str->length(), const_cast<char **>(tmp_str->get_buffer()), false);
  ...
  case Tango::DEV_FLOAT:
    hist_any >>= tmp_fl;
    new_tmp_fl =
        new DevVarFloatArray(tmp_fl->length(), tmp_fl->length(), const_cast<float *>(tmp_fl->get_buffer()), false);
    live_any <<= new_tmp_fl;
    break;

  case Tango::DEV_BOOLEAN:
    hist_any >>= tmp_boo;
    new_tmp_boo = new DevVarBooleanArray(tmp_boo->length(), tmp_boo->length(),
                                         const_cast<DevBoolean *>(tmp_boo->get_buffer()), false);
    live_any <<= new_tmp_boo;
    break;

  case Tango::DEV_USHORT:
    hist_any >>= tmp_ush;
    new_tmp_ush = new DevVarUShortArray(tmp_ush->length(), tmp_ush->length(),
                                        const_cast<DevUShort *>(tmp_ush->get_buffer()), false);
  ...
  case Tango::DEV_UCHAR:
    hist_any >>= tmp_uch;
    new_tmp_uch =
        new DevVarCharArray(tmp_uch->length(), tmp_uch->length(), const_cast<DevUChar *>(tmp_uch->get_buffer()), false);
