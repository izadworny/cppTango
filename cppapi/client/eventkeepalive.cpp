...
// file :				event.cpp
//
// description :		C++ classes for implementing the event system
// KeepAliveThread. This class cheks that the
//						heartbeat event are well received. It also manages the stateless
// subscription
// because this 						is this class which will regularely re-try to
// subsribe to event in case it is needed. 						Finally, it is also this
// class which generates the re-connection in case a device server sending
// events is stopped and re-started
//
...
//
// description :
//		Method to reconnect the process to an event channel in case of
// reconnection to a notifd
//
// argument :
//		in :
//			- ipos : An iterator to the EventChannel structure to reconnect to in
// the Event Channel map
    ...
    if(epos->second.channel_name == ipos->first)
    {
      ...
      if(need_reconnect == true)
      {
        try
        {
          ...
          event_consumer->connect_event_channel(adm_name, epos->second.get_device_proxy().get_device_db(), true, dummy);
...
//
// description :
//		Method to reconnect the process to a ZMQ event channel in case of
// reconnection
//
// argument :
//		in :
...
bool EventConsumerKeepAliveThread::reconnect_to_zmq_channel(const EvChanIte &ipos, EventConsumer *event_consumer,
                                                            DeviceData &dd)
{
  bool ret = true;
  EvCbIte epos;
    ...
    if(epos->second.channel_name == ipos->first)
    {
      ...
      if(need_reconnect == true)
      {
        try
        {
          ...
          //
          // Forget exception which could happen during massive restart of
          // device server process running on the same host
          //
          try
          {
          ...
          event_consumer->connect_event_channel(adm_name, epos->second.get_device_proxy().get_device_db(), true,
                                                subscriber_out);
...
//
// description :
//		Method to reconnect each event associated to a specific event channel to
// the just reconnected event
// channel
//
// argument :
//		in :
//			- ipos : An iterator to the EventChannel structure in the Event Channel
// map
//			- event_consumer : Pointer to the EventConsumer singleton
//
//--------------------------------------------------------------------------------------------------------------------
    ...
    if(epos->second.channel_name == ipos->first)
    {
      ...
      if(need_reconnect == true)
      {
        ...
        catch(...)
        {
          ...
          ss << "EventConsumerKeepAliveThread::reconnect_to_event() cannot get "
                "callback monitor for "
             << epos->first;
...
// argument :
//		in :
//			- epos : An iterator to the EventCallback structure in the Event
// Callback map
...
{
  //
  // Build a filter using the CORBA Notify constraint Language (use attribute
  ...
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                              "Caught CORBA::COMM_FAILURE exception while "
                              "creating event filter (check filter)");
  }
  catch(...)
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
...
void EventConsumerKeepAliveThread::reconnect_to_zmq_event(const EvChanIte &ipos, EventConsumer *event_consumer,
                                                          DeviceData &dd)
{
  EvCbIte epos;
      ...
      if(need_reconnect == true)
      {
        try
        {
          ...
          try
          {
            ...
            event_consumer->connect_event_system(d_name, epos->second.obj_name, epos->second.event_name, vs, ipos,
                                                 epos->second, dd, ipos->second.valid_endpoint);
        ...
        catch(...)
        {
          ...
          ss << "EventConsumerKeepAliveThread::reconnect_to_zmq_event() cannot "
                "get callback monitor for "
             << epos->first;
          au->print_error_message(ss.str().c_str());
        }
      }
...
void *EventConsumerKeepAliveThread::run_undetached(TANGO_UNUSED(void *arg))
{
  ...
  while(exit_th == false)
  {
    ...
    //
    // Go to sleep until next heartbeat. Wait on a monitor. This allows another
    // thread to wake-up this thread before the end of the
    // EVENT_HEARTBEAT_PERIOD time which is 10 seconds. Only one command can now
    // be send to the thread. It is a stop command
    //
    ...
    //
    // Be sure to have valid event consumer object (In case of long startup OS
    // with some notifd event(s) subscribed at the end of the process startup.
    // Verified with some ESRF HdbEventHandler process)
    //
...

    {
      ...
      for(ipos = event_consumer->channel_map.begin(); ipos != event_consumer->channel_map.end(); ++ipos)
      {
        try
        {
          ...
          //
          // Check if a heartbeat have been skipped. If a heartbeat is missing,
          // there are four possibilities : 1 - The notifd is dead (or the crate
          // is rebooting or has already reboot) 2 - The server is dead 3 - The
          // network was down; 4 - The server has been restarted on another
          // host.
          //
          ...
          {
            // When the heartbeat has worked, mark the connection to the notifd
            // as OK
        ...
        catch(...)
        {
          ...
          ss << "EventConsumerKeepAliveThread::run_undetached() timeout on "
                "callback monitor of "
             << epos->first;
          au->print_error_message(ss.str().c_str());
        }
...
//
// description :
//		Try to connect not yet connected event(s). Try first with ZMQ event
// consumer. 		If ZMQ is not used (API_CommandNotFound exception), use notifd.
//
// argument :
//		in :
  ...
  if(!event_consumer->event_not_connected.empty())
  {
        ...
        /*vpos++*/)
    {
      ...
      if((now - vpos->last_heartbeat) >= (EVENT_HEARTBEAT_PERIOD - 1))
      {
        ...
        {
          // try to subscribe
          event_consumer->connect_event(vpos->device, vpos->attribute, vpos->event_type, vpos->callback, vpos->ev_queue,
                                        vpos->filters, vpos->event_name, vpos->event_id);
        ...
        catch(Tango::DevFailed &e)
        {
          ...
          if(reason == API_CommandNotFound)
          {
            try
            {
              ...
              notifd_event_consumer->connect_event(vpos->device, vpos->attribute, vpos->event_type, vpos->callback,
                                                   vpos->ev_queue, vpos->filters, vpos->event_name, vpos->event_id);
        ...
        catch(...)
        {
          ...
          ApiUtil *au = ApiUtil::instance();
          au->print_error_message("During the event subscription an exception was sent which is "
...
//
// description :
//		Try to connect not yet connected event(s). This method is called only in
// case of forwarded attribute
// with
  ...
  if(!event_consumer->event_not_connected.empty())
  {
        ...
        /*vpos++*/)
    {
      ...
      catch(...)
      {
        ...
        ApiUtil *au = ApiUtil::instance();
        au->print_error_message("During the event subscription an exception was sent which is not "
                                "a Tango::DevFailed exception!");
      }
...
//
// description :
//		Confirm event subscription for all events coming from a specified event
// channel (Device server)
//
// argument :
//		in :
//			- event_consumer : The ZMQ event consumer object
                                                        ...
                                                        const std::map<std::string, EventChannelStruct>::iterator &ipos)
{
  ...
  if(ipos->second.channel_type == ZMQ && cmd_params.empty() == false)
  {
    ...
    catch(Tango::DevFailed &e)
    {
      ...
      {
        //
        // We are connected to a Tango 8 server which do not implement the
        // EventConfirmSubscription command Send confirmation the old way
        //
...
//
// description :
//		Main method executed to send error to the user callback or to reconnect
// the event
//
// argument :
//		in :
                                                  ...
                                                  const std::map<std::string, EventChannelStruct>::iterator &ipos)
{
  ...
  if(ipos->second.channel_type == NOTIFD)
  {
    ...
    {
      //
      // Check if the device server is now running on a different host. In this
      // case we have to reconnect to another notification daemon.
      //
      DeviceInfo info;
      try
      {
    ...
    //
    // if the connection to the notify daemon is marked as ok, the device server
    // is working fine but the heartbeat is still not coming back since three
    // periods: The notify deamon might have closed the connection, try to
    // reconnect!
    //
    ...
    //
    // Re-build connection to the event channel. This is a two steps process.
    // First, reconnect to the new event channel, then reconnect callbacks to
    // this new event channel
    //
  ...
  errors[0].desc                 = Tango::string_dup("Event channel is not responding anymore, maybe the "
                                                                     "server or event system is down");
        ...
        for(esspos = epos->second.callback_list.begin(); esspos != epos->second.callback_list.end(); ++esspos)
        {
          ...
          if(event_name == CONF_TYPE_EVENT)
          {
            ...
            if(callback != NULL)
            {
              ...
              catch(...)
              {
                ...
                ss << "EventConsumerKeepAliveThread::run_undetached() "
                      "exception in callback method of  "
                   << epos->first;
                au->print_error_message(ss.str().c_str());
              }
          ...
          else if(event_name == EventName[INTERFACE_CHANGE_EVENT])
          {
            DevIntrChangeEventData *event_data =
                new DevIntrChangeEventData(esspos->device, event_name, domain_name, (CommandInfoList *) NULL,
                                           (AttributeInfoListEx *) NULL, false, errors);
            // if a callback method was specified, call it!
            if(callback != NULL)
            {
              try
          ...
          {
            PipeEventData *event_data = new PipeEventData(esspos->device, domain_name, event_name, dev_pipe, errors);
          ...
          {
            FwdEventData *event_data = new FwdEventData(esspos->device, domain_name, event_name, dev_attr, errors);
...
//
// description :
//		Re subscribe to the event after a successfull reconnection to the event
// channel (device server)
//
// argument :
//		in :
...
void EventConsumerKeepAliveThread::re_subscribe_after_reconnect(
    ZmqEventConsumer *event_consumer, NotifdEventConsumer *notifd_event_consumer,
    const std::map<std::string, EventCallBackStruct>::iterator &epos,
    const std::map<std::string, EventChannelStruct>::iterator &ipos, const std::string &domain_name)
{
  DeviceData subscriber_in;
  std::vector<std::string> subscriber_info;
    ...
    if((ev_name == "change") || (ev_name == "quality") || (ev_name == "archive") || (ev_name == "user_event"))
    {
      ...
      try
      {
        ...
        *da = device.read_attribute(epos->second.obj_name.c_str());
        ...
        // when the notifd has not closed the connection. Increase the counter
        // to detect when the heartbeat is not coming back.
        //
      ...
      for(esspos = epos->second.callback_list.begin(); esspos != epos->second.callback_list.end(); ++esspos)
      {
        ...
        if(callback != NULL)
        {
          ...
          catch(...)
          {
            ...
            ss << "EventConsumerKeepAliveThread::run_undetached() exception in "
                  "callback method of "
               << epos->first;
            au->print_error_message(ss.str().c_str());
          }
    ...
    else if(epos->second.event_name == EventName[INTERFACE_CHANGE_EVENT])
    {
      ...
      for(esspos = epos->second.callback_list.begin(); esspos != epos->second.callback_list.end(); ++esspos)
      {
        ...
        if(cb_ctr != cb_nb)
        {
          ...
          *cil_copy  = *cil;
          event_data = new DevIntrChangeEventData(esspos->device, ev_name, dom_name, cil_copy, aie_copy, true, err);
        }
        else
        {
...
// argument :
//		in :
//			- vpos : An iterator in the vector of non-connected event for the
// concerned event
...
void EventConsumerKeepAliveThread::stateless_subscription_failed(const std::vector<EventNotConnected>::iterator &vpos,
                                                                 const DevFailed &e, const time_t &now)
{
  //
  // Subscribe has not worked, try again in the next hearbeat period
  //
  ...
  //
  // The event can still not be connected. Send the return error message as
  // event to the client application. Push an event with the error message!
  //
     ...
     (vpos->event_name == "periodic") || (vpos->event_name == "user_event"))
  {
    ...
    if(vpos->callback != NULL)
    {
      ...
      catch(...)
      {
        ...
        ss << "EventConsumerKeepAliveThread::stateless_subscription_failed() "
              "exception in callback method of "
           << domain_name;
        au->print_error_message(ss.str().c_str());
      }
  ...
  {
    DevIntrChangeEventData *event_data =
        new DevIntrChangeEventData(vpos->device, vpos->event_name, domain_name, (CommandInfoList *) NULL,
