...
//
// description :  	C++ include file for implementing the TANGO event related
// client classes - EventConsumer and others.
//             		These classes are used to receive events from the server and
...
//
// These template specialization allow us to set or get sequences within a
// AttrValUnion union. Union method to get/set sequence value are named
// short_att_value(), double_att_value(), float_att_value()....
//
...
//
// Create a dummy empty sequence and init union with this dummy sequence
// (minimun data copy) Retrieve a reference to that sequence once it is in the
// union and replace its data pointer by the data in the CdrMemoryStream. Use
// set_seq(), get_seq() specialized template created just above to set/get union
// sequences T is the basic data type (DevShort) TA is the sequence data type
// (DevVarShortArray)
//
...
typedef struct event_callback : public EventCallBackBase, public EventCallBackZmq
{
  ...
  std::string get_client_attribute_name() { return client_attribute_name; }
} EventCallBackStruct;
...
class EventConsumer
{
  typedef void (*EventCallbackFunction)(std::string event_name, std::string event_type,
...
public:
  ...
  int connect_event(DeviceProxy *, const std::string &, EventType, CallBack *, EventQueue *,
                    const std::vector<std::string> &, std::string &, int event_id = 0);
  ...
  int subscribe_event(DeviceProxy *device, const std::string &attribute, EventType event, CallBack *callback,
                      const std::vector<std::string> &filters, bool stateless = false);
  int subscribe_event(DeviceProxy *device, const std::string &attribute, EventType event, int event_queue_size,
                      const std::vector<std::string> &filters, bool stateless = false);
...
protected:
  int subscribe_event(DeviceProxy *device, const std::string &attribute, EventType event, CallBack *callback,
                      EventQueue *ev_queue, const std::vector<std::string> &filters, bool stateless = false);
  ...
  static std::map<std::string, std::string> device_channel_map; // key - device_name, value - channel name (full adm
                                                                // name)
  static std::map<std::string, EventChannelStruct> channel_map; // key - channel_name (full adm name), value - Event
                                                                // Channel info
  ...
  void get_fire_sync_event(DeviceProxy *, CallBack *, EventQueue *, EventType, std::string &, const std::string &,
                           EventCallBackStruct &, std::string &);
  ...
  virtual void connect_event_system(const std::string &, const std::string &, const std::string &e,
                                    const std::vector<std::string> &, const EvChanIte &, EventCallBackStruct &,
                                    DeviceData &, size_t) = 0;
  ...
  virtual ReceivedFromAdmin initialize_received_from_admin(const Tango::DevVarLongStringArray *pArray,
                                                           ...
                                                           const std::string &adm_name, bool device_from_env_var) = 0;
};
...
protected:
  ...
  virtual void connect_event_channel(const std::string &, Database *, bool, DeviceData &) override;
  virtual void disconnect_event_channel(const std::string &channel_name, const std::string &endpoint,
...
private:
  ...
  // variable to count the number of ZMQ_DELAY_EVENT requests currently in
  // progress:
  ...
  FwdEventData *newFwdEventData(zmq::message_t &event_data, DeviceProxy *device, DevErrorList &errors,
                                std::string &event_name, std::string &full_att_name, long vers,
                                const DeviceAttribute *dev_attr, bool no_unmarshalling, unsigned int cb_nb,
                                unsigned int cb_ctr, const CallBack *callback) const;
};
...
public:
  ...
  void stateless_subscription_failed(const std::vector<EventNotConnected>::iterator &, const DevFailed &,
                                     const time_t &);
...
private:
  ...
  void main_reconnect(ZmqEventConsumer *, NotifdEventConsumer *, std::map<std::string, EventCallBackStruct>::iterator &,
                      const std::map<std::string, EventChannelStruct>::iterator &);
  void re_subscribe_after_reconnect(ZmqEventConsumer *, NotifdEventConsumer *,
                                    const std::map<std::string, EventCallBackStruct>::iterator &,
                                    const std::map<std::string, EventChannelStruct>::iterator &, const std::string &);
};
...
{
public:
  DelayedEventSubThread(EventConsumer *ec, DeviceProxy *_device, const std::string &_attribute, EventType _event,
                        CallBack *_callback, EventQueue *_ev_queue, const std::string &_ev_name, int _id)
      : omni_thread(),
        ev_cons(ec),
        device(_device),
