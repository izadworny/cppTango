...
/**
 * High level class which provides the client with an easy-to-use interface to
 * TANGO devices.
 *
 * The high level object which provides the client with an easy-to-use interface
 * to TANGO devices. DeviceProxy is a handle to the real Device (hence the name
 * Proxy) and is not the real Device (of course). DeviceProxy provides
 * interfaces to all TANGO devices. The DeviceProxy manages timeouts, stateless
 * connections (new DeviceProxy() nearly always works), and reconnection if the
...
public:
   ...
   *
   * Create a DeviceProxy to a device of the specified name. The TANGO_HOST
   * environment variable is used to determine which TANGO database to connect
   * to. The client can specify an ORB as argument if she wants to. The
   * constructor will connect to the TANGO database, query for the client’s
   * network address and build a connection to the device. If the device is
   * defined in the TANGO database but the device server is not running
   * DeviceProxy will try to build a connection every time the client tries to
   * access the device. If the device is not defined an exception is thrown.
   * Example : \code DeviceProxy *my_device = new DeviceProxy(“my/own/device”);
   * \endcode
   * See appendix on device naming in
   * <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a> for all details about Tango device naming syntax.
   * If an alias name is
   * defined for the device, this alias name can be used to create the
   ...
   * Example : \code cout << " device info : " << std::endl DeviceInfo dev_info
   * = my_device->info() << std::endl; cout << " dev_class " <<
   * dev_info.dev_class; cout << " server_id " << dev_info.server_id; cout << "
   * server_host " << dev_info.server_host; cout << " server_version " <<
   * dev_info.server_version; cout << " doc_url " << dev_info.doc_url; cout << "
   * device_type " << dev_info.dev_type; \endcode All DeviceInfo fields are
   * strings except server_version which is a long integer.
   *
   ...
   * Get device state.
   *
   * A method which returns the state of the device as a Tango::DevState type.
   * Example : \code Tango::DevState dev_state = my_device->state(); \endcode
   *
   ...
   * client computer the command was executed. This method allocates memory for
   * the vector of strings returned to the caller. It is the caller
   * responsibility to delete this memory.
   *
   * @param [in] nb Number of requested records. If more records than available
   * is requested, only the available records are returned
   ...
   *
   * Returns the name of the corresponding administrator device. This is useful
   * if you need to send an administration command to the device server e.g.
   * restart it.
   *
   * @return The administrator device name
   * @throws ConnectionFailed, CommunicationFailed
   ...
   * DevFailed exception with the reason field set to Db_AliasNotDefined.
   *
   ...
   * returns a single CommandInfo type.
   *
   ...
   *
   * Query the device for info on all commands. This method returns a vector of
   * CommandInfo types. This method allocates memory for the vector of
   * CommandInfo returned to the caller. It is the caller responsibility to
   * delete this memory
   *
   ...
   *
   * Return the names of all commands implemented for this device as a vector of
   * strings. This method allocates memory for the vector of strings returned to
   * the caller. It is the caller responsibility to delete this memory.
   *
   ...
   *
   * Return command information for the list of specified commands. This method
   * allocates memory for the vector of CommandInfo returned to the caller. It
   ...
   * depth. This method returns a vector of DeviceDataHistory types. This method
   * allocates memory for the vector of DeviceDataHistory returned to the
   * caller. It is the caller responsibility to delete this memory. See chapter
   * "Advanced Feature" <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a> for all details regarding polling.
   * @code
   ...
   * @return The command information std::list: One CommandInfo structure per
   * command
   * @throws NonSupportedFeature, ConnectionFailed, CommunicationFailed,
   ...
   * returns a single AttributeInfoEx type.
   *
   * @b NOTE on compatibility between Tango V4 and Tango V5 regarding attribute
   * properties:
   * @n Between Tango V4 and Tango V5, attribute configuration has been modified
   * to incorporate alarm and event related parameters. This explains why it
   * exists two structure types for attribute configuration parameters. All
   * Tango V4 parameters are defined in a structure called AttributeInfo and a
   * new structure called AttributeInfoEx has been defined for all Tango V5
   * parameters. Nevertheless, AttributeInfoEx inherits from AttributeInfo and
   * it is always possible to call the Tango V5 DeviceProxy::attribute_query()
   * method and to store its result in one AttributeInfo structure thus allowing
   * compatibility for client written for Tango V4 but linked with Tango V5. It
   * is also possible for a client written and linked with Tango V5 to call
   * Tango V5 DeviceProxy::attribute_query() method to all kind of Tango
   ...
   *
   * Query the device for info on all attributes. This method returns a vector
   * of AttributeInfo types. This method allocates memory for the vector of
   * AttributeInfo structures returned to the caller. It is the caller
   * responsibility to delete this memory.
   * @n See DeviceProxy::attribute_query for a note about compatibility between
   * attribute properties structure
   *
   ...
   *
   * Query the device for info on all attributes. This method returns a vector
   * of AttributeInfoEx types. This method allocates memory for the vector of
   * AttributeInfoEx structures returned to the caller. It is the caller
   * responsibility to delete this memory.
   * @n See DeviceProxy::attribute_query for a note about compatibility between
   * attribute properties structure
   *
   * @return A vector of AttributeInfoEx structures with one element per
   * attribute
   * @throws ConnectionFailed, CommunicationFailed, DevFailed from device
   */
   ...
   * of strings. This method allocates memory for the vector of strings returned
   ...
   *
   * Return the attribute configuration for the list of specified attributes. To
   * get all the attributes pass a vector containing the string AllAttr (defined
   * in tango_const.h). This method allocates memory for the vector of
   * AttributeInfo returned to the caller. It is the caller responsibility to
   * delete this memory.
   * @n See DeviceProxy::attribute_query for a note about compatibility between
   * attribute properties structure
   *
   ...
   * attributes. To get all the attributes pass a vector containing the define
   * AllAttr (defined in tango_const.h). This method allocates memory for the
   * vector of AttributeInfoEx returned to the caller. It is the caller
   * responsibility to delete this memory.
   * @n See DeviceProxy::attribute_query for a note about compatibility between
   * attribute properties structure
   *
   ...
   * @throws ConnectionFailed, CommunicationFailed, DevUnlocked, DevFailed from
   * device
   */
   ...
   *
   * Read the list of specified attributes. To extract the value you have to use
   * the operator of the class DeviceAttribute which corresponds to the data
   * type of the attribute. NOTE: There is no automatic type conversion from the
   * attribute native type to user type e.g. if an attribute returns a short you
   * cannot extract it as a double, you have to extract it as a short. By
   * default, if the server reports error for one of the attribute in the list,
   * this error will be passed to the user using exception when he (she) will
   * try to extract the data form the corresponding See sub-chapter on
   * DeviceAttribute to learn how to change this default behaviour.
   * DeviceAttribute object. This method allocates memory for the vector of
   * DeviceAttribute objects returned to the caller. This is the caller
   * responsibility to delete this memory. Example :
   * @code
   ...
   * @return A std::vector of DeviceAttribute instances with one element for
   * each read attribute
   * @throws ConnectionFailed, CommunicationFailed
   */
   ...
   *
   * Read a single attribute. To extract the value you have to use the operator
   * of the class DeviceAttribute which corresponds to the data type of the
   * attribute. NOTE: There is no automatic type conversion from the attribute
   * native type to user type e.g. if an attribute returns a short you cannot
   * extract it as a double (this will return 0) you have to extract it as a
   * short. See usage eample in DeviceProxy::read_attributes
   *
   * @param [in] att_name Attribute name
   ...
   *
   * Write the specified attributes. To insert the values to write you have to
   * use the operator of the DeviceAttribute class which corresponds to the data
   * type of the attribute. NOTE: There is no automatic type conversion from the
   * user type to the attribute native type e.g. if an attribute expects a short
   * you cannot insert it as a double (this will throw an exception) you have to
   * insert it as a short. Note that this is the only API call which could throw
   * a NamedDevFailedList exception. See @ref except to get all the details on
   ...
   * @throws ConnectionFailed, CommunicationFailed, DeviceUnlocked, DevFailed or
   * NamedDevFailedList from device
   */
  virtual void write_attributes(const std::vector<DeviceAttribute> &attr_in);
  /**
   * Write a single attribute
   *
   * Write a single attribute. To insert the value to write you have to use the
   * operator of the class DeviceAttribute which corresponds to the data type of
   * the attribute. NOTE: There is no automatic type conversion from the user
   * type to the attribute native type e.g. if an attribute expects a short you
   * cannot insert it as a double (this will throw an exception) you have to
   * insert it as a short. See Deviceproxy::write_attributes for a usage
   * example.
   *
   * @param [in] attr_in Attribute name and value
   * @throws ConnectionFailed, CommunicationFailed, DeviceUnlocked, DevFailed
   * from device
   */
   ...
   * (serialisation by device), the execution of this call in the server can’t
   * be interrupted by other clients. To insert/extract the value to write/read
   * you have to use the operator of the class DeviceAttribute which corresponds
   * to the data type of the attribute. NOTE: There is no automatic type
   * conversion from the user type to the attribute native type e.g. if an
   * attribute expects a short you cannot insert it as a double (this will throw
   ...
   * (serialisation by device), the execution of this call in the server can’t
   * be interrupted by other clients. On the server side, attribute(s) are first
   * written and if no exception has been thrown during the write phase,
   * attributes will be read. To insert/extract the value to write/read you have
   * to use the operator of the class DeviceAttribute which corresponds to the
   ...
   * expects a short you cannot insert it as a double (this will throw an
   * exception) you have to insert it as a short.
   *
   ...
   * from device
   */
  virtual std::vector<DeviceAttribute> *write_read_attributes(const std::vector<DeviceAttribute> &attr_in,
   ...
   * argument is the attribute name. The second argument is the wanted history
   * depth. This method returns a vector of DeviceAttributeHistory types. This
   * method allocates memory for the vector of DeviceAttributeHistory returned
   * to the caller. It is the caller responsibility to delete this memory. See
   * chapter on Advanced Feature in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a> for all details regarding polling.
   * @code
   * DeviceProxy dev = new DeviceProxy("...");
   ...
   * ((*hist)[i].get_err_stack())[0].desc << std::endl;
   *    }
   ...
   *
   * Return the pipe configuration for the list of specified pipes. To get all
   * the pipes pass a vector containing the define AllPipe (defined in
   * tango_const.h). This method allocates memory for the vector of PipeInfo
   ...
   *
   * Write then read a pipe in a single network call. By default (serialisation
   * by device), the execution of this call in the server can’t be interrupted
   * by other clients.
   *
   * @param [in] pipe_data Data to be sent to the device through the pipe
   * @return The pipe value in a DevicePipe instance
   ...
   *
   * Read asynchronously (polling model) a single attribute. This call returns
   * an asynchronous call identifier which is needed to get the attribute value.
   *
   ...
   *
   * Read asynchronously (polling model) the list of specified attributes. This
   * call returns an asynchronous call identifier which is needed to get
   * attributes value.
   *
   ...
   * model). id is the asynchronous call identifier. If the reply is arrived and
   ...
   * DeviceAttribute which corresponds to the data type of the attribute. NOTE:
   * There is no automatic type conversion from the attribute native type to
   * user type e.g. if an attribute returns a short you cannot extract it as a
   * double, you have to extract it as a short. Memory has been allocated for
   * the DeviceAttribute object returned to the caller. This is the caller
   * responsibility to delete this memory.
   *
   * @param [in] id The call identifier
   * @return The attribute(s) data
   * @throws AsynCall, AsynReplyNotArrived, CommunicationFailed, DevFailed from
   * device
   */
   ...
   * model). id is the asynchronous call identifier. If the reply is arrived and
   * if it is a valid reply, it is returned to the caller in a DeviceAttribute
   * object. If the reply is an exception, it is re-thrown by this call. If the
   * reply is not yet arrived, the call will wait (blocking the process) for the
   * time specified in timeout. If after timeout milliseconds, the reply is
   * still not there, an exception is thrown. If timeout is set to 0, the call
   * waits until the reply arrived. To extract attribute value, you have to use
   * the operator of the class DeviceAttribute which corresponds to the data
   * type of the attribute. NOTE: There is no automatic type conversion from the
   * attribute native type to user type e.g. if an attribute returns a short you
   * cannot extract it as a double, you have to extract it as a short. Memory
   * has been allocated for the DeviceAttribute object returned to the caller.
   ...
   * model). id is the asynchronous call identifier. If the reply is arrived and
   * if it is a valid reply, it is returned to the caller in a DeviceAttribute.
   * If the reply is an exception, it is re-thrown by this call. An exception is
   * also thrown in case of the reply is not yet arrived. To extract attribute
   ...
   * type conversion from the attribute native type to user type e.g. if an
   * attribute returns a short you cannot extract it as a double, you have to
   * extract it as a short. Memory has been allocated for the DeviceAttribute
   * object returned to the caller. This is the caller responsibility to delete
   * this memory.
   *
   * @param [in] id The call identifier
   ...
   * model). id is the asynchronous call identifier. If the reply is arrived and
   * if it is a valid reply, it is returned to the caller in a DeviceAttribute.
   * If the reply is an exception, it is re-thrown by this call. If the reply is
   * not yet arrived, the call will wait (blocking the process) for the time
   * specified in timeout. If after timeout milliseconds, the reply is still not
   * there, an exception is thrown. If timeout is set to 0, the call waits until
   * the reply arrived. To extract attribute value, you have to use the operator
   * of the class DeviceAttribute which corresponds to the data type of the
   * attribute. NOTE: There is no automatic type conversion from the attribute
   * native type to user type e.g. if an attribute returns a short you cannot
   * extract it as a double, you have to extract it as a short. Memory has been
   * allocated for the DeviceAttribute object returned to the caller. This is
   ...
   * automatic type conversion from the user type to the attribute native type
   * e.g. if an attribute expects a short you cannot insert it as a double (this
   * will throw an exception) you have to insert it as a short. This call
   * returns an asynchronous call identifier which is needed to get the server
   * reply.
   *
   ...
   * DeviceAttribute which corresponds to the data type of the attribute. NOTE:
   * There is no automatic type conversion from the user type to the attribute
   * native type e.g. if an attribute expects a short you cannot insert it as a
  ...
  /**
   * Check if the answer of one asynchronous write_attribute (single attribute)
   * call is arrived
   *
   * Check if the answer of an asynchronous write_attribute is arrived (polling
   * model). id is the asynchronous call identifier. If the reply is arrived and
   * if it is a valid reply, the call returned. If the reply is an exception, it
   * is re-thrown by this call. An exception is also thrown in case of the reply
   * is not yet arrived.
   *
   * @param [in] id The call identifier
   * @throws AsynCall, AsynReplyNotArrived, CommunicationFailed, DevFailed from
  ...
  /**
   * Check if the answer of one asynchronous write_attribute call (single
   * attribute) is arrived with timeout
   *
   * Check if the answer of an asynchronous write_attribute is arrived (polling
   * model). id is the asynchronous call identifier. If the reply is arrived and
   * if it is a valid reply, the call returned. If the reply is an exception, it
   * is re-thrown by this call. If the reply is not yet arrived, the call will
   * wait (blocking the process) for the time specified in timeout. If after
   * timeout milliseconds, the reply is still not there, an exception is thrown.
   ...
   *
   * Read asynchronously (callback model) a single attribute. The last argument
   * is a reference to a callback object. This callback object should be an
   * instance of a user class inheriting from the Tango::CallBack class with the
   * @e attr_read() method overloaded.
   *
   * @param [in] att_name The attribute name
   ...
   *
   * Write asynchronously (callback model) a single attribute. The last argument
   * is a reference to a callback object. This callback object should be an
   * instance of a user class inheriting from the Tango::CallBack class with the
   * @e attr_written() method overloaded.
   *
   * @param [in] argin The attribute name and value
   * @param [in] cb The call-back object
   * @throws ConnectionFailed
   ...
   *
   * Write asynchronously (callback model) an attribute list. The last argument
   * is a reference to a callback object. This callback object should be an
   * instance of a user class inheriting from the Tango::CallBack class with the
   * @e attr_written() method overloaded.
   *
   ...
   * @li CALLBACK : Returns only device callback model asynchronous request
   * number
  ...
  virtual long pending_asynch_call(asyn_req_type req)
  {
    if(req == POLLING)
      return pasyn_ctr;
    else if(req == CALL_BACK)
      return pasyn_cb_ctr;
    else
      return (pasyn_ctr + pasyn_cb_ctr);
  }
   ...
   *
   * Returns true if the attribute "att_name" is polled. Otherwise, returns
   * false.
   *
   * @param [in] att_name The attribute name
   ...
   *
   * Returns the command "cmd_name" polling period in mS. If the command is not
   * polled, it returns 0.
   *
   * @param [in] cmd_name The command name
   ...
   *
   * Returns the attribute "att_name" polling period in mS. If the attribute is
   ...
   * @li The exception parameters in case of the last command/attribute
   * execution failed
   *
   ...
   * Poll a command
   *
   * Add the command "cmd_name" to the list of polled command. The polling
   * period is specified by "polling_period" (in mS). If the command is already
   * polled, this method will update the polling period according to
   * "polling_period".
   *
   * @param [in] cmd_name The command name
   ...
   * Poll an attribute
   *
   * Add the attribute "att_name" to the list of polled attributes. The polling
   * period is specified by "polling_period" (in mS). If the attribute is
   ...
   * fired only when the state changes. Events consist of an attribute name and
   * the event reason. A standard set of reasons are implemented by the system,
   * additional device specific reasons can be implemented by device servers
   * programmers.
   * The attribute parameter is the device attribute name which will be sent as
   * an event e.g. “current”, event parameter is the event reason, cb is a
   * pointer to a class inheriting from the Tango CallBack class and
   * implementing a push_event() method. The subscribe_event() call returns an
   * event id which has to be specified when unsubscribing from this event.
   * Please, note that the cb parameter is a pointer. The lifetime of the
   * pointed to object must at least be equal to the time when events are
   * requested because only the pointer is stored into the event machinery. The
   * same thing is true for the DeviceProxy instance on which the
   * subscribe_event() method is called.
   *
   * @param [in] att_name The attribute name
   ...
   * an event e.g. “current”, event parameter is the event reason, cb is a
   * pointer to a class inheriting from the Tango CallBack class and
   * implementing a push_event() method. The last call parameter is named
   * stateless. When the stateless flag is set to false, an exception will be
   * thrown when the event subscription encounters a problem. With the stateless
   * flag set to true, the event subscription will always succeed, even if the
   * corresponding device server is not running. The keep alive thread will try
   * every 10 seconds to subscribe for the specified event. At every
   * subscription retry, a callback is executed which contains the corresponding
   * exception. The subscribe_event() call returns an event id which has to be
   * specified when unsubscribing from this event. Please, note that the cb
   * parameter is a pointer. The lifetime of the pointed to object must at least
   * be equal to the time when events are requested because only the pointer is
   * stored into the event machinery. The same thing is true for the DeviceProxy
   * instance on which the subscribe_event() method is called.
   *
   * @param [in] att_name The attribute name
   * @param [in] event The event type
   * @param [in] cb The callback object
   ...
   *
   * The client call to subscribe for event reception in the pull model. Instead
   * of a callback method the client has to specify the size of the event
   ...
   * events.
   * @li Event reception buffer size = 1 : The client is interested only in the
   * value of the last event received. All other events that have been received
   * since the last reading are discarded.
   ...
   * received events. The buffer size is unlimited and only restricted by the
   * available memory for the client.
   *
   * All other parameters (att_name, event, stateless) are similar to those
   * described in DeviceProxy::subscribe_event(const string &,EventType,CallBack
   * *,bool)
   *
   * @param [in] att_name The attribute name
   * @param [in] event The event type
   ...
   * @throws EventSystemFailed
   */
  virtual int subscribe_event(const std::string &att_name, EventType event, int event_queue_size,
                              bool stateless = false);
  /**
   * Subscribe for device event reception with stateless support
   *
   * The client call to subscribe for <B>device</B> event reception in the push
   * model. The client implements a callback method which is triggered when the
   * event is received. Filtering is done based on the event type. Today, only
   * one event type is supported for <B>device</B> event. This event type is a
   * device interface change event. cb is a pointer to a class inheriting from
   * the Tango CallBack class and implementing a push_event() method. The last
   * call parameter is named stateless. When the stateless flag is set to false,
   * an exception will be thrown when the event subscription encounters a
   * problem. With the stateless flag set to true, the event subscription will
   * always succeed, even if the corresponding device server is not running. The
   * keep alive thread will try every 10 seconds to subscribe for the specified
   * event. At every subscription retry, a callback is executed which contains
   * the corresponding exception. The subscribe_event() call returns an event id
   * which has to be specified when unsubscribing from this event. Please, note
   * that the cb parameter is a pointer. The lifetime of the pointed to object
   * must at least be equal to the time when events are requested because only
   * the pointer is stored into the event machinery. The same thing is true for
   * the DeviceProxy instance on which the subscribe_event() method is called.
   *
   * @param [in] event The event type
   * @param [in] cb The callback object
   * @param [in] stateless The stateless flag
   ...
   *
   * The client call to subscribe for <B>device</B> event reception in the pull
   * model. Today, only one event type is supported for <B>device</B> event.
   * This event type is a device interface change event. Instead of a callback
   * method the client has to specify the size of the event reception buffer.
   * The event reception buffer is implemented as a round robin buffer. This way
   * the client can set-up different ways to receive events.
   * @li Event reception buffer size = 1 : The client is interested only in the
   * value of the last event received. All other events that have been received
   * since the last reading are discarded.
   * @li Event reception buffer size > 1 : The client has chosen to keep an
   ...
   * @li Event reception buffer size = ALL_EVENTS : The client buffers all
   * received events. The buffer size is unlimited and only restricted by the
   * available memory for the client. The last call parameter is named
   * stateless. When the stateless flag is set to false, an exception will be
   * thrown when the event subscription encounters a problem. With the stateless
   * flag set to true, the event subscription will always succeed.
   *
   * @param [in] event The event type
   * @param [in] event_queue_size The event queue size
   * @param [in] stateless The stateless flag
   ...
   * event_id is the event identifier returned by the
   ...
   * executes the callback method cb for every event. During event subscription
   * the client must have chosen the <B>pull model</B> for this event. event_id
   * is the event identifier returned by the DeviceProxy::subscribe_event()
   * method.
   *
   * @param [in] event_id The event identifier
   * @param [in] cb The callback object
   ...
   *
   * The method extracts all waiting events from the event reception buffer. The
   * returned event_list is a vector of EventData pointers. The EventData object
   * contains the event information as for the callback methods. During event
   * subscription the client must have chosen the <B>pull model</B> for this
   * event. event_id is the event identifier returned by the
   * DeviceProxy::subscribe_event() method.
   *
   * @param [in] event_id The event identifier
   ...
   * AttrConfEventData pointers. The AttrConfEventData object contains the event
   * information as for the callback methods.
   * During event subscription the client must have chosen the <B>pull model</B>
   ...
   * The method extracts all waiting attribute configuration events from the
   * event reception buffer. The returned event_list is a vector of
   * DataReadyEventData pointers. The DataReadyEventData object contains the
   * event information as for the callback methods. During event subscription
   * the client must have chosen the <B>pull model</B> for this event. event_id
   * is the event identifier returned by the DeviceProxy::subscribe_event()
   * method.
   *
   ...
   * The method extracts all waiting attribute configuration events from the
   * event reception buffer. The returned event_list is a vector of
   * DevIntrChangeEventData pointers. The DevIntrChangeEventData object contains
   * the event information as for the callback methods. During event
   * subscription the client must have chosen the <B>pull model</B> for this
   * event. event_id is the event identifier returned by the
   * DeviceProxy::subscribe_event() method.
   *
   ...
   * buffer. The returned event_list is a vector of PipeEventData pointers. The
   * PipeEventData object contains the event information as for the callback
   * methods. During event subscription the client must have chosen the <B>pull
   * model</B> for this event. event_id is the event identifier returned by the
   * DeviceProxy::subscribe_event() method.
   *
   * @param [in] event_id The event identifier
   * @param [out] event_list The event(s) list
   ...
   * every call to DeviceProxy:get_events(), the event queue size is 0. During
   * event subscription the client must have chosen the <B>pull model</B> for
   * this event. event_id is the event identifier returned by the
   * DeviceProxy::subscribe_event() method.
   *
   * @param [in] event_id The event identifier
   ...
   * buffer. After every call to Device- Proxy:get_events(), the event reception
   * buffer is empty. In this case an exception will be returned. During event
   * subscription the client must have chosen the <B>pull model</B> for this
   * event. event_id is the event identifier returned by the
   * DeviceProxy::subscribe_event() method.
   *
   ...
   *
   * Get a single property for a device. The property to get is specified as a
   * string. Refer to DbDevice::get_property() and DbData sections below for
   * details on the DbData type.
   *
   * @param [in] prop_name The property name
   * @param [out] db The property value
   * @throws NonDbDevice, ConnectionFailed, CommunicationFailed, DevFailed from
   * database device
   */
   ...
   * as a vector of strings. Refer to DbDevice::get_property() and DbData
   ...
   *
   * Get property(ies) for a device. Properties to get are specified using the
   * DbData type. Refer to DbDevice:: get_property() and DbData sections below
   * for details.
   *
   ...
   *
   * Put property(ies) for a device. Properties to put are specified using the
   * DbData type. Refer to DbDevice:: put_property() and DbData sections below
   * for details.
   *
   ...
   *
   * Delete property(ies) for a device. Properties to delete are specified using
   * the DbData type. Refer to DbDevice:: get_property() and DbData sections
   * below for details.
   *
   ...
   *
   * Get the list of property names for the device. The parameter filter allows
   * the user to filter the returned name list. The wildcard character is ’*’.
   * Only one wildcard character is allowed in the filter parameter. The name
   * list is returned in the vector of strings passed as the method second
   * argument.
   *
   ...
   *
   * Adds a new logging target to the device. The target_type_name input
   * parameter must follow the format: @b target_type::target_name. Supported
   * target types are:
   * @li console
   * @li file
   * @li device
   *
   * For a device target,
   * the target_name part of the target_type_target_name parameter must contain
   * the name of a log consumer device (as defined in A.8). For a file target,
   * target_name is the full path to the file to log to. If omitted, the
   * device’s name is used to build the file name (which is something like
   * domain_family_member.log). Finally, the target_name part of the
   * target_type_target_name input parameter is ignored in case of a console
   ...
   * @throws DevFailed from device
   */
  virtual void add_logging_target(const char *target_type_name) { add_logging_target(std::string(target_type_name)); }

  /**
   * Remove a logging target from the device
   *
   * Removes a logging target from the device’s target list. The
   * target_type_name input parameter must follow the format:
   * target_type::target_name. Supported target types are:
   * @li console
   * @li file
   * @li device
   *
   * For a
   * device target, the target_name part of the target_type_target_name
   * parameter must contain the name of a log consumer device (as defined in ).
   * For a file target, target_name is the full path to the file to remove. If
   * omitted, the default log file is removed. Finally, the target_name part of
   * the target_type_target_name input parameter is ignored in case of a console
   * target and can be omitted. If target_name is set to "*", all targets of the
   * specified target_type are removed.
   *
   * @param [in] target_type_name The target type and name
   */
   ...
   * Each vector element has the following format: target_type::target_name. An
   * empty vector is returned is the device has no logging targets.
   *
   ...
   * Lock a device
   *
   * Lock a device. The lock_validity is the time (in seconds) the lock is kept
   * valid after the previous lock call. A default value of 10 seconds is
   * provided and should be fine in most cases. In case it is necessary to
   * change the lock validity, it’s not possible to ask for a validity less than
   * a minimum value set to 2 seconds. The library provided an automatic system
   * to periodically re lock the device until an unlock call. No code is needed
   * to start/stop this automatic re-locking system. The locking system is
   * re-entrant. It is then allowed to call this method on a device already
   * locked by the same process. The locking system has the following features:
   * @li It is impossible to lock the database device or any device server
   ...
   * another client:
   * @li command_inout call except for device state and status requested via
   * command and for the set of commands defined as allowed following the
   * definition of allowed command in the Tango control access schema.
   ...
   * Unlock a device
   *
   * Unlock a device. If used, the method argument provides a back door on the
   * locking system. If this argument is set to true, the device will be
   * unlocked even if the caller is not the locker. This feature is provided for
   * administration purpose and should be used very carefully. If this feature
   * is used, the locker will receive a DeviceUnlocked exception during the next
   * call which is normally protected by the locking Tango system.
   *
   ...
   * @li "Device <device name> is locked by CPP or Python client with PID <pid>
   * from host <host name>" in case the device is locked by a CPP client
   * @li "Device <device name> is locked by JAVA client class <main class> from
   * host <host name>" in case the device is locked by a JAVA client
   *
   ...
   * process informations in the structure passed as argument. If the device is
   * not locked, the method returns false. The LockerInfo structure definition
   * is
   * @code
   * typedef union
   * {
  ...
  virtual int subscribe_event(const std::string &attr_name, EventType event, CallBack *,
                              ...
                              bool stateless); // For compatibility with Tango < 8
