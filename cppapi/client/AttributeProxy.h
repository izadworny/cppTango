 ...
 * to TANGO device attributes. AttributeProxy is a handle to the real attribute
 * (hence the name Proxy) and is not the real attribute (of course). The
 * AttributeProxy manages timeouts, stateless connections (new AttributeProxy()
...
private:
  ...
  public:
    ...
    AttributeProxyExt(const std::string &name)
...
public:
   ...
   * constructor will connect to the TANGO database, query for the device to
   * which the attribute belongs to network address and build a connection to
   * this device. If the device to which the attribute belongs to is defined in
   * the TANGO database but the device server is not running, AttributeProxy
   * will try to build a connection every time the client tries to access the
   * attribute. If an alias name is defined for the attribute, this alias name
   * can be used to create the AttributeProxy instance. If a device name alias
   * is defined for the device, it can be used instead of the three fields
   * device name. If the device to which the attribute belongs to is not defined
   * in the database, an exception is thrown. Examples :
   * @code
   ...
   * @exception ConnectionFailed, CommunnicationFailed, DevFailed from device
   * @deprecated Use the set_config() method with AttributeInfoEx parameter data
   * type
   */
   ...
   * the class DeviceAttribute which corresponds to the data type of the
   ...
   * insert it as a short.
   *
   * @param [in] da The new attribute value
   * @exception ConnectionFailed, CommunnicationFailed, DevUnlocked, DevFailed
   * from device
   */
   ...
   *
   * Retrieve attribute history from the attribute polling buffer. The argument
   * is the wanted history depth. This method returns a vector of
   * DeviceAttributeHistory types. This method allocates memory for the vector
   * of DeviceAttributeHistory returned to the caller. It is the caller
   * responsibility to delete this memory. See Tango book chapter on Advanced
   ...
   * @exception ConnectionFailed, CommunnicationFailed, NonSupportedFeature,
   ...
   *
   * Check if the answer of an asynchronous read is arrived (polling model). id
   * is the asynchronous call identifier. If the reply is arrived and if it is a
   * valid reply, it is returned to the caller in a DeviceAttribute object. If
   ...
   *
   * Write the attribute asynchronously (polling model). To insert the value to
   * write you have to use the operator of the class DeviceAttribute which
   * corresponds to the data type of the attribute. NOTE: There is no automatic
   * type conversion from the user type to the attribute native type e.g. if an
   * attribute expects a short you cannot insert it as a double (this will throw
   * an exception) you have to insert it as a short. This call returns an
   * asynchronous call identifier which is needed to get the server reply.
   *
   ...
   *
   * Check if the answer of an asynchronous write is arrived (polling model). id
   * is the asynchronous call identifier. If the reply is arrived and if it is a
   * valid reply, the call returned. If the reply is an exception, it is
   * re-thrown by this call. An exception is also thrown in case of the reply is
   * not yet arrived.
   *
   * @param [in] id The asynchronous identifier
   * @exception AsynCall, AsynReplyNotArrived, CommunicationFailed, DevFailed
   ...
   * is the asynchronous call identifier. If the reply is arrived and if it is a
   * valid reply, the call returned. If the reply is an exception, it is
   * re-thrown by this call. If the reply is not yet arrived, the call will wait
   * (blocking the process) for the time specified in timeout. If after timeout
   * milliseconds, the reply is still not there, an exception is thrown. If
   ...
   *
   * Read the attribute asynchronously using the callback model. The argument is
   ...
   * instance of a user class inheriting from the Tango::CallBack class with the
   * attr_written() method overloaded.
   *
   * @param [in] da The new attribute value
   * @param [in] cb The callback object
   * @exception ConnectionFailed
   ...
   *
   * Returns the attribute polling period in mS. If the attribute is not polled,
   * it returns 0.
   *
   ...
   *
   * Add the attribute to the list of polled attributes. The polling period is
   * specified by "period" (in mS). If the attribute is already polled, this
   * method will update the polling period according to "period".
   *
   ...
   * received either by polling or a dedicated thread. Filtering is done based
   * on the reason specified and the event type. For example when reading the
   * state and the reason specified is "change" the event will be fired only
   * when the state changes. Events consist of an attribute name and the event
   * reason. A standard set of reasons are implemented by the system, additional
   * device specific reasons an be implemented by device servers programmers. cb
   * is a pointer to a class inheriting fromthe Tango CallBack class and
   * implementing a push_event() method. The lifetime of the pointed to object
   * must at least be equal to the time when events are requested because only
   * the pointer is stored into the event machinery. The subscribe_event() call
   * returns an event id which has to be specified when unsubscribing from this
   * event.
   *
   ...
   * last section. It adds an additional flag called stateless. When the
   * stateless flag is set to false, an exception will be thrown when the event
   * subscription encounters a problem.
   * With the stateless flag set to true, the event subscription will always
   * succeed, even if the corresponding device server is not running. The keep
   * alive thread will try every 10 seconds to subscribe for the specified
   ...
   * AttributeProxy::subscribe_event() method.
   *
   ...
   * the client must have chosen the pull model for this event. event_id is the
   ...
   * @exception EventSystemFailed
   */
  virtual void get_events(int event_id, EventDataList &event_list) { dev_proxy->get_events(event_id, event_list); }
   ...
   * event subscription the client must have chosen the pull model for this
   ...
   * buffer. After every call to AttributeProxy:get_events(), the event
   * reception buffer is empty. In this case an exception will be returned.
   * During event subscription the client must have chosen the pull model for
   * this event. event_id is the event identifier returned by the
   * AttributeProxy::subscribe_event()method.
   *
   ...
   * as a string. Refer to DbDevice:: get_property() and DbData sections for
   * details on the DbData type.
   *
   * @param [in] prop_name The property name
   * @param [out] db Property value
   * @exception NonDbDevice, ConnectionFailed, CommunicationFailed, DevFailed
   * from database
   */
   ...
   * specified as a vector of strings. Refer to DbDevice::get_property() and
   * DbData sections for details on the DbData type.
   *
   ...
   *
   * Get property(ies) for the attribute. Properties to get are specified using
   * the DbData type. Refer to DbDevice:: get_property() and DbData sections for
   * details.
   *
   ...
   *
   * Put property(ies) for an attribute. Properties to put are specified using
   * the DbData type. Refer to DbDevice:: put_property() and DbData sections for
   * details.
   *
   * @param [in,out] db Properties value
   * @exception NonDbDevice, ConnectionFailed, CommunicationFailed, DevFailed
   ...
   * specified as a string. Refer to DbDevice:: delete_property() and DbData
   * sections for details on the DbData type.
   *
   * @param [in] prop_name The property name
   * @exception NonDbDevice, ConnectionFailed, CommunicationFailed, DevFailed
   * from database
   ...
   *
   * Delete a list of properties for an attribute. The properties to delete are
   * specified as a vector of strings. Refer to DbDevice::get_property() and
   * DbData sections for details on the DbData type.
   *
   ...
   * using the DbData type. Refer to DbDevice::get_property() and DbData
  ...
  virtual int subscribe_event(EventType event, CallBack *,
                              const std::vector<std::string> &filters); // For compatibility
  virtual int subscribe_event(EventType event, CallBack *, const std::vector<std::string> &filters,
                              bool stateless); // For compatibility
  virtual int subscribe_event(EventType event, int event_queue_size, const std::vector<std::string> &filters,
