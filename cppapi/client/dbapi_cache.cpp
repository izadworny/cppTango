...
//
// description:
//		Constructor of the DbServerCache class. This class manage a data cache
// used during the device server 		process startup phase.
//
// arguments:
// 		in :
...
DbServerCache::DbServerCache(Database *db, const std::string &ds_name, const std::string &host)
{
  ...
  //
  // Extract the different blocks from the big list. First the stored procedure
  // release nb The release number is stored as:
...
//
// description:
//		This method returns to the caller of the info necessary to import the DS
// admin device. The returned data 		are the same than the one returned by the
// classical API
//
// return:
//		This method returns a pointer to a DevVarLongStringArray initilised with
// the adm device import data
//
//------------------------------------------------------------------------------------------------------------------
...
// event associated to the notifd
// running
// 		on the DS host. The returned data are the same than the one returned by
// the classical API
//
// return :
//		This method returns a pointer to a DevVarLongStringArray initilised with
// the notifd event  import data
//
//------------------------------------------------------------------------------------------------------------------
...
// description :
//		This method returns to the caller of the info necessary to import the
// event associated to the DS server. 		The returned data are the same than
// the one returned by the classical API
//
// return :
//		This method returns a pointer to a DevVarLongStringArray initilised with
// the DS event  import data
//
//-------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		This method returns to the caller the wanted class properties. The
// returned data are the same than the
// one 		returned by the classical API
//
// argument :
// 		in :
...
//
// return :
//		This method returns a pointer to a DevVarStringArray initilised with the
// class properties
//
//-------------------------------------------------------------------------------------------------------------------
...
// all the data returne by the DB
// server.
...
//			- dev_prop : Boolean set to true in case of device property. For device
// property, a undefined prop is 						 returned with a value set to " " !
//  	out :
//			- obj : Reference to the structure which will be initialsed by this
// method
//
//------------------------------------------------------------------------------------------------------------------
...
void DbServerCache::get_obj_prop(DevVarStringArray *in_param, PropEltIdx &obj, bool dev_prop)
{
  ...
  //	cout4 << "DbCache --> Data returned for a get_obj_property for object " <<
  //(*in_param)[0] << std::endl; 	for (unsigned int ll=0;ll<
  // ret_obj_prop.length();ll++) 		cout4 << "    DbCache --> Returned
  // std::string = " << ret_obj_prop[ll] << std::endl;
}
...
//
// description :
//		This method returns to the caller the wanted device properties The
// returned data are the same than the
// one 		returned by the classical API
//
// argument :
...
const DevVarStringArray *DbServerCache::get_class_att_property(DevVarStringArray *in_param)
{
  ...
  //	cout4 << "DbCache --> Returned data for a get_class_att_property for class
  //" << (*in_param)[0] << std::endl; 	for (unsigned int ll=0;ll<
  // ret_obj_att_prop.length();ll++) 		cout4 << "    DbCache --> Returned
  // object att prop = " << ret_obj_att_prop[ll] << std::endl;
...
const DevVarStringArray *DbServerCache::get_dev_att_property(DevVarStringArray *in_param)
{
  ...
  //	cout4 << "DbCache --> Returned data for a get_dev_att_property for device
  //" << (*in_param)[0] << std::endl; 	for (unsigned int ll=0;ll<
  // ret_obj_att_prop.length();ll++) 		cout4 << "    DbCache --> Returned
  // object att prop = " << ret_obj_att_prop[ll] << std::endl;
...
// passed in the in parameter. These
// indexes
...
//
// description :
//		This method computes where is the last data related to the object (for
// attribute related object) in the
// big 		array coming from the Db server
//
// argument :
// 		in :
...
//
// description :
//		This method returns to the caller the wanted object properties. The
// returned data are the same than the
// one 		returned by the classical API
//
// argument :
...
//
// description :
//		This method returns to the caller the device property names. The returned
// data are the same than the one 		returned by the classical API
//
// argument :
// 		in :
...
//
// retrun :
//		This method returns a pointer to a DevVarStringArray initialised with the
// device property names
//
//------------------------------------------------------------------------------------------------------------------
...
void DbServerCache::get_obj_prop_list(DevVarStringArray *in_param, PropEltIdx &obj)
{
  ...
  {
    //
    // Check according to the user wildcard, if we have to returned this
    // property name Note: The property name is case independant
    //
...
//
// description :
//		This method returns to the caller of the info necessary to import the TAC
// device. 		The returned data are the same than the one returned by the
// classical API
//
// return :
//		This method returns a pointer to a DevVarLongStringArray initilised with
// the Tango access control import
// data
//
//-------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		This method returns to the caller the class pipe properties for the class
// and pipes specified in
// 		the in parameters. The returned data are the same than the one returned by
// the classical API
//
// argument
// 		in :
...
const DevVarStringArray *DbServerCache::get_class_pipe_property(DevVarStringArray *in_param)
{
  ...
  if(proc_release < 109)
  {
    std::string mess("Your database stored procedure is too old to support "
                     "pipe. Please update to stored procedure "
                     "release 1.9 or more");
  ...
  //	cout4 << "DbCache --> Returned data for a get_class_pipe_property for
  // class " << (*in_param)[0] << std::endl; 	for (unsigned int ll=0;ll<
  // ret_obj_pipe_prop.length();ll++) 		cout4 << "    DbCache --> Returned
  // object pipe prop = " << ret_obj_pipe_prop[ll] << std::endl;
...
//
// description :
//		This method returns to the caller the device pipe properties for the
// device and pipes specified in 		the in parameters. The returned data are the
// same than the one returned by the classical API
//
// argument
// 		in :
...
const DevVarStringArray *DbServerCache::get_dev_pipe_property(DevVarStringArray *in_param)
{
  ...
  if(ret_value != -1)
  {
    ...
    for(int loop = 0; loop < wanted_pipe_nb; loop++)
    {
      ...
      {
        if(TG_strcasecmp((*in_param)[loop + 1],
                         (*data_list)[classes_idx[class_ind].devs_idx[dev_ind].dev_pipe_prop.atts_idx[ll]]) == 0)
        {
  ...
  //	cout4 << "DbCache --> Returned data for a get_dev_pipe_property for device
  //" << (*in_param)[0] << std::endl; 	for (unsigned int ll=0;ll<
  // ret_obj_pipe_prop.length();ll++) 		cout4 << "    DbCache --> Returned
  // object pipe prop = " << ret_obj_pipe_prop[ll] << std::endl;
