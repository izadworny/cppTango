 ...
 *
 * This class is a singleton. Therefore, it is not necessary to create it. It
 * will be automatically done. A static method allows a user to retrieve the
 * instance
 *
 * $Author: taurel $
 * $Revision: 1 $
...
public:
   ...
   *
   * Get environment variable. On Unixes OS, this call tries to get the variable
   * in the caller environment then in a file @e .tangorc in the user home
   * directory and finally in a file @e /etc/tangorc. On Windows, this call
   * looks in the user environment then in a file stored in
   * %TANGO_HOME%/tangorc. This method returns 0 of the environment variable is
   ...
   * and attribute) with already arrived replied. Returns immediately if there
   * is no replies already arrived or if there is no asynchronous requests.
   */
   ...
   *
   * Fire callback methods for all (any device) asynchronous requests (command
   * and attributes) with already arrived replied. Wait and block the caller for
   ...
   * sub-model. See Tango book chapter 4.5 to read the definition of these
   * sub-models. By default, all Tango client using asynchronous callback model
   * are in pull sub-model. This call must be used to switch to the push
   * sub-model. NOTE that in push sub-model, a separate thread is spawned to
   * deal with server replies.
   *
  ...
  TANGO_IMP_EXP static inline bool _is_instance_null() { return _instance == NULL; }
