      ...
      db_tg(NULL)
{
  ...
  if(ret == -1)
  {
    ...
    desc << "TANGO_HOST env. variable not set, set it and retry (e.g. "
...
#ifdef _TG_WINDOWS_
Database::Database(ORB *orb_in, std::string &ds_exec_name, std::string &ds_inst_name)
    : Connection(orb_in),
      ext(new DatabaseExt),
      access_proxy(NULL),
      access_checked(false),
...
void Database::check_tango_host(const char *tango_host_env_c_str)
{
  ...
  if(separator != std::string::npos)
  {
    ...
    while((separator = tango_host_env.find(',', separator + 1)) != std::string::npos)
    {
      ...
      if(host_sep != std::string::npos)
      {
        ...
        {
          TangoSys_MemStream desc;
          desc << "TANGO_HOST env. variable syntax incorrect (e.g. "
                  "TANGO_HOST=<host>:<port>,<host>:<port>)"
               << std::ends;
          TANGO_THROW_API_EXCEPTION(ApiConnExcept, API_TangoHostNotSet, desc.str());
        }
  ...
  else
  {
    ...
    //
    // If localhost is used as host name, try to replace it by the real host
    // name
    //
    ...
    //
    // Get FQDN but store original TANGO_HOST (for event in case of alias used
    // in TANGO_HOST)
    //
...
Database::Database(const std::string &in_host, int in_port, ORB *orb_in)
    : Connection(orb_in),
      ext(new DatabaseExt),
      access_proxy(NULL),
      access_checked(false),
...
//-----------------------------------------------------------------------------
//
// Database::unexport_device() - public method to unexport a device from the
// Database
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::delete_device() - public method to delete a device from the
// Database
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::delete_server() - public method to delete a server from the
// Database
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::unexport_server() - public method to unexport a server from the
// Database
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::delete_device_property() - public method to delete device
// properties
//                      from the Database
//
//-----------------------------------------------------------------------------
...
void Database::get_class_property(std::string device_class, DbData &db_data, DbServerCache *db_cache)
{
  ...
  //
  // Build data returned to caller from those received from db server or from
  // cache
  //
...
//-----------------------------------------------------------------------------
//
// Database::delete_class_property() - public method to delete class properties
// from
//                                   the Database
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::get_class_attribute_property() - public method to get class
// attribute
...
DbDatum Database::get_device_name(const std::string &device_server, const std::string &device_class,
...
//-----------------------------------------------------------------------------
//
// Database::get_device_exported() - public method to get exported device names
// from
//                               the Database
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::get_attribute_alias_list() - public method to get attribute alias
// list
//				          Wildcard (*) is suported
//
//-----------------------------------------------------------------------------
...
void Database::get_device_property_list(std::string &dev, const std::string &wildcard,
                                        std::vector<std::string> &prop_list, DbServerCache *db_cache)
{
  if(db_cache == NULL)
  {
...
//-----------------------------------------------------------------------------
//
// Database::get_instance_name_list() - Query the database for a list of
// instance
//                                      names registred for specified server
//                                      name
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::get_server_list() - Query the database for a list of servers
// registred
//                               in the database
//
//-----------------------------------------------------------------------------
...
//                               (ie non-device) for which properties are
//                               defined
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::get_device_exported_for_class() - Query database for list of
// exported
...
//-----------------------------------------------------------------------------
//
// Database::delete_attribute_alias() - Delete an attribute alias from the
// database
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::get_property_history() - Returns the history of the specified
// object
//                                    property
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::get_device_property_history() - Returns the history of the
// specified
//                                           device property
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Database::unexport_event() - public method to unexport an event from the
// Database
//
//-----------------------------------------------------------------------------
...
CORBA::Any *Database::fill_server_cache(const std::string &ds_name, const std::string &loc_host)
{
  ...
  if(pos == std::string::npos)
  {
    TANGO_THROW_EXCEPTION(API_MethodArgument, "The device server name parameter is incorrect. "
                                              "Should be: <ds_exec_name>/<inst_name>");
  }

  //
  // Read Db device "StoredProcedureRelease" attribute. In case the stored
  // procedure release is >= 1.9, add Tango major release at the end the host
  // name. This is required to manage compatibility between different Tango
  // releases used by device server(s) process when stored procedure supporting
  // pipes is installed (release 1.9 or higher) To read the attribute, we do not
  // have a DeviceProxy.
  //
...
void Database::delete_all_device_attribute_property(std::string dev_name, const DbData &db_data)
{
  ...
  if(filedb != 0)
  {
    TANGO_THROW_EXCEPTION(API_NotSupportedFeature, "The underlying database command is not implemented "
                                                   "when the database is a file");
  }
  else
...
AccessControlType Database::check_access_control(const std::string &devname)
{
  ...
  try
  {
    if((access_checked == false) && (access_proxy == NULL))
    {
      ...
      if(ret == -1)
      {
        ...
        {
          //
          // No access service defined, give WRITE ACCESS to everyone also on
          // the database device
          //
  ...
  catch(Tango::DevFailed &e)
  {
       ...
       ::strcmp(e.errors[0].reason.in(), DB_DeviceNotDefined) == 0)
    {
      ...
      tmp_err_desc     = tmp_err_desc + "\nControlled access service defined in Db but "
                                        "unreachable --> Read access given to all devices...";
...
bool Database::is_command_allowed(const std::string &devname, const std::string &cmd)
{
  ...
  {
    //
    // In case of Database object, the first command uses the default access
    // right (READ) Therefore, this is_command_allowed method is called and the
    // access are checked by the check_access_control() call upper in this
    // method. If the access is WRITE, force true for the retrun value of this
    // method
    //
...
//-----------------------------------------------------------------------------
//
// Database::get_device_attribute_list() - Get list of attributes with data in
// db for a specified device
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------------------------------
//
// Database::get_class_pipe_property() - public method to get class pipe
// properties from the Database
//
//-----------------------------------------------------------------------------------------------------------------
  ...
  else
  {
    ...
    catch(Tango::DevFailed &e)
    {
      ...
      if(err_reason == DB_ClassNotFoundInCache || err_reason == DB_TooOldStoredProc)
      {
        ...
        {
          cout << "WARNING: You database stored procedure is too old to "
                  "support device pipe"
               << std::endl;
...
//------------------------------------------------------------------------------------------------------------------
//
// Database::get_device_pipe_property() - public method to get device pipe
// properties from the Database
//
//------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------
//
// Database::delete_class_pipe_property() - public method to delete class pipe
// properties from the Database
//
//------------------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------------------
//
// Database::delete_device_pipe_property() - public method to delete device pipe
// properties from the Database
//
//--------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//
// Database::get_class_pipe_list() - Query the database for a list of pipes
...
//-------------------------------------------------------------------------------------------------------------------
//
// Database::get_device_pipe_list() - Get list of pipes with data in db for a
// specified device
//
//-------------------------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------------------------------------------
//
// Database::delete_all_device_pipe_property() - Call Db server to delete all
// the property(ies) belonging to the specified device pipe(s)
//
//----------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//
// Database::put_class_pipe_property() - public method to put class pipe
...
//-------------------------------------------------------------------------------------------------------------------
//
// Database::put_device_pipe_property() - public method to put device pipe
// properties into the Database
//
//-------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------
//
// Database::get_class_pipe_property_history() - Returns the history of the
// specified class pipe property
//
//------------------------------------------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------------------------------------
//
// Database::get_device_pipe_property_history() - Returns the history of the
// specified device pipe property
//
//-----------------------------------------------------------------------------------------------------------------
