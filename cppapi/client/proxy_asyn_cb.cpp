  ...
  if(!CORBA::is_nil(env) && (env->exception() != NULL))
  {
    ...
    if((unk_ex = CORBA::UnknownUserException::_downcast(ex_ptr)) != NULL)
    {
      ...
      else
      {
        ...
        {
          m_ex  = *multi_serv_ex;
          err_3 = Tango::NamedDevFailedList(m_ex, dev_name(), (const char *) "Connection::Cb_WriteAttr_Request()",
  ...
  if(get_pasyn_cb_ctr() != 0)
  {
    ...
    else
    {
      ...
      while(get_pasyn_cb_ctr() != 0)
      {
        ...
        //
        // Retrieve this request in the cb request map and mark it as "arrived"
        // in both maps
        //
