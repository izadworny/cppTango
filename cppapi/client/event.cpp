...
//  file :  			event.cpp
//
//	description : 		C++ classes for implementing the event server and client
// singleton classes -
// EventConsumer.
//
...
//
// description :
//		This function will be executed at process exit or when the main returned.
// It has to be executed to
// properly 		shutdown and destroy the ORB used by as a server by the event
// system. The ORB loop is in EventConsumer thread. 		Therefore, get a
// reference to it, shutdown the ORB and wait until the thread exit. It also
// destroys the 		heartbeat filters
//
//--------------------------------------------------------------------------------------------------------------------
...
{
  //
  // Build and store the fqdn prefix for devices in the TANGO_HOST environment
  // variable (in lower case letters)
  //
  ...
  if(env_var_fqdn_prefix.empty() == true)
  {
    try
    {
      ...
      for(size_t loop = 0; loop < env_var_fqdn_prefix.size(); ++loop)
        std::transform(env_var_fqdn_prefix[loop].begin(), env_var_fqdn_prefix[loop].end(),
                       env_var_fqdn_prefix[loop].begin(), ::tolower);

      //
      // Also get Db server defined in DB but not in the user TANGO_HOST env.
      // variable
      //
  ...
  if(keep_alive_thread == NULL)
    ...
    //
    // Install a function to be executed at exit. This is the only way I found
    // to properly shutdown and destroy the ORB. Don't do this for windows DLL.
    //
  ...
  //
  // Create and start the EventConsumerKeepAliveThread. Do this at the method's
  // end because the keep_alive_thread ptr is also used as a "init done" flag.
  //
...
//
// description :
//		Get from a Tango control system which TANGO_HOST are available. This info
// comes from the Tango CS
// database
//
// argument :
//		in :
...
void EventConsumer::get_cs_tango_host(Database *db)
{
  try
  {
    ...
    // It true and if we don't know this alias, store its definition in the
    // alias map
    //
...
//
// description :
//		Method to stop the keep alive thread and to disconnect from all used event
// channels
//
//--------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		This method is a wrapper around the connection to the event channel
//(heartbeat event)
//
// argument :
//		in :
...
void EventConsumer::connect(DeviceProxy *device_proxy, const std::string &d_name, DeviceData &dd,
                            const std::string &adm_name, bool &necm)
{
  std::string channel_name = adm_name;

  //
  // If no connection exists to this channel then connect to it. Sometimes, this
  // method is called in order to reconnect to the notifd. In such a case, the
  // lock is already locked before the method is called
  //
...
//
// description :
//		Method to initialize in the DeviceAttribute instance given to the user the
// attribute value which are
// received
...
void EventConsumer::attr_to_device(const AttributeValue *attr_value, const AttributeValue_3 *attr_value_3, long vers,
...
void EventConsumer::attr_to_device(const AttributeValue_4 *attr_value_4, DeviceAttribute *dev_attr)
{
  ...
  //
  // Warning: Since Tango 9, data type SHORT is used for both short attribute
  // and enumeration attribute! Therefore, we need to store somewhere which
  // exact type it is. With IDL 5, it is easy, because the data type is
  // transferred on the network (modified IDL), For previous release, we do not
  // have enumerated data type and therefore the data type could be used only
  // for SHORT.
  //
...
//		out :
//			- dev_attr  : Pointer to the DeviceAttribute which will be given to the
// user
//
//-------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Method to subscribe to an event with the callback mechanism. Can be called
// in a stateless way, that it
// even 		works when the attribute is not available.
//
// argument :
//		in :
...
//          - stateless : Flag to enable the stateless connection when set to
//          true
//
//-------------------------------------------------------------------------------------------------------------------

int EventConsumer::subscribe_event(DeviceProxy *device, const std::string &attribute, EventType event,
                                   CallBack *callback, const std::vector<std::string> &filters, bool stateless)
{
...
//
// description :
//		Method to subscribe to an event with the event_queue mechanism. Can be
// called in a stateless way, that it even 		works when the attribute is not
// available.
//
// argument :
//		in :
...
//-------------------------------------------------------------------------------------------------------------------
int EventConsumer::subscribe_event(DeviceProxy *device, const std::string &attribute, EventType event,
                                   int event_queue_size, const std::vector<std::string> &filters, bool stateless)
{
...
//--------------------------------------------------------------------------------------------------------------------
int EventConsumer::subscribe_event(DeviceProxy *device, const std::string &attribute, EventType event,
                                   CallBack *callback, EventQueue *ev_queue, const std::vector<std::string> &filters,
                                   bool stateless)
{
  ...
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_InvalidArgs,
                              "The quality change event does not exist any more. A change event is "
                              "fired on a quality change!");
  }
  else
  ...
  // In such a case, we have to do  the subscription in a thread  otherwise we
  // have a deadlock due to the event consumer thread which can not at the same
  // time execute the call back and register the new event
  //
  ...
  //
  // Take a writer lock right now and not in the connect_event method. In case
  // of stateless subscription and if the device is not there, the lock will
  // still be valid when the data will be inserted into the vector of
  // non-connected events. Also ask the main ZMQ thread to delay all incoming
  // event until this method exit. A dead lock could happen if we don't do this
  // (really experienced!)
  //
  ...
  catch(Tango::DevFailed &e)
  {
    ...
    // when the subscribe event has not worked, store the connection data in a
    // vector of not yet connected events. Retry to connect in the next
    // heartbeat period.
...
int EventConsumer::subscribe_event(DeviceProxy *device, EventType event, CallBack *callback, bool stateless)
{
  ...
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_InvalidArgs,
                              "Device,callback pointer NULL or unsupported event type");
  }
...
int EventConsumer::connect_event(DeviceProxy *device, const std::string &obj_name, EventType event, CallBack *callback,
                                 EventQueue *ev_queue, const std::vector<std::string> &filters, std::string &event_name,
                                 int event_id)
{
  ...
  //
  // Inform server that we want to subscribe (we cannot use the asynchronous
  // fire-and-forget request so as not to block the client because it does not
  // reconnect if the device is down !) To do this, we need to build DS adm
  // device proxy. If it is not the first call for this DS, we should find it in
  // map. Otherwise, get it.
  //
    ...
    if(dd.is_empty() == true)
    {
      if(device->get_from_env_var() == false)
      {
        ...
        if(fqdn_prefix != env_var_fqdn_prefix[0])
        {
          ...
          o << " is too old to generate event in a multi TANGO_HOST "
               "environment. Please, use Tango >= 7.1"
            << std::ends;
  ...
  catch(Tango::DevFailed &e)
  {
    ...
    if(reason == API_CommandNotFound)
      ...
      TANGO_RETHROW_API_EXCEPTION(EventSystemExcept, e, API_DSFailedRegisteringEvent,
                                  "Device server send exception while trying to register event");
  }

  //
  // Change event name if it is IDL 5 compatible:
  // This code is Tango 9 or more. If the remote device is IDL 5 (or more),
  // insert tango IDL release number at the beginning of event name.
  //
  ...
  //
  // Do we already have this event in the callback map? If yes, simply add this
  // new callback to the event callback list If it's a ATTR_CONF_EVENT, don't
  // forget to look for the two different event kinds
  //
  ...
  if(iter != event_callback_map.end())
  {
    ...
    get_fire_sync_event(device, callback, ev_queue, event, event_name, obj_name, iter->second,
  ...
  //
  // Check if this subscription is for a fwd attribute root attribute (when
  // relevant)
  //
  ...
  //
  // Create a callback monitor and set its timeout to 1000ms not to block the
  // event consumer for too long.
  //
  ...
  //
  // If we have a CS for which TANGO_HOST is one alias (host name in alias map),
  // set flag in map
  //
  ...
  if(!ret.second)
  {
    ...
    o << "Failed to connect to event channel for device " << device_name
      << "\nCorrupted internal map: event callback already exists. Please "
         "report bug!"
      << std::ends;
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed, o.str());
  }
  ...
  //
  // Read the attribute/pipe by a simple synchronous call.This is necessary for
  // the first point in "change" mode Force callback execution when it is done
  //

  get_fire_sync_event(device, callback, ev_queue, event, event_name, obj_name, iter->second, local_callback_key);

  //
  // Sleep for some mS in order to give to ZMQ some times to propagate the
...
{
  if(filters.size() == 1)
    return filters[0]; // here filters[0] is expected to be user defined
                       // attribute name
...
void EventConsumer::unsubscribe_event(int event_id)
{
  if(event_id == 0)
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_EventNotFound,
                              "Failed to unsubscribe event, the event id specified does not "
                              "correspond with any known one");
  }
  ...
  // Do not take a WriterLock because the push_structured_event method already
  // holds a Reader lock In such a case, the real unsubscribe will be done later
  // via a thread
  //

  {
    ...
    for(epos = event_callback_map.begin(); epos != event_callback_map.end(); ++epos)
    {
      for(esspos = epos->second.callback_list.begin(); esspos != epos->second.callback_list.end(); ++esspos)
      {
        ...
        {
          //
          // If the unsubscribe is done while the callback is being executed,
          // mark the callback as unusable (event_id < 0) and start a thread
          // which will do the unsubscribe when the callback execution will be
          // finished
          //
          ...
          if(thread_id != 0)
          {
            ...
            if(omni_thread::self()->id() == thread_id)
            {
              //                           cout << event_id << ": Unsubscribing
              //                           for an event while it is in its
              //                           callback
              //                           !!!!!!!!!!" << endl;
              esspos->id = -event_id;
  ...
  //
  // Ask the main ZMQ thread to delay all incoming event until this method exit.
    ...
    for(esspos = evt_cb.callback_list.begin(); esspos != evt_cb.callback_list.end(); ++esspos)
    {
      if(esspos->id == event_id)
      {
        ...
        if(evt_cb.callback_list.empty() == true)
        {
          ...
          if(evt_ch.channel_type == NOTIFD)
          {
            ...
            catch(...)
            {
              TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_EventNotFound,
                                        ...
                                        "remove_filter() "
                                        "or destroy() (hint: check the Notification daemon is "
                                        "running ");
            }
          }
          else
          ...
          //
          // This channel is not used anymore in the app, remove its entry in
          // the channel maps
          //
              ...
              if(chan_pos->first == deleted_channel_name)
              {
                if(evt_ch.adm_device_proxy != NULL)
                {
                  if(evt_ch.channel_type == NOTIFD)
                  {
                    ...
                    {
                      TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_EventNotFound,
                                                "Failed to unsubscribe event, caught exception while "
                                                "calling remove_filter() or destroy() on "
                                                "the heartbeat filter (hint: check the Notification "
                                                "daemon is running ");
                    }
                  }
                  else
  ...
  TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_EventNotFound,
                            "Failed to unsubscribe event, the event id "
...
//
// description :
//		Add a not connected event to the list of not connected event managed in
// the event consumer in order  to
//      periodically retry the subscription. This method is supposed to be
//      called when event are subscribed within a event callback. In this case
//      the subsciption is done in a thread (DelayedEventSubThread class)
//
// argument :
//		in  :
...
//
// description :
//		Return a vector with all events stored in the event queue. Events are kept
// in the buffer since the last 		extraction with get_events().
...
void EventConsumer::get_events(int event_id, EventDataList &event_list)
{
  ...
  TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_EventNotFound,
                            "Failed to get event, the event id specified does "
                            "not correspond with any known one");
}
...
//
// description :
//		Return a vector with all attribute configuration events stored in the
// event queue.
//      Events are kept in the buffer since the last extraction with
//      get_events(). After returning the event data, the event queue gets
//      emptied!
//
// argument :
//		in  :
...
//
// description :
//		Return a vector with all device interface change events stored in the
// event queue.
//      Events are kept in the buffer since the last extraction with
//      get_events(). After returning the event data, the event queue gets
//      emptied!
...
int EventConsumer::add_new_callback(DeviceProxy *device, EvCbIte &iter, CallBack *callback, EventQueue *ev_queue,
                                    int event_id)
{
...
{
  //
  // A small mS sleep here! This is required in case there is a push_event in
  // the read_attribute (or pipe) method on the device side. This sleep gives
  // time to ZMQ to send its subscription message
  //
     ...
     (event == PERIODIC_EVENT))
  {
    ...
    if(callback != NULL)
    {
      ...
      {
        std::cerr << "EventConsumer::subscribe_event() exception in callback "
                     "method of "
                  << callback_key << std::endl;
      }
  ...
  else if(event == ATTR_CONF_EVENT)
  {
    ...
    FwdAttrConfEventData *event_data = new FwdAttrConfEventData(device, local_domain_name, local_event_name, aie, err);
    AutoTangoMonitor _mon(cb.callback_monitor);
...
ChannelType EventConsumer::get_event_system_for_event_id(int event_id)
{
  ...
  //
  // Also search in the not connected event vector. The returned value in this
...
EventData::~EventData() { delete attr_value; }
...
void EventData::set_time() { reception_date = make_TimeVal(std::chrono::system_clock::now()); }
...
FwdEventData::FwdEventData(DeviceProxy *dev, const std::string &_s1, const std::string &_s2,
                           Tango::DeviceAttribute *_da, const DevErrorList &_del)
...
FwdEventData::FwdEventData(DeviceProxy *dev, const std::string &_s1, const std::string &_s2,
                           Tango::DeviceAttribute *_da, const DevErrorList &_del, zmq::message_t *_m)
    : EventData(dev, _s1, _s2, _da, _del),
      av_5(nullptr),
      event_data(_m)
{
...
AttrConfEventData::~AttrConfEventData() { delete attr_conf; }
...
void AttrConfEventData::set_time() { reception_date = make_TimeVal(std::chrono::system_clock::now()); }
...
DataReadyEventData::DataReadyEventData(DeviceProxy *dev, AttDataReady *dr, const std::string &evt,
...
void DataReadyEventData::set_time() { reception_date = make_TimeVal(std::chrono::system_clock::now()); }
...
DevIntrChangeEventData::DevIntrChangeEventData(DeviceProxy *dev, const std::string &evt, const std::string &d_name,
                                               DevCmdInfoList_2 *c_list, AttributeConfigList_5 *a_list, bool d_s,
                                               const DevErrorList &errors_in)
    : event(evt),
      device_name(d_name),
      dev_started(d_s),
...
DevIntrChangeEventData::DevIntrChangeEventData(DeviceProxy *dev, const std::string &evt, const std::string &d_name,
                                               CommandInfoList *c_list, AttributeInfoListEx *a_list, bool d_s,
                                               const DevErrorList &errors_in)
    : event(evt),
      device_name(d_name),
      dev_started(d_s),
...
void DevIntrChangeEventData::set_time() { reception_date = make_TimeVal(std::chrono::system_clock::now()); }
...
PipeEventData::~PipeEventData() { delete pipe_value; }
...
void PipeEventData::set_time() { reception_date = make_TimeVal(std::chrono::system_clock::now()); }
