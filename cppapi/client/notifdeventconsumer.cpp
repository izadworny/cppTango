...
{
  cout3 << "calling "
           "Tango::NotifdEventConsumer::disconnect_structured_push_consumer() \n";
}
    ...
    if(evt_ch.adm_device_proxy != NULL)
    {
      ...
      if(evt_ch.channel_type == NOTIFD)
      {
        try
        {
          ...
          // cout << "EventConsumer::cleanup_EventChannel_map(): Disconnect push
          // supplier!" << std::endl;
        ...
        {
          std::cerr << "Could not remove heartbeat filter from notification "
                       "daemon for "
...
//                  - valid_end : Valid endpoint in case the DS has retruned
//                  several
//                                possible endpoints
//
//-----------------------------------------------------------------------------

void NotifdEventConsumer::connect_event_system(const std::string &device_name, const std::string &att_name,
                                               const std::string &event_name, const std::vector<std::string> &filters,
                                               const EvChanIte &evt_it, EventCallBackStruct &new_event_callback,
                                               TANGO_UNUSED(DeviceData &dd), TANGO_UNUSED(size_t valid_end))
{
  //
...
void NotifdEventConsumer::connect_event_channel(const std::string &channel_name, Database *db, bool reconnect,
                                                TANGO_UNUSED(DeviceData &dd))
{
  ...
  if(db != NULL)
  {
    ...
    catch(...)
    {
      ...
      o << "Maybe the server is not running or is not linked with Tango "
           "release 4.x (or above)... "
        << std::ends;
  ...
  if(channel_exported)
  {
    ...
    {
      TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                                "Failed to narrow EventChannel from notification daemon (hint: make "
                                "sure the notifd "
  ...
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_EventChannelNotExported,
                              "Failed to narrow EventChannel (hint: make sure a notifd process is "
  ...
  try
  {
    ...
    {
      // cerr << "Could not get CosNotifyChannelAdmin::ConsumerAdmin" <<
      // std::endl;
      TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                                "Failed to get default Consumer admin from "
                                "notification daemon (hint: make sure the "
  ...
  try
  {
    proxySupplier = consumerAdmin->obtain_notification_push_supplier(CosNotifyChannelAdmin::STRUCTURED_EVENT, proxyId);
    if(CORBA::is_nil(proxySupplier))
    {
      // cerr << "Could not get CosNotifyChannelAdmin::ProxySupplier" <<
      // std::endl;
      TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                                "Failed to obtain a push supplier from notification daemon (hint: "
                                "make sure the notifd "
                                "process is running on this host)");
    }
    ...
    {
      // cerr << "Tango::NotifdEventConsumer::NotifdEventConsumer() could not
      // get CosNotifyChannelAdmin::StructuredProxyPushSupplier" << std::endl;
      TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                                "Failed to narrow the push supplier from "
                                "notification daemon (hint: make sure the "
                                "notifd process is running on this host)");
    }
    ...
    // This is necessary in case of maany threads doing subscribe/unsubscribe as
    // fast as they can
    //
  ...
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                              "Failed to get PushSupplier from notification daemon due to "
                              "AdminLimitExceeded (hint: "
                              "make sure the notifd process is running on this host)");
  }
  ...
  {
    std::cerr << "Tango::NotifdEventConsumer::NotifdEventConsumer() caught "
  ...
  if(res == 1)
  {
    ...
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotificationServiceFailed,
                              "Caught exception while adding constraint for "
                              "heartbeat (check filter)");
  }
}
...
void NotifdEventConsumer::push_structured_event(const CosNotification::StructuredEvent &event)
{
  ...
  //	cout << "Received event: domain_name = " << domain_name << ", event_type =
  ...
  if(event_name == "heartbeat")
  {
    ...
    // 2 - The fully qualified device name but with the Tango database host name
    // specifed without fqdn (in case of) 3 - The device name (for old servers)
    //
    ...
    if(ipos != channel_map.end())
    {
      ...
      {
        std::cerr << "Tango::NotifdEventConsumer::push_structured_event() "
  ...
  else
  {
    ...
    // 2 - The fully qualified attribute event name but with the Tango database
    // host name specifed without fqdn (in case of) 3 - In case of Tango system
    // with multi db server, replace the db server defined by the event with the
    // first one in the client list (like for the heartbeat event)
    //
    ...
    if(ipos != event_callback_map.end())
    {
      ...
      try
      {
        ...
        if(ty->kind() == tk_struct)
        {
          ...
          else
          {
            ...
            errors[0].desc     = Tango::string_dup("Unknown structure used to pass attribute "
                                                       "value (Need compilation ?)");
            dev_attr           = NULL;
          }
        }
        else
          ...
          if(esspos->id > 0)
          {
            ...
            if((ev_attr_conf == false) && (ev_attr_ready == false))
            {
              ...
              if(cb_ctr != cb_nb)
              {
                ...
                event_data = new EventData(esspos->device, fq_dev_name, event_name, dev_attr_copy, errors);
              }
              else
              {
              ...
              if(callback != NULL)
              {
                ...
                {
                  std::cerr << "Tango::NotifdEventConsumer::push_structured_"
                               "event() exception in callback method of "
                            << ipos->first << std::endl;
                }
            ...
            else if(ev_attr_ready == false)
            {
              ...
              if(cb_ctr != cb_nb)
              {
                ...
                *attr_info_copy                 = *attr_info_ex;
                event_data = new AttrConfEventData(esspos->device, fq_dev_name, event_name, attr_info_copy, errors);
              }
              else
              {
                event_data = new AttrConfEventData(esspos->device, fq_dev_name, event_name, attr_info_ex, errors);
              }
            ...
            {
              DataReadyEventData *event_data = new DataReadyEventData(esspos->device, att_ready, event_name, errors);
              // if a callback method was specified, call it!
              if(callback != NULL)
              {
                try
      ...
      catch(...)
      {
        ...
        std::cerr << "Tango::NotifdEventConsumer::push_structured_event() "
                     "timeout on callback monitor of "
                  << ipos->first << std::endl;
      }
    }
    else
...
NotifdEventConsumer::initialize_received_from_admin(TANGO_UNUSED(const Tango::DevVarLongStringArray *dvlsa),
                                                    const std::string &local_callback_key, const std::string &adm_name,
