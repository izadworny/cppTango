...
NamedDevFailedList::NamedDevFailedList(const Tango::MultiDevFailed &corba_ex, std::string dev_name, const char *op_name,
                                       const char *reason)
{
  unsigned int i;
  ...
  {
    NamedDevFailed tmp_err(corba_ex.errors[i].err_list, corba_ex.errors[i].name.in(), corba_ex.errors[i].index_in_call);
