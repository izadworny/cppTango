...
public:
   ...
   *
   * A constructor for a DbServer object for a server in the TANGO database
   * specified by the TANGO_HOST environment variable.
   *
   ...
   *
   * A constructor for a DbServer object for the server in the specified
   * database. This method reuses the Database supplied by the programmer
   *
   * @param [in] server_name	The device server name
   ...
   *
   * Add a group of devices to the database. The device names, server names and
   * classes are specified in the vector of DbDevInfo structures
   *
...
public:
   ...
   *
   * A constructor for a DbClass object for a class in the TANGO database
   * specified by the TANGO_HOST environment variable
   *
   ...
   *
   * A constructor for a DbClass object for the Tango class in the specified
   * database. This method reuses the Database supplied by the programmer.
   *
   * @param [in] class_name	The Tango class name
   * @param [in] db The database object
   ...
   * Database::get_class_property() for an example of how to specify and
   ...
   * Database::put_class_property() for an example of how to specify the
   * properties.
   *
   ...
   *
   * Delete the list of specified properties for this class in the database. See
   * Database::delete_property() for an example of how to specify the
   * properties.
   *
   ...
   * Database::get_class_attribute_property() for an example of how to specify
   ...
   * Database::put_class_attribute_property() for an example of how to specify
   * the properties.
   *
   * @param [in] db Property name(s) and value
   *
   ...
   * in the database. See Database::delete_class_attribute_property() for an
   ...
   * Database::get_class_pipe_property() for an example of how to specify and
   * retrieve the properties.
   *
   * @param [in,out] db Property name(s) and value
   *
   ...
   * Database::put_class_pipe_property() for an example of how to specify the
   * properties.
   *
   * @param [in] db Property name(s) and value
   *
   ...
   *
   * Delete all properties for the list of specified pipes for this class in the
   * database. See Database::delete_class_pipe_property() for an example of how
 ...
 * for inserting and extracting C++ native types. This is the fundamental type
 * for specifying database properties. Every property has a name and has one or
 * more values associated with it. The values can be inserted and extracted
 * using the operators << and >> respectively. A status flag indicates if there
 * is data in the DbDatum object or not. An additional flag allows the user to
 * activate exceptions.
 *
 * @headerfile tango.h
 * @ingroup DBase
...
public:
   ...
   *
   * Create an instance of the DbDatum class with name set to the specified
   * parameter
   *
   * @param [in] name	The CORBA ORB pointer. Default value is fine for 99 % of
   * cases
   *
   */
   ...
   * DbDatum my_float_vector("my_float_vector"),
   * my_double_vector("my_double_vector");
   *
   * std::string a_string;
   * short a_short;
   * DevLong a_long;
   ...
   * string my_string >> a_string; // extract a string my_float_vector <<
   * a_float_vector // insert a vector of floats my_float_vector >>
   * a_float_vector; // extract a vector of floats my_double_vector <<
   * a_double_vector; // insert a vector of doubles my_double_vector >>
   * a_double_vector; // extract a vector of doubles
   * @endcode
   *
   ...
   * trying to extract data from an empty DbDatum object. The default is to not
   * throw exception. The following flags are supported :
   * @li @b isempty_flag - throw a WrongData exception (reason =
   * API_EmptyDbDatum) if user tries to extract data from an empty DbDatum
   * object
   ...
   * different than the type used for insertion
   *
   ...
   * whether the DbDatum object contains data or not. It can be used to test
   * whether a property is defined in the database or not e.g.
   * @code
   ...
   * else
   * {
   *     cout << device_name << " has no parity defined in database !" <<
   * std::endl;
   * }
   * @endcode
   *
...
public:
  ...
  std::string stopped_date; ///< Date of the last device un-export (empty if not
                            ///< set in DB)
...
//
// DbServerData object to implement the features required to move a complete
// device server proces configuration from one database to another one
//

/**
 * Class used to move/copy a complete device server process database
 * configuration from one Tango host to another
 *
 * @headerfile tango.h
 * @ingroup DBase
 */
...
public:
   ...
   *
   * A constructor for a DbServerData object for a device server process defined
   * in the TANGO database specified by the TANGO_HOST environment variable
   *
   ...
   *
   * Return true if one of the device defined in the device server process with
   * configuration in this instance is already defined in the database defined
   * by the Tango host given as parameter
   *
   * @param [in] tg_host The tango host
   * @return Boolean set to true if one of the device server device is defined
   * in the Tango host given as parameter
   */
   ...
   * specified by the given Tango host parameter
   *
   * @param [in] tg_host The tango host
   */
   ...
   *
   * Remove the device server process database configuration from the database
   * specified by the given Tango host parameter
   *
   * @param [in] tg_host The tango host
   */
   ...
   *
   * Remove the device server process database configuration from the database
   * specified by the environment variable TANGO_HOST
   *
   */
  void remove();
