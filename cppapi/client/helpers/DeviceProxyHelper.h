...
//
//
//   description :  This utility class helps to use
//   command_inout,read_attributes and write_attribute(s)
//                on a DeviceProxy , from a Tango C++ Client.
//                Its role is :
//                  - to provide syntactic sugar to avoid the heavy manipulation
//                  of DeviceData objects
//                            used to exchange data between a C++ client and a
//                            server
//                  - to handle exception in an uniform way using the
//                  TangoExceptionHelper.h file
//
// Usage examples :
//
//    Step 1)  create a DeviceProxyHelper object (in your client init_device for
//    instance)
//
//    tangoCA = new DeviceProxyHelper("lucia/sls/tangoca",this);
//      ==>  A DeviceProxy is internally created by DeviceProxyHelper ( note :
//      the this pointer is used for logging purpose)
//
//    Step 2)  USE your remote Device !!
//
//     FOR COMMANDS (Note that  DEVVAR<X>STRINGARRAY argin or argout types are
//     not supported)
...
//  Note :  the methods are Templates so you don't care about type (while they
//  are supported by DeviceData!!!)
//
//  COMMAND EXAMPLES :
//                      a) execute Command Reset on the deviceProxy : No argin
//                      nor argout required tangoCA->command("Reset");
//
//                      b) execute Command GotoPosition on the deviceProxy with
//                      argin= 12.3 tangoCA->command_in("GotoPosition", 12.3);
//
//                      c) execute Command ReadPosition on the deviceProxy :
//                      argout will be put in read_value
//                      tangoCA->command_out("ReadPosition", read_value);
//
//                      d) execute Command ReadDoublePv on the deviceProxy :
//                      verticalGapPV.c_str() is the argin and
//                      *attr_VerticalGap_read is the argout.
//                      tangoCA->command_inout("ReadDoublePV",verticalGapPV.c_str(),
//                                            *attr_VerticalGap_read)
//                          ==> Automatically  the CommandInOutHelper has done
//                          all the DeviceData encapsulation
//                             and transfers the result of the command_inout in
//                             your argout
//
//
//        Handling of composite type like DEVVAR<X>STRINGARRAY in argin and/or
//        argout (with <X> = LONG or DOUBLE):
//           The <X> part and the STRING part must be seperated by user in two
//           vectors, std::vector<X> and std::vector<std::string>. The following
//           must be called:
//
//        - command with argin only:
//               void command_in (
//                      const std::string& cmd_name,
//                      const std::vector<_IN>& _nv_in,           // numerical
//                      part of the input DEVVAR<X>STRINGARRAY const
//                      std::vector<std::string>& _sv_in    // string part of
//                      the input DEVVAR<X>STRINGARRAY
//                    )
//
//        - command with argout only:
//              void command_out (
//                       const std::string& cmd_name,
//                       std::vector<_OUT>& _nv_out,              // numerical
//                       part of the ouput DEVVAR<X>STRINGARRAY
//                       std::vector<std::string>& _sv_out        // string part
//                       of the ouput DEVVAR<X>STRINGARRAY
//                     )
//
...
//                        part of the input DEVVAR<X>STRINGARRAY const
//                        std::vector<std::string>& _sv_in, // string part of
//                        the input DEVVAR<X>STRINGARRAY std::vector<_OUT>&
//                        _nv_out,             // numerical part of the ouput
//                        DEVVAR<X>STRINGARRAY std::vector<std::string>& _sv_out
//                        // string part of the ouput DEVVAR<X>STRINGARRAY
//                       )
//
//
//  ATTRIBUTES EXAMPLES :
//                      a) execute  read_attribute  on the deviceProxy and store
//                      value in omega_on_axis variable
//                             _omegaProxy_helper
//                             ->read_attribute("AxisCurrentPosition" ,
//                             omega_on_axis);
//
//                      b) execute  write_attribute  on the deviceProxy and with
//                      argin = 12.3
//				                    _omegaProxy_helper		->
// write_attribute("AxisCurrentPosition", 12.3);
...
// * Update DeviceProxyHelper with Florent modification ( read the write part of
// an attribute)
//
// Revision 1.3  2005/04/27 10:22:28  buteau
// - Under linux , for all methods of DeviceProxyHelper (i.e read_attribute,
// command_in, command_out , etc .. ) , the filename and the line number of
// client Device appears in Exceptions (instead of line number and filenmae in
// DeviceProxyHelper.h)
// - under WIN32, VC++6 : same feature but only on read_attribute,
// write_attribute, command methods of DeviceProxyHelper
...
// - Changed the only one command_inout overloaded 4 times by four differents
// methods:
//  -command (no argin,no argout)
//  -command_inout (argin and argout)
//  -command_in (only argin)
...
//
//	Definition of macros to benefit from expansion of __FILE__ and __LINE__
// keywords in Device caller source file
//
...
public:
  ...
  //---------------------------------------------------------------------------
  inline Tango::DeviceProxy *get_device_proxy(void) { return device_proxy_; }

  //---------------------------------------------------------------------------
  //  HelperBase::operator->
  //  returns the underlying device
  //---------------------------------------------------------------------------
  inline Tango::DeviceProxy *operator->(void) { return device_proxy_; }
...
public:
  ...
  void internal_command(const std::string &cmd_name, std::string file, int line)
  {
    if(device_proxy_)
    {
      _DEV_TRY_FILE_LINE(
          ...
          //- origin
          "CommandInOutHelper::command", file, line);
    }
  }
  ...
  void internal_command_inout(const std::string &cmd_name, const _IN &argin, _OUT &argout, std::string file = __FILE__,
                              int line = __LINE__)
  {
    if(device_proxy_)
    {
      ...
      _DEV_TRY_FILE_LINE(
          ...
          std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
          //- origin
          "CommandInOutHelper::command_inout", file, line);
      _DEV_TRY_FILE_LINE(
          //- try
          dd_out_ >> argout,
          //- what do we tried
  ...
  //---------------------------------------------------------------------------
  //  CommandInOutHelper::command_inout
  //  exec a DEVVAR<X>STRINGARRAY/DEVVAR<X>STRINGARRAY command on the underlying
  //  device cmd_name : the name of the TANGO command _nv_in : numerical part of
  //  the input DEVVAR<X>STRINGARRAY _sv_in : string part of the input
  //  DEVVAR<X>STRINGARRAY _nv_out : numerical part of the ouput
  //  DEVVAR<X>STRINGARRAY _sv_out : string part of the ouput
  //  DEVVAR<X>STRINGARRAY
  //---------------------------------------------------------------------------
  //  template arg _IN must be supported by DeviceData::.insert
  //  template arg _OUT must be supported by DeviceData::.insert
  //---------------------------------------------------------------------------
  template <class _IN, class _OUT>
  void internal_command_inout(const std::string &cmd_name, const std::vector<_IN> &_nv_in,
                              const std::vector<std::string> &_sv_in, std::vector<_OUT> &_nv_out,
                              std::vector<std::string> &_sv_out, std::string file = __FILE__, int line = __LINE__)
  {
    if(device_proxy_)
    {
      Tango::DeviceData dd_in;
  ...
  //---------------------------------------------------------------------------
  template <class _IN>
  void internal_command_inout(const std::string &TANGO_UNUSED(cmd_name), const _IN &TANGO_UNUSED(argin),
                              DevVarDoubleStringArray *TANGO_UNUSED(argout), std::string file = __FILE__,
                              int line = __LINE__)
  {
  ...
  template <class _IN>
  void internal_command_inout(const std::string &TANGO_UNUSED(cmd_name), const _IN &TANGO_UNUSED(argin),
                              DevVarLongStringArray *TANGO_UNUSED(argout), std::string file = __FILE__,
                              int line = __LINE__)
  {
    TangoSys_OMemStream o;
    o << " [" << file << "::" << line << "]" << std::ends;
  ...
  //---------------------------------------------------------------------------
  template <class _IN>
  void internal_command_in(const std::string &cmd_name, const _IN &argin, std::string file = __FILE__,
                           int line = __LINE__)
  {
    if(device_proxy_)
    {
      ...
      _DEV_TRY_FILE_LINE(
          ...
          std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
          //- origin
          "CommandInOutHelper::command_in", file, line);
    }
  }
  ...
  //---------------------------------------------------------------------------
  template <class _IN>
  void internal_command_in(const std::string &cmd_name, const std::vector<_IN> &_nv_in,
  ...
  void internal_command_out(const std::string &cmd_name, _OUT &argout, std::string file = __FILE__, int line = __LINE__)
  {
    if(device_proxy_)
    {
      _DEV_TRY_FILE_LINE(
          ...
          std::string("command_inout on ") + FULL_CMD_NAME(cmd_name),
          //- origin
          "CommandInOutHelper::command_out", file, line);
      _DEV_TRY_FILE_LINE(
          //- try
          dd_out_ >> argout,
          //- what do we tried
  ...
  //---------------------------------------------------------------------------
  template <class _OUT>
  void internal_command_out(_OUT TANGO_UNUSED(dummy), DevVarDoubleStringArray *TANGO_UNUSED(argout),
  ...
  //---------------------------------------------------------------------------
  template <class _OUT>
  void internal_command_out(_OUT TANGO_UNUSED(dummy), DevVarLongStringArray *TANGO_UNUSED(argout),
                            std::string file = __FILE__, int line = __LINE__)
  {
    TangoSys_OMemStream o;
    o << " [" << file << "::" << line << "]" << std::ends;
  ...
  //---------------------------------------------------------------------------
  template <class _OUT>
  void internal_command_out(const std::string &cmd_name, std::vector<_OUT> &_nv_out, std::vector<std::string> &_sv_out,
                            std::string file = __FILE__, int line = __LINE__)
  {
    if(device_proxy_)
    {
...
public:
  ...
  //---------------------------------------------------------------------------
  Tango::DeviceAttribute get_device_attribute() { return da_out_; }
  ...
  void internal_write_attribute(const std::string &attr_name, const _VAL &attr_value, std::string file, int line)
  {
    if(device_proxy_)
    {
      ...
      _DEV_TRY_FILE_LINE(
          ...
          //- origin
          "AttributeHelper::write_attribute", file, line);
    }
  }
  ...
  void internal_read_attribute(const std::string &attr_name, _VAL &attr_value, std::string file, int line)
  {
    if(device_proxy_)
    {
      _DEV_TRY_FILE_LINE(
          ...
          //- origin
          "AttributeHelper::read_attribute", file, line);
      _DEV_TRY_FILE_LINE(
          //- try
          da_out_ >> attr_value,
          //- what do we tried
  ...
  void internal_read_attribute_w(const std::string &attr_name, _VAL &w_attr_value, std::string file, int line)
  {
    if(device_proxy_)
    {
      _DEV_TRY_FILE_LINE(
          ...
          std::string("read_attribute on ") + FULL_ATTR_NAME(attr_name),
          //- origin
          "AttributeHelper::read_attribute_w", file, line);
