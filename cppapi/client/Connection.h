 ...
 *
 * Base class for Tango device access. This class is pure virtual and can be
 * instantiated as is.
 *
 * $Author: taurel $
 * $Revision: 1 $
...
public:
   ...
   * Set device timeout
   *
   * Set client side timeout for device in milliseconds. Any method which takes
   * longer than this time to execute will throw an exception.
   *
   ...
   * The DevSource is an enumerated type which can be one of {DEV, CACHE,
   * CACHE_DEV}. See chapter on Advanced Feature in <a
   ...
   * for command_inout and read_attribute methods. The DevSource is an
   * enumerated type which can be one of {DEV, CACHE, CACHE_DEV}. The default
   * value is CACHE_DEV. See chapter on Advanced Feature for all details
   * regarding polling
   *
   ...
   * communication error between client and server. The API will try to re-build
   * the network connection between client and server as soon as an error is
   ...
   * result is returned in a DeviceData object
   *
   ...
   * object, output is returned as a DeviceData object.
   *
   * @param [in] cmd_name The command name
   ...
   * arguments are passed in a DeviceData object (see following chapters on how
   * to insert data into DeviceData object). The last argument is a fire and
   * forget flag. If this flag is set to true, this means that the client does
   * not care at all about the server answer and will even not try to get it. A
   * false default value is provided. Please, note that device re-connection
   * will not take place (in case it is needed) if the fire and forget mode is
   * used. Therefore, an application using only fire and forget requests is not
   ...
   *
   * Execute asynchronously (polling model) a command on a device which takes no
   * input argument. The last argument is a fire and forget flag. If this flag
   * is set to true, this means that the client does not care at all about the
   * server answer and will even not try to get it. A false default value is
   * provided. Please, note that device re-connection will not take place (in
   * case it is needed) if the fire and forget mode is used. Therefore, an
   * application using only fire and forget requests is not able to
   * automatically re-connnect to device. This call returns an asynchronous call
   * identifier which is needed to get the command result.
   *
   * @param [in] cmd_name The command name
   * @param [in] forget Fire and forget flag
   * @return The call identifier
   ...
   * if it is a valid reply, it is returned to the caller in a DeviceData
   * object. If the reply is an exception, it is re-thrown by this call. An
   * exception is also thrown in case of the reply is not yet arrived. Example :
   * @code
   ...
   *
   * Check if the answer of an asynchronous command_inout is arrived (polling
   model). id is the asynchronous
   * call identifier. If the reply is arrived and if it is a valid reply, it is
   returned to the caller in a DeviceData
   * object. If the reply is an exception, it is re-thrown by this call. If the
   reply is not yet arrived, the call will
   * wait (blocking the process) for the time specified in timeout. If after
   timeout milliseconds, the reply is still
   * not there, an exception is thrown. If timeout is set to 0, the call waits
   ...
   * no input argument. The last argument is a reference to a callback object.
   ...
   * arguments are passed in a DeviceData object (see following chapters on how
   * to insert data into DeviceData object). The last argument is a reference to
   * a callback object. This callback object should be an instance of a user
   * class inheriting from the Tango::CallBack class with the cmd_ended() method
   * overloaded.
   *
   * @param [in] cmd_name The command name
   ...
   * replied. Returns immediately if there is no replies already arrived or if
   * there is no asynchronous request for the device. Example :
   * @code
   ...
   * identifier. This is a call local to the client. It simply allows the caller
   * not to get the answer of the asynchronous request. It does not interrupt
   ...
   *
   * Cancel all pending polling asynchronous requests. This is a call local to
   * the client. It simply allows the caller not to get the answers of the
