...
//  file :      	event.h
//
// 	description :   C++ include file for implementing the TANGO event server and
// client singleton classes -
//					EventSupplier and EventConsumer.
//             		These classes are used to send events from the server to the
 ...
 *
 * This class is used to pass data to the callback method when an event is sent
 * to the client
 *
 *
 * @headerfile tango.h
...
public:
  ...
  EventData(DeviceProxy *dev, const std::string &nam, const std::string &evt, Tango::DeviceAttribute *attr_value_in,
...
public:
  ...
  FwdAttrConfEventData(DeviceProxy *, const std::string &, const std::string &, Tango::AttributeInfoEx *,
                       const DevErrorList &);
 ...
 *
 * This class is used to pass data to the callback method when an attribute data
 * ready event is sent to the client.
 *
 *
 * @headerfile tango.h
 * @ingroup Client
...
public:
  ...
  int ctr;               ///< The user counter. Set to 0 if not defined when sent by the
                         ///< server
...
public:
  ...
  DevIntrChangeEventData(DeviceProxy *, const std::string &, const std::string &, DevCmdInfoList_2 *,
                         AttributeConfigList_5 *, bool, const DevErrorList &);
  DevIntrChangeEventData(DeviceProxy *, const std::string &, const std::string &, CommandInfoList *,
                         AttributeInfoListEx *, bool, const DevErrorList &);
  ...
  bool dev_started;             ///< Device started flag (true when event sent due to device
                                ///< being (re)started and with only a possible but not sure
                                ///< interface change)
 ...
 *
 * This class is used to pass data to the callback method when a pipe event is
 * sent to the client
 *
 *
 * @headerfile tango.h
...
public:
  ...
  PipeEventData(DeviceProxy *dev, const std::string &nam, const std::string &evt, Tango::DevicePipe *pipe_value_in,
                const DevErrorList &errors_in);
