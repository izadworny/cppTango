...
public:
   ...
   * @endcode
   *
   * @param [in] orb	The CORBA ORB pointer. Default value is fine for 99 % of
   * cases
   *
   */
   ...
   * specified in the DbDevInfo structure. Example :
   * @code
   ...
   * @endcode
   *
   * @param [in] dev_info A reference to a DbDevInfo instance with all device
   * info.
   *
   ...
   * (DB_SQLError, DB_DeviceNotDefined)
   */
   ...
   * server, class, pid and version are specified in the DbDevExportInfo
   * structure. Example :
   * @code
   ...
   * The device names, server names and classes are specified in a vector of
   ...
   * vector of DbDevExportInfo structures.
   *
   ...
   * the DbDatum object contains pair of strings &lt;instance_name&gt; followed
   * by &lt;device name&gt;.
   * @code
   ...
   * DbDatum structures. The method returns the properties in the same DbDatum
   * structures. To retrieve the properties use the extract operator >>. Here is
   * an example of how to use the DbData type to specify and extract properties
   * :
   * @code
   * DbData db_data;
   ...
   * property names and their values are specified by the vector of DbDatum
   * structures. Use the insert operator >> to insert the properties into the
   * DbDatum structures. Here is an example of how to insert properties into the
   ...
   * are specified by the vector of DbDatum structures. Here is an example of
   * how to delete properties from the database using this method :
   * @code
   * DbData db_data;
   * db_data.push_back(DbDatum(“velocity”));
   * db_data.push_back(DbDatum(“acceleration”));
   ...
   * object. The property names are specified by the vector of DbDatum
   * structures. The method returns the properties in the same DbDatum
   * structures. To retrieve the properties use the extract operator >>. Here is
   * an example of how to use the DbData type to specify and extract properties
   * :
   * @code
   ...
   * Note that prop_name can contain a wildcard character (eg: "prop*"). An
   * example of usage of a similar function can be found in the documentation of
   * the get_property_history() function.
   *
   * @param [in] dev_name The device name
   * @param [in] prop_name The property name
   * @return A vector of DbHistory instances
   ...
   * DbDatum structures. The method returns all the properties for the specified
   * attributes. The attribute names are returned with the number of properties
   * specified as their value. The first DbDatum element of the returned DbData
   * vector contains the first attribute name and the first attribute property
   * number. The following DbDatum element contains the first attribute property
   * name and property values. To retrieve the properties use the extract
   * operator >>. Here is an example of how to use the DbData type to specify
   * and extract attribute properties :
   * @code
   * DbData db_data;
   *
   * db_data.push_back(DbDatum("velocity"));
   ...
   * of DbDatum structures. Use the insert operator >> to insert the properties
   * into the DbDatum structures. Here is an example of how to insert/update
   * properties min, max for attribute velocity and properties min, max for
   * attribute acceleration of device id11/motor/1 into the database using this
   * method :
   * @code
   * DbDatum vel("velocity");                // We want to put properties for
   * attribute "velocity" DbDatum vel_min("min"), vel_max("max"); DbDatum
   * acc("acceleration")             // We want to put properties for attribute
   * "acceleration" DbDatum acc_min("min"), acc_max("max"); DbData db_data;
   *
   * vel << 2;                               // Two properties for attribute
   * "velocity" vel_min << 0.0;                         // Value for property
   * min vel_max << 1000000.0;                   // Value for property max
   *
   * db_data.push_back(vel);
   * db_data.push_back(vel_min);
   * db_data.push_back(vel_max);
   *
   * acc << 2;                               // Two properties for attribute
   * "acceleration" acc_min << 0.0;                         // Value for
   * property min acc_max << 8000000;                     // Value for property
   * max
   *
   ...
   * attribute names are specified by the vector of DbDatum structures. Here is
   * an example of how to delete the unit property of the velocity attribute of
   ...
   * property. Note that prop_name and att_name can contain a wildcard character
   * (eg: "prop*"). An example of usage of a similar function can be found in
   * the documentation of the get_property_history() function.
   *
   * @param [in] dev_name The device name
   * @param [in] prop_name The property name
   ...
   * @exception ConnectionFailed,CommunicationFailed,DevFailed from device
   */
  std::vector<DbHistory> get_device_attribute_property_history(const std::string &dev_name,
   ...
   * specified device. Note that this is not the list of all device attributes
   * because not all attribute(s) have some data in database
   *
   * @param [in] dev_name The device name
   ...
   *
   * Get the list of pipe(s) with some data defined in database for a specified
   * device. Note that this is not the list of all device pipes because not all
   * pipe(s) have some data in database
   *
   * @param [in] dev_name The device name
   ...
   *
   * Query the database for a list of device pipe properties for the specified
   * object. The pipe names are specified by the vector of DbDatum structures.
   * The method returns all the properties for the specified pipes. The pipe
   * names are returned with the number of properties specified as their value.
   * The first DbDatum element of the returned DbData vector contains the first
   * pipe name and the first pipe property number. The following DbDatum element
   * contains the first pipe property name and property values. To retrieve the
   ...
   * pipe property names and their values are specified by the vector of DbDatum
   * structures. Use the insert operator >> to insert the properties into the
   * DbDatum structures. Here is an example of how to insert/update properties
   * min, max for pipe velocity and properties min, max for pipe acceleration of
   * device id11/motor/1 into the database using this method :
   * @code
   * DbDatum vel("velocity");                // We want to put properties for
   * pipe "velocity" DbDatum vel_min("min"), vel_max("max"); DbDatum
   * acc("acceleration")             // We want to put properties for pipe
   * "acceleration" DbDatum acc_min("min"), acc_max("max"); DbData db_data;
   *
   * vel << 2;                               // Two properties for pipe
   * "velocity" vel_min << 0.0;                         // Value for property
   * min vel_max << 1000000.0;                   // Value for property max
   *
   * db_data.push_back(vel);
   ...
   *
   * Delete a list of pipe properties for the specified device. The pipe names
   * are specified by the vector of DbDatum structures. Here is an example of
   * how to delete the unit property of the velocity pipe of the id11/motor/1
   ...
   * property. Note that prop_name and pipe_name can contain a wildcard
   * character (eg: "prop*"). An example of usage of a similar function can be
   * found in the documentation of the get_property_history() function.
   *
   * @param [in] dev_name The device name
   ...
   * @exception ConnectionFailed,CommunicationFailed,DevFailed from device
   */
  std::vector<DbHistory> get_device_pipe_property_history(const std::string &dev_name, const std::string &pipe_name,
                                                          const std::string &prop_name);
   ...
   *
   * Query the database for a list of class properties. The property names are
   * specified by the vector of DbDatum structures. The method returns the
   * properties in the same DbDatum structures. To retrieve the properties use
   * the extract operator >>. Here is an example of how to use the DbData type
   * to specify and extract properties :
   * @code
   * DbData db_data;
   * db_data.push_back(DbDatum("velocity"));
   * db_data.push_back(DbDatum("acceleration"));
   ...
   *
   * Insert or update a list of properties for the specified class. The property
   * names and their values are specified by the vector of DbDatum structures.
   * Use the insert operator >> to insert the properties into the DbDatum
   * structures. Here is an example of how to insert properties into the
   * database using this method :
   * @code
   * DbDatum velocity("velocity"), acceleration("acceleration");
   * DbData db_data;
   ...
   *
   * Delete a list of properties for the specified class. The property names are
   * specified by the vector of Db- Datum structures. Here is an example of how
   * to delete properties from the database using this method
   * @code
   * DbData db_data;
   * db_data.push_back(DbDatum("velocity"));
   * db_data.push_back(DbDatum("acceleration"));
   ...
   * of DbDatum structures. Use the insert operator >> to insert the properties
   * into the DbDatum structures. Here is an example of how to insert/update
   * min, max properties for attribute velocity and min, max properties for
   * attribute acceleration properties belonging to class StepperMotor into the
   * database using this method :
   * @code
   ...
   *
   * Delete a std::list of attribute properties for the specified class. The
   * attribute names are specified by the std::vector of DbDatum structures. All
   * properties belonging to the std::listed attributes are deleted. Here is an
   * example of how to delete the unit property of the velocity attribute of the
   ...
   *
   * Get the std::list of the last 10 modifications of the specifed class
   * attribute property. Note that prop_name and att_name can contain a wildcard
   * character (eg: "prop*"). An example of usage of a similar function can be
   * found in the documentation of the get_property_history() function.
   *
   * @param [in] class_name The class name
   ...
   * @exception ConnectionFailed,CommunicationFailed,DevFailed from device
   */
  std::vector<DbHistory> get_class_attribute_property_history(const std::string &class_name,
                                                              const std::string &att_name,
                                                              const std::string &prop_name);
   ...
   *
   * Query the database for a std::list of class pipe properties for the
   * specified object. The pipe names are specified by the std::vector of
   * DbDatum structures. The method returns all the properties for the specified
   * pipes. The pipe names are returned with the number of properties specified
   * as their value. The first DbDatum element of the returned DbData
   * std::vector contains the first pipe name and the first pipe property
   * number. The following DbDatum element contains the first pipe property name
   * and property values. To retrieve the properties use the extract operator
   ...
   *
   * Insert or update a std::list of pipe properties for the specified class.
   * The pipe property names and their values are specified by the std::vector
   * of DbDatum structures. Use the insert operator >> to insert the properties
   * into the DbDatum structures. Here is an example of how to insert/update
   * min, max properties for pipe velocity and min, max properties for pipe
   * acceleration properties belonging to class StepperMotor into the database
   * using this method :
   * @code
   * DbDatum velocity("velocity"), vel_min("min"), vel_max("max");
   * DbDatum acceleration("acceleration"), acc_min("min"), acc_max("max");
   ...
   *
   * Delete a std::list of pipe properties for the specified class. The pipe
   * names are specified by the std::vector of DbDatum structures. All
   * properties belonging to the std::listed pipes are deleted. Here is an
   * example of how to delete the unit property of the velocity pipe of the
   * StepperMotor class from the database using this method :
   * @code
   * DbData db_data;
   * db_data.push_back(DbDatum("velocity"));
   ...
   *
   * Get the std::list of the last 10 modifications of the specifed class pipe
   * property. Note that prop_name and pipe_name can contain a wildcard
   * character (eg: "prop*"). An example of usage of a similar function can be
   * found in the documentation of the get_property_history() function.
   *
   ...
   * @exception ConnectionFailed,CommunicationFailed,DevFailed from device
   */
  std::vector<DbHistory> get_class_pipe_property_history(const std::string &class_name, const std::string &pipe_name,
                                                         const std::string &prop_name);
  //@}
   ...
   *
   * Get the device name from its alias. The device alias is specified by alias
   * and the device name is returned in dev_name. If there is no device defined
   * with the given alias, a DevFailed exception is thrown.
   *
   ...
   * dev_name and the device alias is returned in alias. If there is no alias
   * defined for the device, a DevFailed exception is thrown.
   *
   * @param [in] dev_name The device name
   ...
   * dev_name and the device alias name is returned in dev_alias. If there is no
   ...
   * dev_alias and the device name is returned in dev_name. If there is no
   ...
   * Define device alias
   *
   * Create a device alias. Alias name has to be uniq within a Tango control
   * system and you will receive an exception if the alias is already defined.
   *
   * @param [in] dev_name The device name
   * @param [in] dev_alias The device alias
   *
   ...
   *
   * Get the attribute name from its alias. The attribute alias is specified by
   * alias and the attribute name is returned in att_name. If there is no
   ...
   *
   * Get the attribute alias from its name. The attribute name is specified by
   * att_name and the attribute alias is returned in alias. If there is no alias
   * defined for the attribute, a DevFailed exception is thrown.
   *
   * @param [in] att_name The attribute name
   ...
   * Get attribute name from its alias
   *
   * Get the full attribute name from an alias. The attribute alias is specified
   * by att_alias and the full attribute name is returned in att_name. If there
   * is no attribute with the given alias, a DevFailed exception is thrown.
   *
   ...
   * att_alias and the attribute name is specifed by att_name. If the given
   ...
   *
   * Returns the std::list of all host names registered in the database which
   ...
   *
   * Query the database for a std::list of classes instancied by the specified
   * server. The DServer class exists in all TANGO servers and for this reason
   * this class is removed from the returned std::list.
   * @code
   ...
   *
   * @param [in] ds_name The full device server process name
   * @return The std::list of Tango classes embedded in the specified server
   * process
   *
   * @exception ConnectionFailed,CommunicationFailed,DevFailed from device
   */
   ...
   *
   * Return the std::list of all instance names existing in the database for the
   * specifed server.
   * @code
   ...
   * @return The std::list of all instances for the specified device server
   * process
   *
   * @exception ConnectionFailed,CommunicationFailed,DevFailed from device
   */
   ...
   *
   * Return the std::list of all servers registered in the database which match
   ...
   *
   * @param [in] wildcard The wildcard
   * @return The std::list of device server processes defined in database
   ...
   *
   * Query the database for a std::list of servers registred on the specified
   * host.
   * @code
   ...
   *
   * Query the database for a std::list of devices served by the specified
   ...
   *
   * Query the database for a std::list of exported devices whose names satisfy
   * the supplied filter
   ...
   *
   * Query the database for a std::list of device domain names which match the
   * wildcard provided. Wildcard character * matches any number of characters.
   ...
   *
   * Query the database for a std::list of device family names which match the
   * wildcard provided. Wildcard character * matches any number of characters.
   ...
   *
   * Query the database for a std::list of device member names which match the
   * wildcard provided. Wildcard character * matches any number of characters.
   ...
   *
   * Query the database for a std::list of devices and classes served by the
   * specified server. Return a std::list with the following structure: {device
   ...
   *
   * Query the database for a std::list of object (free properties) for which
   ...
   *
   * Query the database for a std::list of properties defined for the specified
   ...
   *
   * Query the database for a std::list of classes which match the specified
   * wildcard.
   * @code
   ...
   *
   * Query the database for a std::list of attributes defined for the specified
   ...
   *
   * Query the database for a std::list of pipes defined for the specified class
   ...
   *
   * Get device alias std::list. The parameter is a std::string to filter the
   * alias std::list returned. Wildcard (*) is supported. For instance, if the
   * std::string passed as the method parameter is initialised with only the *
   * character, all the defined device alias will be returned. The DbDatum
   * returned by this method is initialised with an array of std::strings and
   * must be extracted into a std::vector<std::string>. If there is no alias
   * with the given filter, the returned array will have a 0 size.
   * @code
   * DbData db_data;
   * std::string filter("*");
   *
   ...
   *
   * Get attribute alias std::list. The parameter is a std::string to filter the
   * alias std::list returned. Wildcard (*) is supported. For instance, if the
   * std::string passed as the method parameter is initialised with only the
   * * character, all the defined attribute alias will be returned. The DbDatum
   * returned by this method is initialised with an array of std::strings and
   * must be extracted into a std::vector<std::string>. If there is no alias
   * with the given filter, the returned array will have a 0 size.
   *
  ...
  void get_device_property_list(std::string &, const std::string &, std::vector<std::string> &,
