...
//
// description :
//		Install a signal handler for SIGINT and SIGTERM but only if nothing is
// already installed
//
//-------------------------------------------------------------------------------------------------------------------
...
void ApiUtil::create_orb()
{
  ...
  //
  // pass dummy arguments to init() because we don't have access to argc and
  // argv
  //
  ...
  //
  // Get user signal handler for SIGPIPE (ORB_init call install a SIG_IGN for
  // SIGPIPE. This could be annoying in case the user uses SIGPIPE)
  //
...
//
// description :
//		Retrieve a Tango database object created from the TANGO_HOST environment
// variable
//
//--------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Try to obtain data returned by a command asynchronously requested. This
// method does not block if the reply is 		not yet arrived. Fire callback for
...
void ApiUtil::get_asynch_replies()
{
  ...
  try
  {
    while(_orb->poll_next_response() == true)
    {
      ...
      //
      // Retrieve this request in the cb request map and mark it as "arrived" in
      // both maps
      //
  ...
  if(asyn_p_table->get_cb_request_nb() != 0)
  {
    ...
    if(call_timeout != 0)
    {
      ...
      while((nb > 0) && (asyn_p_table->get_cb_request_nb() != 0))
      {
        ...
        try
        {
          if(_orb->poll_next_response() == true)
          {
            ...
            //
            // Retrieve this request in the cb request map and mark it as
            // "arrived" in both maps
            //
      ...
      if((nb == 0) && (asyn_p_table->get_cb_request_nb() != 0))
      {
        ...
        desc << "Still some reply(ies) for asynchronous callback call(s) to be "
                "received"
             << std::ends;
    ...
    {
      //
      // If timeout is set to 0, this means wait until all the requests sent to
      // this device has sent their replies
      //
...
//
// description :
//		Set the callback automatic mode (Fired by dedicated call or automatically
// fired by a separate thread)
//
// arg(s) :
//		in :
...
// which is waiting in a CORBA::run() 		indefintely for events. It will then
// trigger the events.
//
//----------------------------------------------------------------------------------------------------------------

void ApiUtil::create_notifd_event_consumer() { notifd_event_consumer = NotifdEventConsumer::create(); }

void ApiUtil::create_zmq_event_consumer() { zmq_event_consumer = ZmqEventConsumer::create(); }

NotifdEventConsumer *ApiUtil::get_notifd_event_consumer() { return notifd_event_consumer; }

ZmqEventConsumer *ApiUtil::get_zmq_event_consumer() { return zmq_event_consumer; }
...
void ApiUtil::attr_to_device(const AttributeValue *attr_value, const AttributeValue_3 *attr_value_3, long vers,
...
//		out :
//			- env_var : Reference to the string initialised with the env. variable
// value (if found)
//
// return :
...
void ApiUtil::get_ip_from_if(std::vector<std::string> &ip_adr_list)
{
  ...
  if(host_ip_adrs.empty() == true)
  {
    ...
    // later. The ifa_addr field points to a structure containing the interface
      ...
      if(ifa->ifa_addr != NULL)
      {
        ...
        if(family == AF_INET)
        {
          ...
          if(s != 0)
          {
            ...
            std::cerr << "ApiUtil::get_ip_from_if: not getting info from "
                         "remaining ifaddrs";
    ...
    if(WSAIoctl(sock, SIO_GET_INTERFACE_LIST, NULL, 0, (LPVOID) &info, sizeof(info), (LPDWORD) &retlen, NULL, NULL) ==
       SOCKET_ERROR)
    {
      closesocket(sock);
      ...
      TangoSys_OMemStream desc;
      desc << "Can't retrieve list of all interfaces addresses (WSAIoctl)! "
              "Error = "
           << err << std::ends;
...
//
// description :
//		Assignement operator for the AttributeInfoEx class from a
// AttributeConfig_2 pointer
//
//----------------------------------------------------------------------------------------------------------------
...
std::ostream &operator<<(std::ostream &o_str, const AttributeInfoEx &p)
{
  ...
  if((p.writable == Tango::WRITE) || (p.writable == Tango::READ_WRITE))
  {
    ...
    case MEMORIZED_WRITE_INIT:
      o_str << "Attribute is memorized and the memorized value is written at "
               "initialisation"
            << std::endl;
      break;
