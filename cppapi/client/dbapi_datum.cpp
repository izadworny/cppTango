...
bool DbDatum::operator>>(bool &datum)
{
  ...
  else
  {
    ...
    if(!istream)
    {
      ...
      {
        TANGO_THROW_API_EXCEPTION(ApiDataExcept, API_IncompatibleArgumentType,
                                  "Cannot extract, data in DbDatum is not a short");
      }
      ret = false;
    }
    else
...
bool DbDatum::operator>>(unsigned char &datum) const
{
  ...
  if(value_string.size() == 0)
  {
    if(exceptions_flags.test(isempty_flag))
    {
      TANGO_THROW_API_EXCEPTION(ApiDataExcept, API_EmptyDbDatum,
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(unsigned short &) - extract a unsigned short from
// DbDatum
//
//-----------------------------------------------------------------------------
...
bool DbDatum::operator>>(DevULong &datum) const
{
  ...
  else
  {
    ...
    if(!istream)
    {
      ...
      {
        TANGO_THROW_API_EXCEPTION(ApiDataExcept, API_IncompatibleArgumentType,
                                  "Cannot extract, data in DbDatum is not a "
  ...
  else
  {
    ...
    if(!istream)
    {
      if(exceptions_flags.test(wrongtype_flag))
      {
        TANGO_THROW_API_EXCEPTION(ApiDataExcept, API_IncompatibleArgumentType,
                                  "Cannot extract, data in DbDatum is not a "
                                  "DevULong64 (unsigned long 64 bits)");
      }
      ret = false;
    }
    else
...
bool DbDatum::operator>>(float &datum) const
{
  ...
  else
  {
    ...
    if(!istream)
    {
      if((TG_strcasecmp("nan", value_string[0].c_str()) == 0) || (TG_strcasecmp("-nan", value_string[0].c_str()) == 0))
      {
      ...
      {
        TANGO_THROW_API_EXCEPTION(ApiDataExcept, API_IncompatibleArgumentType,
                                  "Cannot extract, data in DbDatum is not a float");
      }
      else
      {
        ret = false;
...
bool DbDatum::operator>>(double &datum) const
{
  ...
  else
  {
    ...
    if(!istream)
    {
      ...
      {
        TANGO_THROW_API_EXCEPTION(ApiDataExcept, API_IncompatibleArgumentType,
                                  "Cannot extract, data in DbDatum is not a double");
      }
      else
      {
        ret = false;
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<short> &) - insert a vector<short>
// into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<short> &) - extract a vector<short> from
// DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<unsigned short> &) - insert a
// vector<unsigned short> into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<unsigned short> &) - extract a
// vector<unsigned short> from DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<DevLong> &) - insert a vector<DevLong>
// into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<DevLong> &) - extract a vector<DevLong> from
// DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<DevULong> &) - insert a
// vector<DevULong> into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<DevULong> &) - extract a vector<DevULong>
// from DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<DevLong64> &) - insert a
// vector<DevLong64> into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<DevLong64> &) - extract a vector<DevLong64>
// from DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<DevULong64> &) - insert a
// vector<DevULong64> into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<DevULong64> &) - extract a
// vector<DevULong64> from DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<float> &) - insert a vector<float>
// into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<float> &) - extract a vector<float> from
// DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<double> &) - insert a vector<double>
// into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<double> &) - extract a vector<double> from
// DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator <<(const std::vector<std::string> &) - insert a
// vector<string> into DbDatum
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DbDatum::operator >>(std::vector<std::string> &) - extract a vector<string>
// from DbDatum
//
//-----------------------------------------------------------------------------
