...
struct DataElement
{
   ...
   * Create a DataElement object.
   *
   * Create a DataElement object for device pipe blob extraction. Usefull for
   * extraction into TANGO CORBA sequence. See DevicePipeBlob extraction method
   *
   * @param [in] value The data element value
   */
  DataElement(T value);
...
template <typename T>
DataElement<T>::DataElement(const std::string &_na, T _val)
    : name(_na),
      value(_val)
{
}
...
template <typename T>
DataElement<T>::DataElement(const std::string &_na)
    : name(_na)
{
}

template <typename T>
DataElement<T>::DataElement(T _val)
    : value(_val)
{
}
 ...
 * A device pipe blob
 *
 * A device pipe blob. A blob is used to pack data to be sent through device
 * pipe
 *
 *
 * @headerfile tango.h
...
public:
   ...
   * kinds of insetor methods have been implemented. You can insert data from:
   * @li Scalar data type
   * @li vector
   ...
   *
   * When inserting data using a DataElement<T> instance, the data element name
   * is also set. <B>For insertion from TANGO CORBA sequence type pointer, the
   * insertion method consumes the memory allocated to store the data and it
   * will be freed by the Tango layer.</B>
   *
   ...
   * @li DevVarStateArray &
   *
   * Insert operators for the following CORBA sequence types <B>with memory
   * consumption</B>  (and DataElement<T>):
   * @li DevVarBooleanArray *
   * @li DevVarShortArray *
   * @li DevVarLongArray *
   * @li DevVarLong64Array *
   ...
   * @li DevVarStateArray *
   *
   * Here is an example of inserting data into a DevicePipeBlob instance. We
   * insert 3 data element into the pipe blob with a DevLong, a vector of
   * doubles and finally an array of 100 unsigned short
   * @code
   ...
   *
   * @endcode
   * The same example of inserting data into a DevicePipeBlob instance when we
   * want to set the data element name.
   * @code
   * DevicePipeBlob dpb("MyBlob");
   *
   ...
   *
   * unsigned short *array = new unsigned short [100]; 	// The array is
   * populated by a way or another DevVarUShortArray *dvush =
   * create_DevVarUShortArray(array,100); DataElement<DevVarUShortArray *>
   * de_dvush("ThirdDE",array);
   *
   * try
   * {
   ...
   *
   * Set the blob data element number and names. The data element number is the
   * number of names in the input parameter.
   *
   ...
   *
   * According to the data inside blob data element, several kinds of extractor
   * methods have been implemented. You can extract data into:
   * @li Scalar data type
   * @li std::vector
   ...
   *
   * When extracting data using a DataElement<T> instance, the data element name
   * is also returned. <B>For extraction into C++ vector, data are copied into
   * the vector. It is not the case for extraction into TANGO CORBA sequence
   * type. For extraction into TANGO CORBA sequence types, the extraction method
   * consumes the memory allocated to store the data and it is the caller
   * responsibility to delete this memory.</B>
   *
   ...
   * @li DevVarStateArray *
   *
   * Here is an example of extracting data from a DevicePipeBlob instance. We
   * know that the DevicePipeBlob contains 3 data element with a DevLong, an
   * array of doubles and finally an array of unsigned short
   * @code
   * DevicePipeBlob dpb = .....
   *
   * DevLong dl;
   ...
   * delete dvush;
   * @endcode
   * The same example of extracting data from a DevicePipeBlob instance when we
   * want to retrieve the data element name.
   * @code
   * DevicePipeBlob dpb = .....
   *
   ...
   * }
   *
   * cout << "Data element name = " << de_dl.name << " - Value = " <<
   * de_dl.value << std::endl;
   * ...
   ...
   * }
   * @endcode
   * Note that instead of using DevLong and std::vector<double> data, the
   * extraction can be done using DataElement<T> instances. In this case, the
   * call to the get_data_elt_name() method becomes useless.
   *
   ...
   *when trying to extract data from a DevicePipeBlob object. The following
   *flags are supported :
   * @li @b isempty_flag - throw a WrongData exception (reason =
   *API_EmptyDataElement) if user tries to extract data from one empty blob data
   *element. By default, this flag is set
   ...
   *different than the type used for insertion. By default, this flag is set
   * @li @b notenoughde_flag - throw a WrongData exception (reason =
   *API_PipeWrongArg) if user tries to extract data from a DevicePipeBlob for a
   *data element which does not exist. By default, this flag is set
   * @li @b blobdenamenotset_flag - Throw a WrongData exception (reason =
   *API_PipeNoDataElement) if user tries to insert data into the blob while the
   *name or number of data element has not been set with methods
   *       set_data_elt_nb() or set_data_elt_names()
   * @li @b mixing_flag - Throw a WrongData exception (reason =
   *API_NotSupportedFeature) if user tries to mix insertion/extraction method
   *(<< or >>) with operator[]
   *
   ...
   *
   * Sets one exception flag. See DevicePipeBlob::exceptions() for a usage
   * example.
   *
   * @param [in] fl The exception flag
   */
   ...
   *
   * Allow the user to check if insertion/extraction into/from DevicePipeBlob
   ...
   * into/from DevicePipeBlob failure. This method has to be used when
   * exceptions are disabled. Here is an example of how methods has_failed() and
   * state() could be used
   * @code
   * DevicePipeBlob dpb = ....
   *
  ...
  void print(std::ostream &, int, bool) const;
...
private:
  ...
  DevVarPipeDataEltArray *insert_elt_array; // Ptr for data to be inserted
                                            // (client write/Server read)
  int insert_ctr;                           // Ctr for inserting data elt
  int insert_ind;

  const DevVarPipeDataEltArray *extract_elt_array; // Ptr for data to be extracted (client read/Server
                                                   // write)
...
public:
   ...
   *
   * Create one instance of the DevicePipe class and set its name and its root
   * blob name
   *
   * @param [in] pipe_name The pipe name
...
//@{
   ...
   *
   * Inserting data into a DevicePipe instance is simlar to inserting data into
   * a DevicePipeBlob class instance. See doc of DevicePipeBlob class insertion
   * methods (DevicePipeBlob::operator<<) to get a complete documentation on how
   * to insert data into a DevicePipe
   *
...
//@{
   ...
   *
   * Extracting data from a DevicePipe instance is simlar to extracting data
   * from a DevicePipeBlob class instance. See doc of DevicePipeBlob class
   * extraction methods (DevicePipeBlob::operator>>) to get a complete
   * documentation on how to extract data from a DevicePipe
   *
   ...
   *
   * It's a method which allows the user to switch on/off exception throwing
   *when trying to insert/extract data from a DevicePipe object. The following
   *flags are supported :
   * @li @b isempty_flag - throw a WrongData exception (reason =
   *API_EmptyDataElement) if user tries to extract data from one empty pipe data
   *element. By default, this flag is set
   * @li @b wrongtype_flag - throw a WrongData exception (reason =
   *API_IncompatibleArgumentType) if user tries to extract data with a type
   *different than the type used for insertion. By default, this flag is set
   * @li @b notenoughde_flag - throw a WrongData exception (reason =
   *API_PipeWrongArg) if user tries to extract data from a DevicePipe for a data
   *element which does not exist. By default, this flag is set
   * @li @b blobdenamenotset_flag - Throw a WrongData exception (reason =
   *API_PipeNoDataElement) if user tries to insert data into the blob while the
   *name or number of data element has not been set with methods
   *       set_data_elt_nb() or set_data_elt_names()
   ...
   * Check insertion/extraction success
   *
   * Allow the user to check if insertion/extraction into/from DevicePipe
   * instance was successfull. This method has to be used when exceptions are
   * disabled.
   *
   * @return True if insertion/extraction has failed
   ...
   *
   * Allow the user to find out what was the reason of insertion/extraction
   * into/from DevicePipe failure. This method has to be used when exceptions
   * are disabled. Here is an example of how methods has_failed() and state()
   * could be used
   * @code
   * DevicePipe dpb = ....
   *
   ...
   *
   * Is an utility function to easily print the contents of a DevicePipe object.
   * This function knows all types which could be inserted in a DevicePipe
   * object and print them accordingly. A special string is printed if the
...
template <typename T>
std::ostream &operator<<(std::ostream &, const DataElement<T> &);

template <typename T>
std::ostream &operator<<(std::ostream &, const DataElement<std::vector<T>> &);
