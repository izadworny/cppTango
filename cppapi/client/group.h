...
public:
  ...
  AsynchRequest(long _rid, const std::string &_obj_name, bool ge_enabled = true)
      : rq_id(_rid),
        group_element_enabled_m(ge_enabled)
  {
  ...
  inline bool group_element_enabled() const { return group_element_enabled_m; }
...
public:
  ...
  //- ctor
  GroupReply(const std::string &dev_name, const std::string &obj_name, bool group_element_enabled = true);
  //- ctor
  GroupReply(const std::string &dev_name, const std::string &obj_name, const DevFailed &exception,
   ...
   */
  inline bool has_failed() const { return has_failed_m; }
   ...
   */
  inline const std::string &dev_name() const { return dev_name_m; }
   ...
   */
  inline const std::string &obj_name() const { return obj_name_m; }
   ...
   */
  inline const DevErrorList &get_err_stack() const { return exception_m.errors; }
...
protected:
  ...
  bool has_failed_m;
  //- group_element_enabled_m : true is the associated group member is enabled,
  // false otherwise
  bool group_element_enabled_m;
...
public:
  ...
  GroupCmdReply();
  //-
  GroupCmdReply(const std::string &dev_name, const std::string &obj_name, const DeviceData &data);
  //- ctor
  GroupCmdReply(const std::string &dev_name, const std::string &obj_name, const DevFailed &exception);
  //- ctor
...
public:
  ...
  GroupAttrReply();
  //- ctor
  GroupAttrReply(const std::string &dev_name, const std::string &obj_name, const DeviceAttribute &data);
  //- ctor
  GroupAttrReply(const std::string &dev_name, const std::string &obj_name, const DevFailed &exception);
  //- ctor
...
public:
   ...
   * Check if an error has occurred
   *
   * Returns a boolean set to true if the write_attribute executed on the group
   * has failed for any device member of the hierarchy. Otherwise, returns false
   *
   * @return The error flag
   */
  bool has_failed() const { return has_failed_m; }
...
public:
   ...
   * Check if an error has occurred
   *
   * Returns a boolean set to true if the read_attribute executed on the group
   * has failed for any device member of the hierarchy. Otherwise, returns false
   *
   * @return The error flag
   */
...
{
public:
  //- instanciatethe GroupElement which name matches the specified pattern with
...
private:
  ...
  static DeviceNames resolve_remote_device_names(int db_port, std::string &db_host,
                                                 std::string &name_or_pattern_without_host);
  static DeviceNames add_host_and_port_to_device_names(const DeviceNames &, const std::string &db_host, int db_port);
  static std::string build_full_device_name(const std::string &host, int port, const std::string &device_name);
...
public:
   ...
   *
   * Instanciate an empty group. The group name allows retrieving a sub-group in
   * the hierarchy.
   *
   ...
   * themwhen it is itself deleted. Therefore, never try to delete a Group
   * (respectively a DeviceProxy) returned by a call to @e
   * Tango::Group::get_group() (respectively to @e Tango::Group::get_device()).
   * Use the @e Tango::Group::remove() method instead.
   */
  virtual ~Group();
  //@}
   ...
   * Group attached to a Group. Use the Group::remove() method instead. If
   * timeout_ms parameter is different from -1, the client side timeout
   * associated to each device composing the group added is set to timeout_ms
   * milliseconds. If timeout_ms is -1, timeouts are not changed. This method
   * does nothing if the specified group is already attached (i.e. it is
   * silently ignored) and timeout_ms = -1. If the specified group is already
   * attached and timeout_ms is different from -1, the client side timeout of
   * each device composing the group given in parameter is set to timeout_ms
   * milliseconds.
   *
   ...
   * (e.g. domain_* / family/ member_*). This method first asks to the Tango
   * database the list of device names matching the pattern. Devices are then
   * attached to the group in the order in which they are returned by the
   * database. Any device already present in the hierarchy (i.e. a device
   * belonging to the group or to one of its subgroups) is silently ignored but
   * its client side timeout is set to timeout_ms milliseconds if timeout_ms is
   * different from -1.
   * Set the client side timeout of each device matching the specified pattern
   * to timeout_ms milliseconds if timeout_ms is different from -1.
   *
   ...
   *
   * The patterns parameter can be an array of device names and/or device name
   * patterns. Thismethod first asks to the Tango database the list of device
   * namesmatching one the patterns. Devices are then attached to the group in
   * the order in which they are returned by the database. Any device already
   * present in the hierarchy (i.e. a device belonging to the group or to one of
   * its subgroups), is silently ignored but its client side timeout is set to
   * timeout_ms milliseconds if timeout_ms is different from -1. If timeout_ms
   * is different from -1, the client side timeouts of all devices matching the
   * specified patterns are set to timeout_ms milliseconds.
   *
   ...
   *
   * The pattern parameter can be a group name, a device name or a device name
   pattern (e.g domain_* /family/member_*).
   * Since we can have groups with the same name in the hierarchy, a group name
   can be fully qualified to
   ...
   * gauges->remove("gauges.cell-02.penning") will only remove the specified
   group.
   * If fwd is set to true (the default), the remove request is also forwarded
   to subgroups. Otherwise, it is
   * only applied to the local set of elements. For instance, the following code
   remove any stepper motor in the
   * hierarchy:
   * @code
   * root_group->remove("*/
  stepper_motor /*");
...
#endif
      virtual void
      remove(const std::string &pattern, bool fwd = true);
  /**
   * Removes any group or device which name matches any of the specified
   * patterns.
   *
   * The patterns parameter can be an array of group names and/or device names
   * and/or device name patterns. Since we can have groups with the same name in
   * the hierarchy, a group name can be fully qualified to specify which group
   * should be removed. See previous method for details. If fwd is set to true
   * (the default), the remove request is also forwarded to subgroups.
   ...
   * matches the specified pattern. Returns false otherwise. The pattern can be
   * a fully qualified or simple group name, a device name or a device name
   * pattern. If fwd is set to true (the default), the request is also forwarded
   ...
   * by that name in the group. This method may throw an exception in case the
   * specified device belongs to the group but can’t be reached (not registered,
   * down...). See example below. See also the Tango::DeviceProxy class
   ...
   * named device_name could be found in the local set of devices, the request
   * is forwarded to subgroups). Be aware that a group always gets the ownership
   * of its children and deletes them when it is itself deleted. Therefore,
   * never try to delete a DeviceProxy returned by the
   * Group::get_device()method. Use the Tango::Group::remove() method instead.
   *
   ...
   * hierarchy contains less than "idx" devices. This method may throw an
   ...
   * Tango::DeviceProxy class documentation for details. The request is
   * systematically forwarded to subgroups (i.e. if the local set of devices
   * contains less than "idx" devices, the request is forwarded to subgroups).
   * Be aware that a group always gets the ownership of its children and deletes
   * them when it is itself deleted. Therefore, never try to delete a
   * DeviceProxy returned by the Group::get_device()method. Use the
   ...
   *
   * Returns a reference to the "idx-th" device in the hierarchy or NULL if the
   * hierarchy contains less than "idx" devices. See the Tango::DeviceProxy
   * class documentation for details. The request is systematically forwarded to
   * subgroups (i.e. if the local set of devices contains less than "idx"
   ...
   * itself deleted. Therefore, never try to delete a DeviceProxy returned by
   * the Group::get_device()method. Use the Tango::Group::remove() method
   * instead.
   *
   * @param [in] idx The device name
   * @return device reference
   ...
   *
   * Returns a reference to the specified group or NULL if there is no group by
   that name. The group_name
   ...
   * The request is systematically forwarded to subgroups (i.e. if no group
   named group_name could be
   * found in the local set of elements, the request is forwarded to subgroups).
   * Be aware that a group always gets the ownership of its children and deletes
   them when it is itself
   * deleted. Therefore, never try to delete a Group returned by the
   Group::get_group() method. Use the
   * Tango::Group::remove() method instead.
   *
   ...
   * device in the group) if the forward option is set to true (respectively set
   * to false)
   *
   * @param [in] fwd The forward flag
   * @return group size
   ...
   *
   * Returns the list of devices currently in the hierarchy.
   * If fwd is set to true (the default) the request is forwarded to subgroups.
   * Otherwise, it is only applied to the local set of devices. Considering the
   * following hierarchy:
   * @code
   ...
   * g1->add("my/device/02");
   * @endcode
   * The returned vector content depends on the value of the forward option. If
   * set to true, the results will be organized as follows:
   * @code
   ...
   *
   * Enables group element. The element will participate in all group
   * operations.
   *
   * @param [in] device_name name of the element, can contain wildcards (*).
   * If more than one device matches the pattern, only the first one will be
   * enabled.
   * @param [in] fwd flag to perform recursive search for the element in all
   * sub-groups
   */
   ...
   *
   * Disables group element. The element will be excluded from all group
   * operations.
   *
   * @param [in] device_name name of the element, can contain wildcards (*)
   * If more than one device matches the pattern, only the first one will be
   ...
   *
   * Ping all devices in a group. This method returns true if all devices in the
   * group are alive, false otherwise. If fwd is set to true (the default), the
   * request is also forwarded to subgroups. Otherwise, it is only applied to
   ...
   * milliseconds. Any method which takes longer than this time to execute will
   * throw an exception.
   *
   * @param [in] tmo_ms The timeout value
   */
   ...
   *
   * Executes a Tango command on a group. This method is synchronous and does
   * not return until replies are obtained or timeouts occurred. The parameter c
   * is the name of the command. If fwd is set to true (the default), the
   * request is also forwarded to subgroups. Otherwise, it is only applied to
   * the local set of devices. Command results are returned in a
   * GroupCmdReplyList. See Obtaining command result for details
   * (Chapter 4.7.3.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>). See also Case 1 of executing a command
   * (Chapter 4.7.3.2 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   ...
   *
   * Executes a Tango command on each device in the group. This method is
   * synchronous and does not return until replies are obtained or timeouts
   * occurred. The parameter c is the name of the command. The second parameter
   * d is a Tango generic container for command carrying the command argument.
   ...
   * Command results are returned in a GroupCmdReplyList. See Obtaining command
   * results (Chapter 4.7.3.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details. See also Case 2 of executing a
   * command (Chapter 4.7.3.4 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   * @param [in] c The command name
  ...
  /**
   * Executes a Tango command with the different input data on a group (Using
   * DeviceData class instances)
   *
   * Executes a Tango command on each device in the group. This method is
   * synchronous and does not return until replies are obtained or timeouts
   * occurred. This implementation of command_inout allows passing a specific
   * input argument to each device in the group. In order to use this form of
   * command_inout, the user must have an "a priori" and "perfect" knowledge of
   * the devices order in the group.
   * @n The parameter c is the name of the command.
   * The std::vector d contains a specific argument value for each device in the
   * group. Its size must equal Group::get_size(fwd). Otherwise, an exception is
   * thrown. The order of the argument values must follows the order of the
   * devices in the group (d[0] => 1st device, d[1] => 2nd device and so on). If
   * fwd is set to true (the default), the request is also forwarded to
   * subgroups. Otherwise, it is only applied to the local set of devices.
   * Command results are returned in a GroupCmdReplyList. See Obtaining command
   * results (Chpater 4.7.3.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details. See also Case 3 of executing a
   * command (Chapter 4.7.3.5 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example of this special form of
   * command_inout.
   *
   * @param [in] c The command name
   * @param [in] d The command data
   ...
   * @n The parameter c is the name of the command.
   * The std::vector d contains a specific argument value for each device in the
   * group. Since this method is a template, d is able to contain any Tango
   * command argument type. Its size must equal Group::get_size(fwd). Otherwise,
   * an exception is thrown. The order of the argument values must follows the
   * order of the devices in the group (d[0] => 1st device, d[1] => 2nd device
   * and so on). If fwd is set to true (the default), the request is also
   * forwarded to subgroups. Otherwise, it is only applied to the local set of
   * devices. Command results are returned in a GroupCmdReplyList. See Obtaining
   * command results (Chpater 4.7.3.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details. See also Case 3 of executing a
   * command (Chapter 4.7.3.5 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   ...
   *
   * Executes a Tango command on each device in the group asynchronously. The
   * method sends the request to all devices and returns immediately. Pass the
   * returned request id to Group::command_inout_reply() to obtain the results.
   * The parameter c is the name of the command.
   * The parameter fgt is a fire and forget flag. If set to true, it means that
   * no reply is expected (i.e. the caller does not care about it and will not
   * even try to get it). A false default value is provided. If the parameter
   * fwd is set to true (the default) request is forwarded to subgroups.
   * Otherwise, it is only applied to the local set of devices. See Case 1 of
   * Executing a command (Chapter 4.7.3.2 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   * @param [in] c The command name
  ...
  /**
   * Executes a Tango command with same input data on each device in the group
   * asynchronously.
   *
   * Executes a Tango command on each device in the group asynchronously. The
   * method sends the request to all devices and returns immediately. Pass the
   * returned request id to Group::command_inout_reply() to obtain the results.
   * The parameter c is the name of the command.
   * The second parameter d is a Tango generic container for command carrying
   * the command argument. See the Tango::DeviceData documentation for details.
   * The parameter fgt is a fire and forget flag. If set to true, it means that
   * no reply is expected (i.e. the caller does not care about it and will not
   * even try to get it). A false default value is provided. If the parameter
   * fwd is set to true (the default) request is forwarded to subgroups.
   * Otherwise, it is only applied to the local set of devices. See Case 2 of
   * Executing a command (Chapter 4.7.3.4 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   * @param [in] c The command name
  ...
  /**
   * Executes a Tango command with different input data (using DeviceData
   * object) on each device in the group asynchronously.
   *
   * Executes a Tango command on each device in the group asynchronously. The
   * method send the request to all devices and return immediately. Pass the
   * returned request id to Group::command_inout_reply to obtain the results.
   * This implementation of command_inout allows passing a specific input
   * argument to each device in the group. In order to use this form of
   * command_inout_asynch, the user must have an "a priori" and "perfect"
   ...
   * The parameter fgt is a fire and forget flag. If set to true, it means that
   * no reply is expected (i.e. the caller does not care about it and will not
   * even try to get it). A false default value is provided. If fwd is set to
   * true (the default), the request is also forwarded to subgroups. Otherwise,
   * it is only applied to the local set of devices. See Case 3 of Executing a
   * command (Chapter 4.7.3.5 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example of this special form of
   * command_inout.
  ...
  /**
   * Executes a Tango command with different input data on each device in the
   ...
   * The parameter c is the name of the command.
   * The std::vector d contains a specific argument value for each device in the
   * group. Since it's a template data type, d is able to contain any Tango
   * command argument type. Its size must equal Group::get_size(fwd). Otherwise,
   * an exception is thrown. The order of the argument values must follows the
   * order of the devices in the group (d[0] => 1st device, d[1] => 2nd device
   * and so on). The parameter fgt is a fire and forget flag. If set to true, it
   * means that no reply is expected (i.e. the caller does not care about it and
   * will not even try to get it). A false default value is provided. If fwd is
   ...
   * Executing a command (Chapter 4.7.3.5 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example of this special form of
   * command_inout.
   *
   ...
   *
   * Returns the results of an asynchronous command.
   * The first parameter req_id is a request identifier previously returned by
   * one of the command_inout_asynch methods. For each device in the hierarchy,
   * if the command result is not yet available, command_inout_replywait
   * timeout_ms milliseconds before throwing an exception. This exception will
   * be part of the global reply. If timeout_ms is set to 0, command_inout_reply
   * waits "indefinitely". Command results are returned in a GroupCmdReplyList.
   * See Obtaining command results (Chapter 4.7.3.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details.
   *
   ...
   * and does not return until replies are obtained or timeouts occurred. The
   * parameter a is the name of the attribute to read. If fwd is set to true
   * (the default) request is forwarded to subgroups. Otherwise, it is only
   * applied to the local set of devices. Attribute values are returned in a
   * GroupAttrReplyList. See Obtaining attribute values (Chapter 4.7.4.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details. See also Reading an attribute
   * (Chapter 4.7.4 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   ...
   * synchronous and does not return until replies are obtained or timeouts
   * occurred. The parameter al is the list of attributes to be read. If fwd is
   * set to true (the default) request is forwarded to subgroups. Otherwise, it
   * is only applied to the local set of devices. Attribute values are returned
   * in a GroupAttrReplyList. See Obtaining attribute values (Chapter 4.7.4.1 in
   * <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details. See also Reading an attribute
   * (Chapter 4.7.4 in <a
   ...
   *
   * Reads an attribute on each device in the group asynchronously. The method
   * sends the request to all devices and returns immediately. Pass the returned
   * request id to Group::read_attribute_reply() to obtain the results. The
   * parameter a is the name of the attribute to read. If fwd is set to true
   * (the default) request is forwarded to subgroups. Otherwise, it is only
   * applied to the local set of devices. The last parameter (rsv) is reserved
   * for internal purpose and should not be modify. It may disappear in a near
   * future. See Reading an attribute (Chapter 4.7.4 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   * @param [in] a The attribute name
   ...
   * method sends the request to all devices and returns immediately. Pass the
   * returned request id to Group::read_attribute_reply() to obtain the results.
   * The parameter a is the name of the attribute to read.
   * If fwd is set to true (the default) request is forwarded to subgroups.
   * Otherwise, it is only applied to the local set of devices. The last
   * parameter (rsv) is reserved for internal purpose and should not be modify.
   * It may disappear in a near future. See Reading an attribute (Chapter 4.7.4
   * in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   ...
   * Returns the results of an asynchronous attribute reading.
   * The first parameter req_id is a request identifier previously returned by
   * read_attribute_asynch. For each device in the hierarchy, if the attribute
   * value is not yet available, read_attribute_reply wait timeout_ms
   * milliseconds before throwing an exception. This exception will be part of
   * the global reply. If timeout_ms is set to 0, read_attribute_reply waits
   * "indefinitely". Replies are returned in a GroupAttrReplyList. See Obtaining
   * attribute values (Chapter 4.7.4.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details
   *
   ...
   * occurred. The first parameter d is a Tango generic container for attribute
   * carrying both the attribute name and the value. See the
   * Tango::DeviceAttribute documentation for details. If fwd is set to true
   * (the default) request is forwarded to subgroups. Otherwise, it is only
   * applied to the local set of devices. Acknowledgements are returned in a
   * GroupReplyList. See Obtaining acknowledgements (Chapter 4.7.5.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details. See also Case 1 of Writing an
   * attribute (Chapter 4.7.5.2 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   ...
   * timeouts occurred. The first parameter d is a vector of Tango generic
   * container for attribute carrying both the attribute name and the value. See
   * the Tango::DeviceAttribute documentation for details. If fwd is set to true
   * (the default) request is forwarded to subgroups. Otherwise, it is only
   * applied to the local set of devices. Acknowledgements are returned in a
   * GroupReplyList. See Obtaining acknowledgements (Chapter 4.7.5.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details. See also Case 2 of Writing an
   * attribute (Chapter 4.7.5.3 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
  ...
  /**
   * Writes one attributes on each device in the group with specific value per
   * device
   *
   * Writes an attribute on each device in the group. This method is synchronous
   * and does not return until replies are obtained or timeouts occurred. This
   * implementation of write_attribute allows writing a specific value to each
   * device in the group. In order to use this form of write_attribute, the user
   * must have an "a priori" and "perfect" knowledge of the devices order in the
   * group. The parameter a is the name of the attribute. The std::vector d
   * contains a specific value for each device in the group. Since this method
   * is a template, d is able to contain any Tango attribute type. Its size must
   * equal Group::get_size(fwd). Otherwise, an exception is thrown. The order of
   * the attribute values must follows the order of the devices in the group
   * (d[0] => 1st device, d[1] => 2nd device and so on).
   * If fwd is set to true (the default) request is forwarded to subgroups.
   * Otherwise, it is only applied to the local set of devices. Acknowledgements
   * are returned in a GroupReplyList. See Obtaining acknowledgements
   * (Chapter 4.7.5.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details. See also Case 2 of Writing an
   * attribute (Chapter 4.7.5.3 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   ...
   *
   * Write an attribute on each device in  the group asynchronously. The method
   * sends the request to all devices and returns immediately. Pass the returned
   * request id to Group::write_attribute_reply() to obtain the
   * acknowledgements. The first parameter d is a Tango generic container for
   * attribute carrying both the attribute name and the value. See the
   * Tango::DeviceAttribute documentation for details. If fwd is set to true
   * (the default) request is forwarded to subgroups. Otherwise, it is only
   * applied to the local set of devices. See Case 1 of Writing an attribute
   * (Chapter 4.7.5.2 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   * @param [in] d The attribute name and value
   ...
   * method sends the request to all devices and returns immediately. Pass the
   * returned request id to Group::write_attribute_reply() to obtain the
   * acknowledgements. The first parameter d is a vector of Tango generic
   * container for attribute carrying both the attribute name and the value. See
   * the Tango::DeviceAttribute documentation for details. If fwd is set to true
   * (the default) request is forwarded to subgroups. Otherwise, it is only
   * applied to the local set of devices. See Case 1 of Writing an attribute
   ...
   *
   * Writes an attribute on each device in the group asynchronously. The method
   * sends the request to all devices and returns immediately. Pass the returned
   * request id to Group::write_attribute_reply() to obtain the
   * acknowledgements. This implementation of write_attribute_asynch allows
   * writing a specific value to each device in the group. In order to use this
   * form of write_attribute_asynch, the user must have an "a priori" and
   * "perfect" knowledge of the devices order in the group. The parameter a is
   * the name of the attribute. The std::vector d contains a specific value for
   * each device in the group. Since this method is a template, d is able to
   * contain any Tango attribute type. Its size must equal Group::get_size(fwd).
   * Otherwise, an exception is thrown. The order of the attribute values must
   * follows the order of the devices in the group (d[0] => 1st device, d[1] =>
   * 2nd device and so on). If fwd is set to true (the default) request is
   * forwarded to subgroups. Otherwise, it is only applied to the local set of
   * devices. See Case2 of Writing an attribute (Chapter 4.7.5.3 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for an example.
   *
   * @tparam T The attribute data type
   ...
   * Returns the acknowledgements of an asynchronous attribute writing.
   * The first parameter req_id is a request identifier previously returned by
   * one of the write_attribute_asynch implementation. For each device in the
   * hierarchy, if the acknowledgement is not yet available,
   * write_attribute_replywait timeout_ms milliseconds before throwing an
   * exception. This exception will be part of the global reply. If timeout_ms
   * is set to 0, write_attribute_reply waits "indefinitely". Acknowledgements
   * are returned in a GroupReplyList. See Obtaining acknowledgements
   * (Chapter 4.7.5.1 in <a
   * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
   * target=new>Tango book</a>) for details.
...
private:
  ...
  virtual long command_inout_asynch_i(const std::string &c, const std::vector<DeviceData> &d, bool fgt, bool fwd,
                                      long ari);
  template <typename T>
  ...
  template <typename T>
  long write_attribute_asynch_i(const std::string &a,
...
private:
  ...
  //- ctor: creates a GroupDeviceElement named <name> with timeout set to tmo_ms
  // milliseconds
  ...
  inline DeviceProxy *dev_proxy() { return dp ? dp : connect(); }
...
//=============================================================================
template <typename T>
long Group::command_inout_asynch_i(const std::string &c,
                                   /*const*/ std::vector<T> &d, bool fgt, bool fwd, long ari)
{
  ...
  if(gsize != static_cast<long>(d.size()))
  {
    ...
    desc << "the size of the input argument list must equal the number of "
            "device in the group"
...
//=============================================================================
template <typename T>
long Group::write_attribute_asynch_i(const std::string &a,
                                     /*const*/ std::vector<T> &d, bool fwd, long ari)
{
#ifdef TANGO_GROUP_HAS_THREAD_SAFE_IMPL
  omni_mutex_lock guard(elements_mutex);
