...
// file :               LockThread.cpp
//
// description :        C++ source code for the LockThread class. This class is
// used for the locking thread. The rule of
//						this thread is to regulary send the device locking command to a DS
// admin
// device
//
// project :            TANGO
...
LockThread::LockThread(LockThCmd &cmd, TangoMonitor &m, DeviceProxy *adm, const std::string &dev,
                       LockClock::duration per)
    : shared_cmd(cmd),
      p_mon(m),
      admin_proxy(adm)
{
...
void LockThread::one_more_lock()
{
  ...
  try
  {
    //		cout << "Locking Thread: I am re-locking devices (" <<
    // re_lock_cmd_args.size() << ")" << std::endl;
    admin_proxy->command_inout("ReLockDevices", d_in);
  }
  catch(Tango::DevFailed &e)
  {
    //		cout << "LockThread: Gasp, an exception while sending command to admin
    // device" << std::endl;

    //
    // If we had a locking error, retrieve device name and remove it from the
    // locked devices list
    //
...
//
// description :
//		This method manage the thread update period. The update period is the
// smallest lock validity of all the 		locked device minus 0.5 sec
//
//-------------------------------------------------------------------------------------------------------------------
