 ...
 * operators << and >> respectively and insert() for mixed data types. There are
 * two ways to check if the extraction operator succeed : <ul> <li> 1. By
 * testing the extractor operators return value. All the extractors operator
 * returns a boolean value set to false in case of problem. <li> 2. By asking
 * the DeviceAttribute object to throw exception in case of problem. By default,
 * DeviceAttribute throws exception : <ol> <li> When the user try to extract
 * data and the server reported an error when the attribute was read. <li> When
 * the user try to extract data from an empty DeviceAttribute
 *    </ol>
 * </ul>
 *
 * <B>For insertion into DeviceAttribute instance from TANGO CORBA sequence
 * pointers, the DeviceAttribute object takes ownership of the pointed to
 * memory. This means that the pointed to memory will be freed when the
 * DeviceAttribute object is destroyed or when another data is inserted into it.
 * The insertion into DeviceAttribute instance from TANGO CORBA sequence
 * reference copy the data into the DeviceAttribute object.\n For extraction
 * into TANGO CORBA sequence types, the extraction method consumes the memory
 * allocated to store the data and it is the caller responsibility to delete
 * this memory.</B>
 *
 * $Author: taurel $
 * $Revision: 1 $
...
public:
  ...
  DeviceAttribute();
  /**
   * Create a DeviceAttribute object from attribute name and value for scalar
   * attribute
   *
   * Create a DeviceAttribute object from attribute name and value for @b scalar
   * attribute. These constructors exists for the following data type:
  ...
  /**
   * Create a DeviceAttribute object from attribute name and value for spectrum
   * attribute
   *
   * Create a DeviceAttribute object from attribute name and value for @b
   * spectrum attribute. These constructors exists for the following data type:
  ...
  /**
   * Create a DeviceAttribute object from attribute name and value for image
   * attribute
   *
   * Create a DeviceAttribute object from attribute name and value for @b image
   * attribute. These constructors have two more parameters allowing the user to
   * define the x and y image dimensions. These constructors exists for the
   * following data type:
   * @li DeviceAttribute(const std::string &, const std::vector<bool> &, int,
   * int);
   * @li DeviceAttribute(const std::string &, const std::vector<short> &, int,
   * int);
   * @li DeviceAttribute(const std::string &, const std::vector<DevLong> &, int,
   * int);
   * @li DeviceAttribute(const std::string &, const std::vector<DevLong64>&,
   * int, int);
   * @li DeviceAttribute(const std::string &, const std::vector<float> &, int,
   * int);
   * @li DeviceAttribute(const std::string &, const std::vector<double> &, int,
   * int);
   * @li DeviceAttribute(const std::string &, const std::vector<unsigned char>
   * &, int, int);
   * @li DeviceAttribute(const std::string &, const std::vector<unsigned short>
   ...
   * @li DeviceAttribute(const char *, const std::vector<unsigned char> &, int,
   * int);
   * @li DeviceAttribute(const char *, const std::vector<unsigned short> &, int,
   * int);
   * @li DeviceAttribute(const char *, const std::vector<DevULong> &, int, int);
   * @li DeviceAttribute(const char *, const std::vector<DevULong64>&, int,
   * int);
   * @li DeviceAttribute(const char *, const std::vector<std::string> & , int,
   * int);
   ...
   * and the user application. Nevertheless, C++ vector types are not the CORBA
   * native type and one copy is unavoidable when using vectors. Using the
   * native TANGO CORBA sequence types in most cases avoid any copy but needs
   * some more care about memory usage.
   * @li <B>For insertion into DeviceAttribute instance from TANGO CORBA
   * sequence pointers, the DeviceAttribute object takes ownership of the
   * pointed to memory. This means that the pointed to memory will be freed when
   * the DeviceAttribute object is destroyed or when another data is inserted
   * into it.</B>
   * @li <B>The insertion into DeviceAttribute instance from TANGO CORBA
   * sequence reference copy the data into the DeviceAttribute object.</B>
   *
   ...
   * @li const char *
   *
   * Insert operators for the following C++ vector types for @b spectrum
   * attributes :
   * @li std::vector<bool>
   * @li std::vector<short>
   ...
   *
   * Insert operators for @b spectrum attribute and for the following types by
   * pointer (with memory ownership transfert)
   * :
   ...
   *
   * my_string << std::string("estas lista a bailar el tango ?"); // insert a
   * string my_string >> a_string; // extract a string
   *
   ...
   * @li <B> insert(const std::string &str, std::vector<unsigned char>
   * &data);</B>
   * @li <B> insert(const char *str, DevVarCharArray *data);</B>
   *
   * These three methods do not take ownership of the memory used for the data
   * buffer.
   * @n See DeviceAttribute::operator<< for example of inserting and extracting
   * data to/from DeviceAttribute instance
   *
   ...
   * Insert attribute data for image attribute (from CORBA sequence by
   reference)
   *
   * Insert methods for @b image attribute and for the following types by
   reference.
   ...
   *
   * Insert methods for @b image attribute and pointers. The DeviceAttribute
   ...
   * Nevertheless, C++ vector types are not the CORBA native type and one copy
   is unavoidable when using
   * vectors. Using the native TANGO CORBA sequence types in most cases avoid
   any copy but needs some
   * more care about memory usage.
   * @li <B>For extraction into TANGO CORBA sequence types, the extraction
   method consumes the
   * memory allocated to store the data and it is the caller responsibility to
   delete this memory.</B>
   *
   ...
   * @li DevEncoded
   *
   * Extract operators for the following C++ vector types for @b spectrum and @b
   image attributes :
   * @li std::vector<bool>
   * @li std::vector<short>
   * @li std::vector<DevLong>
   ...
   * @li std::vector<DevState>
   *
   * Extract operators for the following CORBA sequence types <B>with memory
   consumption</B> :
   * @li DevVarBooleanArray *
   * @li DevVarShortArray *
   * @li DevVarLongArray *
   ...
   * the read value can be read by using the methods get_dim_x() and get_dim_y()
   or get_r_dimension().
   ...
   * @li bool DeviceAttribute::extract_read(std::string&, std::vector<unsigned
   char> &);
   ...
   * of the set value can be read by using the methods get_written_dim_x() and
   get_written_dim_y()
   * or get_w_dimension(). The methods use the same return values as the
   ...
   * @li bool DeviceAttribute::extract_set(std::string &, std::vector<unsigned
   char> &);
  ...
  void insert(char *&, unsigned char *&,
              unsigned int); // Deprecated. For compatibility purpose
  //	void insert(const char *str,unsigned char *data,unsigned int length);
  void insert(const std::string &, std::vector<unsigned char> &);
  void insert(std::string &,
              std::vector<unsigned char> &); // Deprecated. For compatibility purpose
  ...
  bool extract(char *&, unsigned char *&,
               unsigned int &); // Deprecated. For compatibility purpose
   ...
   * trying to extract data from a DeviceAttribute object. The following flags
   * are supported :
   * @li @b isempty_flag - throw a WrongData exception (reason =
   * API_EmptyDbDatum) if user tries to extract data from an empty
   * DeviceAttribute object. By default, this flag is set
   * @li @b wrongtype_flag - throw a WrongData exception (reason =
   * API_IncompatibleArgumentType) if user tries to extract data with a type
   * different than the type used for insertion. By default, this flag is not
   * set
   * @li @b failed_flag - throw an exception when the user try to extract data
   * from the DeviceAttribute object and an error was reported by the server
   * when the user try to read the attribute. The type of the exception thrown
   * is the type of the error reported by the server. By default, this flag is
   * set.
   * @li @b unknown_format_flag - throw an exception when the user try to get
   * the attribute data format from the DeviceAttribute object when this
   * information is not yet available. This information is available only after
   * the read_attribute call has been sucessfully executed. The type of the
   * exception thrown is WrongData exception (reason =
   * API_EmptyDeviceAttribute). By default, this flag is not set.
   *
   ...
   */
  std::bitset<numFlags> exceptions() const { return exceptions_flags; }
   ...
   *     ....
   * @endcode
   * There is another usage example in the DeviceAttribute::exceptions() method
   * documentation.
   * @param [in] fl The exception flag
   */
   ...
   * DeviceAttribute failure. This method has to be used when exceptions are
   * disabled. Here is an example of how method state() could be used
   * @code
   * DeviceAttribute da = ....
   *
   ...
   * The following is an example of the three available ways to get data out of
   * a DeviceAttribute object
   * @code
   * DeviceAttribute da;
   ...
   * when the server reports an error when the attribute was read. In the second
   * way, the DeviceAttribute object now does not throw "DevFailed" exception
   * any more and the return value of the extractor operator is checked. IN the
   * last case, the attribute data validity is checked before trying to extract
   * them.
   *
   ...
   * whether the DeviceAttribute object contains data or not. Note that by
   * default, a DeviceAttribute object throws exception if it is empty (See
   * DeviceAttribute::exceptions() method). If you want to use this method, you
   * have to change this default behavior. It can be used to test whether the
   * DeviceAttribute has been initialized or not e.g.
   * @code
   * std::string parity;
   ...
   *    cout << "Last written value " << j+1 << " = " << attr_data[j + read] <<
   * std::endl;
   * @endcode
   *
   * @return The read value number
   ...
   *
   * Returns the attribute data format. Note that this information is valid only
   * after the call to the device has been executed. Otherwise the FMT_UNKNOWN
   * value of the AttrDataFormat enumeration is returned or an exception is
   * thrown according to the object exception flags.
   *
