...
void Connection::connect(const std::string &corba_name)
{
  ...
  while(retry == true)
  {
    try
    {
      ...
      // First, try as a Device_5, then as a Device_4, then as .... and finally
      // as a Device
      //
      // But we have want to know if the connection to the device is OK or not.
      // The _narrow() call does not necessary generates a remote call. It all
      // depends on the object IDL type stored in the IOR. If in the IOR, the
      // IDL is the same than the one on which the narrow is done (Device_5 on
      // both side for instance), then the _narrow call will not generate any
      // remote call and therefore, we don know if the connection is OK or NOT.
      // This is the reason of the _non_existent() call. In case the IDl in the
      // IOR and in the narrow() call are different, then the _narrow() call try
      // to execute a remote _is_a() call and therefore tries to connect to the
      // device. IN this case, the _non_existent() call is useless. But because
      // we don want to analyse the IOR ourself, we always call _non_existent()
      ...
      // Set a flag if the object is running on a host with several net
      // addresses. This is used during re-connection algo.
      //
...
void Connection::reconnect(bool db_used)
{
  ...
  if(connection_state != CONNECTION_OK)
  {
    ...
    if(prev_failed_t0.has_value() && (now - *prev_failed_t0) < RECONNECTION_DELAY)
    {
      ...
      desc << "The last connection request was done less than " << std::chrono::milliseconds(RECONNECTION_DELAY).count()
           << " ms ago" << std::ends;
  ...
  try
  {
    ...
    if(connection_state == CONNECTION_OK)
    {
      try
      {
        ...
        // class. Doing it now, will break compatibility (one more virtual
        // method)
        //
...
//-----------------------------------------------------------------------------
//
// Connection::get_timeout_millis() - public method to get timeout on a TANGO
// device
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Connection::set_timeout_millis() - public method to set timeout on a TANGO
// device
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// Connection::command_inout() - public method to execute a command on a TANGO
// device
//
//-----------------------------------------------------------------------------
        ...
        if(db->is_command_allowed(d_name, command) == false)
        {
          ...
          else
          {
            desc << "Command " << command << " on device " << dev_name()
                 << " is not authorized because an error occurs while talking "
                    "to the Controlled Access Service"
                 << std::ends;
...
void DeviceProxy::parse_name(const std::string &full_name)
{
  ...
  if(dbase_used == false)
  {
    ...
    //
    // Check device name syntax (domain/family/member). Alias are forbidden
  ...
  else
  {
    ...
    else
    {
      ...
      {
        //
        // There is at least one / in dev name but it is not a TANGO_HOST
        // definition. A correct dev name must have 2 /. Check this. An alias
        // cannot have any /
        //
...
//-----------------------------------------------------------------------------
//
// DeviceProxy::get_command_config() - return the command info for a set of
// commands
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DeviceProxy::command_list_query() - return the list of commands implemented
// for this TANGO device
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DeviceProxy::get_command_list() - return the list of commands implemented for
// this TANGO device (only names)
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// DeviceProxy::get_property_list() - get a list of property names from the
// database
//
//-----------------------------------------------------------------------------
...
PipeInfoList *DeviceProxy::get_pipe_config(const std::vector<std::string> &pipe_string_list)
{
  ...
  if(version > 0 && version < 5)
  {
    ...
    ss << "Device " << device_name
       << " too old to use get_pipe_config() call. Please upgrade to Tango "
          "9/IDL5";
...
void DeviceProxy::set_pipe_config(const PipeInfoList &dev_pipe_list)
{
  ...
  if(version > 0 && version < 5)
  {
    std::stringstream ss;
    ss << "Device " << device_name
       << " too old to use set_pipe_config() call. Please upgrade to Tango "
          "9/IDL5";
...
DevicePipe DeviceProxy::read_pipe(const std::string &pipe_name)
{
  ...
  // For thw data elt sequence, we create a new one with size and buffer from
  // the original one. This is required because the whole object received by the
  // call will be deleted at the end of this method
  //
...
DevicePipe DeviceProxy::write_read_pipe(DevicePipe &pipe_data)
{
  ...
  if(version > 0 && version < 5)
  {
    std::stringstream ss;
    ss << "Device " << device_name
       << " too old to use write_read_pipe() call. Please upgrade to Tango "
          "9/IDL5";
...
void DeviceProxy::read_attribute(const std::string &attr_str, AttributeValue_4 *&av_4)
{
  ...
  if(version < 4)
  {
    std::stringstream ss;
    ss << "Device " << dev_name()
       << " is too old to support this call. Please, update to IDL 4 (Tango "
          "7.x or more)";
...
void DeviceProxy::read_attribute(const std::string &attr_str, AttributeValue_5 *&av_5)
{
  ...
  if(version < 5)
  {
    std::stringstream ss;
    ss << "Device " << dev_name()
       << " is too old to support this call. Please, update to IDL 5 (Tango "
          "9.x or more)";
    TANGO_THROW_EXCEPTION(API_NotSupported, ss.str());
  }
...
void DeviceProxy::write_attributes(const std::vector<DeviceAttribute> &attr_list)
{
  ...
  while(ctr < 2)
  {
    ...
    {
      throw Tango::NamedDevFailedList(e, device_name, (const char *) "DeviceProxy::write_attributes",
...
//-----------------------------------------------------------------------------
//
// DeviceProxy::command_history() - get command history (only for polled
// command)
//
//-----------------------------------------------------------------------------
...
//
// description:
//		Create a connection to the admin device of the Tango device server process
...
//
// description :
//		Subscribe to an event- Adds the statless flag for stateless event
// subscription.
//
//-------------------------------------------------------------------------------------------------------------------
                                 ...
                                 const std::vector<std::string> &filters, bool stateless)
{
  ...
  //
  // First, try using zmq. If it fails with the error "Command Not Found", try
  // using notifd
  //
...
//
// description :
//		Subscribe to an event with the usage of the event queue for data
// reception. Adds the statless flag for 		stateless event subscription.
//
//-----------------------------------------------------------------------------------------------------------------

int DeviceProxy::subscribe_event(const std::string &attr_name, EventType event, int event_queue_size,
                                 const std::vector<std::string> &filters, bool stateless)
{
  ApiUtil *api_ptr = ApiUtil::instance();
  if(api_ptr->get_zmq_event_consumer() == NULL)
  ...
  try
  {
    ret = api_ptr->get_zmq_event_consumer()->subscribe_event(this, attr_name, event, event_queue_size, filters,
                                                             stateless);
  }
  catch(DevFailed &e)
  {
    ...
    if(reason == API_CommandNotFound)
    {
      ...
      ret = api_ptr->get_notifd_event_consumer()->subscribe_event(this, attr_name, event, event_queue_size, filters,
                                                                  stateless);
    }
    else
    {
...
//
// description :
//		Subscribe to a device event- Add the statless flag for stateless event
// subscription.
//
//-------------------------------------------------------------------------------------------------------------------
...
int DeviceProxy::subscribe_event(EventType event, CallBack *callback, bool stateless)
{
  if(version < MIN_IDL_DEV_INTR)
  {
    ...
    ss << "Available since Tango release 9 AND for device inheriting from IDL "
          "release 5 (Device_5Impl)";
...
void DeviceProxy::get_events(int event_id, DevIntrChangeEventDataList &event_list)
{
  ...
  {
    std::stringstream desc;
    desc << "Event Device Interface Change not implemented in old Tango event "
            "system (notifd)";
  ...
  if((local_lock_ctr == 0) || (force == true))
  {
...

    {
      ...
      if(pos == au->lock_threads.end())
      {
        ...
        //				o << "Can't find the locking thread for device " <<
        // device_name << " and admin device " << adm_dev_name << ends;
...
bool DeviceProxy::is_locked_by_me()
{
  ...
  else
  {
    ...
    else
    {
      ...
      //
      // If the call is local, as the PID is already the good one, the caller is
      // the locker
      //
      ...
      {
        //
        // Get the host address(es) and check if it is the same than the one
        // sent by the server
        //
...
int DeviceProxy::get_tango_lib_version()
{
  ...
  {
    //
    // IDL 3 is for Tango 5 and 6. Unfortunately, there is no way from the
    // client side to determmine if it is Tango 5 or 6. The beast we can do is
    // to get the info that it is Tango 5.2 (or above)
    //
