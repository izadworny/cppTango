...
DevicePipe::DevicePipe(const std::string &pipe_name)
    : name(pipe_name),
      ext(nullptr)
{
}

DevicePipe::DevicePipe(const std::string &pipe_name, const std::string &root_blob_name)
    : name(pipe_name),
      ext(nullptr)
{
...
DevicePipe::DevicePipe(const DevicePipe &source)
    : ext(nullptr)
{
  name          = source.name;
  time          = source.time;
...
DevicePipe::DevicePipe(DevicePipe &&source)
    : ext(nullptr)
{
...
DevicePipeBlob::DevicePipeBlob(const std::string &blob_name)
    : name(blob_name),
      failed(false),
      insert_elt_array(nullptr),
      insert_ctr(0),
...
//----------------------------------------------------------------------------------------------------------------
//
// DevicePipeBlob::~DevicePipeBlob() - destructor to destroy DevicePipeBlob
// instance
//
//----------------------------------------------------------------------------------------------------------------
...
DevicePipeBlob::DevicePipeBlob(const DevicePipeBlob &source)
    : ext(nullptr)
{
  name             = source.name;
...
//-----------------------------------------------------------------------------
//
// DevicePipeBlob::DevicePipeBlob() - Move copy constructor to create
// DevicePipeBlob
//
//-----------------------------------------------------------------------------

DevicePipeBlob::DevicePipeBlob(DevicePipeBlob &&source)
    : ext(nullptr)
{
  name             = std::move(source.name);
  exceptions_flags = source.exceptions_flags;
...
std::vector<std::string> DevicePipeBlob::get_data_elt_names()
{
  if(extract_elt_array == nullptr)
  {
    ...
    ss << "You try to get data element name(s) for a blob which has never been "
...
//
// description :
//		Get the index of a data element in the extract blob from its name. Throw
...
//
// description :
//		Return the object itself with the extract counter set to the ind of the
// data element with name given as 		parameter. This allows user to write code
// like 			pipe_blob["MyDataElt"] >> sh;
//
// argument :
//		in:
//			- _na : Data element name
...
//
// description :
//		Set the data element name. This method create the pipe data elt array
  ...
  else
  {
    ...
    else
    {
      ...
      if(tmp_ptr != nullptr)
      {
        ...
        if(insert_ind != -1)
        {
          (*insert_elt_array)[insert_ind].inner_blob.replace(max, len, tmp_ptr->get_buffer((CORBA::Boolean) true),
                                                             true);
        ...
        {
          (*insert_elt_array)[insert_ctr].inner_blob.replace(max, len, tmp_ptr->get_buffer((CORBA::Boolean) true),
                                                             true);
...
//
// description :
//		Throw exception when user try to insert/extract too many data from the
// blob
//
// argument:
//		in :
//			- _meth : Caller method name
//			- _extract : Flag set to true if this method is called during
// extraction. Otherwise, false
//
//-------------------------------------------------------------------------------------------------------------------
...
//
// description :
//		Throw exception when user try to insert/extract data from one empty data
// element
//
// argument:
//		in :
...
//
// description :
//		Throw exception when user try to insert data into a device blob while the
// DE name/nb not set
//
// argument:
//		in :
...
//
// description :
//		Throw exception when user try to mix [] and >> (<<) insertion/extraction
// method
//
// argument:
//		in :
...
void DevicePipeBlob::throw_mixing(const std::string &_meth)
{
  ...
  std::string m_name("DevicePipeBlob::");
  m_name = m_name + _meth;
  TANGO_THROW_EXCEPTION(API_NotSupportedFeature, "Not supported to mix extraction type (operator >> (or "
                                                 "<<) and operator [])");
}
...
//
// description :
//		Print blob content. Due to recusrsive type, it was not possible to write
// this feature as a classical
//		<< operator.
//
// argument:
//		in :
...
//			- insert_extract: Flag set to true is insert data should be printed,
// else extract data are
// printed
//
//-------------------------------------------------------------------------------------------------------------------
...
void DevicePipeBlob::print(std::ostream &o_str, int indent, bool insert_extract) const
{
  ...
  if(dvpdea != nullptr)
  {
    for(size_t ctr = 0; ctr < dvpdea->length(); ctr++)
    {
      ...
      case DEVICE_STATE:
        o_str << "Unsupported data type in data element (ATT_NO_DATA, "
                 "DEVICE_STATE)!"
              << std::endl;
        break;
