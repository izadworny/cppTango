 ...
 * commands. The values can be inserted and extracted using the operators << and
 * >> respectively and insert() for mixed data types. A status flag indicates if
 * there is data in the DbDatum object or not. An additional flag allows the
 * user to activate exceptions.
 *
 * <B> Insertion by pointers takes full ownership of the pointed to memory. The
 * insertion copy the data in the DeviceData object and delete the pointed to
 * memory. Therefore, the memory is not more usable after the insertion.\n When
 * using extraction by pointers, the pointed to memory is inside the DeviceData
 * object and its lifetime is the same than the DeviceData object lifetime.</B>
 *
 * $Author: taurel $
 * $Revision: 1 $
 *
...
public:
   ...
   *
   * Operators exist for inserting the native TANGO CORBA sequence types. These
   * can be useful for programmers who want to use the TANGO api internally in
   * their device servers and do not want to convert from CORBA to C++ types.
   ...
   *
   * <B> Insertion by pointers takes full ownership of the pointed to memory.
   * The insertion copy the data in the DeviceData object and delete the pointed
   * to memory. Therefore, the memory is not more usable after the
   * insertion.</B>
   *
   ...
   * native TANGO CORBA sequence types avoid any copy. When using these TANGO
   * CORBA sequence types, insertion into the DeviceData object consumes the
   * memory pointed to by the pointer. After the insertion, it is not necessary
   * to delete the memory. It will be done by the destruction of the DeviceData
   * object. For extraction, the pointer used for the extraction points into
   * memory inside the DeviceData object and you should not delete it Here is an
   * example of creating, inserting and extracting some data type from/into
   * DeviceData object :
   * @code
   ...
   * @li <B>void insert(const char *str, unsigned char *data,unsigned int
   * length);</B>
   *
   ...
   * @li std::vector<double>
   *
   * Operators exist for extracting the native TANGO CORBA sequence types. These
   * can be useful for programmers who want to use the TANGO api internally in
   * their device servers and do not want to convert from CORBA to C++ types.
   * Insert and extract operators exist for the following types :
   ...
   *
   * <B>Note that when using extraction by pointers, the pointed to memory is
   * inside the DeviceData object and its lifetime is the same than the
   * DeviceData object lifetime.</B>
   *
   * Special care has been taken to avoid memory copy between the network layer
   * and the user application. Nevertheless, C++ vector types are not the CORBA
   * native type and one copy is unavoidable when using vectors. Using the
   * native TANGO CORBA sequence types avoid any copy. When using these TANGO
   * CORBA sequence types, for extraction, the pointer used for the extraction
   * points into memory inside the DeviceData object and you should not delete
   * it See DeviceData::operator<< for inserters and extractors usage example
   *
   ...
   * when trying to extract data from a DeviceData object. The following flags
   * are supported :
   * @li @b isempty_flag - throw a Empty DeviceData exception (reason =
   * API_EmptyDeviceData) if user tries to extract data from an empty DeviceData
   ...
   * DeviceData failure. This method has to be used when exceptions are
   ...
   * whether the DeviceData object contains data or not. It can be used to test
   * whether the DeviceData has been initialized or not but you have first to
   * disable exception throwing in case of empty object e.g.
   * @code
   ...
   * This function knows all types which could be inserted in a DeviceData
   * object and print them accordingly. A special string is printed if the
   * DeviceData object is empty
   * @code
