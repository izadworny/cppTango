...
void AttributeProxy::parse_name(std::string &full_name)
{
  ...
  if(dbase_used == false)
  {
    ...
    if(pos == std::string::npos)
    {
      TANGO_THROW_API_EXCEPTION(ApiWrongNameExcept, API_WrongAttributeNameSyntax,
                                "Host and port not correctly defined in device name");
    }
  ...
  do
  {
    ...
    if(pos != std::string::npos)
    {
      ...
      {
        TANGO_THROW_API_EXCEPTION(ApiWrongNameExcept, API_WrongAttributeNameSyntax,
                                  "Attribute name must have four fields separated by /'s or no /'s "
                                  "at all if it is an "
  ...
  {
    TANGO_THROW_API_EXCEPTION(ApiWrongNameExcept, API_WrongAttributeNameSyntax,
                              "Attribute name must have four fields separated "
                              "by /'s or no /'s at all if it is an "
                              "alias (e.g. my/device/name/an_attr or myalias)");
  }
  ...
  if(n_sep == 0)
  {
    ...
    {
      TANGO_THROW_API_EXCEPTION(ApiWrongNameExcept, API_WrongAttributeNameSyntax,
                                "Wrong alias name (: not allowed in alias name)");
    }
    ...
    //
    // Get full attribute name from database but connect to database first if it
    // is not done already
    //
    ...
    {
      TANGO_THROW_API_EXCEPTION(ApiWrongNameExcept, API_WrongAttributeNameSyntax,
                                "Attribute name must have four fields separated by /'s (check the "
                                "alias entry in the database) ");
    }
  ...
  //
  // attribute name has four fields, separate them into device and attribute
  // names but keep attr_name as a case sentitive name
  //
...
//-----------------------------------------------------------------------------
//
// AttributeProxy::~AttributeProxy() - destructor to destroy proxy to TANGO
// device
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//
// AttributeProxy::ping() - ping TANGO device and return time elapsed in
// microseconds
//
//-----------------------------------------------------------------------------

int AttributeProxy::ping() { return (dev_proxy->ping()); }
...
DevState AttributeProxy::state() { return (dev_proxy->state()); }
...
std::string AttributeProxy::status() { return (dev_proxy->status()); }
...
void AttributeProxy::set_transparency_reconnection(bool val) { dev_proxy->set_transparency_reconnection(val); }
...
bool AttributeProxy::get_transparency_reconnection() { return dev_proxy->get_transparency_reconnection(); }
...
AttributeInfoEx AttributeProxy::get_config() { return (dev_proxy->get_attribute_config(attr_name)); }
...
DeviceAttribute AttributeProxy::read() { return (dev_proxy->read_attribute(attr_name)); }
...
int AttributeProxy::get_poll_period() { return (dev_proxy->get_attribute_poll_period(attr_name)); }
...
void AttributeProxy::poll(int period) { dev_proxy->poll_attribute(attr_name, period); }
...
bool AttributeProxy::is_polled() { return (dev_proxy->is_attribute_polled(attr_name)); }
...
void AttributeProxy::stop_poll() { dev_proxy->stop_poll_attribute(attr_name); }
                                    ...
                                    bool stateless)
{
  ...
  try
  {
    // we use filters here to pass user defined attribute name to later use it
    // in event callback see https://github.com/tango-controls/cppTango/pull/423
    std::vector<std::string> non_const_filters;
    non_const_filters.push_back(get_user_defined_name());
    ret = api_ptr->get_zmq_event_consumer()->subscribe_event(dev_proxy, attr_name, event, callback, non_const_filters,
                                                             stateless);
  }
  catch(DevFailed &e)
  {
    ...
    if(reason == API_CommandNotFound)
    {
      ...
      ret = api_ptr->get_notifd_event_consumer()->subscribe_event(dev_proxy, attr_name, event, callback, filters,
                                                                  stateless);
    }
    else
      throw;
...
int AttributeProxy::subscribe_event(EventType event, int event_queue_size, const std::vector<std::string> &filters,
                                    bool stateless)
{
  ApiUtil *api_ptr = ApiUtil::instance();
  if(api_ptr->get_zmq_event_consumer() == NULL)
  ...
  try
  {
    ret = api_ptr->get_zmq_event_consumer()->subscribe_event(dev_proxy, attr_name, event, event_queue_size, filters,
                                                             stateless);
  }
  catch(DevFailed &e)
  {
    ...
    if(reason == API_CommandNotFound)
    {
      ...
      ret = api_ptr->get_notifd_event_consumer()->subscribe_event(dev_proxy, attr_name, event, event_queue_size,
                                                                  filters, stateless);
    }
    else
      throw;
