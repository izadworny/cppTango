...
typedef enum _AttrMemorizedType
{
  ...
  MEMORIZED_WRITE_INIT ///< Memorized attribute with memorized value writen at
                       ///< initialization
} AttrMemorizedType;
...
// TODO: Pipe -> Change the type for writable to replace PIPE_READ,
// PIPE_READ_WRITE by READ, READ_WRITE (IDL limitation)
 ...
 * buffers. This class inherits from the Tango::DeviceData class. One instance
 * of this class is created for each command result history. Within this class,
 * you find the command result data or the exception parameters, a flag
 * indicating if the command has failed when it was invoked by the device server
 * polling thread and the date when the command was executed. For history calls,
 * it is not possible to returns command error as exception. See chapter on
 * Advanced Features in the <a
 * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
 * target=new>Tango book</a> for all details regarding device polling.
 *
 *
 * @headerfile tango.h
 * @ingroup Client
...
public:
   ...
   * DeviceDataHistory object and print them accordingly. It also prints date
 ...
 * buffers. This class inherits from the Tango::DeviceAttribute class. One
 * instance of this class is created for each attribute result history. Within
 * this class, you find the attribute result data or the exception parameters
 * and a flag indicating if the attribute has failed when it was invoked by the
 * device server polling thread. For history calls, it is not possible to
 * returns attribute error as exception. See chapter on Advanced Features in the
 * <a
 * href=http://www.esrf.eu/computing/cs/tango/tango_doc/kernel_doc/ds_prog/index.html
 * target=new>Tango book</a> for all details regarding device polling.
 *
...
public:
   ...
   * Is an utility function to easily print the contents of a
   DeviceAttributeHistory object. This function knows
   * all types which could be inserted in a DeviceAttributeHistory object and
   print them accordingly. It also
...
inline void Connection::add_asyn_cb_request(CORBA::Request_ptr req, CallBack *cb, Connection *con,
                                            TgRequest::ReqType req_type)
{
  omni_mutex_lock guard(asyn_mutex);
...
inline int DeviceProxy::subscribe_event(const std::string &attr_name, EventType event, int event_queue_size,
                                        bool stateless)
{
  std::vector<std::string> filt;
...
#define READ_ATT_EXCEPT(NAME_CHAR, OBJ)                                                                        \
  ...
  catch(CORBA::TRANSIENT & trans) { TRANSIENT_NOT_EXIST_EXCEPT(trans, "DeviceProxy", "read_attribute", OBJ); } \
