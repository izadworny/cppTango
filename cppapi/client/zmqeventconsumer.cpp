...
//
// description : 		C++ classes for implementing the event consumer singleton
// class when used with zmq
//
// author(s) : 			E.Taurel
//
...
void *ZmqEventConsumer::run_undetached(TANGO_UNUSED(void *arg))
{
  ...
  //
  // Create the subscriber socket used to receive heartbeats coming from
  // different DS. This socket subscribe to everything because dedicated
  // publishers are used to send the heartbeat events. This socket will be
  // connected to all needed publishers
  //
  ...
  //
  // Create the subscriber socket used to receive events coming from different
  // DS. This socket subscribe to everything because dedicated publishers are
  // used to send the heartbeat events. This socket will be connected to all
  // needed publishers
  //
  ...
  while(1)
  {
    ...
    //
    // Wait for message. The try/catch is usefull when the process is running
    // under gdb control
    //
    ...
    if(items[1].revents & ZMQ_POLLIN)
    {
      ...
      try
      {
        ...
        if(!res)
        {
          print_error_message("First Zmq recv call on heartbeat socket "
                              "returned false! De-synchronized event system?");
          items[1].revents = 0;
          continue;
        }
        ...
        if(!res)
        {
          print_error_message("Second Zmq recv call on heartbeat socket "
                              "returned false! De-synchronized event system?");
          items[1].revents = 0;
          continue;
        }
        ...
        if(!res)
        {
          print_error_message("Third Zmq recv call on heartbeat socket "
                              "returned false! De-synchronized event system?");
          items[1].revents = 0;
          continue;
        }
    ...
    if(items[2].revents & ZMQ_POLLIN)
    {
      try
      {
        ...
        if(!res)
        {
          print_error_message("First Zmq recv call on event socket returned "
                              "false! De-synchronized event system?");
          items[2].revents = 0;
          continue;
        }
        ...
        if(!res)
        {
          print_error_message("Second Zmq recv call on event socket returned "
                              "false! De-synchronized event system?");
          items[2].revents = 0;
          continue;
        }
        ...
        if(!res)
        {
          print_error_message("Third Zmq recv call on event socket returned "
                              "false! De-synchronized event system?");
          items[2].revents = 0;
          continue;
        }
        ...
        if(!res)
        {
          print_error_message("Forth Zmq recv call on event socket returned "
                              "false! De-synchronized event system?");
          items[2].revents = 0;
          continue;
        }
...
void ZmqEventConsumer::process_heartbeat(zmq::message_t &received_event_name, zmq::message_t &received_endian,
...
void ZmqEventConsumer::process_event(zmq::message_t &received_event_name, zmq::message_t &received_endian,
                                     zmq::message_t &received_call, zmq::message_t &event_data)
{
  // cout << "event name message adr = " << (void *)(&received_event_name) << "
  // - size = " << received_event_name.size()
  // << " - ptr = " << (void *)(received_event_name.data()) << std::endl; cout
  // << "endian message adr = " << (void
  // *)(&received_endian) << " - size = " << received_endian.size() << " - ptr =
  // " << (void *)(received_endian.data())
  // << std::endl; cout << "call info message adr = " << (void
  // *)(&received_call) << " - size = " << received_call.size() << " - ptr = "
  // << (void *)(received_call.data()) << std::endl; cout << "event data message
  // adr = " << (void
  // *)(&event_data) << " - size = " << event_data.size() << " - ptr = " <<
  // (void *)(event_data.data()) << std::endl;
...
// ZMQ_END command has been received).
...
bool ZmqEventConsumer::process_ctrl(zmq::message_t &received_ctrl, zmq::pollitem_t *poll_list, int &poll_nb)
{
  ...
  case ZMQ_CONNECT_HEARTBEAT:
  {
    ...
    //
    // Most of the time, we have only one TANGO_HOST to take into account and we
    // dont need to execute following code. But there are some control system
    // where several TANGO_HOST are defined
    //
  ...
  case ZMQ_DISCONNECT_HEARTBEAT:
  {
    ...
    //
    // Most of the time, we have only one TANGO_HOST to take into account and we
    // don need to execute following code. But there are some control system
    // where several TANGO_HOST are defined
    //
    ...
    //
    // Remove the endpoint in the vector of already connected heartbeat and
    // disconnect the socket to this endpoint
    //
    ...
    //
    // Remove the event endpoint from the already connected event and disconnect
    // the event socket
    //
  ...
  case ZMQ_CONNECT_EVENT:
  {
    ...
    //
    // Most of the time, we have only one TANGO_HOST to take into account and we
    // don't need to execute following code. But there are some control system
    // where several TANGO_HOST are defined!
    //
  ...
  case ZMQ_DISCONNECT_EVENT:
  {
    ...
    if(mcast == false)
    {
      ...
      //
      // Most of the time, we have only one TANGO_HOST to take into account and
      // we don need to execute following code. But there are some control
      // system where several TANGO_HOST are defined
      //
  ...
  {
    // If poll_nb == 1, then we are already in a situation where events are
    // being delayed and we are currently only taking care of messages received
    // on the control socket No need to update old_poll_nb in this case because
    // it is already correct otherwise this would lead to issues like
    // https://github.com/tango-controls/cppTango/issues/686 where events would
    // no longer be received if someone subscribes or unsubscribes to events in
    // an event callback and when the callback is executed during a
    // subscribe_event call
    if(poll_nb != 1)
    {
      old_poll_nb = poll_nb;
      poll_nb     = 1;
  ...
  case ZMQ_RELEASE_EVENT:
  {
    ...
    {
      // Stop delaying events only if there is no other ZMQ_DELAY_EVENT command
      // requested
      poll_nb = old_poll_nb;
    }
  }
  ...
  default:
    print_error_message("ZMQ main thread: Received an unknown command code "
                        "from control socket!");
    break;
  }
...
//
// description :
//		Method to execute a ZMQ socket command (actually only SUBSCRIBE or
// UNSUBSCRIBE) when several TANGO_HOST
...
void ZmqEventConsumer::cleanup_EventChannel_map()
{
  ...
  //
  // Create and connect the REQ socket used to send message to the ZMQ main
  // thread
  //
  ...
  try
  {
    ...
    //
    // Build message sent to ZMQ main thread. In this case, this is only a
    // command code
    //
...
//			- db : Database object
//			- reconnect: Flag set to true in case this method is called for event
...
void ZmqEventConsumer::connect_event_channel(const std::string &channel_name, TANGO_UNUSED(Database *db),
                                             bool reconnect, DeviceData &dd)
{
  //
  ...
  //
  // Do we have this tango host info in the vector of possible TANGO_HOST. If
  // not get them
  //
  ...
  //
  // If the server has returned several possible ZMQ endpoints (because several
  // NIC boards on server host), check which one is correct
  //
  ...
  {
    // ZmqEventSubscriptionChange returns the ZMQ message filters used for the
    // attribute and for the heartbeat events at the end of the string array
    // part of the returned DevVarLongStringArray
    nb_endpoints -= 2;
  }
  ...
  if(nb_endpoints != 1)
  {
    ...
    if(valid_endpoint == nb_endpoints)
    {
      ...
      o << "Impossible to create a network connection to any of the event "
           "endpoints returned by server";
  ...
  try
  {
    ...
    //
    // In case this thread runs before the main ZMQ thread, it is possible to
    // call connect before the main ZMQ thread has binded its socket. In such a
    // case, error code is set to ECONNREFUSED. If this happens, give the main
    // ZMQ thread a chance to run and retry the connect call I have tried with a
    // yield call but it still failed in some cases (when running the DS with a
    // file as database  for instance). Replace the yield with a 10 mS sleep !!!
    //
    ...
    // In this case, this is the command code, the publisher endpoint and the
    // event name
    //
  ...
  if(reply.size() != 2)
  {
    ...
    o << "Failed to create connection to event channel!\n";
    o << "Error while trying to connect or subscribe the heartbeat ZMQ socket "
  ...
  else
  {
    ...
    // set the timeout for the channel monitor to 1000ms not to block the event
    // consumer for to long.
...
//
// description :
//		Disconnect to the event channel. This means that the process should not
// receive the heartbeat event for
// this 		channel. It will be filtered out by ZMQ
//
// argument :
//		in :
//			- channel name : The event channel name (DS admin name)
...
void ZmqEventConsumer::disconnect_event_channel(const std::string &channel_name, const std::string &endpoint,
                                                const std::string &endpoint_event)
{
  ...
  if(reply.size() != 2)
  {
    ...
    o << "Error while trying to unsubscribe the heartbeat ZMQ socket from the "
         "channel heartbeat publisher\n";
    o << "ZMQ message: " << err_mess << std::ends;
...
//
// description :
//		Disconnect to the event. This means that the process should not receive
// the event any more 		It will be filtered out by ZMQ
//
// argument :
//		in :
...
//			- evt_it : Iterator pointing to the event channel entry in channel_map
// map
//			- new_event_callback : Structure used for the event callback entry in
// the event_callback_map
//			- dd : The data returned by the DS admin device xxxSubscriptionChange
// command
//          - valid_end : The valid endpoint in the list of endpoint returned by
//          ZMQEventSubscriptionChange command
//
//--------------------------------------------------------------------------------------------------------------------
...
void ZmqEventConsumer::connect_event_system(TANGO_UNUSED(const std::string &device_name),
                                            ...
                                            EventCallBackStruct &new_event_callback, DeviceData &dd, size_t valid_end)
{
  //
  // Extract server command result
  //
  ...
  try
  {
    ...
    //
    // Build message sent to ZMQ main thread
    // In this case, this is the command code, the publisher endpoint, the event
    // name and the sub hwm
    //
  ...
  if(reply.size() != 2)
  {
    ...
    o << "Failed to create connection to event!\n";
    o << "Error while trying to connect or subscribe the event ZMQ socket to "
         "the new publisher\n";
    o << "ZMQ message: " << err_mess << std::ends;
...
//
// description :
//		Method called when the heartbeat event is received. This method retrieve
// the channel entry in the
// channel_map 		and update the last heartbeat date.
//
// argument :
//		in :
...
void ZmqEventConsumer::push_heartbeat_event(std::string &ev_name)
{
  ...
  if(ipos != channel_map.end())
  {
    ...
    {
      std::string st("Tango::ZmqEventConsumer::push_heartbeat_event() timeout "
                     "on channel monitor of ");
      st = st + ipos->first;
      print_error_message(st.c_str());
    }
  }
      ...
      for(loop = 0; loop < env_var_fqdn_prefix.size(); loop++)
      {
        ...
        else
        {
          ...
          if(ipos != channel_map.end())
          {
            ...
            {
              std::string st("Tango::ZmqEventConsumer::push_heartbeat_event() "
                             "timeout on channel monitor of ");
              st = st + ipos->first;
              print_error_message(st.c_str());
            }
            break;
...
//
// description :
//		Method called when the event is received. This method retrieve the channel
// entry in the channel_map 		and update the last heartbeat date.
//
// argument :
//		in :
//			- ev_name : The fully qualifed event name
...
void ZmqEventConsumer::push_zmq_event(std::string &ev_name, unsigned char endian, zmq::message_t &event_data,
                                      bool error, const DevULong &ds_ctr)
{
  map_modification_lock.readerIn();
  bool map_lock = true;
  ...
  //
  // Search for entry within the event_callback map using the event name
  ...
  {
    //
    // Test different fully qualified event name depending on different
    // TANGO_HOST defined for the control system
    //
    ...
    if(ipos != event_callback_map.end())
    {
      ...
      //
      // Miss some events?
      // Due to LIBZMQ Bug 283, the first event after a process startup is sent
      // two times with the same ctr value. Do not call the user callback for
      // the second times.
      //
      ...
      //
      // If the client TANGO_HOST is one alias, replace in the event name the
      // host name by the alias
      //
      ...
      else
      {
        ...
        // memory address. ZMQ returned memory which is sometimes aligned on a 8
        // bytes boundary but not always (seems to depend on the host
        // architecture) The attribute data transfert starts with the union
        // discriminator (4 bytes), the elt nb (4 bytes) and the element
        // themselves. This means 8 bytes before the real data. There is a trick
        // here. The buffer is always transferred with an extra 4 bytes added at
        // the beginning If the alignememnt is not correct (buffer aligned on a
        // 8 bytes boundary and 64 bits data type), shift the whole buffer by 4
        // bytes erasing the additional 4 bytes sent.
        //
        ...
        if(data_type == PIPE && data64 == true && buffer_aligned64 == false)
        {
          ...
          {
            omniORB::logger log;
            log << "ZMQ: Pipe event -> Shifting received buffer to be aligned "
                   "on a 8 bytes boundary"
                << '\n';
          }
          char *src  = data_ptr + 8;
        ...
        else if(data_type != PIPE && data64 == true && buffer_aligned64 == true && shift_zmq420 == false)
        {
          ...
          {
            omniORB::logger log;
            log << "ZMQ: Classical event -> Shifting received buffer to be "
          ...
          case ATT_VALUE:
            if(evt_cb.device_idl >= 5)
            {
              ...
              try
              {
                ...
                //
                // Update name in DeviceAttribute in case it is not coherent
                // with name received in first ZMQ message part. This happens in
                // case of forwarded attribute but also in case of DS started
            ...
            else if(evt_cb.device_idl == 3)
            {
              ...
              {
                TangoSys_OMemStream o;
                o << "Received malformed data for event (AttributeValue_3 -> "
                     "Device_3Impl....) ";
                o << ev_name << std::ends;
            ...
            else if(evt_cb.device_idl < 3)
            {
              ...
              {
                TangoSys_OMemStream o;
                o << "Received malformed data for event (AttributeValue -> "
                     "Device_2Impl....) ";
                o << ev_name << std::ends;
      ...
      try
      {
        ...
        //
        // In case we have missed some event, prepare structure to send to
        // callback to inform user of this bad behavior
        //
        ...
        if(err_missed_event == true)
        {
          ...
          if((ev_attr_conf == false) && (ev_attr_ready == false) && (ev_dev_intr == false) && (pipe_event == false))
            ...
            missed_conf_event_data = new FwdAttrConfEventData(device, full_att_name, event_name, NULL, missed_errors);
            ...
            missed_dev_pipe_data = new PipeEventData(device, full_att_name, event_name, NULL, missed_errors);
          else
            missed_dev_intr_event_data =
                new DevIntrChangeEventData(device, event_name, full_att_name, (CommandInfoList *) NULL,
                                           (AttributeInfoListEx *) NULL, false, missed_errors);
        }
        ...
        for(esspos = evt_cb.callback_list.begin(); esspos != evt_cb.callback_list.end(); ++esspos)
        {
          ...
          if(esspos->id > 0)
          {
            ...
            if((ev_attr_conf == false) && (ev_attr_ready == false) && (ev_dev_intr == false) && (pipe_event == false))
            {
              FwdEventData *event_dat = newFwdEventData(event_data, esspos->device, errors, event_name, full_att_name,
                                                        vers, dev_attr, no_unmarshalling, cb_nb, cb_ctr, callback);
              //
              ...
              //
              // No calback method, the event has to be inserted into the event
              // queue
              //
            ...
            else if(ev_attr_ready == false && ev_dev_intr == false && pipe_event == false)
            {
              ...
              if(cb_ctr != cb_nb)
              {
                ...
                *attr_info_copy                 = *attr_info_ex;
                event_data_ =
                    new FwdAttrConfEventData(esspos->device, full_att_name, event_name, attr_info_copy, errors);
                if(attr_conf_5 != NULL)
                  event_data_->set_fwd_attr_conf(attr_conf_5);
              }
              else
              {
                event_data_ = new FwdAttrConfEventData(esspos->device, full_att_name, event_name, attr_info_ex, errors);
                if(attr_conf_5 != NULL)
                  event_data_->set_fwd_attr_conf(attr_conf_5);
              }
            ...
            else if(ev_attr_ready == false && pipe_event == false)
            {
              DevIntrChangeEventData *event_data_ =
                  new DevIntrChangeEventData(esspos->device, event_name, full_att_name, &dev_intr_change->cmds,
            ...
            else if(ev_attr_ready == false)
            {
              ...
              if(cb_ctr != cb_nb)
              {
                ...
                *dev_pipe_copy            = *dev_pipe;
                event_data_ = new PipeEventData(esspos->device, full_att_name, event_name, dev_pipe_copy, errors);
              }
              else
              {
                event_data_ = new PipeEventData(esspos->device, full_att_name, event_name, dev_pipe, errors);
              }
            ...
            else
            {
              DataReadyEventData *event_data_ =
                  new DataReadyEventData(esspos->device, const_cast<AttDataReady *>(att_ready), event_name, errors);
              // if a callback method was specified, call it!
              if(callback != NULL)
              {
                try
      ...
      catch(DevFailed &e)
      {
        ...
        {
          std::string st("Tango::ZmqEventConsumer::push_zmq_event() timeout on "
                         "callback monitor of ");
          st = st + ipos->first;
          print_error_message(st.c_str());
        }
...
FwdEventData *ZmqEventConsumer::newFwdEventData(zmq::message_t &event_data, DeviceProxy *device, DevErrorList &errors,
                                                std::string &event_name, std::string &full_att_name, long vers,
                                                const DeviceAttribute *dev_attr, bool no_unmarshalling,
                                                unsigned int cb_nb, unsigned int cb_ctr, const CallBack *callback) const
{
  //
  // In case we have several callbacks on the same event or if the event has to
  // be stored in a queue, copy the event data (Event data are in the ZMQ
  // message)
  //
  ...
  else
  {
    ...
    else
    {
      ...
      {
        return new FwdEventData(device, actual_full_att_name, event_name, const_cast<DeviceAttribute *>(dev_attr),
                                errors);
      }
    }
  }
...
//
// description :
//		Do some ZMQ specific tasks like checking release compatibility or lower
// case the admin device name 		which is used in the heartbeat event name.
//
//		Even if the current Tango  version does not work with zmq 3.x we need to
// keep these checks as the 		other tango version across the network might
// still be using an old zmq version.
//
// argument :
//		in :
...
void ZmqEventConsumer::zmq_specific(DeviceData &dd, std::string &adm_name, DeviceProxy *device,
                                    const std::string &obj_name)
{
  ...
  //
  // For event coming from server still using Tango 8.0.x, do not lowercase the
  // adm_name
  //
  ...
  //
  // Check for ZMQ compatible release. Impossible to check if server does not
  // send which ZMQ release it is using.
  //
...
//
// description :
//		Check if the endpoint returned by the ZMQEventSubscriptionChange DS admin
// device command are valid on
// the
//      client side.
//
// argument :
...
//
// return :
//      A boolean set to true if it is possible to establish a connection with
//      this endpoint. Otherwise, returns false
//
//--------------------------------------------------------------------------------------------------------------------
...
bool ZmqEventConsumer::check_zmq_endpoint(const std::string &endpoint)
{
  ...
  if(result == SOCKET_ERROR)
  {
    ...
    if(err_code == WSAEWOULDBLOCK)
    {
      ...
      //
      // Because socket is in non-blocking mode, call select to get connection
      // status
      //
                                                                   ...
                                                                   bool device_from_env_var)
{
  ...
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotSupported,
                              "Server did not send its tango lib version. The "
                              "server is possibly too old. The event "
  ...
  else
  {
    ...
    {
      // For event coming from server still using Tango 8.0.x or below, do not
      // lowercase the adm_name in the channel name
  ...
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotSupported,
                              "Server did not send the event name. The server is possibly too old. "
  ...
  {
    TANGO_THROW_API_EXCEPTION(EventSystemExcept, API_NotSupported,
                              "Server did not send the channel name. The server is possibly too old. "
                              "The event system is not initialized!");
  }
...
void ZmqEventConsumer::disconnect_socket(zmq::socket_t &socket, const char *endpoint)
{
  ...
  {
    // Silently ignore ENOENT as it indicates that endpoint is already
    // disconnected.
    if(e.num() != ENOENT)
    {
      throw;
...
//
// description :
//		Write our own unmarshalling method. The omniORB one allocate memory and
// copy data. We already have
// memory 		allocated in the ZMQ message. No need to allocate once more and to
// copy data. We are doing this only for 		attribute data. For the remaining,
// keep using omniORB stuff
//
// argument :
//		in :
//			- n :
...
void ZmqAttrValUnion::operator<<=(TangoCdrMemoryStream &_n)
{
  ...
  // let omniORB do its stuff. Don't forget to rewind memory ptr before
  // returning to omniORB
  //
...
//
// description :
//		A class to ask the ZMQ main thread to stop receiving external event. This
// is necessary to prevent a
// possible
//      deadlock which could happen if an event is received while a user is
//      calling subscribe or unsubscribe event
//
// argument :
//		in :
      ...
      eve_con(NULL)
{
  ...
  if(str[0] == 'Z')
  {
    ...
    try
    {
      ...
      //
      // In case this thread runs before the main ZMQ thread, it is possible to
      // call connect before the main ZMQ thread has binded its socket. In such
      // a case, error code is set to ECONNREFUSED. If this happens, give the
      // main ZMQ thread a chance to run and retry the connect call. I have
      // tried with a yield call but it still failed in some cases (when running
      // the DS with a file as database  for instance). Replace the yield with a
      // 15 mS sleep !!!
      //
      // Since ZMQ 4, itś possible to connect to the remote socket event if it
      // is not yet bound but the remote socket will hang in the its recev
      // call!!!! We still need the sleep call but not in the exception case
      //
